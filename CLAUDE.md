...
### Concept Before Function — the Horse and the Carriage *(top-principal — 2026-06-21)*

> "A concept is like a horse pulling a carriage. A function without a concept is like a carriage without a horse — it won't go anywhere, and it definitely won't achieve anything." — Eisa, 2026-06-21

Before building, fixing, or re-enabling **any** function or feature, state its **concept** in a clear, sound sentence: the one thing it is *for*, the question it answers, the cognitive purpose it serves. The concept is the horse (the purpose that makes the work move and arrive); the function is the carriage (the mechanism). **If the concept cannot be stated clearly and soundly, do not build the function — find or clarify the concept first.** Effort spent on a function with no sound concept is wasted motion; it cannot achieve anything because there is nothing it is achieving.

This principle is *temporally first* — it precedes the others, which can only judge a function that already has a concept: **Constraint as Design** ("every feature must justify its existence") and **Form-Aligns-To-Purpose** ("every part of a feature must serve its cognitive purpose") both presuppose a concept to justify and to serve. **Knowledge Formulation, not Management** is this same refusal at the level of the whole app. The **concept-paper-per-function** bring-up method (`docs/concept-papers/00-MASTER`) is this principle operationalized: each function is re-enabled only behind a concept paper that states its horse.

How to apply: open every `/migration` Architect doc, concept paper, feature, and behavior-adding fix with a one-line concept statement (the horse) **before** the function (the carriage). It sits alongside **State the Function in Hand** — name the function, AND name its concept.
...
│
├── Library (zero or more — additional registered libraries, ALWAYS under the root — MIG-108)
│    └── Folder
│         └── Note
│
...
9. **The Pending Jobs ledger is the living backlog — reconcile it at the close of EVERY job, and it is the FIRST file opened at that close.** *(Boss-mandated 2026-07-12: "create a process for maintaining the PJ list… the first file you check after every job is finished, to make sure we are following Constellation development and do not miss anything.")* The highest-version `docs/Constellation Pending Jobs vX.Y.md` is the single source of truth for **what is left to do** in Constellation — organized (since v1.18) into five priority groups with a standing **"► Next action"** line at the top. A **"job"** = a `/migration` close, a `PJ-NNN` completion, a standalone fix/feature, or a session-close. At the close of every job, **before** the session-log write (SO#1) and **before** picking the next task, open the ledger and reconcile it **in the same commit as the work**:
   1. **Close what shipped** — flip every PJ/item this job completed to Done, with evidence (MIG#/commit/§/date). Never leave a just-shipped item reading "Open."
   2. **File what surfaced** — allocate the next `PJ-NNN` for any new outstanding item this job discovered (a bug, a debt, a deferred follow-up, an inspection finding not fixed in-pass). A discovered item that lives ONLY in a session log or the Charter and is not in the ledger is a miss — the ledger is the completeness net.
   3. **Re-rank if the close changed priorities** — a Group-1 item done promotes the next; a new app-killer jumps to Group 1; the "► Next action" line always names the current top item.
...
6. **Fix what you discover.** *(Boss-dictated 2026-06-21.)* "If we discover it, we have to fix it." When a review, audit, or investigation surfaces a defect — **any** severity, including P3 and edge-cases — fix it in the same pass, before the Boss test, before declaring the phase done. A documented-but-unfixed finding is a known bug shipped; "logged in the session log" is not "handled." Do NOT present a "deferred / documented" bucket as an acceptable outcome. If something genuinely cannot be fixed now (it needs a separate migration or a Boss design decision), say so explicitly and get a ruling — never silently park it. A fix may be proportionate (an O(1) guard, an honest empty-state, a skip-condition) — it need not be a big refactor — but the issue must be addressed, not noted-and-shipped. (Canonical arc: 2026-06-21 §E — three adversarial review passes surfaced 17 findings; all 17 fixed, none deferred. The principle was set when I proposed deferring 4 P3 findings and the Boss interrupted to forbid it.)
