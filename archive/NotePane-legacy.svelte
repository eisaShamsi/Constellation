<script lang="ts">
	...
	import { parseFrontmatter, extractHeadings, editingTabIds, toggleEditMode, saveTabContent, updateTabContent, buildFullContent, resolveWikilinkCrossLibrary, openNoteTab, openTabs, libraryAppearances, libraries, navigateBack, navigateForward, readNote, appSettings, updateSettings, renameItem } from '$lib/libraries/store';
	...
	let editBody = $state(tab ? parseFrontmatter(tab.content).body : '');
	let latestEditorText = tab ? parseFrontmatter(tab.content).body : ''; // non-reactive, for saving only
	...
	onDestroy(() => {
		...
		// Always update store + save to disk on destroy so reopening gets latest content
		if (tab && latestEditorText) {
			const currentParsed = parseFrontmatter(tab.content);
			const newContent = buildFullContent(currentParsed.properties, latestEditorText);
			updateTabContent(tab.id, newContent);
			saveTabContent(tab.id, tab.path, currentParsed.properties, latestEditorText).catch((e) => console.error('[NotePane] Flush save failed:', e));
		}
