...
// ─── MIG-048 §B — Mind tool helpers ─────────────────────────────
//
// Public functions added to back the `list_recent` and `graph_neighbors`
// chat tools (`mind/orchestrator/tools/list_recent.rs` +
// `mind/orchestrator/tools/graph_neighbors.rs` in §C). They are NOT
// `#[tauri::command]` because the dispatcher calls them in-process via
// `spawn_blocking`, not through the IPC layer.

/// Return the most-recently-modified notes across the active Universe.
///
/// - `since_unix_seconds`: only notes with `modified > this` are
///   returned. Pass 0 for "no threshold" (all notes).
/// - `limit`: max rows (clamped to [1, 100]; default 20).
///
/// Read-only; touches `note_meta` only.
pub fn constellation_search_recent(
    app: &tauri::AppHandle,
    ...
    let rows = stmt
        .query_map(params![since_unix_seconds as i64, limit as i64], |row| {
            Ok(SearchResult {
                path: row.get(0)?,
                name: row.get(1)?,
                library_name: row.get(2)?,
...
/// One graph-neighbour entry returned by [`constellation_graph_neighbors`].
#[derive(Debug, Clone, Serialize)]
pub struct GraphNeighborEntry {
    pub path: String,
    pub name: String,
    pub library_name: String,
    pub link_type: String,
    /// "outgoing" (root → neighbor) or "incoming" (neighbor → root).
    pub direction: String,
    /// 1 = direct neighbor; 2 = neighbor-of-neighbor; etc.
    pub distance: u32,
}

/// Wrapper around [`GraphNeighborEntry`] list returned to the model.
#[derive(Debug, Clone, Serialize)]
pub struct GraphNeighborhood {
    pub root_path: String,
    pub root_name: String,
    pub depth: u32,
    pub neighbors: Vec<GraphNeighborEntry>,
}

/// BFS over `note_links` starting at `note_path`. Follows both outgoing
/// edges (source = current) and incoming edges (target_path = current
/// OR target_name = current's name). `depth` is clamped to [1, 3] to
/// keep tool-result payloads finite — even 3 hops can return hundreds
/// of nodes in a dense Universe.
pub fn constellation_graph_neighbors(
    app: &tauri::AppHandle,
    note_path: String,
    depth: u32,
    ...
    // Look up the root note's name (needed for incoming-edge match by name).
    let (root_name, _root_library): (String, String) = conn
        .query_row(
            "SELECT name, library_name FROM note_meta WHERE path = ?1",
            params![&note_path],
            |row| Ok((row.get(0)?, row.get(1)?)),
        )
        .map_err(|_| format!("note not found in index: {note_path}"))?;

    let mut visited: std::collections::HashSet<String> = std::collections::HashSet::new();
    visited.insert(note_path.clone());

    // Frontier: (path, name) pairs we need to expand on the next round.
    let mut frontier: Vec<(String, String)> = vec![(note_path.clone(), root_name.clone())];
    let mut neighbors: Vec<GraphNeighborEntry> = Vec::new();
    ...
    for current_depth in 1..=depth {
        ...
        for (curr_path, curr_name) in &frontier {
            // Outgoing: rows where source_path = curr_path AND status = 'active'.
            let mut out_stmt = conn
                .prepare(
                    "SELECT l.target_path, l.target_name, l.link_type, m.library_name
                     FROM note_links l
                     LEFT JOIN note_meta m ON m.path = l.target_path
                     WHERE l.source_path = ?1 AND l.status = 'active'",
                )
                .map_err(|e| e.to_string())?;
            let out_rows = out_stmt
                .query_map(params![curr_path], |row| {
                    let target_path: Option<String> = row.get(0)?;
                    let target_name: String = row.get(1)?;
                    let link_type: String = row.get(2)?;
                    let library_name: Option<String> = row.get(3)?;
                    Ok((target_path, target_name, link_type, library_name))
                })
                .map_err(|e| e.to_string())?;
            for r in out_rows {
                let (target_path, target_name, link_type, library_name) =
                    r.map_err(|e| e.to_string())?;
                // Resolved (path present) neighbours go into the result + traversal frontier.
                if let Some(p) = target_path {
                    if visited.insert(p.clone()) {
                        neighbors.push(GraphNeighborEntry {
                            path: p.clone(),
                            name: target_name.clone(),
                            library_name: library_name.unwrap_or_default(),
                            link_type,
                            direction: "outgoing".into(),
                            distance: current_depth,
                        });
                        next_frontier.push((p, target_name));
                    }
                }
                // Unresolved (target_path NULL) edges are included as leaves
                // at distance=current_depth so the model can still see them.
                // They never expand further (no path to traverse from).
            }

            // Incoming: rows where target_path = curr_path OR target_name = curr_name.
            let mut in_stmt = conn
                .prepare(
                    "SELECT DISTINCT l.source_path, l.source_name, l.link_type, m.library_name
                     FROM note_links l
                     LEFT JOIN note_meta m ON m.path = l.source_path
                     WHERE (l.target_path = ?1 OR l.target_name = ?2) AND l.status = 'active'",
                )
                .map_err(|e| e.to_string())?;
            let in_rows = in_stmt
                .query_map(params![curr_path, curr_name], |row| {
                    let source_path: String = row.get(0)?;
                    let source_name: String = row.get(1)?;
                    let link_type: String = row.get(2)?;
                    let library_name: Option<String> = row.get(3)?;
                    Ok((source_path, source_name, link_type, library_name))
                })
                .map_err(|e| e.to_string())?;
            for r in in_rows {
                let (source_path, source_name, link_type, library_name) =
                    r.map_err(|e| e.to_string())?;
                if visited.insert(source_path.clone()) {
                    ...
                    next_frontier.push((source_path, source_name));
                }
            }
        }
...
mod mig048_tests {
    ...
    fn insert_note(conn: &Connection, path: &str, name: &str, library: &str, modified: i64) {
        conn.execute(
            "INSERT INTO note_meta (path, name, library_name, modified)
             VALUES (?1, ?2, ?3, ?4)",
            params![path, name, library, modified],
        )
        .expect("insert note");
    }
    ...
    fn insert_link(
        ...
        conn.execute(
            "INSERT INTO note_links (source_path, source_name, target_path, target_name, link_type, status)
             VALUES (?1, ?2, ?3, ?4, ?5, 'active')",
            params![src_path, src_name, tgt_path, tgt_name, link_type],
        )
        .expect("insert link");
    }

    // The two functions above take an `AppHandle` to look up the
    // `SearchState`. The tests here exercise the SQL surface directly
    // by extracting the bodies into helpers. The full plumbing is
    // exercised in Step §M's audit + the manual verify clause of §E
    // (which drives both fns end-to-end via the orchestrator).

    #[test]
    fn recent_query_orders_by_modified_desc_and_respects_limit() {
        let conn = make_test_conn();
        insert_note(&conn, "/lib/a.md", "A", "lib", 100);
        insert_note(&conn, "/lib/b.md", "B", "lib", 300);
        insert_note(&conn, "/lib/c.md", "C", "lib", 200);
        insert_note(&conn, "/lib/d.md", "D", "lib", 400);

        let mut stmt = conn
            .prepare(
                "SELECT path, modified FROM note_meta WHERE modified > ?1
        ...
        assert_eq!(rows[0].0, "/lib/d.md");
        assert_eq!(rows[0].1, 400);
        assert_eq!(rows[1].0, "/lib/b.md");
        assert_eq!(rows[1].1, 300);
    }

    #[test]
    fn recent_query_filters_by_since_threshold() {
        let conn = make_test_conn();
        insert_note(&conn, "/lib/old.md", "O", "lib", 50);
        insert_note(&conn, "/lib/new.md", "N", "lib", 200);

        let mut stmt = conn
            .prepare(
                "SELECT path FROM note_meta WHERE modified > ?1
                 ORDER BY modified DESC LIMIT ?2",
            )
            .unwrap();
        let rows: Vec<String> = stmt
            .query_map(params![100_i64, 10_i64], |row| row.get::<_, String>(0))
            .unwrap()
            .map(Result::unwrap)
            .collect();

        assert_eq!(rows, vec!["/lib/new.md"]);
    }

    #[test]
    fn graph_neighbors_query_finds_outgoing_and_incoming() {
        let conn = make_test_conn();
        // root → b (outgoing); c → root (incoming); d unrelated.
        insert_note(&conn, "/lib/root.md", "Root", "lib", 100);
        insert_note(&conn, "/lib/b.md", "B", "lib", 100);
        insert_note(&conn, "/lib/c.md", "C", "lib", 100);
        insert_note(&conn, "/lib/d.md", "D", "lib", 100);
        insert_link(&conn, "/lib/root.md", "Root", Some("/lib/b.md"), "B", "supports");
        insert_link(&conn, "/lib/c.md", "C", Some("/lib/root.md"), "Root", "contradicts");

        // Outgoing from root
        let mut out_stmt = conn
            .prepare(
                "SELECT target_path, target_name, link_type FROM note_links
                 WHERE source_path = ?1 AND status = 'active'",
            )
            .unwrap();
        let out: Vec<(Option<String>, String, String)> = out_stmt
            .query_map(params!["/lib/root.md"], |row| {
                Ok((row.get(0)?, row.get(1)?, row.get(2)?))
            })
            .unwrap()
            .map(Result::unwrap)
            .collect();
        assert_eq!(out, vec![(Some("/lib/b.md".into()), "B".into(), "supports".into())]);

        // Incoming to root (by path or name)
        let mut in_stmt = conn
            .prepare(
                "SELECT source_path, link_type FROM note_links
                 WHERE (target_path = ?1 OR target_name = ?2) AND status = 'active'",
            )
            .unwrap();
        let inc: Vec<(String, String)> = in_stmt
            .query_map(params!["/lib/root.md", "Root"], |row| {
                Ok((row.get(0)?, row.get(1)?))
            })
            .unwrap()
            .map(Result::unwrap)
            .collect();
        assert_eq!(inc, vec![("/lib/c.md".into(), "contradicts".into())]);
    }
}
