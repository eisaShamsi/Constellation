...
import { setBuffer, setBufferProps, ensureBuffer, composeBuffer, updateBufferPath, deleteBuffer, clearAllBuffers, parityProbe } from '$lib/editor/noteBuffers';
// MIG-076 §CB-2 — local bindings for the extracted frontmatter cluster
// (the `export ... from` re-export below introduces no local names).
import { parseFrontmatter, buildFullContent } from '$lib/editor/frontmatter';
import type { FrontmatterProperty } from '$lib/editor/frontmatter';
...
// MIG-076 §CB-2 — the frontmatter parse/serialize cluster moved VERBATIM to
// $lib/editor/frontmatter.ts (so the buffer layer composes without a cycle).
// Re-exported here so every existing `from '$lib/libraries/store'` import
// keeps working unchanged.
export { parseFrontmatter, reconstructFrontmatter, buildFullContent, normalizeDateValue } from '$lib/editor/frontmatter';
export type { FrontmatterProperty, PropertyType } from '$lib/editor/frontmatter';
...
export async function saveTabContent(
	...
	try {
		...
		// MIG-076 §CB-2 — the buffer is the ONLY composition source: props (with
		// the auto-date) land in the buffer; the body comes from the buffer's own
		// copy, which is as fresh as (or fresher than) the caller's `body` param —
		// the pane's save path maintains it. A path mismatch is refused+journaled.
		ensureBuffer(tabId, filePath, buildFullContent(updatedProps, body));
		setBufferProps(tabId, updatedProps);
		const composed = composeBuffer(tabId, filePath, 'prop_save');
		if (!composed.ok) return;
		recentWrites.set(filePath, Date.now());
		await writeNote(filePath, composed.content, 'prop_save');
		...
		// §CB-2 — embeds what disk actually received (the buffer's body).
		if (get(appSettings).enabledFeatures?.semanticSearch) {
			const tab = get(openTabs).find(t => t.path === filePath);
			if (tab) {
				invoke('constellation_embed_notes', {
					notes: [{ path: filePath, name: tab.name, content: composed.body }],
					force: true
				}).catch(() => {});
			}
		}
...
export async function openNoteTab(filePath: string, libraryName: string, color: string = '#7c3aed', highlightTerm?: string, newTab?: boolean, fromNotePath?: string) {
	...
	if (!newTab && currentTab) {
		...
		// MIG-076 §CB-1 — mirror the tab slot's new identity+content into its buffer
		{
			...
			parityProbe(currentTab.id, { props: fp.properties, body: fp.body }, 'openNoteTab:reuse');
		}
