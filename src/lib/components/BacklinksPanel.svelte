<script lang="ts">
	...
	// Confidence popover state. Opened via right-click on a backlink row; the
	// popover UI + IPC live in the shared <ConfidencePicker> (MIG-077 A4).
...
<style>
	.backlinks-panel { font-size: calc(0.8rem * var(--rs-scale, 1)); }
	.bl-filter { padding: 2px 8px 4px; }
	.bl-filter input {
		width: 100%; padding: 3px 6px; border: 1px solid var(--border); border-radius: 4px;
		background: var(--bg); color: var(--text); font-size: calc(0.75rem * var(--rs-scale, 1)); font-family: inherit; outline: none;
	}
	...
	.bl-library-label { font-size: calc(0.68rem * var(--rs-scale, 1)); color: var(--text-faint); flex-shrink: 0; }
	.bl-name { color: var(--interactive-accent); font-size: calc(0.8rem * var(--rs-scale, 1)); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
	.bl-context { display: block; color: var(--text-faint); font-size: calc(0.72rem * var(--rs-scale, 1)); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
	.bl-annotation {
	...
	.bl-empty { color: var(--color-base-40); font-size: calc(0.78rem * var(--rs-scale, 1)); padding: 4px 0; }
	.bl-link-btn {
	...
	.bl-traversal-chip {
		...
		color: var(--link-tier-accent, var(--interactive-accent, #7c3aed)); /* MIG-088 §5b — styleable tier accent */
	...
	.bl-tier-emerging {
		/* §5b Boss ruling: one colour per tier — tier var → Accent → theme accent (byte-identical chain) */
