				...
				{#each presets as p (p.id)}
					<div class="sp-row">
						<div class="sp-info">
							...
							{:else}
								<div class="sp-name" dir={detectDir(p.name)}>{p.name}</div>
...
<style>
	...
	.sp-row {
		...
		padding: 7px 8px; border-radius: 8px; border: 1px solid var(--background-modifier-border);
		background: var(--background-primary);
	}
	.sp-row:hover { background: var(--background-modifier-hover); }
	.sp-info { flex: 1; min-width: 0; }
	.sp-name { font-size: 0.9rem; font-weight: 600; color: var(--text-normal); }
	.sp-sections { font-size: 0.7rem; color: var(--text-faint); margin-top: 1px; }
	.sp-rename {
	...
	.sp-actions { display: flex; align-items: center; gap: 4px; }
	.sp-apply {
		padding: 4px 12px; font: inherit; font-size: 0.8rem; font-weight: 600;
	...
	.sp-icon {
		...
		font-size: 0.85rem; padding: 3px 6px; border-radius: 5px;
	}
