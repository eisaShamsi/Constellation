<script lang="ts">
	...
	let nodePositions: { node: GraphNode; x: number; y: number; r: number }[] = [];
	...
	function layout() {
		...
		// Assign vault colors
		const vaultNames = [...new Set(nodes.map(n => n.vaultName))];
	...
	function draw(dpr?: number) {
		...
		for (const pos of nodePositions) {
			...
			const color = vaultColorMap.get(pos.node.vaultName) || '#6b7280';
