<script lang="ts">
	...
	import { libraryStats, loadLibraries, loadAllStats } from '$lib/libraries/store';
		...
		<div class="vs-list">
			{#each childUniverses as child}
				<div class="vs-item vs-child-universe">
					...
					<span class="vs-count">{child.library_count} {child.library_count === 1 ? 'library' : 'libraries'}</span>
				</div>
			{/each}
		</div>
	{/if}
