...
| **MIG-073** Knowledge Health snapshot cache (circulatory-aggregate layer) | Architect→Plan→Build P1/P2 | 🔨 **In build** *(2026-06-10)*. Perf-Rule-8 fix for the KH overlay's ~11s cold-scan freeze: general `link_stats_cache` (stale-while-revalidate, background recompute on a dedicated connection) — the data layer CCS §8 assumes; KH = first consumer. Precursor fix `aa9941ee`: `constellation_link_decay` made READ-ONLY (the old write-loop compounded decay into raw weights on every panel/boot/24h call — decay is display-only per Living-Links-Guide §7). P1 `556d401e` backend; P2 `77ed2786` panel. Found during build: the right-sidebar 'health' tab = **TensionPanel** (`detect_tensions`), NOT the KH overlay — its stuck-"Loading…" is a separate pre-existing bug. Docs: `lab/reports/MIG-073-KH-CACHE-{ARCHITECT,PLAN}.md`. |
