...
**A different kind of finding: entries that point at notes which no longer exist.** Separately from the counts above, the band may tell you that a number of *entries in the search index point at notes that no longer exist on disk* — that they can show up as search results that open nothing, and as connections to notes that aren't there, and that nothing has been changed, they have only been counted. These are leftovers — a note's file was deleted or moved outside Constellation long ago, the folder it lived in is no longer part of any of your libraries, and nothing you did to that note (no link you strengthened, no review you recorded) is attached to it. They are why a search result can occasionally open nothing.

This sentence sits on its own row, with its own **✕** that dismisses only it, and unlike the one above it, **has no Repair now button** — deliberately. The repair works by walking your libraries and re-reading the files it finds; one of these leftovers has neither a library nor a file, so a repair cannot reach it. A button that could not do anything would be worse than no button.

**To remove them, go to Settings → Index.** When there are any, a **Remove stale index entries** item appears there, telling you how many. Pressing **Remove** asks you to confirm first — the question names the number, and removing is permanent. Constellation then removes those entries from the search index and from every view built on it: search, backlinks, tag counts, the review schedule and Sky View. **Your note files are never touched** — these entries point at files that are already gone, which is what makes them leftovers.

A record is kept, and you can read it. Before each entry is removed, a line describing it is written to a file inside your universe folder. **You can read that record in Settings → Universe & Libraries → Deleted notes**, which lists every removal this universe has recorded and lets you open any one of them to see the text that was kept.

**Be clear about what that record is, because it is easy to assume more.** It keeps the note's text *as the search index held it* — and the search index does not store your file. It stores a stripped version, built for searching: the note's properties are gone entirely, headings and emphasis are gone, the contents of code blocks are gone, web addresses behind links are gone, links appear as bare text without the note you wrote beside them, and Arabic vowel marks are removed. One real note of 8,037 characters keeps 713. It is **proof of what was removed, not a copy you could rebuild the note from.**

Three honest limits, then. It is a record, **not an undo** — there is no button that puts a note back. The text it keeps is a stripped rendering, as above. And it is **not a complete record**: a note whose identity is blank in the search index is removed without leaving an entry at all — whether Constellation never gave it one, or another file had already claimed the same one. Treat the removal itself as permanent.

The connections are a separate matter, and the record does not cover them. Removing these entries also removes the links they carried — in one measured case, 603 entries carried 19,472 links between them — and **none of those links is written into the record.** Nothing you earned is lost this way (a link's strength, confidence and history live on the note that points *outward*, and those notes are not touched), but if you want to know what a removed entry pointed at, the record will not tell you.

Afterwards a short receipt appears under the button — *"Removed 603 entries. A record of each was saved in your universe folder."* It reports honestly rather than flatteringly: if some entries were kept because the note reappeared, or some could not be removed, or the removal stopped early because you switched universes mid-run, each of those gets its own line. The control disappears once there is nothing left to remove.

**A third kind of finding: a note Constellation cannot see at all.** The two above are about entries in the index. This one is the opposite — a note that is on your disk, with your text in it, that Constellation has never been able to read. **A file whose name begins with a dot is a hidden file.** That is a long-standing convention on every system Constellation runs on, and it is the rule that keeps Constellation's own `.constellation` and `.trash` out of your index, alongside other applications' `.obsidian` and `.git`. The price of honouring it is that a real note saved under such a name is invisible *twice over*: it is never indexed, **and it cannot be reported as missing either**, because the check that hunts for missing notes skips it for exactly the same reason. Until now the app had no way to tell you such a note existed.

It now counts them and says so, on its own row: *"Constellation is not searching 2 notes on disk, because a file name that begins with a dot marks the file as hidden. It can neither find such a note nor report it as missing. Removing the leading dot from the file name lets Constellation see the note again."* Like the row above it, it has **no Repair now button**, and for a sharper reason: a repair walks your libraries and re-reads the files it finds, and it would skip these on the very rule that hid them. A button that reliably did nothing would be worse than none.

**The remedy is a rename, and it is yours to make.** In your file manager, remove the leading dot from the file's name — leave everything else alone — and Constellation reads the note at the next start. It matches links by a note's **title**, which lives inside the file and is not touched by renaming the file, so any links pointing at that note begin resolving as well. Constellation will no longer produce such a name itself: not when you create a note, not when you import one (in any format, including a folder or Obsidian vault), and not when you save a template. It can still happen if you *rename* a note or folder to one yourself, or type one into a folder setting — those doors are known and are being closed.

**A fourth kind of finding: a library Constellation has been told about but cannot reach.** This one is rarer than the others, and it comes from a collision between two things you did. A folder *above* one of your registered libraries contains a `universe.json` file — either on its own or inside a hidden `.constellation` folder — which is how Constellation marks a folder as a universe in its own right. Constellation stops reading at that mark, because a universe is never content of another universe. But the library beneath it is still registered as belonging to *this* universe, so the app is in two minds: it keeps the notes it already indexed, and never looks at them again.

**What that means in practice.** The notes stay in your search index and keep appearing in results, so nothing looks broken. What stops is *updating* — an edit you make to one of those notes, in Constellation or anywhere else, is not picked up. The bar says so rather than letting it pass in silence, and like the two rows above it carries **no Repair now button**, for the sharpest reason yet: the repair walks your libraries and re-reads their files, and it stops at exactly the same mark. It would report success and change nothing.

**The remedy is on disk.** Delete the `universe.json` file in the folder above the library, and Constellation reads that library again at the next start. One such file can sit above more than one library, so removing it may clear several at once. **Removing the library from this universe is not an alternative fix** — it stops the message, but the notes then go from flagged to silently ignored: still in the index, still never re-read, and no longer counted anywhere. If you want those notes out, remove them as notes; if you want them read, remove the marker above them.

Two things worth knowing about the count. It is **conservative by design**: anything Constellation cannot be certain about is left alone and not counted — in particular, if a drive or folder holding one of your libraries is temporarily unavailable, it reports nothing at all rather than mistaking your real notes for leftovers. And **zero is a normal, healthy answer**: a universe whose libraries all sit under its own folder cannot accumulate these at all, so the sentence never appearing means there is nothing to report.
...
**Renaming updates links automatically.** When you rename a note — from the file tree (right-click → **Rename**) or by editing its title at the top of the page — Constellation rewrites every link that points to it, so links never silently break. This is the **Auto-update links on rename** setting under **Settings → Links**, and it is on unless you switch it off. A brief "Updating links…" overlay appears on the affected note(s) while this runs (the editor pauses typing for that moment); the old title is also kept as an alias, so anything still naming the old title keeps resolving.

**Every form of link is updated — not just the plain ones.** The rewrite covers a plain link (`[[Note Name]]`), a link with your own display text (`[[Note Name|the words you wanted to read]]`), a **typed** link that carries a relationship (`[[supports::Note Name]]`) including one with your annotation on it (`[[supports::Note Name|why it supports]]`), and a link that points *inside* a note — at a heading (`[[Note Name#A Heading]]`) or at a block (`[[Note Name^blockid]]`). Only the name changes: the relationship type, your display text, your annotation, and the heading or block you pointed at are left exactly as you wrote them. Typed links and pointed-inside links were previously left behind, still naming the old title, with nothing said about it — a broken link with no warning. A link that spells out the folder on the way to the note (`[[Folder Name/Note Name]]`) is updated too, and keeps its folder exactly as written — the note was retitled, not moved. **One case is not yet covered:** a link that spells the title with different capitalisation from the note itself (`[[note name]]` for a note titled "Note Name") still resolves, but after a rename it is left naming the old title.

**Every library in your universe is covered.** The update reaches notes in **all** the libraries of the universe you have open, not only the library the renamed note happens to live in — so a link that crosses from one library to another survives the rename. **Renaming a note that lives in a Linked Universe works too:** the rename heals the referring notes *inside that same Linked Universe*, using that universe's own link vocabulary, and its bookkeeping lands in that universe's own records — where it belongs. Notes in *your* universe that link to the renamed Linked-Universe note keep their old link text for now (a later update will heal those as well), but clicking such a link still opens the renamed note — Constellation remembers the old title. A rename never crosses into a third universe.

**If some notes could not be updated, you are told.** Now and then a note's file is locked, read-only, or held open by another program. Constellation names those notes and says their links still point at the old title, instead of reporting a clean success — so you know exactly what to check.
...
- **Knowledge Health** — the open note's intellectual *tensions*: contradicted by another note, orphaned (nothing links to it), a single point of failure, or in a structural gap. The whole-library Knowledge Health dashboard is a separate full-page view (Command Palette → Knowledge Health). For a note in a **Linked Universe** the panel shows *"Analysis unavailable"* — health analysis currently covers your own universe's notes, and Constellation says so honestly instead of computing a result from the wrong universe's data. (A federated health check is planned.)
