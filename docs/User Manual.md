...
Three honest limits, then. It is a record, **not an undo** — there is no button that puts a note back. The text it keeps is a stripped rendering, as above. And it is **not a complete record**: a note that Constellation never gave an identity of its own is removed without leaving an entry at all. Treat the removal itself as permanent.
