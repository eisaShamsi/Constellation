...
**Every library in your universe is covered.** The update reaches notes in **all** the libraries of the universe you have open, not only the library the renamed note happens to live in — so a link that crosses from one library to another survives the rename. Notes in a **linked (child) universe** are deliberately left untouched: that is a separate knowledge base, and Constellation does not rewrite its notes.
