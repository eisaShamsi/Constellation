...
| **MIG-074** CCS — Constellation Circulatory System (left-dock Core Plug-in; the circulatory register of the Connection question) | `lab/reports/MIG-074-CCS-ARCHITECT.md` | 🟡 **Architect produced 2026-06-10 — awaiting Eisa's ratification (Q1–Q8).** Per the ratified Concept Paper v1.1: seven §6 registers on the MIG-073 `link_stats_cache` (+~4–5 additive `ccs_*` keys in the existing recompute), retire the right-sidebar LinkDashboard, re-point the MIG-007 hub, coordinate (not subsume) Knowledge Health. Proposed: detect_tensions OUT (CNS-MIG material) · read-time dormancy IN · trends deferred · KH = mutual deep-links. No Plan, no code yet. |
