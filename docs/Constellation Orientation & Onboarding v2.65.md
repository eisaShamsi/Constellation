...
| **MIG-074** CCS — Constellation Circulatory System (left-dock Core Plug-in; the circulatory register of the Connection question) | `lab/reports/MIG-074-CCS-{ARCHITECT,PLAN}.md` | 🟡 **Architect RATIFIED 2026-06-10 (all Q1–Q8 as recommended); Plan produced — awaiting Plan approval.** Per the ratified Concept Paper v1.1: seven §6 registers on the MIG-073 `link_stats_cache` (6 additive `ccs_*` keys in the existing recompute + the Q3 derived-dormancy repair), the CCSView left-dock surface beside CNS, retire the right-sidebar LinkDashboard + re-point the MIG-007 hub (atomic), coordinate (not subsume) Knowledge Health via mutual deep-links. Rulings: detect_tensions OUT (CNS-MIG material) · read-time dormancy IN · trends deferred · crossLibrary/broken drop documented. Build = §A–§F, three ★ Boss-test stops. |
...
| **CCS — Constellation Circulatory System** (the circulatory register for the Connection question; peer of CNS; the pulse of the link graph) | [`docs/Constellation-Circulatory-System-Concept-Paper-v1.1.md`](docs/Constellation-Circulatory-System-Concept-Paper-v1.1.md) (RATIFIED) · Architect: [`lab/reports/MIG-074-CCS-ARCHITECT.md`](lab/reports/MIG-074-CCS-ARCHITECT.md) (awaiting ratification) |
