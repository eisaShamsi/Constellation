...
- **Universe Digest** — the *primary* place summaries live: a dedicated left-dock pane that lists every note in your Universe (tiered Library → Folder → Note), with each row showing the headline beneath the name. Click a row to expand and read the full multi-sentence summary inline. Search/filter narrows the whole list. See *The Digest* for the full topic.

By default summaries fill in **lazily and gently**: as cards scroll into view, as search results appear, as you open a note, as you expand a term, hover a bubble, or scroll the Digest, Constellation computes the missing summaries a few at a time, pausing whenever a Library classification scan is running so the two never compete for resources. This keeps the app responsive — you may briefly see a card / result / opened note / row / tooltip / Digest row before its summary appears, then the summary pops in a moment later.
