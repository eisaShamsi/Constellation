...
And a third sentence can appear in that band, for the opposite problem: a note that is on disk but that Constellation has never been able to see. **A file whose name begins with a dot is a hidden file** — a long-standing convention on every system Constellation runs on, and it is what keeps Constellation's own `.constellation` and `.trash` out of your index alongside other apps' `.obsidian` and `.git`. The cost of honouring it is that a real note saved under such a name is invisible twice over: it is never indexed, **and it cannot be reported as missing either**, because the check that looks for missing notes skips it for the very same reason. Constellation now counts these and says so. That sentence also carries **no button** — a repair works by walking your libraries and re-reading their files, and it would skip the same file on the same rule. The remedy is a rename in your file manager: remove the leading dot, and the note is read at the next start. Constellation itself will no longer produce such a name when you create a note, when you import one, or when you save a template.

A fourth sentence can appear for a rarer situation: a library Constellation has been **told about but cannot reach**. A folder above one of your registered libraries carries a `universe.json` file — the mark that makes a folder a universe of its own — and Constellation stops reading at that mark, because a universe is never content of another universe. The library beneath it is still registered here, so its notes stay in the index and keep appearing in search, while edits to them are never picked up. That sentence carries **no button** for the sharpest reason of the three: the repair walks your libraries and re-reads their files, and stops at the very same mark. The remedy is to delete that `universe.json`, after which the library is read again at the next start; one such file can sit above several libraries. Removing the library from this universe is **not** an equivalent fix — it silences the message and leaves the notes indexed but permanently unread.
