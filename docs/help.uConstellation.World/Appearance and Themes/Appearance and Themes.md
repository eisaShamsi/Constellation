...
- **Sky View / CNS / OrgChart / Index / Cataloger / Shell** — the plugin surfaces. The **Sky View** surface includes a **Canvas** element whose **Background** colour sets the backdrop behind the graph bubbles — independent of the panel colour (see *Sky View canvas*, below). The **CNS** surface styles the Nervous System's gravity well (see *CNS — style the gravity well*, below).
...
- **The Editor, Sky View, and CNS categories** show a **preview in the centre** — a sample note, a labelled bubble board, or a miniature gravity well, filling the centre zone. Click a part of the preview and its controls appear on the right; the preview updates instantly as you edit. (These surfaces draw on busy canvases, so a focused preview beats hunting a colour change in a live graph.)
