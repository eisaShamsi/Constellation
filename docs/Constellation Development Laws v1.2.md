...
**Statement.** Constellation organizes knowledge in a four-level structural hierarchy with an *optional federation layer at the top*:
...
The structural levels of stored knowledge are **Universe → Library → Folder → Note** (four levels). **`cUniverse` is a sibling federation mechanism at the Universe level**, not a level a user has to traverse to reach a Library — a Universe holds its Libraries directly, and *optionally* also links to other Universes whose Libraries are federated in.

- **Universe** — top-level container. Holds its own libraries (manifested in `<universe>/.constellation/libraries.json`), settings, bases, bookmarks, AND an optional list of cUniverse children (manifested in `<universe>/universe.json`'s `children` array). One Universe is "active" per Constellation instance.
- **Library** — a complete, self-contained knowledge base, **a direct child of a Universe**. First-class citizen with its own color, appearance, tags, links, and index. Registered in `libraries.json`. Multiple libraries coexist in one Universe. Read in place — never copied.
- **cUniverse (Child Universe)** — *optional layer*. A linked Universe whose libraries get federated into the parent at runtime. Each cUniverse is itself a full Universe (with its own libraries and its own optional cUniverse children); the loader recursively flattens the federation tree into one library list. Enables viewing notes from multiple independent Universes in one window. **A Universe with zero cUniverses is a complete, valid setup** — federation is opt-in.
...
**Code reference.** `src-tauri/src/universe.rs::resolve_libraries_recursive` is the canonical loader — it reads `libraries.json` directly under the active Universe, then recurses into each cUniverse child declared in `universe.json`. The recursion is what makes `cUniverse` a federation surface rather than a hierarchy level.

**Canonical violation.** 2026-05-05. Laws v1.0/v1.1, CLAUDE.md, and the orientation all carried a five-level diagram that drew `Library` *only* as a child of `cUniverse`, implying federation was a required intermediate layer. Boss spotted the inconsistency while reviewing the Laws and asked: "could it be Libraries under the Universe directly, with each library having folders and notes?" — the correct framing. The fix is in v1.2: hierarchy redrawn as 4 + optional-1, with the code reference making the verification path explicit. Logged as a reminder that even high-tier Laws can carry stale diagrams; verify against code, not memory.

**Source.** `CLAUDE.md` → Constellation Knowledge Hierarchy (corrected 2026-05-05); `src-tauri/src/universe.rs::resolve_libraries_recursive` for the canonical loader; orientation v1.0+ (text body, which always described the relationship correctly even though the diagram in CLAUDE.md/Laws didn't).
