...
## 2. Purpose
Surface the actionable commitments embedded in notes — `- [ ]` lines with optional due date (`📅`/`due::`), priority (`⏫🔼🔽`/`priority::`), tags, created/done dates — and let the user complete them in place. In Five-Acts terms this is the weakest-justified function in the set: it serves **Conviction** only obliquely (a held position often implies "do X next"), and it is closer to PKM-as-management than PKM-as-*formulation*. **It must justify itself in bring-up** against Constraint-as-Design: a task list is a file-manager affordance, and the core paper says Constellation is *not* a file manager. The honest position: it earns its place only if framed as "the open loops my thinking left behind," not as a productivity to-do app. If it cannot, it should stay disabled.
...
- [ ] **Serves Constellation's core purpose:** §2 self-justification resolved — framed as "open loops in my thinking" (Conviction), not a productivity to-do app; else stays disabled (Constraint as Design).
...
Notes: Two real components verified (`TasksPanel.svelte`, `GlobalTasksView.svelte`) over `src-tauri/src/tasks.rs`. **Three bring-up blockers:** (1) **Rule 8 violation** — both surfaces recompute by re-walking the disk; needs a persisted `note_tasks` index + write-path trigger + resumable back-fill. (2) **No right-click menu** at all — gap vs. 00 §5; must be the shared menu, not hand-rolled. (3) **Editor gate-bypass** — `toggle_task` writes via the WriteGate directly without firing the Editor's reindex, so search/backlinks/tags can drift after a toggle. Also: no `enabledFeatures.tasks` gate (add one); `TasksPanel` is missing a `dir` attribute (GlobalTasksView has it). The deepest open question is §2: whether a task list belongs in a *formulation* (not management) system at all — to be ruled by the Boss in bring-up.
