...
| 12 | 360° Inspector | `inspector360.rs` (445 lines) | N/A — pure read aggregation across 9 phases |
...
`docs/CANONICAL-FILENAME-ARCHITECTURE.md`. Format:
...
**Generation algorithm** ([`canonical.rs:49-93`](src-tauri/src/canonical.rs:49)):
- Timestamp source priority: frontmatter `created:` → filesystem creation → modification → `Utc::now()`.
- Format: `dt.format("%Y%m%dT%H%M%SZ")`.
- KIND: file_kinds classification.
- XXXX: 4-char uppercase hex.
- Collision avoidance: try 10 hex suffixes, fallback increment timestamp by 1 second.
...
`ensure_cid_cn()` (line 1222) injects on first read; legacy `cid:` migrated in-place.

**Resolution order**: title exact → aliases → original_filename → broken (red).

### 6.2 Other canonical commands

- `canonicalize_preview` / `canonicalize_execute` — preview rename map, then apply.
- `auto_canonicalize_all` — emits `canonical-progress` events (scanning / canonicalizing / done).
- `inject_cid_library` (line 794) — non-destructive: adds `cid_cn:` only, no rename. Migrates legacy `cid:`.
- `de_canonicalize_library` (line 901) — renames back to human filenames. **Preserves `cid_cn`** so traversal weights survive. Deletes `.meta.json` sidecars.
- `repair_external_libraries_on_startup` (line 1300) — scans registered libraries, detects accidentally-canonicalized external vaults, silently de-canonicalizes them.

**6 unit tests** ([`canonical.rs:1329-1401`](src-tauri/src/canonical.rs:1329)): `test_canonical_name_format`, `test_is_canonical`, `test_inject_frontmatter_new`, `test_inject_frontmatter_merge`, `test_sidecar_path` (and one more by index).

**MIG-003** (human-name filenames overlay) is queued — not started.
...
| **MIG-003** Human-name Filenames | (no plan doc) | 🔲 Not started. |
...
- **Badge letter M** — defined in `CAT_COLORS` of both ConstellationMap.svelte and ConstellationSight2.svelte (cyan `#06b6d4`) but no design doc covers it. To be clarified by project owner. *(W resolved as Wikilink in v1.3 via Badge-Taxonomy.md.)*
...
*End of v1.3. Maintained per Standing Order #6.*
