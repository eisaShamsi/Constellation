...
description: Notizzusammenfassungen geben Ihnen einen kurzen Abriss einer Notiz in einfacher Sprache, sodass Sie sie beurteilen können, ohne sie zu öffnen. Constellation respektiert immer eine Zusammenfassung, die Sie selbst geschrieben haben — im Frontmatter oder in einem Zusammenfassungs-Callout — und generiert nur dann eine, wenn Sie es nicht getan haben. Generierte Zusammenfassungen sind extraktiv (die zentralsten Sätze der Notiz selbst), schreibgeschützt (werden nie in Ihre Datei zurückgeschrieben) und vollständig auf Ihrem Gerät berechnet. Zusammenfassungen erscheinen überall in der App, wo eine Notiz vorkommt — im **Klassifikator**, in den **Suchergebnissen**, im Band des **Editors**, im Panel der **Backlinks**, im Panel der **Outgoing Links**, im **Index**, im Hover-Tooltip der **Himmelsansicht** und im **Universum-Digest**.
---
...
Eine **Notizzusammenfassung** ist ein kurzer Abriss einer Notiz — ein paar Sätze, die Ihnen auf einen Blick sagen, worum es in der Notiz geht. Zusammenfassungen werden vom **Note Summary Creator (NSC)** erzeugt. Sie sehen sie **überall in der App, wo eine Notiz vorkommt**: unter dem Titel jeder Karte in der **Klassifikator**- / **Source-Review**-Warteschlange (wo Sie entscheiden, wie eine Karte zu klassifizieren ist, ohne die Notiz dahinter zu öffnen), als feine kursive Zeile unter jedem Treffer in den **Suchergebnissen** (die Ihnen sagt, *worum* eine Notiz geht, neben dem Ausschnitt, der zeigt, warum sie traf), als schmales Band über der Notiz, wenn Sie sie im **Editor** öffnen (damit der Kern der Notiz beim Lesen oder Schreiben im Kontext bleibt), unter jeder verlinkten Quelle im Panel der **Backlinks** und unter jedem verlinkten Ziel im Panel der **Outgoing Links** (damit eine lange Liste verwandter Notizen als Ideen statt nur als Titel überflogen werden kann), unter jeder Notizerwähnung, wenn Sie einen Begriff im **Index** aufklappen (damit die Notizen eines Begriffs auf einen Blick erkennbar sind), im Hover-Tooltip der **Himmelsansicht**, wenn Sie auf eine Blase zeigen (damit ein voll gepackter Graph lesbar bleibt, ohne durchklicken zu müssen), und als *primärer Inhalt* jeder Zeile im neuen Panel des **Universum-Digest** (wo die gesamte Wissensbasis eine scrollbare Liste von Zusammenfassungen ist — siehe *Das Digest*).
...
- **Backlinks-Panel** — unter jeder Quellzeile, die auf die gerade gelesene Notiz verlinkt. Eine lange Liste eingehender Erwähnungen wird so überfliegbar: Sie lesen den Kern der verlinkenden Notiz als einzelne kursive Zeile unter ihrem Titel, ohne jede einzelne öffnen zu müssen, um sich daran zu erinnern, was sie ist.
- **Outgoing-Links-Panel** — unter jeder Zielzeile, auf die die gerade gelesene Notiz hinausverlinkt. Dieselbe Form wie bei den Backlinks; Sie sehen auf einen Blick, worum es bei jeder ausgehenden Verbindung geht.
- **Index** — wenn Sie einen Begriff aufklappen, zeigt jede Notiz, die den Begriff verwendet, die Zusammenfassung als feine Zeile unter ihrem Titel (und unter dem Treffer-Kontext-Ausschnitt, falls vorhanden). Ein Begriff, der in dutzenden Notizen auftaucht, wird so zu einer Liste von Ideen statt nur Dateinamen.
- **Hover in der Himmelsansicht** — wenn Sie eine Blase im Graphen der **Himmelsansicht** überfahren, zeigt der schwebende Tooltip den Notiznamen in der ersten Zeile und die Zusammenfassungs-Schlagzeile in einer zweiten kursiven Zeile, damit Sie lesen können, was eine Blase *bedeutet*, ohne den Graphen zu verlassen.
- **Universum-Digest** — der *primäre* Ort, an dem Zusammenfassungen leben: ein eigenes Panel im linken Dock, das jede Notiz Ihres Universums auflistet (gestaffelt Library → Ordner → Notiz), wobei jede Zeile die Schlagzeile unter dem Namen zeigt. Klicken Sie eine Zeile an, um sie aufzuklappen und die volle mehrsätzige Zusammenfassung inline zu lesen. Such-/Filter grenzt die gesamte Liste ein. Siehe *Das Digest* für das vollständige Thema.

Standardmäßig füllen sich Zusammenfassungen **träge und behutsam**: Während Karten ins Sichtfeld scrollen, während Suchergebnisse erscheinen, während Sie eine Notiz öffnen, während Sie einen Begriff aufklappen, eine Blase überfahren oder das Digest scrollen, berechnet Constellation die fehlenden Zusammenfassungen in kleinen Mengen und pausiert immer dann, wenn ein Library-Klassifizierungsscan läuft, sodass die beiden niemals um Ressourcen konkurrieren. Das hält die App reaktionsfähig — Sie sehen eine Karte / ein Ergebnis / eine geöffnete Notiz / eine Zeile / einen Tooltip / eine Digest-Zeile möglicherweise kurz, bevor ihre Zusammenfassung erscheint, dann taucht die Zusammenfassung einen Moment später auf.
