...
Tragen ältere Vorlagen noch einen solchen Stempel, zeigt Constellation nach dem Start einen Hinweis mit der Schaltfläche **Zu reparierende Vorlagen prüfen** (auch unter Einstellungen → Universum & Bibliotheken → Vorlagen). Die Prüfung listet jede betroffene Datei auf, zeigt an einem Beispiel genau die beiden Zeilen, die sich ändern, und schreibt nichts, bis Sie auf **Diese … Vorlagen reparieren** klicken. Von jeder Datei wird zuvor eine Sicherung angelegt, und die Reparatur verweigert den Start, solange eine der aufgelisteten Dateien in einem Tab geöffnet ist — schließen Sie diesen Tab hinter dem Dialog und klicken Sie dann auf **Ich habe sie geschlossen — erneut versuchen**. Während eine Reparatur läuft, lässt sich der Dialog absichtlich nicht schließen.
