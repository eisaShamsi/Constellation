...
Tragen ältere Vorlagen noch einen solchen Stempel, zeigt Constellation nach dem Start einen Hinweis mit der Schaltfläche **Zu reparierende Vorlagen prüfen** (auch unter Einstellungen → Universum & Bibliotheken → Vorlagen). Die Prüfung listet jede betroffene Datei auf, zeigt an einem Beispiel genau die beiden Zeilen, die sich ändern, und schreibt nichts, bis Sie auf **Diese … Vorlagen reparieren** klicken. Von jeder Datei, die tatsächlich verändert wird, wird zuvor eine Sicherung angelegt, und die Reparatur verweigert den Start, solange eine der aufgelisteten Dateien in einem Tab geöffnet ist — schließen Sie diesen Tab hinter dem Dialog und klicken Sie dann auf **Ich habe sie geschlossen — erneut versuchen**. Während eine Reparatur läuft, lässt sich der Dialog absichtlich nicht schließen.

Zum Abschluss legt ein Bildschirm mit der Überschrift **Was die Reparatur bewirkt hat** Rechenschaft über jede Datei ab — ein schlichter Satz je Art von Ergebnis, und kein Satz für ein Ergebnis, das nicht eingetreten ist: *repariert*, *behalten* (es war nichts zu tun: bereits repariert, seit der Prüfung bearbeitet oder nicht mehr vorhanden), *konnte nicht repariert werden* (der jeweilige Grund steht unter jeder Datei) und *nicht mehr erreicht* (die Reparatur hat vorher angehalten; dort wurde nichts verändert). **Eine behaltene Datei gilt niemals als Fehlschlag und hält die Reparatur niemals an.** Hat die Reparatur vorzeitig angehalten, nennt eine Zeile den Grund, und eine weitere weist Sie darauf hin, dass Sie sie jederzeit erneut über Einstellungen → Universum & Bibliotheken ausführen können.
