...
**Version 2.10 | 2026-09-01 — DRAIN CYCLE, first two Boss-validated closes.** **PJ-454's GUARD HALF CLOSED** (the Two-Signal Choke Point, Boss-tested on all three cases — template in-folder, self-declared template out-of-folder, ordinary note) and **PJ-455 CLOSED** (a tree-refresh bug the Boss found mid-test: newly created templates now appear without a reload; plus the outside-universe folder warning he ruled). **THE COUNT IS 43, not 102** — 102/67/0 all refuted; the repair engine is built + tested (`mold_repair.rs`) but has NO UI door yet. Two decisions queued with briefs, NOT started: the **107-file second wave** (approve by enumeration, never by rule) and the **18 GB duplicate** inside Eisa Universe. New follow-ups filed: **PJ-456…PJ-459**.
