...
## 2a. WA#5 cross-check against proven methods (Boss-required before approval, 2026-06-17)

Verdict: **the design is the textbook pattern; no change.** The cross-check sharpened the framing into a *split* that the field consistently uses.

- **The correct split — aggregate vs access-path.** Industry guidance on *materialized view vs index*: a **materialized view** earns its keep when the read query is an expensive **aggregate/multi-join over large data** at high read-rate; an **index** is the right tool to "quickly search records based on columns." It is "moving work from read time to write/refresh time — and that shift adds overhead to every write path." → Constellation's **COUNT** (a `COUNT(DISTINCT)` aggregate) is correctly **materialized write-time** (§C.2a `incoming_count`). The **ROWS** are a bounded **indexed access path** (one note's neighbors), not an aggregate → a **read-time indexed query is the correct tool**, NOT a materialized row-cache. Materializing rows would duplicate edge data per-target AND tax the save path — the wrong trade for a system whose keystroke/save latency is sacred (Rule 1). The read-time query touches NO write path. ([Materialized Views vs Indexing](https://medium.com/@jaywang.recsys/materialized-views-vs-indexing-to-improve-query-performance-by-b4935414a7cb), [Real tradeoffs of materialized views](https://technori.com/news/materialized-views-production-tradeoffs/))
- **IR / inverted-index theory (the cleanest model).** A note's backlinks == the **posting list** for that note's name in the inverted index `target_name → source_paths`. Lucene/IR **retrieve a term's posting list on demand** from the index; holding **all** postings in RAM is "infeasible for large datasets." `idx_nl_tnl` IS that inverted index; the per-note query retrieves one posting list. Constellation's CURRENT 234k-edge JS array IS the in-memory-everything anti-pattern. ([Lucene inverted index](https://j.blaszyk.me/tech-blog/exploring-apache-lucene-index/), [In-memory index scalability](https://shbakram.github.io/assets/papers/ismm2023-lucene.pdf))
- **SQLite-as-graph practice.** "Querying neighbors per node index is the preferred approach over loading the entire graph"; depth-1 neighbor lookups are **millisecond-fast with proper indexes** in the tens-of-thousands-of-nodes range (Constellation: 7,653 notes / 234k edges — comfortably inside). ([SQLite as a graph DB](https://dev.to/rohansx/sqlite-as-a-graph-database-recursive-ctes-semantic-search-and-why-we-ditched-neo4j-1ai), [A+ Indexes adjacency lists](https://arxiv.org/pdf/2004.00130))
- **PKM precedent.** Obsidian's NATIVE backlinks "iterate through every cached file and its links" (the in-memory-filter-everything Constellation uses) — "fast enough" for TYPICAL vaults but it degrades at scale, which is why the **Backlink Cache** plugin exists for large vaults; Logseq's in-memory DataScript graph hit 15–20 s page delays on large graphs and its **DB Version moved to persistent SQLite**. The field is converging on indexed/persistent retrieval, away from hold-it-all-in-memory. ([Obsidian Backlink Cache](https://github.com/mnaoumov/obsidian-backlink-cache), [Logseq query/perf](https://github.com/logseq/logseq/issues/4285))

**Net:** keep COUNT write-time (§C.2a), make ROWS a read-time indexed per-note query (§C.2c), virtualize the render. This is the dominant pattern on every axis checked — and it's the one that leaves the write/keystroke path untouched.
