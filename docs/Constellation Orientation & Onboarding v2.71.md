...
>   - *(2026-06-11 afternoon — all four follow-ups SHIPPED on Eisa's "Pass. Proceed with the follow-ups")*: **FU-2+FU-4** (`57f230c4`) — the ONE null-type definition now lives in `link_types.rs::is_null_type` + `linkTypeRegistry.ts::isNullLinkType` (mirrored, both call sites re-pointed) and `diversivity` is dropped from the centrality payload (computed-never-read); **FU-3** (`aa8626bf`) — the Style Setter gains a **CNS category** (Background live on the well root; hover-label bg/text, mount-read → apply on next CNS open) with a mini gravity-well preview; **FU-1** — the CNS help topic re-translated ×14 (the stale 2026-05-16 mirrors overwritten) **and the CCS topic translated ×14 for the first time** (28 files, locale-verbatim UI vocabulary from each `{lang}.json`, `translation_status: AI-generated 2026-06-11`). The four "noted follow-ups" above are all closed; `styleSetter.labels` for the 3 new FU-3 controls ride the built-in English fallback until the next labels batch.
