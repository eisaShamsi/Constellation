...
>   - *(2026-06-11 afternoon — all four follow-ups SHIPPED on Eisa's "Pass. Proceed with the follow-ups")*: **FU-2+FU-4** (`57f230c4`) — the ONE null-type definition now lives in `link_types.rs::is_null_type` + `linkTypeRegistry.ts::isNullLinkType` (mirrored, both call sites re-pointed) and `diversivity` is dropped from the centrality payload (computed-never-read); **FU-3** (`aa8626bf`) — the Style Setter gains a **CNS category** (Background live on the well root; hover-label bg/text, mount-read → apply on next CNS open) with a mini gravity-well preview; **FU-1** — the CNS help topic re-translated ×14 (the stale 2026-05-16 mirrors overwritten) **and the CCS topic translated ×14 for the first time** (28 files, locale-verbatim UI vocabulary from each `{lang}.json`, `translation_status: AI-generated 2026-06-11`). The four "noted follow-ups" above are all closed. *(FU-3 corrected same afternoon — Eisa's screenshot caught two defects: raw `styleSetter.labels.*` keys rendered (the `L()` fallback never fired — svelte-i18n echoes the key itself on a miss, which is truthy, so `|| en` was dead for ANY unkeyed label) and **no mini gravity-well** (every category except editor/sky is two-zone = no centre stage, so the FU-3 preview branch was unreachable). Fix: `L()` made miss-robust (key-echo = miss → English fallback — protects every future label), **CNS joined the three-zone preview set** (same MIG-072 §2 rationale as Sky View, plus the harder one: the hover-label vars are mount-read, so ONLY the mini-well can preview them live), and the 4 labels keys added **natively ×15** (ar chip = Eisa's **الجهاز العصبي للمعرفة**) — gated insertion, leaf-delta +4 each. Note the model shift: on the CNS category the Setter no longer live-pushes to the real app (three-zone clears the live layer); the mini-well IS the live preview, **Keep** applies the background to the real well immediately, labels on next CNS open. Mid-re-test Eisa dictated a new standing rule — **the Style Setter Preview Rule** (now in CLAUDE.md): *"Taking advantage of the entire center zone. Not to squeeze any element mimicry in a tiny box."* — the well preview was resized to fill the stage (`--cns` card modifier = the `--sky` pattern; the well `flex: 1`; rings 28/56/84% of its height).)*
