...
>
> **How job IDs work.** Each job has a stable `PJ-NNN` identifier. Renumbering is forbidden. Jobs that close move to the **Done** section keeping their ID. Jobs that split into multiple smaller jobs keep their ID and gain `PJ-NNNa / PJ-NNNb / …` siblings.
...
|---|---|---|---|---|
| PJ-007 | Note-stage taxonomy decision (Zettelkasten vs Living Link) | **Open** | P1 | Boss design call |
| PJ-006 | Living Link Architecture P2–P5 implementation | **Open** | P1 | Multi-MIG |
...
### PJ-007 — Note-stage taxonomy decision (Zettelkasten vs Living Link)

**Status.** Open · **Severity.** P1 · **Effort.** Boss design call (then small migration)

Two stage taxonomies coexist in the codebase. **Zettelkasten** (4 stages: `fleeting / literature / permanent / synthesis`) used by `PropertyEditor.svelte:481-484`. **Living Link lifecycle** (6 stages: `spark / birth / growth / maturity / dormancy / archival`) used by `LinkStage` in `store.ts:1521`. Boss's "A Trip to the Moon" note has `stage: birth` — empirical signal that lifecycle vocabulary feels natural on Notes too. The breadcrumb i18n labels for all 6 lifecycle stages were quick-fixed in commit `c95a0e6`; the deeper architecture question stays open.

Three paths:

- **Path A — Notes only use Zettelkasten stages.** Reject Living Link values on Notes; treat Boss's `stage: birth` as data anomaly to be scrubbed.
- **Path B — Notes can use both taxonomies.** PropertyEditor offers all 10 with visual grouping. Document the choice in User Manual.
- **Path C — Unify on Living Link lifecycle.** Pick the Constellation-native taxonomy and apply it consistently. Migrate existing Zettelkasten values to closest lifecycle equivalents.

Recommendation in the memory entry: **Path C**, because CLAUDE.md frames Constellation around "Living Link Architecture" as core, six lifecycle values give more granularity, and Boss is already empirically using lifecycle vocab on a Note. But this is a Boss design call.

**Source.** `project_note_stage_taxonomy_decision.md`.

**Acceptance.** Boss picks A / B / C. Whichever path lands as a focused MIG: PropertyEditor scope updated, frontmatter migration utility (if Path C), User Manual section, possibly a Settings preference (if Path B).
...
1. **Adding a job.** Append to the appropriate `§N` section with the next available `PJ-NNN` ID. Bump the version. Commit + push as a new file (`v1.1.md` etc.).
2. **Updating a job.** Edit the existing entry (status, severity, source, acceptance). Bump the version if the change is structural; same version if just refining wording.
3. **Closing a job.** Move the entry to `§7 Done`, strikethrough the title, append closing date and commit hash. Bump the version.
4. **Splitting a job.** Keep the original ID; add `PJ-NNNa`, `PJ-NNNb`, etc. as siblings. Cross-reference in both directions.
5. **Renumbering.** Forbidden. Stable IDs let session logs cite jobs durably.
6. **Filename convention.** Same as orientation + Laws docs. New version = new file alongside the previous. Older versions stay as historical record.
