...
| 7 | Review Pulse | `review.rs` | ✅ Write-time (MIG-083) — `review_schedule` table maintained in `index_note`, read via `get_due_notes` (Mode 1/3 SELECT ∪ Mode-2 staleness JOIN); `.constellation/review-pulse.json` is the durable action source (File-Over-App); reconcile self-heals |
