...
- **Fix all 3 quirks IN the swap** (so the new behavior is the *corrected* one): (a) **stratum** = the note's real maturity (not the faked `2u8`); (b) **interval ladder** = the documented **1 → 3 → 7 → 14 → 30** (cap 30), not `1·2·4·8·16·30`; (c) the **`stale`** reason is emitted. The rehearsal harness therefore asserts the indexed read == the **corrected** reference computation (not parity with today's buggy scan).
...
- The action writers `mark_reviewed`/`snooze_note`/`dismiss_note`/`record_note_visit`: each mutates `review-pulse.json` (source) **and** upserts/deletes the note's `review_schedule` row, using the corrected ladder.
**Verify (one commit):** `cargo test` — saving a note upserts the correct row; `mark_reviewed` advances `due_days` by the corrected ladder + updates the row; `dismiss` drops the row; a body-only edit is a no-op delta. `svelte-check` 0; build. **Reads still use the old scan** (not yet stamped) → no user-visible change yet. Typing-latency unchanged (type-burst test).
...
- Swap `get_due_notes`: when `review` is stamped → `SELECT … FROM review_schedule WHERE due_days <= today AND reason != dismissed ORDER BY stratum DESC, days_overdue DESC` (compute `days_overdue` at read; never an `is_due` flag); else → the legacy scan (safety). Keep the `DueNote` shape (`note_name` joined from `note_meta`).
...
With the cheap schedule live: §F builds the note-context **Review tab** (`get_note_review_status`, O(1)) + adds `'review'` to `NOTE_SCOPED_TABS`; the new **full-page reviewer** (`ReviewerView`) over the now-cheap `get_due_notes`; and the `record_note_visit → openNoteTab` wiring (so opening a note counts as visiting it). Then §G closes MIG-080.
