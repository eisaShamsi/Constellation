...
| **MIG-076** Write Integrity — "Single Write Authority" (the stop-everything response to BUG-023; kills the 4-incident cross-identity write class once and forever) | `lab/reports/MIG-076-WRITE-INTEGRITY-{ARCHITECT,PLAN}.md` | 🟢 **IN FLIGHT — §A+§B+§C+§D+§E-1 SHIPPED + Boss-validated; §E-2 next, §F to close.** The WriteGate (`write_gate.rs`): L0 per-path serialization + L1 atomic ReplaceFileW replace + the `write-journal.jsonl`; ALL ~24 Rust note-writers routed (creates = create-exclusive); L2 identity+freshness CAS in SHADOW (`WRITE_GATE_ENFORCE=false` until §F1); **self-attestation** (incoming cid_cn vs disk, zero frontend plumbing). **§C single content ownership** (`noteModel`/`noteSession`/`SINGLE_OWNERSHIP`) — the content-integrity bug class structurally closed (Boss 5/5; `milestone/mig-076-single-ownership`). **§D quiesced rename + title-rename re-land** + the 6× embed-lock perf win (rename-after-edit 10.6s→1.77s; lock scoped off `run_embedding` inference); the §13 title-rename gap closed. **§E-1 the universe-wide name-collision dialog** (`CollisionDialog.svelte`; create-typed-name + rename → Change name / Overwrite-to-trash / Cancel; `resolveWikilinkCrossLibrary`, cUniverse-inclusive; Quick Capture + folders exempt) + the **trash de-collide fix** (`move_to_trash` suffixes on a `.trash` name clash — never clobber); Boss Stage 1+2 PASS, disk+content proven (renamed note kept its own cid + old name as alias; displaced original intact in `.trash`). Eisa rulings: speed rider; collision = conventional (§E1b); scope §E = collision (done) + diagnostics (§E-2). **Remaining: §E-2** write-integrity diagnostics line (journal-read IPC + Settings → Debug readout) · **§F** enforce flip + refusal/quarantine/freshness recovery UX + permanent regression suite + 3-agent audit + `/simplify` + close. Two trash-setting gaps logged (delete_item ignores `trashDestination` — always permanent; Overwrite always per-library `.trash`). Tests: harness 28/28; gate 14 + suite 923/923. Docs: v2.72–v2.76 preambles, `SESSION-LOG-2026-06-11.md`. |
