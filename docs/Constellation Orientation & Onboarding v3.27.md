...
**Version 3.27 | 2026-07-06**

> **What changed in v3.27** (**MIG-093 — the Quick Switcher, the pure title jumper. The Boss reproduced Ctrl+O's 2+ minute, thrashing result retrieval on an instrumented build (Reproduce-First): the switcher's embedded content search ran FTS5 native `snippet()` over EVERY matching row per keystroke-pause (~20-27s each), stale runs piled up on the db lock, and the wanted TITLE hits were buried by an unranked, path-matching, cap-20 local filter. Boss ruling: *"Ctrl+O should focus only on the titles"* + a research directive. Shipped: title+alias-only ranked switcher (~2ms over 9,161 federated candidates, Boss-measured) + the two-phase engine fix for BOTH search commands. Boss-validated end-to-end.** Ultracode):
> - **MIG-093 shipped (§D → §A → §B → §C → §D-2 → §E).** Research first (workflow `wf_7786efda-5db`: Obsidian names+aliases-only · VS Code banded fuzzyScorer · fzf/fzy subsequence scoring · peers always demote content): **§D/§D-2** the two-phase rank-then-fetch rewrite of `lexical_search_in_schema` AND the universal command's `search_contents` — FTS5 native `snippet()` is GONE from every production path (per-row cost bounded by LIMIT, never match count); `include_snippet` honored end-to-end (was declared, never read). **§A** `read_notes` federated (`read_notes_in_schema` over `get_federated_schemas`; `federation:ready` re-fetches the core snapshot) — `allNotes` spans the ONE universe (~9.1k titles incl. cUniverses; was main-only, a ruling violation). **§B** `$lib/searchFold.ts` (`foldForMatch` + `stemArabicLight10`, Rust-parity documented; IndexPanel's 55-line inline copy re-pointed). **§C** `$lib/switcherRank.ts` (banded: exact 1<<18 > prefix 1<<17 > word-boundary 1<<16 > fzf-style fuzzy; recency/compactness/shorter/collator tie-breaks; alias rows with within-band penalty; 1-2-char fuzzy skip; multi-word all-match) + `QuickSwitcher.svelte` rewritten — titles+aliases only, folded once per cache refresh, ZERO IPC per keystroke (MIG-058's debounced-$state kept at 100ms + composing guard), empty query = recents, pinned **Create-note** + **Search-in-Search-Hub** rows. The embedded content search DELETED from the switcher. Pinned test: query `islam` ranks the exact title "Islam" #1. **§E** instrumentation removed; `quickSwitcher.recent/createNote/searchInHub` ×15 locales.
> - **Boss-measured before/after:** `islam` 25.0s + heavy thrashing → **2.7ms over 9,161 candidates**; `knowledge` 29.3s → **1.6ms**; the Search Hub content search ~25s → "much better," calm (Boss pass). Docs `docs/MIG-093-Architect-Plan-Quick-Switcher.md`.
> - **QUEUED (ranked):** (1) **PJ-069** whole-entity dedup; (2) **PJ-068** SS rework (parked); plus the older freeze queue (L′/F1/F3), `get_360_view` index-read `/migration`, `init_db` profiling, MIG-088 Phases 6–10, the Arabic callout End/Home caret.
>
> ---

**Version 3.26 | 2026-07-06**

> **What changed in v3.26** (**MIG-092 — Collections on the Search Hub. The parked MIG-090 Workbench's durable-basket machinery, repurposed (Boss-approved) into a "Collections" feature: search → hand-pick results → a named, persistent, task-scoped basket. Boss rulings: UNIFY the former Bookmarks in (Starred = a pinned collection); a "Results | Collections" header tab inside the Search Hub; the "done" concept dropped (a note is in a collection or you ✕ it out). Boss-validated over two staged tests + several UX fixes. Committed + pushed to `main`.** Ultracode):
> - **MIG-092 shipped (~11 commits + close-out).** `collections.rs` (`collections_hydrate`, note-only) · `universe.rs` (`read/save_universe_collections`; a legacy `workbench.json` is migrated-then-retained; `read_universe_bookmarks` kept READ-ONLY for the one-time migration) · pure `collectionsLogic.ts` reducers (unit-tested) + a thin `store.ts` shell (`collectionSets`; `loadCollections` runs the idempotent Bookmarks→Starred migration; `hydrateCollectionNotes`) · `CollectionsPanel.svelte` (switch/create/rename/delete, remove members, four state chips via `collectionChips.ts`, debounced liveness) · `SearchHub.svelte` (Results | Collections tabs) · `+layout.svelte` ("Add to collection ▸" submenu on file-tree + search results; the sidebar "Bookmarks" section now renders the **Starred** collection with per-title-RTL rows + a right-click note menu; ⭐ → `toggleStarred`). **RETIRED:** `WorkbenchView`, the `enabledFeatures.workbench` flag + its Settings toggle, the vestigial `bookmarks` store, `save_universe_bookmarks`. Shared `NoteRow` gained per-title direction (Language-First). Docs `docs/MIG-092-Collections-Architect.md` + `…-Plan.md`.
> - **The unify story (whole-entity, zero-duplication).** The adversarial dedup pass caught an existing **Bookmarks** feature absent from BOTH the Boss's dedup list AND the v2 whole-entity map. Boss ruled **UNIFY** → Collections is now the ONE hand-picked-set mechanism; Bookmarks = the pinned "Starred" collection (two mounts: the Search-Hub tab + the sidebar shortcut). Gap-defined vs Search (query, transient) · Bases (dynamic view) · File Explorer (hierarchy). This REDUCES the entity's surface count — the concept-paper §3 ideal.
> - **Bookmarks sidebar reworked (Boss findings, staged tests).** Flat list; each row shows its location breadcrumb **cUniverse / library / folder** on the reading-END side (no repeated per-row library label); per-title row direction (RTL title → fully-RTL row); a right-click note menu (Open / Open-in-new-tab / Reveal / **Remove bookmark** / Add-to-collection ▸ / Copy).
> - **QUEUED (ranked):** (1) **Quick Switcher speed** (reproduce-first — Ctrl+O already reads the in-memory cache; measure the delay); (2) **PJ-069** whole-entity dedup; **PJ-068** SS rework (parked); plus the older freeze queue (L′/F1/F3), `get_360_view` index-read `/migration`, `init_db` profiling, MIG-088 Phases 6–10, the Arabic callout End/Home caret.
>
> ---

**Version 3.25 | 2026-07-05**

> **What changed in v3.25** (**A very long day. F2′ (app-created notes now surface in the file tree via a `note-created` event) → the dead Shift+digit shortcut class fixed → **Batch W** (11 panel fs-walk/writer commands off the dispatch thread) → the tag-click "127 counted, 0 listed" one-authority fix → cross-library **embeds** + the **ONE-universe resolution ruling** → **Dataview removed entirely** (a week-3 vestige, never a real function) → the **Notes Navigator saga**: the All-Notes-Base direction built then REVOKED, a concept-led rewrite (Workbench built, then also revoked) that converged on the Boss's real answer — **empower the File Explorer** — shipped as **MIG-091** (filter · created/size sort · multi-select · batch bar) with the old two-pane Navigator RETIRED; plus the **cid_cn partial-index** data-loss fix. All Boss-validated. Committed + pushed to `main`.** Ultracode):
> - **The Notes Navigator is RESOLVED (MIG-090 → MIG-091).** Two false starts taught the lesson the hard way: a notes TABLE is what **Bases** are (All-Notes-Base direction, revoked); a grammar-free semantic finder is what **Search Hub** is (the Workbench Intent Bar, revoked). The Boss's ruling: *"a file-tree management system should be limited to the file system."* → **MIG-091: empower the File Explorer** with the Navigator's keepable file-management muscle, and RETIRE the old two-pane Navigator. Shipped + Boss-validated: (A) name **filter** (searches ALL libraries — reveals+loads collapsed ones, restores on clear; names only, never content); (A) **sort** += created + size (8-state cycle; `FileEntry` gained `created`/`size` from the same metadata read); (B) **multi-select** (Ctrl/⌘-click toggle, Shift-click range, Escape clears; files + folders); (C) the **batch bar** (Move / Add-tag / Delete, each LOOPING the existing gated single-item handler — `moveItem`/`deleteWithSetting`/`addTagToNote` — NO YAML splice; federated read-only members excluded); (D virtualize DEFERRED — Reproduce-First, no slowness). **RETIRED:** the sidebar 'list' mode + SS 'list' branch, `enabledFeatures.notesNavigator`, the 5-component chain + the dead `navigator/data.svelte.ts` fossil, `collect_library_notes_with_metadata` + `search_by_property` (Rust + regs), `handleListNoteContextMenu`, orphaned `navigator.*` i18n. Shared `NoteRow`/`NoteList` primitive introduced (PJ-069 seed). Docs `docs/MIG-091-Architect-Plan-File-Explorer.md`.
> - **Two PJs filed from the saga.** **PJ-068** (Pending Jobs v1.15): the Second-Screen **contextual-companion rework** — the SS must be CONTEXTUAL + never REPLICATE the main screen (Boss ruling); concept paper `docs/concept-papers/PJ-068-...`; per-mode retire/redesign rulings at reopen (PARKED). **PJ-069** (Pending Jobs v1.16): the **whole-entity deduplication pass** — the map found tags×6, folders×4, recents×3, orphan/fragile×5, hubs 3-way vs CNS's one-home claim, note-lists hand-rolled ×9. Both Concept-Paper-first.
> - **The Workbench (MIG-090) is PARKED → repurpose to Collections.** Built end-to-end (workbench.json + `workbench_hydrate` + chips + the shared primitive, flag-off), then the Boss ruled it "another search engine." **Approved disposition: fold the durable-basket machinery into Search Hub as "Collections"** (search → hand-pick results → a persistent task-scoped basket with live state); the Intent-Bar-as-search + standalone page get deleted. QUEUED.
> - **ONE-universe resolution (Boss ruling, load-bearing).** *"It is ONE universe, regardless of the libraries it contains."* Every name resolver — embeds, wikilinks, pickers, any future one — spans the WHOLE universe INCLUDING federated child-universes (via `resolve_universe_libraries`). Fixed for embeds (`resolve_embed` cross-library fallback); never a single-library scope again.
> - **cid_cn UNIQUE-collision fix (pre-existing data-loss, Boss-flagged, reproduced red→green).** Externally-created notes index with `cid_cn=''`; the FULL unique index + path-only upsert conflict made the SECOND such note fail (SQLITE_CONSTRAINT_UNIQUE) and stay INVISIBLE to search. Fix: the index is now **PARTIAL** (`WHERE cid_cn != ''`) — `''` is a sentinel, not an identity — **self-migrating** (init_db drops+rebuilds a legacy FULL variant). RULE banked: any parameterized cid lookup needs `AND cid_cn != ''` to use the partial index.
> - **Dataview REMOVED.** Boss: *"We don't have Dataview!"* — verified: week-3 code (`48e4a073`), no concept, no toggle, ZERO manual mention; only rendered pasted Obsidian dataview code-blocks in previews. 1,090 lines deleted; such blocks now render as ordinary code blocks. Constellation's query surfaces are Bases + Search.
> - **Batch W + the tag/embed fixes (Boss-validated).** 11 fs-walk/writer commands → `(async)` with stale-result seq guards + two Rust single-flight locks (embeds BUILD_LOCK, cece RELIABILITY_LOCK). Tag-click: THREE tag definitions coexisted (chips counted from the index; the click fetched via an inline-`#tag`-only fs regex → 127 vs 0; the block-list YAML arm didn't strip quotes). Fixed: `search::parse_frontmatter` is THE single tag authority; `notes_by_tag`/`scan_library_tags` delegate to it. Independent tag-column scrolling shipped.
> - **F2′ + the dead-shortcut fix (Boss-validated).** All frontend note-creation funnels through the store's `createNote`, which now emits `note-created`; +layout coalesces the tree refresh; second-screen listens too (cross-window). And EVERY Shift+digit shortcut was dead since birth (the matcher normalized letters but not `Digit*` codes) → `Ctrl+Shift+2` (second screen) works for the first time.
> - **QUEUED (ranked):** (1) **Workbench → Collections on Search Hub** (approved); (2) **Quick Switcher speed** (reproduce-first — Ctrl+O already reads the in-memory cache; measure the delay); (3) **PJ-069** whole-entity dedup; **PJ-068** SS rework (parked). Plus the older freeze queue (L′/F1/F3), `get_360_view` index-read `/migration`, `init_db` profiling, MIG-088 Phases 6–10, the Arabic callout End/Home caret.
>
> ---
>
> *Below: the v3.24 preamble, retained for diff visibility.*



> **What changed in v3.24** (**A marathon day: BATCH-2 — the note-file-writing command family migrated to async behind new locked write-gate primitives (§B2-1..B2-5), the hardest arc (3 rename-bug rounds, journal-forensic diagnosis); the BASE table gained right-click + search + a multi-script letter rail; and a full WIDE-APP FREEZE AUDIT (5 agents, 174 cmds × 7 park mechanisms) → the universe-switch/boot freeze fixed (Batch S/D). All Boss-validated. Committed to `main`.** Ultracode):
> - **The freeze program, whole picture.** The felt "app freeze" is one class: a SYNC `#[tauri::command]` on the single WebView2 IPC dispatch thread waiting on a slow thing (DB writer lock held by a background reindex; the 20–40 s cold `init_db`; a multi-second fs walk). Cure = `#[tauri::command(async)]` (off the dispatch thread), proven across ~50 commands today in three sweeps: **Batch-1** (24 user-action cmds + 9 stale-result seq guards, v3.23), **Batch-2** (note-file writers, below), **Batch S/D** (switch/boot path). **Canonical rule (now near-universal): any new `#[tauri::command]` touching `state.db`/`init_lock`/a long walk MUST be `(async)`; any frontend caller of an async command that writes shared state on resolve needs a stale-result seq/gen guard.**
> - **Batch-2 — note-file-writing family (the hard arc).** §B2-1: new **write-gate primitives** — `gate_rmw(path, surface, closure)` / `gate_rmw_rename` / `gate_delete` / `with_path_lock` — hold the per-path lock across the WHOLE read-modify-write (or destructive) cycle: the explicit replacement for the serialization SYNC dispatch gave for free. **Two hard caller rules:** no nested gate_* on the same path (non-reentrant Mutex), and NO `state.db` wait inside a path lock (would re-freeze the dispatch thread through the back door). 8 concurrency `#[test]`s prove lost-update/deadlock/exclusion. §B2-2: 7 async-only-safe flips (link confidence/archive, sources rejects, classifier_suggest — ONNX off the IPC thread). §B2-3: toggle_task/update_note_property/sources_set_manual/content_type/cece_resolve/resolve_structural_conflict → gate_rmw + `(async)` + frontend flush-first wrappers (the toggleTaskReconciled recipe). §B2-4: the rename/cascade family — `rename_item` → `gate_rmw_rename` (dual-locked read→title-rewrite→write→rename as ONE section); `update_links_on_rename` per-file gate_rmw; `delete_path` gate_delete; **`delete_item` RETIRED** (Boss-ruled — zero callers, superseded by delete_path); flush-before-rename in the shared `renameItem` wrapper. §B2-5: `set_active_universe` gains `switch_lock` + the epoch-capture hole in `ensure_search_db_ready` closed + `(async)`.
> - **THE RENAME BUG (3 Boss-test rounds, journal-forensic).** R1: a FileTree Enter+blur **double-commit** ran two concurrent rename flows once rename went async → `renamesInFlight` reentrancy guard. R2 (single-fire, same symptom): the invoke **NEVER SETTLED** — `rename_item`'s DB tail **parked on the unbounded writer mutex**, invisible once async → the whole post-rename orchestration (tab/tree/cascade) silently never ran → fixed by detaching the DB tail to `spawn_blocking` (`rename_item_db_tail`); the IPC settles the moment the FILE state is final. **Permanent forensic journal markers** added (`rename_return`/`cascade_enter` + `journal_frontend_marker` capturing swallowed-exception text release-visibly). R4 PASS (258 ms in the failing config). **Lesson: lifecycle/content-integrity bugs are journal-forensic, not static-analysis — reproduce, read the trace, fix the pinned mechanism.**
> - **Base table (Boss request):** rows gained the MIG-077 B2 **SAFE note right-click** subset (open/bookmark/copy/reveal/style — no rename/move/delete from a non-refreshing list; shared builder, zero new menu i18n); an in-header **search** (in-memory filter over name + rendered cell text, zero IPC per keystroke; virtualizer windows the filtered list); a **letter rail** whose letters are **harvested from the data** (Latin + Arabic أ ب ت… + any script — Boss-validated both showing), collator-ordered, RTL-aware, name-sorts-then-jumps. 3 new `lensBlock` keys ×15.
> - **Wide-app freeze audit (5 agents, verifier: nothing material refuted):** 174 sync commands × 7 park mechanisms → 9 HIGH / 26 MED / 97 LOW / 42 dormant. **Batch S** (18 switch/boot ensure-callers + slow boot walks → async — the reproduced universe-switch freeze) + **Batch D** (3 spawners: `ensure_search_db_ready` moved INSIDE the worker — kills the classifier boot+5 s auto-fire landmine) shipped. **R2 measured + REFUTED:** `set_active_universe` is ~5 ms (instrumented then removed) — the switch command is NOT the lag; the residual is switch-back-before-the-departing-universe-settles (transient, own pass). **Honest unfixable-by-async:** the 20–40 s cold `init_db` itself (now background; panels fill when done) — maskable by a progress gate, or shrinkable only after a measured init_db phase-profiling pass (UNMEASURED — do not guess).
> - **Canonical facts (v3.24):** (1) write-gate has **locked-RMW/delete primitives** — every note-file command runs its whole cycle under the per-path lock; (2) `delete_item` RETIRED (use `delete_path`/`deleteWithSetting`); (3) the rename DB tail + cascade reindex loop are **detached** (`spawn_blocking`) → note_meta/note_links path rows trail a rename by the tail duration (eventual-consistency, heals on tail/next reindex); (4) permanent rename forensics write release-visible checkpoints to `write-journal.jsonl`; (5) `set_active_universe` is `(async)` + `switch_lock`-serialized + measured ~5 ms; (6) the Base tab has search + letter rail + safe RC.
> - **QUEUED (ranked — the audit's deferred batches):** **F2** (stop refreshing the tree on our own 1.5 s autosaves — create-path verification needed) · **W** (panel fs-walk sweep → async, 2 WA#4 writer walks) · **L** (importer family + `cache_full_links` federated-hold hygiene) · **F1** (**Focus mode saves per KEYSTROKE via sync write_note** — content-integrity class, ships ALONE through the Editor-Surface Gate harness) · **F3** (FileTree virtualization, CNS Louvain → Web Worker, sky 234k-parse off main thread, second-screen per-note reads). Plus: the `get_360_view` index-read `/migration`; the switch-back-fast residual; init_db profiling; MIG-088 Phases 6–10; the Arabic callout End/Home caret.
>
> ---
>
> *Below: the v3.23 preamble, retained for diff visibility.*

> **What changed in v3.23** (**The NOTE-OPEN FREEZE CLASS — root-mechanism mapped end-to-end and CLOSED for every everyday action: 24 SYNC writer-lock commands moved off the IPC dispatch thread + 9 stale-result guards + a review-pulse file lock. Boss-validated ×2 stages. The note-file-writing command family explicitly DEFERRED to a gated pass. Committed to `main`.** Ultracode):
> - **Boss arc:** "I didn't recall facing note-open freeze. Remind me!" → traced honestly: he never faced it under that name — he faced it as the v3.21 **"reset freeze"** (10 s→39 s). Then: **"Proceed with this issue. Fix it."**
> - **THE MECHANISM (4-agent discovery `wf_2d6ff3e5-edb`, all converging, verified):** note-OPEN itself is writer-lock-clean (ms; no reindex/embed/backfill fires on a plain open). The multi-second holder is the **SAVE-path flush**: leaving an edited note fires `constellation_search_reindex` (async, background) which holds the DB writer mutex **CONTINUOUSLY 2.5 s warm / 11 s cold**. The freeze = any of the **59 remaining SYNC writer-lock commands** dispatched during that window parking the single WebView2 IPC dispatch thread on the mutex → whole-app freeze. **The built-in collision:** *edit a note → click a wikilink* — the flush reindex and SYNC `constellation_link_traverse` fire at the same moment by construction.
> - **Batch 1 SHIPPED (caller-verification workflow `wf_7ba74bfc-7b7` ran the WA#4 per-caller ordering check first):** **24 × `#[tauri::command(async)]`** — search ×2 (every keystroke) · link_traverse (every wikilink click) · review reads+actions ×6 (+ a **`PULSE_LOCK`** static mutex: the three review actions do an unlocked whole-file RMW of `review-pulse.json`, safe only under sync dispatch — a genuine hazard the verification caught) · maturity/strata · CCS/KH snapshots · link_archived · ctse concept filter · sources reads ×2 · embed_text/texts (e5 inference off the IPC thread) · embedding_status · link_counts · resolve_wikilink (nav-path FS walk) · provenance chain (the un-migrated `get_360_view` sibling) · suggest_related_notes. Plus **stale-result seq guards ×9 files** (async completion can reorder): SearchHub, ConstellationMap, ConstellationSight2, GraphMindView, OrgChart, CatalogerView, CCSView, KnowledgeHealthDashboard, +layout (provenance gate + link-counts seq). 9 other callers verified already-guarded (QuickSwitcher/gen-tokens/`_srpLoadSeq`/`semanticFetchToken`/keyed writes).
> - **Boss test PASS ×2:** Stage 1 (the collision recipe — edit → wikilink hop ×4) = instant, app alive. Stage 2 (search typing + Review actions inside the busy window) = keystrokes instant, results match the final query, no freeze anywhere.
> - **Batch 2 — DEFERRED EXPLICITLY (not silently):** every command that **writes note files** — `rename_item`/`move_item`/`delete_item`/`delete_path`/`update_links_on_rename`, `toggle_task`, `update_note_property`, `resolve_structural_conflict`, `constellation_link_set_confidence`/`archive`/`unarchive`, sources/CECE accepts, `set_active_universe` — sync→async changes their ordering vs the debounced save = **content-integrity territory (BUG-023 / Editor-Surface Gate)** → own gated pass with the reproduction harness. They still freeze if clicked during a reindex window (rarer, deliberate actions).
> - **/simplify (SO #4): 1 fix applied** (CatalogerView redundant increment), rest clean ×4 (hand-rolled guards = repo convention; PULSE_LOCK = the `journal_lock()` idiom; per-command flips = Tauri's only mechanism, no global switch). **1 characterized risk banked (verified against Tauri 2.10.3 source, not guessed):** a sync-bodied `(async)` command runs via `async_runtime::spawn` on the **core Tokio pool** — parked lock-waiters can crowd it in a pathological window (bounded degradation: async features queue a few seconds; UI never freezes; self-heals on lock release; strictly better than parking the one UI thread). **This upgrades the priority of the queued `with_read_conn` routing pass** (reads stop touching the writer lock entirely — search results during a reindex become instant too).
> - **Canonical facts (v3.23):** (1) **the freeze class is CLOSED for everyday actions** — the felt "note-open freeze" was *SYNC-command-vs-held-writer-lock*, never a note-open job; (2) the save-path reindex still holds the writer mutex continuously 2.5–11 s (chunking it = queued follow-up); (3) **any NEW `#[tauri::command]` that touches `state.db` MUST be `(async)`** (and ideally read via `with_read_conn`) — the PJ-066 rule is now near-universal (45 async commands); (4) **any frontend caller of an async command that writes shared state on resolve needs a stale-result guard** (monotonic seq / gen token — the repo convention); (5) 15 dormant SYNC lock-takers have zero frontend call sites (cece history pair, map_data, link-stats family, sources/content_type CRUD…) — convert if revived; Sight Wings convert when re-enabled.
> - **NEXT:** the queued `with_read_conn` routing pass (now evidence-prioritized) · Batch 2 (note-file writers, gated) · the `get_360_view` index-read `/migration` · MIG-088 Phases 6–10 · Arabic callout End/Home caret.
>
> ---
>
> *Below: the v3.22 preamble, retained for diff visibility.*

> **What changed in v3.22** (**The deferred `get_360_view` note-open freeze — reproduced-first, measured, and FIXED via the SAFE tier (async + loading-state isolation); Boss-validated. The Rule-8 index-read rewrite consciously QUEUED as its own `/migration` (it moves verdicts). Committed to `main`.** Ultracode):
> - **The one real perf bug from v3.21 — CLOSED (safe tier).** Boss chose it over MIG-088 Phase 6. **Reproduce-First, measured not guessed:** a `#[test]` running the REAL `scan_all_notes` + `precompute_all_strata` over the worst-case library (`Eisa Cognitive Knowledge` → `History`, **549 md / 36 MB**) clocked **2.80 s cold / ~0.35 s warm**. So `get_360_view`'s ISOLATED cost is a real SYNC-on-IPC-thread freeze but **~2.8 s, NOT the "tens of seconds"** — that was the **note-open indexing** (separate issue) with the 360 scan stacked on top, and the 360 fetch fires **only when the 360 surface is visible** (`+layout.svelte:1433` — `(rightSidebarOpen && rightSidebarTab==='inspector360') || showInspector360`, debounced+seq-guarded). Confirms the v3.21 phantom-reset conclusion.
> - **Fix Step 1 — `get_360_view` → `#[tauri::command(async)]`** (`inspector360.rs:91`). One-token thread-offload (body has NO `.await`) — the ~2.8 s disk walk runs on a Tokio worker, the UI never freezes. Byte-identical output → **zero verdict change**. Same idiom as `constellation_embed_notes`/`scan_unlinked_mentions`/`save_universe_link_types`. The disk-walk COST is unchanged — only its thread.
> - **Fix Step 2 — settled-vs-loading isolation.** Because the fetch is now async, a note-switch could leave the PREVIOUS note's 360 data on screen for ~2.8 s. New `inspector360Loading` `$state` in `+layout.svelte`: the fetch `$effect` drops the prior data + shows a **"Loading 360° view…" spinner** on a new-note key, clears on settle. `Inspector360.svelte` got a `loading` prop + a loading branch (compact + full) + a self-contained `.i360-spinner` (theme-var colours, `prefers-reduced-motion` aware). i18n `inspector360.loading` ×15. Reactivity verified (Perf Rule 2): the new-note path short-circuits before reading `inspector360Data`, so the bare `= null` write is not a tracked dep → no `$effect` self-trigger.
> - **Design fan-out (`wf_f5388985-d2a`, 3 agents) — why the index-read rewrite was NOT done now.** (1) **Feasible:** the index already holds everything `get_360_view` re-derives (`note_meta.outgoing_links_json`/`incoming_count`/`outgoing_count`/`tags_json`/`word_count`/`body_text`/`properties_json`; `note_links` = indexed 234k-row graph) → the command can be O(neighbours), no disk walk. (2) **But NOT a pure re-plumb:** only the counts stay identical (already index-sourced, MIG-085); maturity/orphan likely-identical; **stratum / SPOF / provenance / stage / tags / gaps / trails = drift-risk** — from cross-library-vs-same-library scope, DISTINCT-vs-occurrence definitional split, and parser mismatches (has_external not stored as a bool; `extract_stage` quirks; 360 reads inline `#tags` only today). → it needs conscious per-verdict decisions = its **own `/migration`**. Shipping it silently would break WA#4.
> - **/simplify (SO #4):** 4 agents (reuse/simplification/efficiency/altitude) — **all CLEAN, shipped as-is.** Reuse: per-component spinner CSS is the convention (no shared Spinner util). Simplification: imperative `$state` is minimal (a `$derived` can't track async settlement + a non-reactive guard). Efficiency: no `$effect` self-trigger; spinner destroyed with the element. Altitude: async is the right depth, host-owned loading is the correct altitude (sibling of `data`).
> - **Canonical facts (v3.22 — supersede v3.21 fact #4):** (1) **`get_360_view` is now `#[tauri::command(async)]`** — the full-library disk re-walk (`scan_all_notes`) runs off the IPC thread, so the 360 panel no longer freezes the UI; it STILL re-walks disk (the Rule-8 write-time-derivation fix is queued as its own `/migration`). (2) The 360 panel now shows a **loading spinner** during the async fetch (`inspector360Loading`); it fires only when the 360 surface is visible. (3) The **note-open indexing freeze is a SEPARATE, still-open issue** — the 360 async fix removes only the 360 view's contribution. (4) Isolate settled-vs-loading FIRST for any async-ification (the resume's rule, honored here).
> - **NEXT:** the **`get_360_view` index-read rewrite** (its own `/migration`; Architect input = the verdict-parity findings above); the **note-open indexing freeze** (separate reproduce-first pass); MIG-088 Phases 6–10; the Arabic callout End/Home caret known-issue.
>
> ---
>
> *Below: the v3.21 preamble, retained for diff visibility.*

> **What changed in v3.21** (**The "8 typed-link type colours" backlog item — SO#8-verified ALREADY SHIPPED (MIG-067), marked obsolete; two real defects it surfaced FIXED; the Links "Reset this element" consolidated; and a multi-round "reset freeze" chase RESOLVED as a phantom (note-open indexing, not the reset). All Boss-validated. Committed to `main`.** Ultracode):
> - **"8 typed-link type colours" — NOT BUILT (SO#8 said already shipped).** A 6-agent discovery (`wf_3950f8af-08e`, adversarial verify) confirmed MIG-067's per-Universe **Link-Type Registry** (`linkTypeRegistry.ts` `SEED_DEFAULTS` + user deltas → `.constellation/link-types.json`, mirroring Rust `link_types::seeds()`) is the SINGLE colour authority; the user already recolours the 8 (+ custom types) via **Style Setter → Links** (`<LinkTypesEditor embedded />`), live-propagated by `subscribeLinkTypes`/`$linkTypesStore`. **Zero drift on any live surface** — editor inline (`livePreview` `typeDeco`; `SEED_COLORS` is a byte-identical boot-edge fallback), LinkTypePill, Inspector360 matrix, KH/CCS bars, GraphMind PIXI edges all resolve through `linkTypeColor()`. Building the planned CSS-var set would have created a SECOND competing authority → correctly declined.
> - **Two defects that discovery surfaced — FIXED (WA#6).** (1) **Inspector360 live-recolour:** its `TYPE_*` were setup-time consts → a §G recolour left the open matrix stale until remount. Now `TYPE_COLORS` is reactive (colours update live) while `TYPE_ORDER`/`TYPE_LABEL_KEYS` derive from a **primitive id-key** (`ids.join(',')`, memoized by value) so they recompute ONLY on a vocabulary change — the heavy `matrix` $derived does NOT re-run on a colour edit. (2) **Dead data removed:** `DEFAULT_SETTINGS.linkPills.fill/.text` (pre-MIG-067) had zero readers (every consumer reads `linkPills.shape`); dropped from defaults + the `AppSettings` interface.
> - **Links "Reset this element" — consolidated (Boss-validated).** The Links Style-Setter element showed TWO resets: the standard top-right "↺ Reset this element" (permanently greyed — Links is registry/settings-backed, no CSS vars) + the embedded editor's own "Reset colours" link. Boss ruling: **use the top-right one.** New `seedColorsDiffer()`/`resetSeedColors()` in the registry; StyleSetter `resetElement('links')` = whole-element reset (seed colours→defaults, toggles→on, pill shape→defaults) with a `linksDirty` $derived driving the enable-state; the editor's own link + CSS + the dead `settings.linkTypes.resetColours` i18n key (×15) removed.
> - **`save_universe_link_types` — async + vocabulary-change guard.** Now `#[tauri::command(async)]` (off the IPC dispatch thread) + only calls `on_link_vocabulary_changed` when `snapshot().fingerprint()` (FNV-1a over ORDERED IDS only) actually changes → a colour-only recolour/reset SKIPS the DB-writer-lock trigger recreation + backfill schedule entirely. A real perf improvement (recolours/resets are cheaper Rust-side), kept on its own merit.
> - **The "reset freeze" — RESOLVED as a PHANTOM.** Multi-round chase (10s→39s) of a reported freeze on the Links reset. 6+ SME agents ruled out EVERY reset-path node (async+guarded save, lockless list, once-tick O(8) fan-out, guarded no-op toggle reconfigure). Boss decisive isolation test — **reset on a SETTLED note (~60s after open) = instant** — confirmed the reset was never the culprit. The freeze was the **note-open background indexing** that the test instructions themselves kept triggering; the reset click landed inside it. **Reproduce-First lesson (violated twice): isolate settled-vs-loading FIRST; never ship a fix for an unreproduced mechanism.**
> - **REAL SEPARATE FINDING (flagged, deferred by Boss — NOT the reset):** opening a link-dense note freezes the UI for tens of seconds. Confirmed cause candidate: **`inspector360.rs::get_360_view` is SYNC and does a full-library filesystem re-scan (`scan_all_notes`) on every call** — the CLAUDE.md Rule 8 anti-pattern — plus the note-open embed/reindex. Its own reproduce-first investigation later (likely: make `get_360_view` async + read the index instead of re-walking disk).
> - **Canonical facts (v3.21):** (1) the Link-Type Registry (`linkTypeRegistry.ts`, MIG-067) is THE colour authority for all 8 cognitive types + custom types; recolour via Style Setter → Links; never add a second CSS-var colour set for these. (2) Inspector360's `TYPE_COLORS` is reactive to recolour; `TYPE_ORDER`/`TYPE_LABEL_KEYS` recompute only on vocabulary change (primitive-key memo pattern — the fix for "a $derived returning a new array every tick re-runs the heavy downstream $derived"). (3) `save_universe_link_types` is async + skips re-materialization on colour-only saves. (4) **`get_360_view` re-scans the whole library from disk (Rule-8 debt) → opening a 360 tab / link-dense note is heavy; a known deferred perf issue.** (5) reset "freeze" was note-open indexing, not the reset.
> - **NEXT:** MIG-088 Phases 6–10 (search/index badges · Sky/OrgChart/Map D3 colours · calendar · dialogs/global · audit); the deferred **`get_360_view` note-open freeze** (its own reproduce-first pass); the **Arabic callout End/Home caret** known-issue.
>
> ---
>
> *Below: the v3.20 preamble, retained for diff visibility.*

> **What changed in v3.20** (**MIG-088 Phase 4 CLOSED (§4c dialog-scrim opacity) + Phase 5 (right-sidebar panels) SHIPPED end-to-end — all six panel elements Boss-validated. Committed to `main`.** Ultracode):
> - **MIG-088 §4c — dialog-scrim opacity (Boss PASS).** New **Global → "Overlays" → "Dimmed opacity"** control (`--modal-overlay-alpha`, a `range` 0–100 `%`). A scrim is a see-through veil, so it's an **opacity**, not a colour (a colour picker can't express see-through). Discovery (`wf_04df3f6e-594`, 25 agents, adversarial mount-verify) reframed the plan's "4 backdrops" as **~20 live modal scrims in two groups**: an **11-dialog family already sharing** `--background-modifier-cover` (Settings/Rename/Move/Create/CommandPalette/QuickSwitcher/Workspace/3×Template/Confirm) + **9 hardcoded rogues**. **Boss ruling: FULL unification** — ONE `theme.css` redefinition (`--background-modifier-cover: rgba(0,0,0, var(--modal-overlay-alpha, 0.3/0.5))`, late-bound custom-property substitution) sweeps the family; the 9 rogues each wire `rgba(0,0,0, var(--modal-overlay-alpha, <own decimal>))`, byte-identical until dragged. **Popovers correctly EXCLUDED (Boss-ruled):** context menus (shared `ContextMenu.svelte` — a scrim-less `position:fixed` popover, click-outside via a document listener, NO backdrop element), pickers, tooltips never dim — the app's principled line is **modals dim, popovers don't** (Obsidian/VS Code/macOS/Windows). Phase 4 (Chrome) is now COMPLETE (§4a + §4b + §4c).
> - **MIG-088 Phase 5 — right-sidebar panel badges (all Boss PASS).** New **"Panels"** Style-Setter category — **3-zone** (added to the twoZone-exception list, like Properties, because the docked Setter panel would occlude the right sidebar it styles) with a live centre preview mimicking each surface (same vars+fallbacks → re-colours live). SO#8 discovery (`wf_82af0a05-bb4`, 13 agents) confirmed all six live + not-shipped + orthogonal to Phase-2 (Phase 2 shipped the cognitive DOT colours; Phase 5 = the CHROME those never touched). Six elements:
>   - **§5a Health cards** (`KnowledgeHealthDashboard` `.khd-card` bg/label-text/radius) · **Provenance tag** (`ProvenancePanel` `.prov-external-tag` text/border/radius) · **Task badges** (`TasksPanel` `.tp-due.overdue/.due-today` + `.tp-tag`, each state = ONE colour driving tint+text via `color-mix`, the §2 Confidence pattern; `.upcoming` left as-is). **§5a FIX (Boss Step-4):** the **Global Tasks full-page view is a SEPARATE component** (`GlobalTasksView`) that didn't follow — its `.gt-due`/`.gt-tag` now chain the shared control UNDER its pre-existing per-view overrides (`var(--gt-overdue, var(--task-overdue, #ef4444))`). TasksPanel + GlobalTasksView are the ONLY two badge surfaces (swept).
>   - **§5b Stale badge** (`ReviewStatusPanel` `.rsp-stale` bg/radius) · **360 markers** (`Inspector360`: Tensions/Fragile/Blind-spots each drive BOTH the column flag border AND the warn icon; + Card border [frame + grid lines] + radius; per-theme fallbacks kept) · **Traversal chips** (`BacklinksPanel` + `OutgoingLinksPanel` mirrors). **§5b FIX (Boss Step-3):** the chips shipped with one shared Accent; **Boss ruling — one colour per tier** (the Task-badges pattern). Now **Accent (master) + Emerging + Established + Load-bearing + Stale**, each `var(--link-tier-<t>, var(--link-tier-accent, var(--interactive-accent, #7c3aed)))` (tier → master → theme; the previously-empty `.bl/.ol-tier-emerging` rules now carry the explicit chain). `fresh` (0-traversal) never renders a chip, so 4 tiers cover all rendered chips.
> - **i18n ×15:** every new control localized. Method: **reuse existing slugs** (background/text/border/radius/tag/accent/review/established/…) + **copy from canonical panel keys** for panel-consistent terms (overdue←tasksPanel.overdue, tensions←inspector360.tensions, blind_spots←inspector360.blindSpots, knowledge_health←knowledgeHealth.title, etc.) + **native-localizer workflows** for the genuinely-new element names (health_cards/provenance_tag/task_badges/label_text/stale_badge/360_markers/traversal_chips/card_border/card_radius). Preview sample fillers (Ibn Khaldun/Review draft/…) English-fallback like "Apple (Fruit)".
> - **Canonical facts (v3.20):** (1) Style-Setter now: **Global** += Overlays (scrim opacity); a new **Panels** category (3-zone) with Health cards · Provenance tag · Task badges · Stale badge · 360 markers · Traversal chips. (2) **modals dim, popovers don't** — `ContextMenu.svelte` (all context menus), pickers, tooltips have NO scrim by design; only true modal dialogs do. (3) `--modal-overlay-alpha` is deliberately never given a default (unset until dragged → byte-identical). (4) **Global Tasks is a separate component from the right-sidebar Tasks panel** — badge/theming wiring must cover BOTH (the §5a-class parity miss); same for Backlinks/Outgoing (mirror components). (5) `color-mix(in srgb, VAR N%, transparent)` == the old `rgba(...,0.N)` tint — the byte-identical way to make ONE colour drive a badge's fill+text.
> - **STANDING RULE added (memory `feedback_build_binary_before_test_instructions`):** finish the release-binary build (freshness-verified) FIRST, then send the Boss-test tutorial — never deliver a tutorial for a binary that doesn't exist yet. If the running app blocks the linker, hold the tutorial until the rebuild actually completes.
> - **DEFERRED (unchanged):** the **8 typed-link type colours** (cross-surface cognitive set — next); disabled-Wing scrims/shadows; the **Arabic callout End/Home caret** known-issue; help-topic folders ×15.
> - **NEXT:** the **8 typed-link type colours**, then MIG-088 Phases 6–10 (search/index · Sky/OrgChart/Map D3 · calendar · dialogs/global · audit).
>
> ---
>
> *Below: the v3.19 preamble, retained for diff visibility.*

> **What changed in v3.19** (**MIG-088 Phase 3 §3b–§3d — the editor colour sweep — SHIPPED end-to-end + Boss-validated; then Phase 4 (Chrome) OPENED — §4a Tab-bar extras + §4b Shadow consolidation SHIPPED + Boss-validated. All pushed to `main`.** Ultracode):
> - **MIG-088 §3b–§3d — editor colour sweep (all Boss PASS).** Every remaining hardcoded editor colour/radius is now a Style-Setter control, byte-identical until edited (unify-on-demand: each surface keeps its own current value as the `var()` fallback):
>   - **§3b Highlight (`fc18fa00`,`9b1c484b`):** the HTML `<mark>`, the markdown `==…==`, and the toolbar `H` chip now share **`--highlight-bg` + `--highlight-text` + `--highlight-radius`**. The **Text** control was a Boss follow-up at test — a dark highlight background swallowed the (inherited) text, so a paired text colour keeps it legible.
>   - **§3c Syntax tokens (`963cfd15`):** `tags.url` → **`--url-color`** (URL). The markdown FORMATTING MARKS (`#`/`**`/`==`/`>`/`` ` ``, `processingInstruction`+`meta`) → **`--syntax-meta-color`**, control **relabelled "Markup marks"** (Boss ruling). NB: these marks are HIDDEN off the active line, and this is NOT the frontmatter (Constellation folds that into the Properties panel — styled via the Properties category). The inherited `/* frontmatter fences */` comment was wrong.
>   - **§3d Link chip (`97d6d6ce`,`c7f185b8`):** reworked twice at Boss test (single "Lens count text" → both bg+text → "why two badges?" → **Boss ruling "Link chip only"**). Styles the **×N link-traversal chip** — **`--link-chip-bg` (fills SOLID when set) + `--link-chip-text` + `--link-chip-radius`**. The lens-count pill reverted to its hardcoded look (out of scope). `/simplify` cleanup `07436595` renamed the element key `badges`→`linkChip` and reused `.ss-link`.
> - **MIG-088 Phase 4 — Chrome (OPENED; §4a + §4b shipped).** SO#8 discovery (`wf_ba6db77c-a00`): **most chrome is ALREADY styleable** (tabs/dock/layout-bar/right-sidebar/status-bar/flank-handle/all pane+dock dividers via `cTabs`/`cDock`/…/`--border`). Boss ruling: also consolidate shadows + scrims.
>   - **§4a Tab bar extras (`b2e7f487`, Boss PASS):** the new-tab `+` button (`--tab-new-border`/`-bg`/`-bg-hover`/`-color-hover`, `--tab-new-bulb-color`) + tab-scroll arrows (`--tab-scroll-arrow-bg`/`-bg-hover`) → a new **`cTabExtras`** element in **Components** (2-zone, live-behind). Consistency fix: `.tab-lib-name` bg `#e8e8ec` → `var(--topbar-bg, #e8e8ec)`.
>   - **§4b Shadow consolidation (`dd08026a`,`c543046c`, Boss PASS):** **3 shadow tokens** — `--modal-shadow` (6 live dialogs), `--popover-shadow` (ConfidencePicker/LinkTypePicker/CalendarPanel/ColorField), `--tooltip-shadow` (HelpTip) — unify-on-demand + byte-identical; a new **`gShadows`** element ("Shadows", `select` presets None/Soft/Medium/Strong/Dramatic) in **Global**. **CANONICAL LESSON:** the discovery flagged hardcoded shadows in **STALE components** — `CodeMirrorEditor.svelte` has 0 importers, `EditorContextMenu` + `FormattingToolbar` are imported only by it (dead). Those 6 wirings + the dead `--dropdown-shadow` control were reverted. **The add-a-styleable-element recipe MUST verify the component is actually MOUNTED.**
> - **Canonical facts (v3.19):** (1) Style-Setter **Editor** category now also has **Highlight · Syntax tokens · Link chip**; **Components** has **Tab bar extras**; **Global** has **Shadows**. (2) **"Markup marks"** = the in-editor markdown syntax marks (visible only on the active line), NOT the frontmatter. (3) unify-on-demand shadows = `var(--<tier>-shadow, <surface's own current shadow>)`. (4) **`CodeMirrorEditor.svelte` + `EditorContextMenu` + `FormattingToolbar` are STALE/unmounted** — the live editor is `NotePane` (`.e-tb` toolbar); the note-body right-click is the shared `ContextMenu` (`--shadow-l`). Never target these for styling. (5) the Style-Setter apply path (`+layout.svelte:~1985`) writes ALL `styleOverride`/`liveStyleDraft` vars to `document.body.style` generically — new vars need no allow-listing.
> - **DEFERRED (honest, noted):** **§4c dialog-scrim** consolidation (the 4 inconsistent backdrop opacities — needs an OPACITY control, a colour picker can't express a see-through scrim); **disabled-Wing shadows** (Sight OFF/Map/GraphMind/SenseMaking/Inspector360-panel/tour); the **8 typed-link type colours** (a cross-surface cognitive colour set — its own step); the **Arabic callout End/Home caret** known-issue (from v3.18); **help-topic folders ×15** for callout customisation.
> - **NEXT:** MIG-088 **§4c** (scrim opacity control) or **Phase 5** (right-sidebar panels: KH/Provenance/Tasks/Review/Inspector360 badges) · the typed-link colours · then Phases 6–10.
>
> ---
>
> *Below: the v3.18 preamble, retained for diff visibility.*

> **What changed in v3.18** (**MIG-088 Phase 3 §3a (callout COLOURS) + §3b-pre (per-element Reset) SHIPPED; then MIG-089 "Callout Customisation" SHIPPED end-to-end — per-type icons, user-defined `[!trigger]` types, a unified centre-zone Callouts manager; then a Boss-requested app-wide Language-First / bidi-correctness audit FIXED across the app. All Boss-validated except one deferred known-issue.** Ultracode):
> - **MIG-088 §3a — Callout colours (`d395673f`, Boss PASS).** The 10 callout families (note/abstract/tip/success/question/warning/failure/danger/example/quote) became restyleable: `calloutPlugin.ts` `[data-callout]` rules now read `var(--callout-<family>-color, <today's hex>)`; aliases share the family var; `question`/`warning` keep separate vars (both default `#ff9100`). New `callouts` element in the Editor category. i18n ×15.
> - **MIG-088 §3b-pre — Per-element Reset (`9c9d412f`, Boss PASS).** Boss-discovered footgun: the lone universal Reset nuked the whole theme. New `clearStyleOverrideKeys(keys[])` + a `selVars`/`selHasOverride`/`resetElement()` per-element "↺ Reset this element" that reverts ONLY the selected element (expands `--interactive-accent` → its 5 decomposed keys). i18n ×15.
> - **MIG-089 "Callout Customisation" — SHIPPED (Architect→Plan→Build, Boss-validated).** Concept: *the callout vocabulary is the user's, not the app's.*
>   - **§A — per-type icons:** re-icon any built-in family from the existing **Emoji & Icon Library** (`iconOverrides` + `EmojiIconPicker` + `SlotIcon` — reused, no parallel system). `calloutPlugin` reads `resolveOverrideSync('callout.'+family)`; widget renders emoji (textContent) or SVG (innerHTML); `eq()` compares icon; a `refreshCallouts` StateEffect + a guarded NotePane `$effect` repaint an OPEN editor live (colours already ride CSS).
>   - **§B — custom callout types:** a per-Universe registry `appSettings.customCallouts: {slug,name,color,icon}[]` (`src/lib/theme/customCallouts.ts`; default `[]`). `calloutFamilies.ts` (new, dependency-free) holds the family data shared by `calloutPlugin` + `customCallouts` (breaks a cycle). A custom `[!trigger]` injects its colour **inline** as `--callout-color` on the CM6 line decoration (built-ins use the CSS theme var); icon from the registry. Slug sanitisation keeps **Unicode** (`\p{L}\p{N}\p{M}` — `[!فكرة]` works); the parser uses `[^\]\n]+` (Language-First). Built-in collision blocked. The header shows the type's display **name** bold when no explicit title (Obsidian default-title). `ColorField.svelte` (new) gives the custom colours the **saved-colours palette** (fixed-position popover). `IconRef.svelte` (new) renders a raw icon ref. Edit mode (✎) for name/trigger with a trigger-change hint.
>   - **§B redesign — unified Callouts manager (Boss suggestion):** one box in the **centre zone** (Full-Center-Zone rule) — every callout (built-in + custom) is a uniform row `[colour] [icon] [name · aliases / [!trigger]]` with a left border in its own colour; built-ins, a divider, custom callouts, an Add row. Built-in colours write the draft vars via `getDraftColor/setDraftColor` (= StyleSetter `curVal/setVar`). The right rail is **hidden for the Callouts category** (`.ss--norail`); a working **Reset** (built-in colours + icons; active when either is overridden; custom callouts keep their ✕) lives in the manager header.
>   - **DEFERRED known-issue (Boss "stop patching"):** the **End/Home caret in an Arabic callout *in the editor*** still misbehaves — a CM6 RTL-caret issue on callout lines (the speculative `isolate→plaintext` fix was reverted; not reproduced from the agent side). Likely the callout's `Decoration.replace` ranges interacting with RTL; a real fix is structural. **Not fixed; documented.**
> - **Language-First / bidi audit — Boss-requested ("ensure no future issues like this"), FIXED app-wide (`wf_463a3f8c-047` found it; Passes 1–4 shipped).** Root finding: only the editor body (`.cm-line`) and the new callout input had the caret-repair `unicode-bidi: plaintext`; **no other text input had it.**
>   - **Pass 1 (Boss PASS):** a zero-specificity global rule in `theme.css` — `input:where([type=text],search,url,email,tel,password,:not([type])), textarea { unicode-bidi: plaintext; text-align: start; }` (any component rule still wins) + `dir="auto"` on ~15 high-severity user-content inputs.
>   - **Pass 2:** display `dir="auto"` on user-text spans (file-tree names, tab titles, status-bar names, search snippets, backlink/outgoing context+annotation, index ref-names, structure crumbs).
>   - **Pass 3:** new `isEditableTarget(e)` in `utils.ts`; the global `handleGlobalKeydown` early-returns for bare caret/edit keys when an editable is focused (Escape still closes; modifier combos still fire) — closes the latent "rebound key swallows typing" landmine; GraphMindView's fly-through guards use it too.
>   - **Pass 4:** Inspector360 back-arrow flips under `[dir=rtl]`; ContextMenu submenu + Dashboard/SecondScreen tag panels + SourceReview accent → logical CSS (`inset-inline-*` / `border-inline-start` / `margin-inline-start`).
> - **Canonical facts (v3.18):** (1) callout COLOURS = per-Universe `--callout-<family>-color` (byte-identical fallback); built-in ICONS = per-Universe `iconOverrides` `callout.<family>` slots; CUSTOM types = per-Universe `appSettings.customCallouts` (colour injected inline on the CM6 line deco, icon from the registry). (2) family data lives in `src/lib/editor/calloutFamilies.ts` (dependency-free). (3) the Callouts Style-Setter element renders the **unified manager in the centre zone**; its category hides the right rail (`.ss--norail`). (4) **Language-First input pattern:** every text `<input>`/`<textarea>` gets `unicode-bidi: plaintext` from the global `theme.css` rule + `dir="auto"` on user-content fields. (5) keyboard handlers that `preventDefault` bare keys must guard with `isEditableTarget(e)`. (6) **open known-issue:** Arabic callout End/Home caret in the editor (deferred).
> - **NEXT:** **MIG-088 §3b–§3d** (the PAUSED editor colour sweep — Highlight/unify, Syntax tokens, Editor badges) resumes after this; then Phases 4–10. Plus the deferred callout-caret known-issue.
>
> ---
>
> *Below: the v3.17 preamble, retained for diff visibility.*

> **What changed in v3.17** (**A marathon: the app-wide RIGHT-CLICK editor surface — note body + frontmatter + Search-results — SHIPPED; then MIG-088 "Style Setter Completeness" OPENED (149-element audit + 10-phase plan) with Phase 1 (frontmatter pills) + ALL of Phase 2 (5 unified Cognitive-colour sets) SHIPPED. All Boss-validated.** Ultracode):
> - **MIG-077 §F — app-wide right-click, the editor surface (SHIPPED, Boss-validated).** The shared `<ContextMenu>` + `contextMenuBuilder` now covers: rich **Note/Folder/Library** menus (Bookmark, Copy path ▸ submenu, Open-in-default-app, Show-in-explorer, Style…); the **note body** RC (NotePane via the shared menu — Add link, Format▸/Paragraph▸/Insert▸, clipboard, Select all, Style→editor); the **frontmatter (Properties panel)** RC (Copy value/name, Remove/Add property + the full editor menu, Style→the new *Properties* category); and **Search-results** RC (B2 — Open / Open-in-new-tab / Reveal-in-tree / Copy / Bookmark / Show-in-explorer / Style→cognitive; **SAFE subset, NO rename/move/delete** — the non-refreshing-list / Notes-Navigator lesson). Submenus flip left at the right edge (overflow fix). Commits `fa98bf6b`·`698da978`·`42b0a585`·`4ea7e9fd`.
> - **MIG-088 "Style Setter Completeness" — OPENED + Phases 1–2 SHIPPED.** A 149-element audit (`lab/reports/MIG-088-STYLESETTER-AUDIT.md`, 50 high-pri) catalogued every user-visible element NOT yet exposed in the Style Setter. Boss ruled **"build everything"** (10-phase plan: `lab/reports/MIG-088-STYLESETTER-COMPLETENESS-PLAN.md`).
>   - **Phase 1 — Properties category (`dbf5507e`):** new **"Properties"** Style-Setter category (3-zone, legend preview) — **Property tags** (`.pe-tag`: bg/text/radius/height) + **Taxonomy pills** (`.pe-taxo-pill`: bg/text/radius). Established the **add-an-element recipe** (memory `project_stylesetter_add_element_recipe`). Boss's trigger: "how can I restyle those tags?"
>   - **Phase 2 — Cognitive colours (`ea68a565`·`2d7f1ac3`·`605642d8`·`c1bed90e`):** new **"Cognitive colours"** category consolidating the 5 repeated cognitive-vocabulary colour sets — **Maturity · Confidence · Origin · Stage · Match-category** — into ONE shared control each, wired across every CSS surface (file-tree, tabs, Inspector360, ConfidencePicker, Knowledge Health, Provenance, editor search-highlight, SearchHub). **Boss ruling — "unify on demand":** each surface keeps its *exact current colour* as a fallback (byte-identical today) until the user sets the shared var, then all surfaces snap to it. **§2f** extended the SearchHub result highlights to per-category — the badge, the in-note editor highlight, AND the result highlight now move on one control. Canvas/D3 surfaces (Map/OrgChart/Sight) + Sky's own `--skyview-*` controls deferred to **Phase 7**.
> - **Canonical facts (v3.17):** (1) the Style Setter now has **two new categories — Properties + Cognitive colours** (both 3-zone). (2) `.ss-stage` lives inside `.ss-center`, which `.ss--twozone` HIDES — two-zone categories rely on the *live app behind* the right-anchored panel; a surface the panel would occlude (e.g. the right-sidebar Properties panel) needs a 3-zone centre preview. (3) Add-an-element = ELEMENTS entry + CATEGORY + a `pk` preview case + the component wired to `var(--x, <today's value>)` + an `L()` slug (REUSE existing slugs for zero new i18n). LL-032 still holds: the Setter render path never touches `BUILTIN_THEMES`. (4) **"Unify on demand"** is the consolidation pattern for any cross-surface semantic colour: shared var + per-surface fallback = byte-identical until edited.
> - **i18n:** every new control localized ×15, reusing in-app vocabulary (`knowledgeHealth.confidence.*`, `notePane.stage.*`, `provenancePanel.*`) for state names + Sonnet for genuinely-new labels. **Boss Arabic review caught + fixed:** `styleSetter.labels.maturity` was *undefined* → English-fallback in ALL locales (added ×15; ar = "نضوج"); ar `mixed` مختلطة→مختلط (incl. the source `provenancePanel.mixed`).
> - **NEXT:** MIG-088 **Phases 3–10** (editor specifics · chrome · panels · search/index · Sky/OrgChart/Map D3 colours · calendar · dialogs/global · audit); the paused **MIG-077 surfaces** (B3 Tags · B4 Calendar · B5 Sky · diagnostic panels · A5 GraphMind fold); the **Notes Navigator rework** (re-enable its RC, task `task_fcc8396c`).

> **What changed in v3.16** (**PJ-065 — the Structural / Parent-TOC link type: DESIGNED via the four-phase `/migration`, BUILT, and Boss-validated end-to-end. Plus two fixes the work surfaced — the app-wide 24 s rename slowdown and a latent Sight bug — both fixed.** Ultracode):
> - **Concept (the horse).** A note's place in a *composition* — Book → Part → Chapter → Scene — is **structural**, not cognitive. It answers "where does this sit in the whole work?", not "how does this idea relate to that one?" So PJ-065 adds a **new, NON-cognitive link kind** — the *compositional spine* — that is **excluded from every cognitive surface** (maturity, strata, 360, centrality, tension, health, Sky, aggregates, search grammar, the GraphMind colour map). It is **not** a 9th cognitive type; the 8 cognitive types (+`associative`) are untouched.
> - **Two link types, authored in frontmatter as quoted wikilinks.** `parent:` (child → its one parent) and `contains:` (parent → ordered child list, carries a nullable `seq`). Both authored as `"[[Target Name]]"` under those YAML keys (decision **D3** — wikilink form means they ride the **existing whole-file rename cascade**, so they are rename-safe by construction; no separate maintenance path). Stored in `note_links` with `link_type IN ('parent','contains')` + the new nullable `note_links.seq`.
> - **Read-time resolution (no derived table, no boot walk — LL-022-safe).** Single-parent: a child's own `parent:` wins; ties broken by smallest path; a `contains:` claim that contradicts the child's own parent is **overruled** (strict tree). Acyclicity via a visited-set cycle guard. Both computed lazily in `structural.rs` when the panel opens.
> - **The Structure panel (`StructuralOutlinePanel.svelte`).** Shows the **whole work** (root resolved from `ancestors[0]`), not just a breadcrumb, with a **focus toggle** (whole-work ⇄ this-subtree), a *you-are-here* highlight, and a clamped tooltip. Boss: *"YES! This is more like it… You got what's in my mind and more."*
> - **Guards + one-click resolve (Boss-chosen "strict tree + conflict flag").** A contested child (its own `parent:` says X, but Y's `contains:` also claims it) renders as a two-line row with **Keep** (keep the child's declared parent) / **Move here** (accept the claimant) buttons → `resolve_structural_conflict` edits the one frontmatter field via the proven `gate_write → reindex → cascade:rewrote` path (6 unit tests). Boss validated both actions.
> - **PropertyEditor authoring (Boss directive).** `parent` and `contains` are **preset property choices**; both author as **`list` chips** with bracket-guarded auto-wrap — a user who types `Owner A` (no brackets) gets `[[Owner A]]` stored, with no triple-bracket artefacts. (Arc: link-type auto-wrap double-wrapped → Boss suggested `list` type → fixed.)
> - **Phase-4 audit (3 agents) + fixes.** Exclusion confirmed correct on every user-visible surface; **2 P1 leaks closed** — `sky_backfill.rs` INSERT and `search.rs::extract_wikilinks` (frontmatter structural keys) both now apply `structural_not_in_clause` / skip structural-keyed frontmatter — plus P3 hardening (GraphMind colour map → `cognitiveLinkTypes()`) and a comment sweep.
> - **Bonus fix #1 — the 24 s rename (felt app-wide).** Root cause: a **dead `UPDATE note_links SET target_path = …` in `rename_item`** — `target_path` is **NULL on every row by design** (vestigial; link targets are resolved by `target_name`), so the UPDATE was a full table scan over 234 k rows for zero benefit. **Deleted → rename is instant.** (Mis-diagnosed twice by static reasoning before per-step instrumentation pinned it — Reproduce-First.)
> - **Bonus fix #2 — latent Sight bug (`sight_v6.rs`).** 6 production subqueries (`link_in_count`, contested-EXISTS) + the link-set SELECT matched the inbound side by the always-NULL `target_path`, so they returned 0 / empty whenever Sight v6 is re-enabled. Migrated to match by **`(target_path OR target_name_lower)`** (the established `COALESCE(target_path, target_name)` fallback). Verified live: "ISBN (identifier)" inbound **0 → 5359**. Pre-existing bug; Sight is a disabled Wing → no live impact, no rebuild needed.
> - **Style Setter — "Tab left offset" control** (Boss "Pass"): a range slider on the `cTabs` element drives `--tab-bar-offset` (the tab-bar `padding-inline-start`).
> - **Rollback caveat (P2).** PJ-065's schema delta is **purely additive and inert without the feature**: the `note_links.seq` column is nullable; the `parent`/`contains` registry seeds and the frontmatter `[[ ]]` values are ordinary YAML/rows that the cognitive engine already **excludes**. To roll back, drop the two registry types — the structural `note_links` rows and frontmatter keys can be left in place harmlessly (they contribute to nothing cognitive) or swept with `link_type IN ('parent','contains')`. No data migration is required to disable.
> - **Canonical facts (update mental model):** (a) structural links are a **separate kind** — gate exclusion via `link_types::structural_not_in_clause(col)` / `is_structural_type` / `STRUCTURAL_SEED_IDS = ['parent','contains']`; (b) they are **rename-safe via the whole-file `[[ ]]` cascade** (D3), not a bespoke path; (c) `note_links.target_path` is **NULL on every row** — *never* match link targets by it; resolve by `target_name` / `target_name_lower` (reinforces the PJ-066 "no `COALESCE(col,expr)` in a WHERE" rule); (d) the Structure panel reads **lazily at panel-open** (`structural.rs`), no derived table.
>
> ---
>
> *Below: the v3.15 preamble, retained for diff visibility.*

> **What changed in v3.15** (**Item 7 — PJ-066 connect-freeze: the felt freeze when connecting a link on a link-dense note ROOT-CAUSED + fixed + Boss-validated ("instant, no freeze at all"). Backlog item 7 of 7 — FINAL; the 7-item Boss backlog is COMPLETE.** Ultracode):
> - **Reproduce-First (5 instrumented measurement rounds)** on the heaviest note (Ancient history, 533 links). The filed "~3 s residual" framing was wrong by an order of magnitude — a single connect froze the app **~47 s**. The diagnosis refuted *two* prior hypotheses (writer-lock-read, frontend-re-render) **and** a mid-session "reindex storm" hypothesis (the caller-tagged trace proved **one connect = exactly one reindex**; the "7 reindexes" seen earlier were the Boss clicking connect *during* the freeze). The lesson: instrument → reproduce → read the trace; never fix on a plausible mechanism.
> - **Four root causes, four surgical fixes** (no schema change, no `/migration` — all additive/behaviour-identical):
>   - **#1 `constellation_embed_notes` was SYNC** → the e5 ONNX inference (~32 s on this note) ran on the single IPC dispatch thread and froze the whole app on every save. → **`#[tauri::command(async)]`** (the sibling of the §C1 reindex-async fix, which had been missed).
>   - **#2 redundant re-embed on a frontmatter-only connect** — a typed-link connect only adds a YAML property (body unchanged) yet re-embedded the whole body. → `saveTabContent(…, bodyUnchanged=true)` → **`force:false`** skips the re-embed when the note is already embedded.
>   - **#4 THE dominant cost — a hidden full-table scan.** `resolve_incoming_target_paths` (the save-path backlink/sky maintenance, run by `maintain_incoming_after_save` + `sky_affected_paths`) resolved a target name via `WHERE COALESCE(name_lower, LOWER(name)) = ?` — the **`COALESCE(...)` in the WHERE defeated every index → a full SCAN of `note_meta` that reads the wide `body_text` rows = 22 s warm / 31 s cold for a SINGLE name resolve.** → new covering index **`idx_note_name_lower(name_lower, path)`** + an index-seeking `UNION` rewrite (post-backfill `name_lower = ?` arm UNION the pre-backfill `name_lower IS NULL AND LOWER(name) = ?` fallback — verified identical to the COALESCE over 200 real names): **21,915 ms → 0.06 ms.**
>   - **#3 `scan_unlinked_mentions` was a SYNC command holding the writer `db` lock** → it blocked the IPC thread for the reindex's full duration (the felt freeze even after the reindex went async). → **`#[tauri::command(async)]` + routed through `with_read_conn`** (the read-only WAL reader, §C3) so it never waits on the writer lock; eventual-consistency is correct for a suggestion panel.
> - **Result (Boss-verified, trace-confirmed):** connect is **instant, no freeze at all**. `maintain_incoming` **31,362 ms → 154 ms**; **zero** `[PJ066-SLOW]` IPC-thread blocks; the reindex (11 s cold / 2.5 s warm) is now **entirely background** — no read blocks the UI.
> - **Canonical facts (update mental model):** (a) the save-path name→path resolve seeks **`idx_note_name_lower`** — **never** put `COALESCE(col, expr)` in a WHERE (it kills the index *and* drags the wide `body_text` rows through a scan); (b) `scan_unlinked_mentions` reads via **`with_read_conn`** (read-only WAL), `async`; (c) the general rule this closes: **any multi-second derived-data command must be `#[tauri::command(async)]` and read via `read_db`/`with_read_conn` — never SYNC on the IPC thread holding the writer lock.**
> - **THE 7-ITEM BOSS BACKLOG (order 1-2-5-4-7-6-3) IS COMPLETE.** Items shipped this arc: §D i18n sentence-embeds · §E live properties count · broader i18n sweep (854 strings) · PJ-067 concept (ratified) · **PJ-066 connect-freeze (this)** · StyleSetter labels · PJ-065 concept (ratified).
>
> ---
>
> *Below: the v3.14 preamble, retained for diff visibility.*
...
> ---
>
> *Below: the v3.13 preamble, retained for diff visibility.*

> **What changed in v3.13** (**Item 5 — the broader i18n remainder: TRIAGED + Boss-ruled (thorough sweep) → 854 genuinely-live English-left UI strings translated across the 13 non-Arabic locales, Boss-validated**. Backlog item 5 of 7; Ultracode):
> - **Triage first (Boss-mandated).** The ~382 "English-left" strings categorized: **INTENTIONAL** (≈75%) — disabled **Sight** (feature OFF, MIG-038), **CECE en+ar bilingual taxonomy** (PJ-043 by design), **brand/product names** (HTML/CSV/Notion/Bear/360.3D/cUniverse), **plurals CLDR structure** (CJK *other*-only + Russian no-*other* — correct, not gaps), `_comment` dev keys, the fixed **Badge-Taxonomy codes** (`searchBadges.*`/`sidebar.match*` = T/C/P/S/W/#), `app.name`, `searchOps.*` (functional search keywords) — PLUS **styleSetter** (→ Item 6). The genuinely-live translatable remainder = SearchHub panel + note-lifecycle stages + live Settings/Index/AI labels.
> - **Boss ruling: the thorough sweep (Tier 1 + Tier 2).** **174 distinct keys → 854 (key,locale) translations** (Arabic was near-complete — 5 stragglers). A Workflow of 14 native-speaker localizers (`wf_265811e3-626`) translated each locale's English-left keys (placeholder-preserving, native-UI consistent; lifecycle stages as natural progressions). Applied byte-identical (CRLF), validated (all keys covered, placeholders intact, zero empties), **svelte-check 0**, all 14 parse. **Boss Stage-1 (non-Arabic: SearchHub + stages, fr/es): PASS.**
> - **Canonical fact (don't re-flag):** the remaining partial-locale gaps are **intentional** — disabled Sight, the CECE **en+ar** scholarly taxonomy (PJ-043), brand/product names, the CLDR plural category sets, and the fixed Badge-Taxonomy letter codes. Future audits that count "English-left strings" should subtract these buckets.
> - **NEXT (backlog):** item 6 — **StyleSetter category labels** (hardcoded English in the component → localize ×15 via `$t`, reuse existing vocab); then item 7 — **PJ-066 speed** (residual ~3 s on mega-notes + finish the read-connection split; Reproduce-First).
>
> ---
>
> *Below: the v3.12 preamble, retained for diff visibility.*

> **What changed in v3.12** (**PJ-067 — the Living Link Relationship Model v2 CONCEPT PAPER + Boss-RATIFIED; the R4 grammar question wargamed → `problematizes/answers` = its own interrogative family; no code**. Backlog item 4 of 7; Ultracode):
> - **PJ-067 concept RATIFIED (build nothing).** SO #8 cross-check OK (Concept-Paper-first; depends on the shipped MIG-086 fold). Two frontier Workflows + the Living-Link philosophy → the concept paper `docs/concept-papers/PJ-067-Living-Link-Relationship-Model-v2-Concept-Paper.md`.
> - **The model — two layers.** **(A) DIMENSIONS:** make each link type's structural algebra **declarable** (OWL-2 characteristics: symmetry/transitive/inverse/functional/reflexive) → **typed backlinks from both ends** (store one direction, derive the reverse — SKOS), **transitive closure ON READ** (recursive CTE, *never stored* — the LL-XXX rule), `part-of` transitive only *within one* meronymy subtype. **(B) FAMILIES by the Five Acts** (Tension + Synthesis are the thinnest → priority): a **thematic family** (used-for/prerequisite-of/precedes/near = the Connection act); the **Tension** vocabulary (`undermines`/`undercuts` as children of `contradicts`, `qualifies`, the question relations, refutes-via-confidence); the **Synthesis** frontier (analogy `maps-to` + n-ary `co-completes`).
> - **Two load-bearing flags:** **`complements` → `co-completes`/`jointly-constitutes`** (the naming trap — "complementarity" is the lexical *opposite*; and it's *n-ary*); the **n-ary SYNTHESIS NODE** — a new **`SYNTH` note-kind** the N constituents point into, with confidence/weight *on the node* (W3C Pattern 1 + Davidsonian) — **shared infrastructure with analogy's mature form; build once.**
> - **R4 (the grammar ruling) wargamed → RULED FAMILY.** At the Boss's request, three steelman defence papers (FAMILY / NINTH / UNTYPED) + a neutral findings (`docs/concept-papers/PJ-067-R4-Wargame-Question-Relation.md`). **Boss ruling: `problematizes / answers` enters as its own first-class INTERROGATIVE FAMILY** — NOT a 9th cognitive act and NOT left untyped. The canonical **8 (the answer/declaration acts) + the §7 derived order stay FROZEN**; the untyped link stays the default open question; the question gets a typed home as a distinct *mood* (`problematizes` = directed verdict-free doubt; `answers` = resolution-of-a-question). Guardrail: scope `answers` to question-targets; typing is opt-in.
> - **Hierarchies in Constellation — four kinds, one machinery (new cross-reference in BOTH PJ-065 §2.1 and PJ-067 §2):** taxonomy (`generalizes`/`exemplifies` — *the taxonomy link, already shipped*) · meronymy (`part-of`) · lineage (`derives-from`/`supersedes`) — **cognitive** — vs the **structural** compositional outline (PJ-065). They differ in meaning but share closure-on-read + acyclicity + inverse + ordering — *build the primitive once.*
> - **Canonical fact:** the Living Link vocabulary is becoming **multi-family** — the 8 cognitive/answer acts · the structural parent/TOC link (PJ-065) · the proposed thematic family · the ratified **interrogative** family (questions). "The 8 cognitive acts" + the §7 order are **unchanged**. R1/R2/R3/R5/R6 are ruled at the `/migration` Architect (Tension-first).
> - **NEXT (backlog):** item 5 — the **broader i18n remainder** (~382 partial-locale gaps): **TRIAGE FIRST + a Boss scope ruling** before any sweep (much is intentional — CECE bilingual taxonomy [PJ-043], disabled Sight strings, `_comment` keys, philosopher names); then StyleSetter labels (item 6), PJ-066 speed (item 7).
>
> ---
>
> *Below: the v3.11 preamble, retained for diff visibility.*

> **What changed in v3.11** (**PJ-065 — the structural (parent/TOC) link-type CONCEPT PAPER produced + Boss-RATIFIED; no code (Concept-Before-Function)**. Backlog item 3 of 7; Ultracode):
> - **PJ-065 concept RATIFIED (build nothing).** SO #8 cross-check confirmed PJ-065 (Boss-conceived 2026-06-24, from the MIG-086 fold discovery) is valid + unblocked (its dependency, the frontmatter fold, shipped). A 4-front grounding Workflow (`wf_240289a1-cdd`: codebase · Breadcrumbs/Tana/Dendron/Logseq/MOC · SKOS/Dublin-Core/schema.org/OWL · Scrivener/Workflowy/screenwriting) + the Living-Link philosophy → the concept paper `docs/concept-papers/PJ-065-Parent-TOC-Link-Type-Concept-Paper.md`.
> - **The horse:** the structural link is the **compositional spine** — it answers *"what is the ordered shape of the work I'm composing?"*, NOT the cognitive *"how do these ideas relate?"*. It is a **distinct kind** from the 8 cognitive acts: **ordered** (a TOC is an ordering), a **tree/DAG** (acyclic), **settled by authorship not inquiry**, and **exempt from the living-link apparatus** (no weight/confidence/decay) and the topology signals. It serves **Externalization** (the act after Conviction: constellation of ideas → composed work) — NOT the cognitive `part-of` (epistemic meronymy). Use case: authors/screenwriters (chapters/scenes/acts), MOC/outline hierarchies.
> - **The pattern (WA#5, all prior art agrees):** assert one direction → derive the reverse write-time; **order = a separate sort-key** (not on the edge); store direct edges, compute closure on read; enforce acyclicity write-time. The one primitive Constellation lacks: **persisted sibling ORDER** on a relationship (`note_links` has no order column; OrgChart reads the filesystem, not links).
> - **Boss ruling: "Approved"** — concept + design direction ratified (multi-parent with one primary spine; MVP = TOC panel + breadcrumb). **The precise NAME + final design decisions are ruled at the `/migration` Architect; build deferred.** PJ-065 status: Open/Deferred → **Concept RATIFIED, /migration-ready.**
> - **NEXT (backlog):** item 4 — **PJ-067** (Living Link Relationship Model v2; **concept-paper-first, STOP at the paper, build nothing**); then the broader i18n triage, StyleSetter labels, PJ-066 speed.
>
> ---
>
> *Below: the v3.10 preamble, retained for diff visibility.*

> **What changed in v3.10** (**MIG-087 §E — the live PROPERTIES (خصائص) count + an all-noun Arabic singular-"one" refinement, Boss-validated**. Backlog item 2 of 7; Ultracode):
> - **MIG-087 §E — live properties count.** The status-bar خصائص count read `sidebarProperties.length` (a `$derived` of `sidebarTab.content`, which is STABLE during editing because the editor owns content) — so it refreshed only on save/tab-switch, never as you edit properties. Fixed by **mirroring §C's live counter exactly**: a one-way, display-only `onLiveProps(tabId, count)` observer from `PropertyEditor` → (`NotePane` → `NoteEditor` →) `+layout`, emitted at the top of `debouncedSave()` — the single edit choke-point every mutation routes through (and the init-sync `$effect` does NOT, so it fires on real edits only, like §C's `onDocChange`). It reports the **non-empty-key** count, which `parseFrontmatter` (`if (key)` push) confirms equals the settled `sidebarProperties.length` → no flicker. `+layout` gained a `propCount` `$state` baselined by an `$effect` (re-syncs on tab-switch/save, dormant during editing) + `handleLiveProps` (focused-tab-guarded) — the same two-writer shape as wordCount/charCount. Wired on **both** PropertyEditor instances (embedded + right-sidebar) and both NoteEditor mounts. **Never writes content; `noteModel` stays non-reactive** (outside the BUG-015/§C-2 vector). Verified: svelte-check 0, **MIG-076 harness 28/28**, Boss Stage-1 PASS.
> - **All-noun Arabic singular-"one" refinement (Boss-ruled mid-test).** Boss: at count 1, Arabic should spell out "one" — "خاصية واحدة", not bare "خاصية". Ruling (AskUserQuestion): apply to **ALL** Arabic counted nouns, gender-correct (واحدة fem / واحد masc). All **27** `ar` `plurals.*.one` forms updated via a native-Arabic verify pass (genders all confirmed; 2 naturalness fixes: `selected` → "عنصر واحد محدد", `hidden` → "عنصر واحد مخفي"). Because `notes.one`/`more.one` also fill the §D sentence embeds, those improved too ("ستُحذف ملاحظة واحدة، …"; "…وواحد آخر…"). **Dual (count=2) unchanged** ("خاصيتان", "ملاحظتان"). Byte-identical `ar.json` round-trip (27/27 `.one` only). Boss re-test PASS.
> - **Canonical fact (update mental model):** Arabic `plurals.*.one` now reads "{noun} واحد/واحدة" (word-only, no digit) for every counted noun. The live status-bar counts (words/chars from §C, properties from §E) are all one-way display observers from the editor surfaces; `noteModel` remains non-reactive.
> - **NEXT (backlog order):** item 3 — **PJ-065** (frontmatter parent/table-of-contents link type; concept-first); then PJ-067 (concept-paper-first), the broader i18n triage, StyleSetter labels, PJ-066 speed.
>
> ---
>
> *Below: the v3.09 preamble, retained for diff visibility.*

> **What changed in v3.09** (**MIG-087 §D — the filed i18n SENTENCE-EMBED remainder shipped + Boss Stage-1-validated (Arabic, counts 1/2/3/11)**. Backlog item 1 of 7; Ultracode):
> - **MIG-087 §D — three full-sentence count strings made plural-aware.** The §B preamble's "Remaining (filed)" item — `dialogs.batchDeleteTrash`/`batchDeleteSystem` ("Delete {count} notes? …"), `settings.arabicOverrides.reindexed` ("Reindexed {count} note(s)"), `importer.andMore` ("and {count} more…") — glued `{count}` to a noun in English word order, grammatically wrong at 1/2/11+. Each is now rewritten ×15 with a single **`{noun}` placeholder in native word order**, filled at the call site by **`$tn('plurals.<noun>', count)`** — **reusing the existing `plurals.*` namespace (no new noun data)**: the three notes-strings ← `plurals.notes`, `andMore` ← `plurals.more`.
> - **Method — Workflow (15 native-speaker rewriters → 15 adversarial verifiers).** The verifiers substituted every CLDR category at counts 1/2/3/11 and caught real agreement bugs OUTSIDE `{noun}`: **en** "They will be moved" (wrong at 1) → "The selection will be moved"; **en** doubled "more" → `and {noun}...`; **he/es/fr/pt** verb/participle number+gender disagreement → agreement-neutral nominalizations; **hi** plural pronoun इन्हें at 1 → impersonal passive. The **Arabic headline**: passive framing `ستُحذف {noun}` puts the count as نائب فاعل (subject of a passive verb), so the bare nominative dual **ملاحظتان** is correct (avoids the object-slot accusative ملاحظتين); 11+ correctly shows singular تمييز ("11 ملاحظة").
> - **Frontend-only.** Call sites: `NotebookNavigator.svelte`, `ArabicOverridesPanel.svelte` (dropped the dead `||fallback`+`.replace`), `ImporterModal.svelte`; `tn` import added to the first two. Verified: byte-identical CRLF round-trip (only the 4 keys/locale changed — 60/60), **svelte-check 0**, runtime simulation of the real `tn`/`t` logic (ar dual + 11+ تمييز + ru one/few/many all correct), frontend+binary rebuilt. **Boss Stage-1 PASS.** Help/User-Manual: none (localization-correctness only).
> - **Canonical fact:** for any counted noun in a **sentence**, use a `{noun}` placeholder filled by `$tn('plurals.<noun>', count)` — never `{count} <noun-word>` inline. The plural form carries the digit (or not, per locale category); surrounding words must be **number-neutral** outside `{noun}`.
> - **Process note (Boss correction, doc-truth):** the **note list has no Constellation right-click menu** (the Back/Refresh/Print menu is the WebView2 native one). Multi-select in the **Note Navigator** = the per-row **checkbox** (or ↑/↓ + Space); the **File Explorer tree has no multi-select** (by design); batch delete = the bottom **NavBatchBar** Delete (حذف) → styled `ConfirmDialog`. (Out-of-scope follow-up flagged: no Ctrl/Shift+click row multi-select.)
> - **NEXT (backlog order):** item 2 — the live **خصائص (properties)** count (mirror §C's one-way observer); then PJ-065, PJ-067 (concept-paper-first), the broader i18n triage, StyleSetter labels, PJ-066 speed.
>
> ---
>
> *Below: the v3.08 preamble, retained for diff visibility.*

> **What changed in v3.08** (**MIG-087 — the broader i18n localization pass + a plural-aware count system (CLDR), Boss-validated; plus a live word/character counter fix**. One session, Ultracode):
> - **MIG-087 §A — broader i18n pass (34 keys ×14 locales).** A proactive audit (locale value == en value AND contains Latin) found **40 keys English in all 14 non-en locales**. **34 translated** by native-speaker agents (one Workflow per concern, each reusing its locale's already-shipped vocabulary): **22 clear UI labels** (`nav.home`/`libraries`/`skills`, `tabs.newTab`, `commands.quickCapture`, `settings.plugins.commandPalette`, the `universe.setup.*` Import flow ×10, `libraries.linkLibrary`/`newLibrary`(+Desc), `orgChart.reset`, `settings.skyview.linking`) + **12 brand keys** per **Boss ruling**: feature names **Sight/Map/CNS → native** (reuse the already-shipped `lens.title`=الجهاز العصبي للمعرفة, `orgChart.title`=مخطط هيكلي; ar Sight = **بصيرة المعرفة**, Map = **خريطة المعرفة**), **Dataview → native** (عرض البيانات), **"Return to CNS/OrgChart" → native verb + native name**. **Kept English** (Boss ruling): `app.name` "Constellation" (product brand), `importer.formats.html`/`csv`, `fontSets.cjk`, the numeric `360.3D`. Byte-identical Python round-trip (+34/−34 per locale, zero structural churn).
> - **MIG-087 §B — plural-aware count system (the headline; Boss-caught).** The status bar used the plural form unconditionally — wrong Arabic ("19 مكتبات" / "7659 ملاحظات"; Arabic 11+ reverts the noun to **singular** تمييز). Built a **CLDR plural engine** on the browser-native **`Intl.PluralRules`** (WA#5 — the ICU/CLDR standard, not hand-rolled): new **`tn(key, count)`** / **`tnIn(loc, …)`** in `src/lib/i18n/index.ts` (category → other → one → en fallback; `{count}` interpolation). A canonical **`plurals.<noun>`** namespace (27 counted nouns) in all 15 locales, native forms by per-locale agents — **ar** one/two/few/many/other (**word-only for 1 & 2** per Boss ruling: 1=مفرد، 2=مثنى، 3–10=جمع، 11+=مفرد), **ru** one/few/many, **he** one/two/other, the rest one/other, **CJK** other-only. **Wired app-wide (~50 call sites)** `{N} {$t('noun')}` → `{$tn('plurals.noun', N)}` (status bar both windows, Backlinks/Inspector360/Sky/Search/Index/Calendar/CCS/OrgChart/Importer/SenseMaking/Tension/Dataview/NavBatch/…). Proven by simulation across 0/1/2/3/5/11/21/100 + Boss Stage-1 (status bar, Arabic): **PASS** (8 خصائص · 22 كلمة · 216 حرفًا · 19 مكتبة · 7659 ملاحظة — all grammatically correct).
> - **MIG-087 §C — live word/character counter (Boss-caught at the §B test).** The status-bar word/char count never updated while typing (it read the tab *store's* content, which the editor deliberately doesn't touch per keystroke — the editor-owns-content rule) AND counted frontmatter YAML (a 2-word note read "22 كلمة"). Fix: a **one-way, display-only** `onLiveStats(id, doc)` from `NoteEditor` (in the existing `onDocChange` handler) → `+layout` debounced counter; it **never writes back into the editor's content**, so it sits OUTSIDE the BUG-015 / §C-2 vector. Both the live path and the on-open baseline now count **body only** (frontmatter excluded — fixes the inflation). `noteModel` stays **deliberately non-reactive** (untouched). Verified: svelte-check 0, **MIG-076 content-integrity harness 28/28**, Boss re-test: **PASS.**
> - **Remaining (filed, flagged to Boss):** 3 **full-sentence** count embeds (the *"Delete N notes?"* confirm dialog, the *"Reindexed N note(s)"* toast, *"…and N more"*) need sentence-level plural forms (a distinct, word-order-sensitive pattern) — Boss ruling pending. The 382 scattered partial-locale gaps (CECE bilingual taxonomy [PJ-043, intentional], disabled Sight v3/v6 strings, `_comment` dev keys, philosopher proper-names) remain the documented broader remainder. Help/User-Manual: no teachable feature change (localization-correctness only); possible help-doc terminology drift from the brand rename is a follow-up.
>
> ---
>
> *Below: the v3.07 preamble, retained for diff visibility.*

> **What changed in v3.07** (**MIG-086 §E SHIPPED + Boss-validated → MIG-086 COMPLETE. The "Suggested Connections" feature is fully localized (×15) + RTL-correct + documented (help + User-Manual ×15); plus a right-sidebar direction root-fix + "Sky View" localized everywhere**. Commits `9772…` arc: i18n/RTL/help close-out → `19448e1c`; Backlinks/Outgoing chrome dir → `29199057`; rs-inner UI-direction root fix → `1d5712b0`; Sky View ×7 keys → `37bfbabb`/`51fc8c85`):
> - **MIG-086 §E — SHIPPED, Boss-validated → MIG-086 is COMPLETE.** The one-click typed "Suggested connections" feature (all 5 hosts, shipped in §D) is now finished: **i18n ×15** (the 2 new keys `panels.suggestedConnections` + `graphView.suggestConnections` + a localized `backlinksPanel.filter`, native + file-consistent in all 14 non-en locales), **RTL polish**, and a **"Suggested Connections" help topic + User-Manual section in all 15 languages** (each reusing that locale's verbatim UI strings + native link-type names). A latent **NUL byte** in `RelatedCandidates.svelte` (a dedup-key separator that made the file "binary" to tools) was removed.
> - **Right-sidebar direction — root fix (Boss RTL finding during §E test).** The right sidebar was `.rs-inner dir={noteDir}` (it followed the OPEN NOTE's direction), so an **LTR note in an RTL UI** rendered the panel chrome (section headers, outer titles) left-to-right. **Fix: `.rs-inner dir={$dir}`** — the right sidebar is CHROME (panels ABOUT the note, in the UI language), so it now follows the **INTERFACE direction**; an 8-panel audit added `dir="auto"` to genuine note-content text (note names, tags, task text, property values, outline headings) so each still reads in its own script. This corrected Backlinks/Outgoing/Health/Tasks/Review/Properties/Provenance/SourceReview in one change. (Architectural note: **the right sidebar follows UI direction; note-content elements within carry `dir="auto"`.** RelatedCandidates/Inspector360/TagsPanel already self-handled their dir.)
> - **"Sky View" localized everywhere (Boss finding).** `panels.skyView` (+ 6 sibling keys: `commands.skyView`, `ribbon.graphView`, `sidebar.graphViewTitle`, `layout.skyViewTitle`, `secondScreen.graphView`, `settings.plugins.graphView`) were the lone labels kept English; all now native (Boss-chosen Arabic **منظر السماء**; others matched to the "view of the sky" concept, distinct from Constellation Map). Per the full-localization top-principal (everything adapts to the UI language).
> - **Broader i18n gap surfaced (NEW follow-up, NOT swept):** a proactive audit (ar==en + Latin text) found **~44 keys still English** in the Arabic UI — ~20 clear UI labels (Quick Capture, Command Palette, New Tab, nav Home/Libraries/Skills, the Import setup flow ×10, Link/Create Library, Reset view, plugin descriptions) + brand-name decisions (Constellation / Constellation Sight / Constellation Map / CNS / Dataview) + keep-English technical (HTML/CSV/CJK). Filed for a dedicated localization pass (needs Boss ruling on the brand names).
> - **NEXT — none blocking.** Open follow-ups: the broader i18n pass (above), **PJ-067** (Living Link relationship typology), the **PJ-066 follow-up** (residual ~3 s on mega-notes + complete read-routing), and PJ-065.
>
> ---
>
> *Below: the v3.06 preamble, retained for diff visibility.*

> **What changed in v3.06** (**MIG-086 §D SHIPPED + Boss-validated — the one-click typed connect is now in ALL 5 hosts; + PJ-066 SHIPPED & CLOSED — the link-dense-note reindex freeze went ≈2 min → ~3 s (≈15×), Phase-4 audited**. Commits `eb0cc280`/`f4e1f3cd`/`f5492543` (§D) · `8bd9039b`→`a0454fb9`+`4a1a290e` (PJ-066)):
> - **MIG-086 §D — `<RelatedCandidates>` wired into the 4 remaining hosts (SHIPPED, Boss-validated all 3 rounds).** The "Suggested connections" surface now lives in **all 5 hosts**: the Reviewer (§B), the **NotePane Backlinks tab** (outbound), the **360° Inspector ×2** (inbound), the note's **Health/TensionPanel** (inbound), and **Sky View's per-node right-click menu** (outbound, popover). Direction is **host-set, not a user toggle** — diagnostic hosts (Inspector/Health) suggest **inbound** ("who should point here"), general hosts (Backlinks/Sky) suggest **outbound** ("what should this point to"); the user In/Out/Both control is deferred to PJ-067. A `direction` prop on the shared component routes `choose()`'s source/target accordingly, with a snapshot guard (re-checks `notePath`/`libraryPath` after the await so a fast note-switch can't write to the wrong note). An architectural review caught + fixed 4 issues before the Boss test; a 2-round fix landed the **`LinkTypePicker` viewport clamp** (`max-height = innerHeight − top − pad` — bulletproof against a stale measured height; benefits the Reviewer picker too). **§E remains** (i18n×15, RTL polish, help/User-Manual×15, mark MIG-086 shipped).
> - **PJ-066 — sky-trigger reindex perf — SHIPPED & CLOSED.** The ~2-min app freeze when connecting a link on a link-dense note (e.g. *Ancient history*, 533 links) is fixed across four grounded layers (WA#5 — researched the field's patterns, didn't reinvent): **§A1** rewrote the sky `COUNT(DISTINCT)` `(target=id OR target IN alias-subq)` disjunction → `IN (SELECT id UNION SELECT alias…)` — **5,572 ms → 26 ms (215×)**, 0 mismatches. **§B** replaced the per-edge sky stratum/maturity triggers with a **save-path recompute-affected diff** (`maintain_sky_after_save`, mirroring MIG-079 §C.2a's incoming maintenance) + a `recompute_all_sky` reconcile self-heal. **§C1** flipped the reindex command to `#[tauri::command(async)]` (off the WebView2 UI thread — LL-021). **§C2** made `index_note`'s link rebuild **incremental (diff-edges)** — only the changed edge is touched, preserving every other edge's earned weight/confidence/traversal. **§C3 root-caused the residual freeze:** it was not the panel queries (those use their own read-only WAL connection) but the **`ensure_search_db_ready` readiness check** taking `db.lock()` just to test "ready?", blocking for the whole reindex; fixed with a **lock-free `db_ready: AtomicBool`** + a **second READ-ONLY WAL connection (`read_db`)** that reads route to. **Net: ≈2 min → ~3 s (≈15×), data correct.** Boss ruling: lock in.
> - **PJ-066 §E — Phase-4 audit (3 dims × adversarial verify) + /simplify.** The audit confirmed **1 P1** (dismissed 2 false positives): the §C3 reader open + readiness publish didn't re-check `federation_generation`, so a universe switch during the slow reader open could publish a stale reader for the old universe — **fixed in-pass** (`4a1a290e`, WA#6) by publishing `db` + `read_db` + `db_ready` together under one gen-validated lock block. `/simplify` found the diff clean (mirrors the proven recompute/maintain/State patterns); 3 quality suggestions folded into the follow-up. Full Rust suite **982 pass**.
> - **The search-engine connection/readiness model (now canonical — update your mental model):** `SearchState` has **two** SQLite connections — `db` (the single writer; WAL) and **`read_db`** (a READ-ONLY WAL reader; SQLite WAL = concurrent readers + 1 writer). Steady-state readiness is a **lock-free `db_ready` atomic** (set after init, cleared on universe switch); `with_read_conn()` routes a read to `read_db`, falling back to `db` only pre-init. Sky aggregates (stratum/maturity) are now **maintained write-time by a save-path diff**, NOT per-edge triggers — same shape as incoming/outgoing (`maintain_*_after_save` + `recompute_all_*` reconcile). `index_note` rebuilds links **incrementally** (diff-edges, traversal-preserving).
> - **PJ-066 follow-up filed (not started):** the residual ~3 s on the heaviest notes (during the now-background reindex) + completing the **full** read-connection split (route search/stats reads off the writer lock; make the sibling read commands async for the same off-UI-thread consistency `/simplify` flagged).
> - **NEXT — MIG-086 §E** (i18n×15 for the §D host strings, RTL polish, help/User-Manual×15 now the connect is fully user-actionable, mark MIG-086 shipped).
>
> ---
>
> *Below: the v3.05 preamble, retained for diff visibility.*

> **What changed in v3.05** (**MIG-086 §F1–§F3 SHIPPED + Boss-validated — typed links now live in the FRONTMATTER (the "fold"); + a doc-drift correction (no `LINK` file exists); + the relationship-typology exploration → PJ-067**. Commit `dc6056a4`):
> > *The directional/symmetric discovery and the typed-link typology below were a Boss-driven exploration mid-MIG-086; the deeper vocabulary work is deferred to PJ-067 (Concept-Paper-first). MIG-086 §D/§E finish on the CURRENT 8 types.*
>
> - **MIG-086 §C reborn as the "frontmatter fold" (Boss ruling 2026-06-24).** §C originally appended `[[type::Target]]` to the note **body**; Boss: *"a typed link hanging there without context is illogical."* Cross-system research (Obsidian Properties, **Breadcrumbs**, Dataview, Logseq/Roam/Tana, RDF/property-graph KR — workflow, fact-checked) confirmed: a **declared** relationship (the one-click kind, not woven into prose) belongs as **frontmatter metadata**. So a typed link is now authored as a **type-as-property** entry — `supports:\n  - "[[Target]]"` — chosen over a structured `links:` object-list. Research doc: `docs/MIG-086-Architect-Frontmatter-Typed-Links.md`; Plan Part 2 in `docs/MIG-086-Plan.md`.
> - **§F1 — dual-source typed-link indexing (SHIPPED).** `search.rs::index_note` now derives `note_links` from **BOTH** the body (`[[type::target]]`, *contextual* links) **and** the frontmatter (type-as-property, *declared* links), merged + deduped on `(type::target)`. New `extract_frontmatter_typed_links` (block-aware; scalar / inline-array / block-list; a **known-type key → its real type**, a non-type key's wikilink → `associative` — byte-for-byte back-compat with the prior full-content scan). 7 unit tests. Existing body links unchanged; no forced migration.
> - **§F2 — the one-click connect writes the frontmatter property (SHIPPED + Boss-validated).** `store.ts::addLinkToNote` + new `addTypedLinkToProps` go through the proven **props save path** (open → `composeNoteModel`+`saveTabContent`; closed → `readNote`+`writeNote`+reindex) — **the body is never touched, so the BUG-015 surface is gone** (the §C dangling-append is retired). `reconstructFrontmatter` now **quotes list items** (valid/interoperable YAML wikilinks; round-trip byte-stable since the block-list parser strips quotes on read). The connect is **non-blocking** (fire-and-forget reindex + optimistic Reviewer) — a ~2-min latency was traced to a **pre-existing sky-trigger reindex storm** on link-dense notes (→ **PJ-066**, Boss-deferred). Default confidence `hypothesis` (C-4) still comes from `index_note`. Boss-validated: clean frontmatter link, full content-integrity, instant, open-note refresh (`reloadTabsFromDisk`).
> - **§F3 — frontmatter links survive rename (SHIPPED).** No cascade change needed — `update_links_on_rename` already rewrites raw file content, so a quoted `"[[Old]]"` in frontmatter becomes `"[[New]]"` in place; locked with a regression test.
> - **§A uncapped (SHIPPED).** Suggestions now show **ALL** related notes closest-first (Boss), not just 5; the shared-term "why" comes from **FTS-index term-membership** instead of per-candidate re-tokenization, so the whole BM25 pool is processed cheaply (Rule 8). The §C picker UI (`LinkTypePicker` — 8-type, viewport-clamped, custom types) + RelatedCandidates wiring shipped; the Reviewer's orphan "Connect" button was removed.
> - **DOC-DRIFT CORRECTED (load-bearing for any future link work):** CLAUDE.md + this orientation describe the Living Link as a first-class **`LINK` file** on disk (`…_LINK_XXXX.md`) that is the source of truth with `note_links` as "just the index." **That file does NOT exist in code.** Links are 100% derived (now from body **and** frontmatter) into the `note_links` SQLite table; the rich earned properties (weight/confidence/traversal) live ONLY in that table. The table **is** the property-graph engine + the de-facto source for the earned properties; the *birth* of a link is its file text (body inline or frontmatter type-as-property). Treat the "LINK file" language as aspirational-never-built until PJ-067 decides the end-state.
> - **Relationship-typology exploration → PJ-067.** The directional-vs-symmetric split opened a much larger map (sourced survey: `docs/Living-Link-Relationship-Typology-Research-2026-06-24.md`): a typed link varies on **DIMENSIONS** (symmetry · **transitivity** · **inverse/converse** · **arity: binary↔n-ary** · cardinality · **taxonomic↔thematic**) and **FAMILIES** (7 covered; uncharted: thematic/functional, **analogy/structure-mapping**, **n-ary synthesis**, argument-attack **undercuts/undermines**, **problematizes/answers**, qualifies). The thinnest **Five Acts** are **Tension** + **Synthesis** — exactly the gap for a *formulation* tool. Two flags: rename the proposed `complements` → **`co-completes`** (lexical "complementarity" = the OPPOSITE); "together form an idea" is genuinely **n-ary** (a synthesis node). **Boss ruling: Concept-Paper this (PJ-067); MIG-086 finishes on the current 8.**
> - **New Pending Jobs (v1.14):** **PJ-065** (frontmatter parent/TOC link type), **PJ-066** (sky-trigger reindex perf — P1, the ~2-min link-dense-note reindex), **PJ-067** (Living Link Relationship Model v2 — Concept-Paper-first).
> - **NEXT — MIG-086 §D** (wire `<RelatedCandidates>` into the 4 other hosts: NotePane Backlinks tab, 360 Inspector ×2, Health/TensionPanel, Sky node menu — host-set direction: diagnostic→inbound, general→outbound; user In/Out/Both → PJ-067) → **§E** (i18n×15, RTL, type-pill/clickable polish, **help/User-Manual ×15** now the feature is user-actionable, Orientation v-bump, MoCh, mark MIG-086 shipped). **Help/UM still deferred to §E** (feature not fully shipped yet — §D pending).
>
> **What changed in v3.04** (**MIG-086 §A+§B — related-note link suggestions: the BM25 "More Like This" backend + the read-only `<RelatedCandidates>` surface in the Reviewer, both Boss-validated; + an FTS-health forensic pass (term_vocab is ~92% empty; the index is NOT fragmented)**):
>
> - **MIG-086 — concept + plan.** When the Reviewer flags an **orphan** ("connect me") or **fragile** ("shore me up") note, it now answers *which* notes to connect — a ranked **"Connect to:"** list with the **shared distinctive terms that explain *why*** (mandatory — the concept's legible reason), and (in §C) a one-click **typed** link. **Concept (the horse):** *reveal a note's unconnected relatives and let the user **formulate** each as a typed connection* — the keystone safeguard is Boss's **"always ask the type"** ruling, which makes each connect an act of formulation, not link-spraying. Surface-validated against ALL ~21 left-dock plugins → **5 hosts**: the Reviewer, the NotePane sidebar, the **360.3D Inspector (both mounts)**, the note's **Health tab (TensionPanel)**, and **Sky View's per-node right-click menu** (canvas excluded — Form-Aligns-To-Purpose). Docs: `docs/MIG-086-Architect-Reviewer-Link-Suggestions.md`, `docs/MIG-086-Plan.md`.
> - **§A — `suggest_related_notes` (SHIPPED).** `libraries.rs` — BM25 "More Like This" over the source note's distinctive terms, **query-time over the always-current `notes_fts` index** (Rule 8; no model, no precomputed matrix). df from **`notes_vocab`** (the live FTS5 vocabulary — `term_vocab` is drift-broken, see below). **A real perf war (60 s → bounded):** empirical probes found the killer — adding `JOIN note_meta` + `NOT IN` post-filters to the bm25 `ORDER BY rank LIMIT` defeats FTS5's rank-limit fast path (**12 ms** bare vs **12 s**); fix = rank with the bare FTS query, exclude in Rust on the top-60; plus capped tokenization + bounded df probes + a capped candidate loop. Live results on the 3 LARGEST notes: 1.1 s / 2.3 s / 0.5 s; **typical orphans <300 ms** (async, spinner-backed, on-demand — never the keystroke path). Quality verified excellent across English (Cathedrals + Gothic architects), Arabic (الدولة العثمانية / مملكة الحجاز), and science. 3 unit tests + a live-DB rehearsal.
> - **§B — `<RelatedCandidates>` (SHIPPED + Boss-validated, read-only).** The shared component (names + shared-term chips + snippet + an inert Link button), mounted in the Reviewer's orphan/fragile detail; `suggestRelatedNotes` wrapper; 6 `reviewer.suggest*` keys ×15. The Boss test caught 4 issues, ALL fixed: a **`$effect` loop** (read+wrote `lastKey` $state → cancelled its own fetch → permanent "Finding…"; fixed by a plain non-reactive var — Rule 2); **"Review Pulse" → "Reviewer"** (new `reviewer.title` ×15); and **two RTL rounds** ending in the rule *mirror when the UI is RTL **or** the content is RTL* (covers all UI×content combos incl. the empty state).
> - **FTS-health forensics (Boss-requested).** **`term_vocab` is ~92% empty** — its one-time first-fill (MIG-013 §1D) was queued + abandoned, so it only ever recorded words from notes edited while running; its `doc_count` is a dead column. **No user-facing number is wrong** (the Index panel reads `notes_vocab`); the only casualty is the "≈ similar" membership filter, which silently under-delivers. P2 — recommended fix: re-point the consumer to `notes_vocab` (Rule-8 canonical). **The FTS5 index is NOT significantly fragmented** (9 segments, 1 dominant; an optimize-on-copy test barely changed it) — the slow live reads are the **1.9 GB index size + concurrent app access**, not fragments. Report: `docs/FTS-Health-Forensics-2026-06-23.md`.
> - **NEXT — MIG-086 §C** (the one-click typed-link action — the SAVE-PATH code; build FRESH with the full Editor-Surface Gate) → §D (wire the other 4 hosts) → §E (i18n×15 finish, RTL, /simplify, docs, help/manual). Help/User-Manual are **deferred to §C** (§B's feature is read-only, not yet user-actionable — documenting a half-feature would mislead).
>
> ---
>
> *Below: the v3.03 preamble, retained for diff visibility.*

> **What changed in v3.03** (**MIG-085 §B — a Unicode case-folding bug fixed (accented-title notes were false orphans) + maturity single-sourced across every surface**):
>
> - **The discovery (the headline).** What began as a P3 "make maturity agree across surfaces" tidy-up surfaced a real, multi-surface **correctness bug**, found by measuring on the live 7,660-note universe. SQLite's built-in `LOWER()` folds **ASCII A–Z only**; it leaves accented capitals (É, Î, Ś, Đ, Š) unfolded. Note **names** were matched to wikilink **targets** via SQL `LOWER(name)` while targets were Rust-folded (full Unicode) — so **13 notes whose titles carry a non-ASCII capital** (`Île-de-France` 17 inbound, `Śramaṇa` 26, `Étienne-Jules Marey` 16, `Émilie du Châtelet`, `Đông Sơn culture`, `Abū Ḥanīfa`, …) **failed to match their own inbound links → `incoming_count = 0`**: shown as **false orphans** in the new Reviewer's 🔗 lens, wrong (sapling) Sky maturity/stratum, 0 backlink badge. The FS-scan surfaces (maturity panel, 360 Inspector — Rust-folded) were *correct*, which **inverted** the original "single-source to `incoming_count`" premise (the DB surfaces were the wrong ones). Boss ruling: **fix the accent bug first, then maturity; include NFC.**
> - **§B.0 — Unicode name-fold (the gating fix).** Research-validated (WA#5, 5-source web research) to the field's dominant pattern — **store one Rust-folded key column, match key-to-key** (Lucene index-time fold / Postgres normalized columns; NOT a `ulower()` SQL function — per-connection-registration landmine; NOT ICU — not in rusqlite bundled). `fold_match_key()` = **NFC → full-Unicode lower → NFC** (no Arabic strip — the target side never stripped, so a strip would fold the name side only and break Arabic links). One canonical fold reused by the new `note_meta.name_lower`, `sky_nodes.id`, `parse_link_body` targets, and aliases. Name-side matches → `COALESCE(name_lower, LOWER(name))` (the column's NULL-ness IS the rollout gate — pre-backfill rows keep today's behaviour, no race). A resumable `name_fold_backfill` fills `name_lower`, NFC-re-folds `alias_lower`, and recomputes the accented notes' `incoming_count` + `sky_nodes.id` + maturity/stratum. NFC also closes the cross-device (macOS NFD vs Windows NFC) duplicate-link class.
> - **§B.1 — maturity single-source.** `MATURITY_SQL_EXPR` inbound: `COUNT(*)` → `COUNT(DISTINCT source_path)` and `status='active'` → `status != 'archived'`, so the Sky trigger's maturity **equals `note_meta.incoming_count` by construction**. `maturity.rs` rewritten from a full-filesystem walk to a pure `note_meta` read (a **Rule-8 win** + agreement); `inspector360` overrides `total_inbound` with `incoming_count`. STRATUM's ≥5 inbound-edge signal stays `COUNT(*)` (it must match the FS `strata.rs` the 360 uses — the one place DISTINCT was *not* applied).
> - **§B.2 — outbound single-source (Boss-caught at the Stage-2 test).** The 360 Inspector header showed **↑34** outgoing while Backlinks/`outgoing_count` showed **16** — the FS scan counts raw `[[link]]` occurrences (a Wikipedia import duplicates targets), the canonical count is the **16 distinct active `note_links` edges**. `inspector360` now overrides BOTH `total_inbound` and `total_outbound` from `note_meta.incoming_count`/`outgoing_count` (one `read_connection_counts` read). The 360 header (↑16 ↓17) now matches the Backlinks panel both directions; the type-breakdown bars + dot matrix still visualize every link *instance* (a separate representation, unchanged). **Boss-validated end-to-end (Stages 1 + 2).**
> - **Verified.** `fold_match_key` + cross-surface agreement tests; full Rust suite **969 passed, 0 failed**; svelte-check 0; **live-copy rehearsal on the real universe** — all 13 accented notes 0→correct count, **zero collateral** on the 7,647 ASCII notes, zero false orphans remain. A high-effort code review's **6 findings were all fixed** (none deferred, WA#6): stratum-DISTINCT over-application reverted; maturity status aligned; backfill INNER-JOIN miss re-keyed; maturity scope trailing-separator trim; `alias_lower` NFC re-fold; `getBacklinks` JS NFC.
> - **Doc:** `docs/MIG-085B-Architect-Plan-Unicode-Name-Fold.md`. **Approved follow-up still open:** MIG-086 — Tier-2 link suggestions for orphans/fragiles (relatedness via the Index FTS5 co-occurrence, one-click create).
>
> ---
>
> *Below: the v3.02 preamble, retained for diff visibility.*

> **What changed in v3.02** (**MIG-080 §F — Review Pulse SPLIT shipped; then MIG-084 — The Rich Reviewer SHIPPED + Boss-validated end-to-end (decision surface, 6 lenses, computed+overridable Priority, per-note Prescription); Phase-4 audited; perf-measured on the live corpus**):
>
> - **MIG-080 §F COMPLETE + Boss-validated** — Review Pulse is SPLIT by scope (Boss ruling): the **note's own status** is a right-sidebar **note-scoped** tab (`ReviewStatusPanel`, O(1) `get_note_review_status`); the **universe queue** moved to a **LEFT-DOCK** full-page reviewer (🕐 clock dock icon). A §F adversarial review caught + fixed 11 findings before the Boss test (incl. the i18n `$t||fallback`-is-dead P1 — a missing key renders the raw path, so all new keys were added to 15 locales; and the reviewer-overlay-stacking P1). The transitional universe `ReviewPulsePanel` was retired.
> - **MIG-084 — The Rich Reviewer COMPLETE + Boss-validated** (Eisa, after §F: "a thin list on a vast empty page → make it a rich decision surface"). The left-dock reviewer is now a **master-detail decision surface**: a six-lens queue (🥀 Stale · 🔄 Due · 🧠 Checkpoints · 🔗 **Orphan — connect me** · ⚠ **Fragile — shore me up** · 📝 Never; all always shown, empty muted+0, collapsible) and a detail pane that **diagnoses → prescribes**: the plain-language why-now, a per-note **Prescription** (the one healthy remedy), a **computed Priority rendered as a readable recipe bar** (two-axis Eisenhower urgency×importance; segments sum to the 0–100 score) that the user can **override** (badged "manual" + "computed would be N" + Reset-to-computed), the always-on summary, CeCe's headline, connection counts, maturity, and hand-offs (Open · 360 · Classify). A top-tab **Return-to-Reviewer** restores the note you were on.
> - **The orphan correction (Eisa):** an orphan is an **ALARM**, never "disposable" — a disconnected note demands attention. Connection counts are a diagnostic in BOTH directions (orphan = "connect me"; fragile = "many depend, little holds it up"). Orphan's action is **Connect**, never a default Dismiss.
> - **The Reviewer is the constellation's TEMPORAL-TRIAGE instrument** (the prescription cross-check): the only **push (not pull)** surface, keyed on **time** — it scans-and-queues; the siblings (Knowledge Health, 360, CeCe, Living-Links) **explain**. It hands off to them, never rebuilds them.
> - **Rule-8 wins:** Orphan/Fragile detect from the write-time `note_meta.incoming_count`/`word_count` (+ a cheap derives count) — no FS walk. The originally-planned maturity-denormalization migration **collapsed** into reusing the existing `maturity_sql` thresholds at read time. The computed-priority engine (`src/lib/reviewer/priorities.ts`, 7 vitest tests) is pure + frontend (no formula in Rust); `review_priority` became a **nullable OVERRIDE** (NULL = use computed). **Perf** (measured on the live 7,660-note universe): the priority pass no longer full-scans `note_meta` (~480 ms) — it fetches only the due notes' rows.
> - **Process:** THREE adversarial reviews (find→verify) across the build fixed 11+12+13 findings, then the **`/migration` Phase-4 audit** (3 agents) found 6 (no P0/P1) — 4 fixed (dismissed-alarm gate; canonical days_overdue = one priority per note across rows + the note tab; 360 SPOF reconciled to outgoing-derives), 2 deferred to focused follow-ups.
> - **Approved follow-ups (build next):** **MIG-085** cross-surface diagnostic consistency (single-source maturity inbound to `note_meta.incoming_count` across maturity.rs/inspector360/the trigger; reconcile tension.rs SPOF to outgoing-derives); **MIG-086** Tier-2 link suggestions for orphans/fragiles (relatedness via the Index FTS5 co-occurrence, one-click create). Observation: the Reviewer is currently scoped to `libraries[0]` — multi-library universes won't aggregate.
>
> ---
>
> *Below: the v3.01 preamble, retained for diff visibility.*

> **What changed in v3.01** (**MIG-083 SHIPPED end-to-end — Review Pulse is now Rule-8 write-time + the new Mode-2 staleness lens, Boss-validated live, triple-reviewed, audited; MIG-080 §F now unblocked**):
>
> - **MIG-083 §A–§E COMPLETE + Boss-validated.** The Review panel that re-walked the whole library filesystem on every open (a Rule-8 violation: `read_dir` recursion + `fs::metadata` per note + `read_to_string`+regex per checkpoint) now reads a **write-time-maintained `review_schedule` SQLite table** — an indexed `SELECT … WHERE due_days <= today` with **zero filesystem access**. Measured on the live **7,660-note** "Eisa Cognitive Knowledge" universe: **get_due_notes 232 ms → ~18 ms**. The legacy `scan_due_recursive` is removed.
> - **Mode-2 "Staleness" — built + Boss-validated live.** A note is flagged **stale** when a **load-bearing OUT-dependency** (`supports`/`contradicts`/`derives-from`/`part-of`/`supersedes`; NOT `associative`) had its **content actually change** (FNV-1a body hash; a touch/sync/frontmatter save does NOT fire) on a later **local** calendar day than the note's last **explicit ✓ review**. A **separate lens** from time-based Due-for-Review (Boss: never merged). Both lenses surface together in `get_due_notes` (each row carries its `reason`; stale rows carry the triggering neighbour/type/date). Resolution via `note_links.target_cid_cn → note_meta.cid_cn`. Boss saw it live: 🥀 on a note whose dependency changed after its review.
> - **Boss rulings this session:** (1) staleness fires on **substantive (plain-body) edits** only — diacritic/formatting-only is a documented fast-follow; (2) the two lenses stay **fully separate** — snooze hides from Due-for-Review (Lens-1) only, a snoozed note can still surface as Stale; (3) a configurable **staleness grace period** Setting (Settings → **Review** → days, min 1, default 1) — a dependency change flags only ≥ grace days after the review (grace=1 = strict next-day). Localized ×15.
> - **Process:** §D went through the §D/§E adversarial pattern — a **24-finding review** + a **4-agent re-verify** + the **`/migration` Phase-4 audit** (invariants/drift/migration-path). Every confirmed finding FIXED (none deferred, WA#6): the P1 mtime-fallback false-fire (→ baseline `content_hash` + require `content_changed_at IS NOT NULL`); snooze-lost-on-reindex + sibling-library bleed + UTC/local day skew + self-links + malformed-date + non-deterministic tie-break; a NEW rename-phantom regression (→ migrate the row + INNER-JOIN guard); and the **missing reconcile self-heal** (`review::recompute_all_in`, now mirroring `tag_counts`). A **live-copy rehearsal harness** (`review_rehearse.rs`) proves on the real corpus: parity vs an independent reference, touch-test 0, <100 ms, Mode-2 fires.
> - **MIG-080 §F now UNBLOCKED** — the note-context **Review tab** (`get_note_review_status`, O(1)) + the full-page **two-lens reviewer** over the now-cheap read. Then §G closes MIG-080. (`record_note_visit→openNoteTab` stays dropped: opening ≠ review.)
>
> ---
>
> *Below: the v3.00 preamble, retained for diff visibility.*

> **What changed in v3.00** (**a marathon session: MIG-080 §B–§E shipped (note-context right-rail splits) + TWO new top-principals + MIG-083 opened (Review Pulse Rule-8 redesign) — architected, planned, §A + §B-1 built, and a NEW research-validated feature concept (change-triggered staleness) defined**):
>
> - **MIG-080 §B–§E COMPLETE + Boss-validated** — the right sidebar is now note-context: §B Tags (note-scoped) · §C Tasks (left-dock agenda + theming + Style-Setter + NL dates) · §D Source Review split (note-rail vs Cataloger) · §E Knowledge Health split (note's tensions vs universe dashboard). **§D + §E were each hardened by adversarial-review passes that caught 17 real defects before the Boss tests — two genuinely dangerous** (a universe-wide "Reject all" one click from a single note's card; a correctness bug + a stale-status flash in §E). All fixed, none deferred. (§F now BLOCKED on MIG-083; §G remains.)
> - **TWO new top-principals (CLAUDE.md + memory):** **Concept Before Function — the Horse and the Carriage** ("a function without a concept is a carriage with no horse — state the concept first"); **Fix What You Discover** (Working Agreement #6 — "if a review surfaces it, fix it in the same pass; no documented-but-unfixed defects").
> - **MIG-083 — Review Pulse → Rule-8 write-time schedule (IN PROGRESS).** Concept-before-function caught that §F's full-page reviewer would be built on `get_due_notes`, a Rule-8 violation (full-FS-walk + content read of every `.md` per open). Boss ruled fix Rule 8 first → MIG-083. **Architected** + **Planned** (`docs/MIG-083-Architect-*.md`, `docs/MIG-083-Plan.md`; 5 phases, Boss-approved): a derived `review_schedule` table maintained write-time in `index_note` (the `tag_counts` pattern), gated on `schema_versions.review`, resumable back-fill; reads become `SELECT WHERE due_days<=today`. **Built: §A** (inert table + corrected pure logic: 1·3·7·14·30 ladder, `tags_json` checkpoint, staleness trigger-types; `c30431e6`) **+ §B-1** (gated DB-maintenance fns + corrected `mark_reviewed` + `record_note_visit` removed; `73a79745`). **18 review unit tests pass; everything INERT (gated off until §C).**
> - **NEW feature concept — Mode 2 "Staleness" (defined + WA#5-validated; concept paper §12):** a note is STALE when a **load-bearing OUT-dependency** (`supports`/`contradicts`/`derives-from`/`part-of`/`supersedes`; NOT `associative`) had its **content change** after its last **explicit ✓ review**; all maturities; 1-hop; a **separate lens** from time-based review. It's the **Truth-Maintenance-System / changed-dependency-invalidation** pattern applied to typed Living Links — **novel in PKM** (no SR tool triggers on a neighbour changing; literal analog: dosu.dev's "Git Age Delta"). Research caught the first predicate would over-fire → scoped to out-deps/load-bearing/content-hash/1-hop. (`record_note_visit→openNoteTab` §F wiring dropped: opening ≠ review.)
> - **NEXT (resume here): MIG-083 §B-2** — the `index_note`/`reindex_delete` hot-path hooks + the action-writer row-sync. Then §C (back-fill+stamp) → §D (read swap + staleness JOIN + harness + Boss test) → §E (cleanup+audit) → MIG-080 §F → §G. **Open §B-2 sub-questions:** the `sky_nodes.stratum` TEXT format to parse for real stratum; the action writers need `app.state::<SearchState>` DB access to sync the row; `content_changed_at` (Mode-2 signal) best folded into §D where the staleness JOIN lives.
>
> ---
>
> *Below: the v2.99 preamble, retained for diff visibility.*

> **What changed in v2.99** (**MIG-080 §E COMPLETE — Knowledge Health split into note-context (right-rail) vs universe (KH Dashboard); 17 findings across THREE adversarial review passes, ALL fixed; + TWO new top-principals canonised**):
>
> - **§E — Knowledge Health split.** *Concept (the horse):* the right-rail Health tab answers **"what is unhealthy about THIS note?"** (contradicted / orphaned / single-point-of-failure / in a structural gap); the universe `KnowledgeHealthDashboard` (full-page) stays whole. **Option B architecture** (chosen after the review exposed the plan's server-side per-note filter as wrong-shaped): `detect_tensions` runs ONCE per library (cached), and `tensionReport` is a **`$derived` client-side slice** (`filterTensionsForNote`) — no per-note-switch recompute (Rule 8). `GapItem.member_names` (full, for correct gap membership); contradictions emit **BOTH perspectives** (source "contradicts X" + target "contradicted by Y" — a contradicted note isn't healthy); a new O(1) `note_tension_status` IPC drives **honest states** ("not analyzed yet" / "shares its title — can't reliably assess") instead of a false ✓; `screen:note-saved` **debounced refresh**; tension **details localized ×15** (`detail_kind`+`detail_args`). 9 new tension tests/strings; 2 new Rust unit tests.
> - **THREE adversarial review passes, 17 findings, ALL fixed (zero deferred).** §E-base (10, incl. the gap-truncation correctness bug + the per-note recompute) → fix-pass-2 (7, incl. a stale-status FLASH that would have defeated the honesty gate, + a Rule-8 re-detect on every autosave). This is the canonical arc behind the new "Fix what you discover" principle.
> - **TWO new top-principals (CLAUDE.md + memory):**
>   - **Concept Before Function — the Horse and the Carriage** *(Architecture Principles)*: state a function's clear, sound CONCEPT (its purpose) before building it; "a function without a concept is a carriage without a horse." Precedes Constraint-as-Design + Form-Aligns-To-Purpose in time. Operationalised by the concept-paper-per-function bring-up method.
>   - **Fix What You Discover** *(Working Agreement #6)*: if a review/audit surfaces it — any severity — fix it in the same pass; no documented-but-unfixed defects.
> - **NEXT — §F (Review Pulse split)** → §G (final reconcile + audit + /simplify).
>
> ---
>
> *Below: the v2.98 preamble, retained for diff visibility.*

> **What changed in v2.98** (**MIG-080 §D COMPLETE — Source Review split into note-context (right rail) vs universe (Cataloger), hardened by a 4-lens adversarial review that caught a DESTRUCTIVE bug before the Boss test + a Cataloger text-resize slider. All Boss-validated**):
>
> - **§D — Source Review split.** The right-sidebar Source Review now shows ONLY the OPEN note's pending source suggestions (`sources_get_suggestions(notePath)`); `'sourceReview'` joined `NOTE_SCOPED_TABS` so it shows "no note selected" when none is open, like Properties/Backlinks. The universe-wide suggestion queue stays in the **Cataloger** (left dock), which mounts the same `SourceReviewPanel` WITHOUT `activeNotePath`. No new Rust IPC — `loadQueue` just branches on `activeNotePath`, with a note-switch `$effect` + a monotonic load-token guard.
> - **The adversarial review earned its keep.** Per WA#4 + secure-don't-muddle, §D got a 4-lens review (reactivity / read-path / gate / regression) → adversarial verify. **7 findings, all confirmed, fixed before the Boss test:** (1) **DESTRUCTIVE** — the per-note rail still showed universe-wide **"Reject all"** (`DELETE FROM sources_suggestions`, no WHERE) → one note's panel could wipe the whole Universe queue → gated on `!activeNotePath`; (2) right-click "Suggest sources" re-routed to the **Cataloger** (it targets an arbitrary note, not the open tab); (3) `handleClassifyAndShow` now ignores foreign events on a note-scoped instance (no cross-note contamination); (4) `loadQueue` cancel-previous race guard; (5) stale comment fixed; (6) `read_suggestions` now propagates real DB errors instead of masking them as "empty".
> - **§D-text — Cataloger text-resize slider** (Style Setter → Cataloger → Text size). Reuses the §H `--rs-scale` the embedded `SourceReviewPanel` already honors: `.cataloger-view { --cat-scale }` drives the chrome AND `.cataloger-queue { --rs-scale: var(--cat-scale) }` drives the cards; the right-rail panel keeps its own scale. Zero new i18n.
> - **NEXT — §E (Knowledge Health split)** → §F (Review Pulse split) → §G (audit + /simplify).
>
> ---
>
> *Below: the v2.97 preamble, retained for diff visibility.*

> **What changed in v2.97** (**MIG-080 §C COMPLETE — the universe Task agenda relocated to the left dock + Global Tasks now follows the universe theme + a Style-Setter "Global Tasks" tab (colours + text size). All Boss-validated. + the Backup & Recovery system banked as a wanted feature**):
>
> - **§C.3 — universe Task agenda → left dock.** The note-context right rail keeps the OPEN note's tasks; the universe/library agenda relocated LEFT per the MIG-080 disposition. **Reuse:** `GlobalTasksView` (already had filters + date-grouping, was command-palette-only) now has a **left-dock "Tasks" button beside the Calendar** (`+layout.svelte`, modeled on the Calendar button). Reused `commands.globalTasks` (×15, zero new i18n).
> - **§C.3b — Global Tasks follows the universe theme + a Style-Setter tab.** **Bug:** the view rendered dark regardless of theme — its background used `var(--bg-primary, …)`, but `--bg-primary` is NOT defined in `theme.css` (the real var is `--background-primary`) → it always fell back to the dark literal. **Fix (the Calendar pattern, 32-replacement rewrite):** every surface → `var(--gt-X, var(--real-theme-var, literal))` — fixes the broken names (`--bg-primary`→`--background-primary`, `--border-faint`→`--border-light`) so it follows the universe theme, AND adds the `--gt-*` override layer. New Style-Setter **"Global Tasks"** category (9 colours + Text size).
> - **§C.3c — Global Tasks text resizing.** 14 font-sizes wrapped `calc(X * var(--gt-scale, 1))`; `.global-tasks { --gt-scale: calc(var(--gt-text-scale,100)/100) }`; a "Text size" control (reuses the `text_size` label). The 7 new Global-Tasks Style-Setter labels (Global Tasks/Surface/Text/Border/Row hover/Overdue date/Due-today date) translated ×15.
> - **§C is COMPLETE** (note-rail tasks ✓ · toggle_task reindex E.2 ✓ · SecondScreen reconcile ✓ · NL date autosuggest ✓ · left agenda ✓ · Global Tasks theming + Style-Setter ✓). **NEXT — §D (Source Review split)** → §E/§F/§G → bring-up §D.
> - **Backup & Recovery system — WANTED FEATURE banked (Boss).** A user-facing worst-case safety net for the PKM/PKF. `docs/concept-papers/Backup-System-Concept-Paper.md` (constraints: File-Over-App restorable-without-Constellation, Local-First, index-rebuilt-on-restore, archival-not-deletion; options; open questions; prior art) + memory `project_backup_system_wanted`. After MIG-080, via `/migration` + WA#5.
>
> ---
>
> *Below: the v2.96 preamble, retained for diff visibility.*

> **What changed in v2.96** (**MIG-080 §C.1/§C.2 — Tasks: the `toggle_task` reindex defect (Debt E.2) FIXED + the SecondScreen reconcile + a research-backed natural-language task-date AUTOSUGGEST. Boss-validated ("a milestone"). + the bring-up cross-check decision + the right-click reference banked**):
>
> - **§C.1 — `toggle_task` reindex (Debt Register E.2) + SecondScreen reconcile (`tasks.rs`, `SecondScreenPage.svelte`).** Ticking a task off wrote via the gate but never reindexed → FTS/body drift; now it calls `reindex_single_note` after the gate write (the `bases.rs:745` pattern). **Freeze-safe (verified):** a checkbox toggle changes no `[[links]]` → `index_note`'s unchanged-edges guard (search.rs:4737) skips the rebuild + the trigger cascade → no ~40 s freeze. SecondScreen's two raw `toggleTask()` sites → `toggleTaskReconciled` (BUG-015 F2 fold-in; **honest residual**: the cross-window main-tab race needs a route-to-main-window refactor — flagged for the Second-Screen bring-up). Editor-Surface Gate: **Boss "All pass."**
> - **§C.2 — natural-language task due dates (research-backed).** Boss finding: tasks don't recognise "tomorrow/next week/etc." First built v1 (auto-convert on Enter) → Boss asked to **research proven methods (WA#5)**. **Research verdict (Workflow `wf_cd63316f`, cited: Obsidian nldates / Obsidian Tasks / Todoist / TickTick / Things / chrono):** mature systems NEVER disambiguate prose-vs-due-date by heuristics — they use a **structural gate** (a `@` trigger, a task-line gate + signifier, a dedicated field, or highlight+undo+toggle). My v1 whole-line auto-convert was the one approach none use. **v2 (Boss-decided): autosuggest-you-accept + the Obsidian `@` trigger + a Settings toggle.** `src/lib/editor/taskDates.ts` (NEW, PURE, unit-tested ×10) resolves keywords (today/tomorrow/yesterday/next week/next month/weekdays/"in N days|weeks"); `createTaskDateCompletion` (completions.ts) is a CM6 source **gated on `TASK_RE`** (prose lines inert → false-positive structurally impossible) + the toggle; type `@` → a date menu, or a bare keyword → a fallback suggestion; accept → inserts `📅 YYYY-MM-DD`. NotePane only (FocusPane plain). Setting `naturalLanguageTaskDates` (default true) + Settings → Editor toggle, i18n ×15.
> - **Bring-up cross-check (Boss re-raised the 2026-06-15 program; SO #5/#8).** The minimal-editor bring-up (`docs/concept-papers/00-MASTER`) has §A/§B/§C done (boot crisis SOLVED by MIG-079); **§D (phase-by-phase re-enable) UNSTARTED** — the last days were features (081/082/080) instead. Key finding: **MIG-080 §C IS Debt Register E.2** (the Tasks bring-up's defect). **Boss decision: FINISH MIG-080 first (§C–§G), THEN bring-up §D fresh.** The bring-up's value now = the systematic per-function correctness pass (Rule-8 / right-click / i18n / the 5 defects), not boot discovery.
> - **Right-click reference banked (Boss-supplied).** Obsidian's Note/Folder/Link/editor-empty context menus → `docs/concept-papers/Right-Click-Reference-Obsidian.md` + memory `project_rightclick_obsidian_targets` — the target for the right-click bring-up (Debt B / concept-paper §5). LATER, not now.
> - **NEXT — §C.3 (the left task agenda)** → §D Source Review / §E Knowledge Health / §F Review Pulse split / §G audit; then the bring-up §D.
>
> ---
>
> *Below: the v2.95 preamble, retained for diff visibility.*

> **What changed in v2.95** (**MIG-080 RESUMED — the right-sidebar → note-context cascade: §0 inspector360 Settings fix + §B Tags-note-scoped (Dashboard reads tag_counts) + §H per-panel right-sidebar TEXT SIZE (Style Setter) + Review-Pulse polish. All Boss-validated; frontend-only; §C next**):
>
> - **MIG-080 cross-checked before resuming (SO #8, 5 agents).** The approved Plan (`docs/MIG-080-Plan.md`) was pre-MIG-081/082; the cross-check confirmed it VALID + found: §A is shipped+superseded (Calendar fully serves daily notes); the "deferred inspector360 bug" IS the Plan's own skipped §A fold-in; §C's `toggle_task` reindex defect + the SecondScreen unreconciled-toggle still real; the Plan's `file:line` anchors stale. Recorded in `lab/reports/SESSION-LOG-2026-06-20.md`.
> - **§0 — inspector360 Settings-UI fix (`f767fca3`).** Added `inspector360` to Settings → Panels placement list (`SettingsModal.svelte` — it was in the schema + tab strip but missing from the UI; the long-deferred bug). +i18n ×15 (`panelInspector360`).
> - **§B — Tags note-scoped (`f767fca3`).** The right-rail Tags tab now shows ONLY the open note's tags (`'tags'` added to `NOTE_SCOPED_TABS`; the All-tags/This-note toggle removed). The universe All-tags browse moved to the **Dashboard**, which now reads the write-time **`tag_counts`**-derived `allLibraryTags` (passed as a prop) instead of the per-library `scanAllLibraryTags` filesystem walk (Rule 8). `#tag`→Search-Hub routing unchanged.
> - **§0b — 360.3D bar-row margin (`0920c5f9`).** `.i360-bar-row` grid `130px 1fr 60px` → `minmax(0,130px) 1fr 60px` so the count column can't overflow a narrow sidebar (the bars sit ≥12px off the edge). All of §0/§B/§0b Boss-validated + pushed.
> - **§H — PER-PANEL right-sidebar text size (Style Setter).** Boss's evolving ask (zoom → text-only global → **per-panel**): each right-sidebar panel has its OWN text size. **181 font-sizes** across 10 panel components + the `.rs-*` rules wrapped `calc(X * var(--rs-scale, 1))` (rem/px only — `em` skipped to avoid double-scaling; `,1` fallback → pixel-identical at default; verified by an adversarial 6-invariant review). `--rs-scale` is set INLINE on `.rs-inner` from the **ACTIVE tab's** token (`var(--rs-text-scale-{tab},100)/100`) — one dynamic binding scopes the scale to whichever panel is open; a panel rendered elsewhere (e.g. PropertyEditor inline in NotePane) stays unscaled. Style Setter: a new **`cRsText` "Right Sidebar Text"** element (Components category) with **10 per-panel sliders** (70–140%, def 100). i18n ×15 (reused the translated panel names; `Right Sidebar Text` + `Source Review` translated natively). **Text-only — spacing untouched.**
> - **§H Review-Pulse polish (Boss findings):** (1) the redundant Review **tab count badge removed** (the section header already shows the count; the `.rs-tab-badge` was the only badge → gone, markup+CSS); (2) the lists **show the FULL set by default** (no "+N more") — plain render below `VLIST_THRESHOLD`(80), `VirtualList` window above (a fresh universe can make "never reviewed" the whole library; `get_due_notes` has no cap → Rule 3 protection). +i18n cleanup.
> - **Editor-Surface Gate UNTOUCHED** — §0/§B/§H are settings/styling/panel-render/i18n only; NO note content/save/lifecycle code. (§C is the first §-phase that touches the write path — `toggle_task` reindex — and will run the full gate.)
> - **NEXT — §C (Tasks):** the `toggle_task` reindex defect (Editor-Surface Gate) + the left task agenda + the SecondScreen unreconciled-toggle fix (Boss-approved fold-in). Design analysis: `wf_29c533af`. Then §D (Source Review) / §E (Knowledge Health) / §F (Review Pulse split) / §G (audit).
>
> ---
>
> *Below: the v2.94 preamble, retained for diff visibility.*

> **What changed in v2.94** (**MIG-082 docs i18n catch-up COMPLETE — the calendar documentation localized ×14; the stale EN help-site Calendar topic brought current; docs-only, no code/schema/Editor-Surface change**):
>
> - **User Manual §14 (the clickable, 8-calendar Calendar) translated into all ×14 non-EN languages.** Each `docs/help.{lang}/User Manual.md` carried a STALE ~10-line calendar section (the pre-MIG-081 "shows notes by date, open from sidebar" placeholder); each was replaced with a faithful, native translation of the current EN §14 — left-dock full-page view; gold/purple/red clickable dots; empty→daily note; task-dot→open-at-line; complete-from-popover; the eight cultural calendars incl. Chinese/Korean lunisolar; Hijri calculation-method + month-correction; Chinese/Korean year-display + native/phonetic month names; Style-Setter theming; the daily-note Hijri stamp + the Properties "+ X" converter. **Heading lines + section numbers preserved (TOC anchors intact); no renumbering.**
> - **The EN help-site Calendar topic was STALE and is now current.** `docs/help.uConstellation.World/Calendar/Calendar.md` had never been updated for MIG-080/081/082 — it still described the Calendar as a *right-sidebar panel* with only purple/red dots and no cultural calendars (last git-touched pre-MIG-080). Rewritten from the validated §14 (left dock, gold daily-note dot, eight calendars, clickable days, cultural-date recording, RTL note). **Then translated into ×14 NEW files** `docs/help.{lang}/Calendar/Calendar.md` (the translated help dirs previously had no Calendar topic at all — this is the first net-new help-site topic coverage for the calendar).
> - **Method = Workflow fan-out** (`wf_62dfd578`, 14 native-translator agents, one per locale, `agentType: general-purpose`, each owning ONLY its locale's two files — no shared files, no positional-mapping risk). **Verified, not trusted** (the §B.3 fabrication lesson): glyph sweep — 五月 / 5월 / 闰六月 / 윤6월 / 丙午年 / 병오년 / 단기 4359 / Wǔyuè / Owol / وُو-يوي / أوه-وُل / `2025-06L-17` / al-Tawfīqāt al-Ilhāmiyyah preserved VERBATIM across all 28 translated files (the lone `단기 4359` grep-miss was Korean's faithful `**단기** 4359` bold markup); structural uniformity (every new topic 90 lines, frontmatter + headings + 3 `[!tip]` callouts intact); the **Arabic + German read directly** — fluent, faithful, no fabrication; phonetics copied verbatim from the Boss-verified source. `git diff --stat`: 14 manuals (+32 each) + 1 EN topic (+92) + 14 new topics.
> - **Correction logged (stale-PJ catch).** The MIG-082 handover's "Boot perf — defer the Sky read off boot" next-option was STALE: that shipped in **MIG-079 §C.2d** (orientation v2.89; commits `23fdd45f`→`cff8f827`→`2f3c8aa0`; measured cold `graph_ready` ~11 s → ~1.5 s, Boss-validated). There is **NO open boot-perf "defer the Sky read" task.** Memory `project_mig079_boot_wtd` corrected to match (it predated §C.2d).
> - **Standing debt cleared:** the calendar docs translation debt (handover §"Standing debts" — "User Manual §14 + help-site updated EN-only; ×14 pending") is RESOLVED for the calendar. App behaviour unchanged.
>
> ---
>
> *Below: the v2.93 preamble, retained for diff visibility.*

> **What changed in v2.93** (**MIG-082 COMPLETE + Phase-4 AUDITED — the clickable Calendar across 8 calendars. §A interactions (dots open items, empty→daily note, open-at-line, toggle-complete with a single-ownership reconcile) + §B Chinese/Korean lunisolar (per-calendar year-display + native/phonetic month-name settings; Arabic phonetic Boss-verified) + §C the daily-note Hijri stamp & the regular-note Gregorian→cultural converter for every selected calendar. The 4-lens migration audit found 3 P1s — ALL FIXED; `/simplify` done. Boss-validated throughout**):
>
> - **MIG-082 — Clickable Calendar + 4 new calendars + open-at-line.** Boss asks (post-MIG-081): the OTHER calendars (Indian/Buddhist/**Chinese/Korean** — all, incl. the hard lunisolar); clickable dots (note/task → open the item, empty cell → daily note, task → open at its line to add/edit); + the **duplicate question — ANSWERED: clicking an existing date OPENS the daily note, never duplicates** (deterministic Gregorian-ISO filename + open-if-exists gate + tab reuse; verified). Note dot = any note edited that day, the **daily note distinguished by colour**. Recon (`wf_c51636b5`) + 4-agent Architect (`wf_f0c6d756`, doc `docs/MIG-082-Architect-Calendar-Interactions.md`) + Plan (`docs/MIG-082-Plan.md`), Boss-approved. **Key Node-verified finding:** the installed Temporal polyfill works for Indian/Buddhist but THROWS for Chinese/Korean (leap-month codes) → Chinese/Korean get a self-contained **Intl-only lunisolar branch** (no Temporal, can't regress the 4 working systems).
> - **§A.1 SHIPPED (`951a464d`)** — cell restructured to a `role=gridcell` with a full-bleed bg button (empty/number → daily note) + per-dot `<button>`s (open the item; a popover for 2+); daily-note dot distinct (`--cal-daily-dot`) + `--cal-dot-size`, backed by a Rust **`is_daily`** flag (`is_daily_note_for` mirrors `get_daily_note_path` — one source of truth) + `library_name`; the scan keeps full per-date `NoteDateEntry[]`/`TaskItem[]` (de-duped). + fixes: Hijri year no longer comma-grouped ("1448"); the **secondary-calendar setting WIRED** (was dead) — `'none'` = single calendar (no per-day 2nd date AND no subtitle range), pick a system → its day under each cell + its range as the subtitle (resolves the §C secondary decision = ACTIVATE).
> - **§A.2 SHIPPED (`1c7c1f44`)** — task/note dot opens the note **AT the task's line** (+ the Tasks panel, reuse). One-shot pending-line-jump (`src/lib/editor/lineJump.ts`, CM6-free) consumed in NotePane's mount block → **selection-only** dispatch + scrollIntoView (no `{changes}`, no save — Gate #2); already-active note jumps via `activeEditor.goToLineIfActive` (path-verified). Adversarial editor-path review: no P0/P1. + the dot-default pin (`--cal-note-dot` no longer accent-coupled).
> - **§A.3 SHIPPED (`e5c18617`)** — toggle a task complete from the calendar popover (a complete-checkbox), **single-ownership reconciled**: `store.ts` **`toggleTaskReconciled`** = flush-if-dirty → `toggleTask` → `reloadTabsFromDisk`, reused by the calendar + Tasks panel + GlobalTasksView (fixes their pre-existing latent reconcile gap). Calendar **live-refreshes** after a toggle (`refreshCalendarData()`). **⚑ Adversarial review caught a P1 (BUG-015 F2 class) — FIXED before ship:** the `reloadTabsFromDisk` `{#key}` remount let the old NotePane's teardown flush revert the toggle on a dirty open note → wrapped the reload in **`markCascading`/`clearCascading`** (the same guard the rename-cascade reload uses). Boss-validated incl. the dirty-note safety gate.
> - **§A.4 SHIPPED (`29fe6644`)** — Indian (Saka) + Buddhist calendars (solar, generic Temporal branch); header labels show **"MonthName (N) Year"** for ALL systems (Eisa: the number helps with unfamiliar cultural months) — built from `{month:'long'}` so the **Persian double-era is fixed** ("Khordad (3) 1405 SH") in the same construction; the secondary subtitle carries the numbers too ("Dhul-Hijjah (12) 1447 – Muharram (1) 1448 AH"). **Hijri month-1 المحرَّم→محرم** in the vendored `hijri.js` + pushed upstream (`eisaShamsi/hijri-calendar@0436e32`, vendor pin updated). **§A is COMPLETE.**
> - **§B SHIPPED + Boss-validated** — Chinese + Korean **lunisolar** calendars, driven entirely by host `Intl` (the Temporal polyfill throws on their leap months). Display month = an ORDINAL (1..12/13) so leap months (闰六月/윤6월) are their own page; new-year scan + 28-day-jump walk, memoised, `timeZone:'UTC'`-pinned (verified across 1900–2200 by an adversarial agent). Each calendar in its OWN script (五月 / 5월; Korea uses the Chinese calendar → identical dates). **User-configurable** (synced settings, shown only when that calendar is active): **year display** — Chinese sexagenary `丙午年`±year / Korean Dangi `단기 4359` / Gregorian; and **month names** — native (五月) OR **phonetic** (Boss-directed): the pronunciation in the UI script, AUTHORED tables (OS can't transliterate) — Latin = Pinyin/Korean-RR, **Arabic Boss-verified** (وُو-يوي / أوه-وُل). Plus **×15 i18n** for every new calendar-settings string (Workflow fan-out). **MIG-082 §A + §B both COMPLETE.**
> - **§C SHIPPED (`9b52c480`, `f2fc85bb`)** — the opt-in daily-note **Hijri stamp** (a `hijri:` frontmatter line on NEW daily notes; Hijri-only; the toggle is disabled unless Hijri is main/secondary; Rust writes it sanitised + creation-only). + the regular-note **Gregorian→cultural converter**: a "+ X" button in a note's Properties for each non-Gregorian calendar the user selected (Hijri/Jalali/Hebrew/Saka/Buddhist/Chinese/Korean) — reads the note's date (`date:`→`created:`→file-creation) and adds the equivalent property; Korean stamps the Dangi year, a lunisolar leap month gets an `L` marker; all via the single-ownership `saveTabContent` path. + the §A-era i18n. **Phase-4 audit (`5925e783`):** migration-path CLEAN; **3 P1s found + ALL FIXED** — the `toggleTaskReconciled` cascade-gate ORDERING (a real BUG-015 F2 fix to already-shipped §A.3 code: mark BEFORE the flush+toggle, not just the reload), the Korean Dangi stored-value mismatch, and the converter's standalone-panel teardown guard. `/simplify` extracted the shared `culturalDateString` helper. **MIG-082 is COMPLETE, audited, and clean.** Per-feature behaviour in the body §4.x.
>
> ---
>
> *Below: the v2.91 preamble, retained for diff visibility.*

> **What changed in v2.91** (**MIG-081 COMPLETE — the Calendar's 4 new functions + per-element Style-Setter theming + full ×15 localization, all Boss-validated; 3-agent migration audit clean**):
>
> - **§C.2f — Month Correction + Calculation Mode (Calendar Settings).** A **Hijri calendar (Islamic)** subsection in Settings → Calendar: a **Calculation method** selector (Astronomical (Lunar Conjunction) / Tabular (al-Tawfīqāt al-Ilhāmiyyah)) + a **Month correction** editor (Hijri year/month + offset, default **0**, ordered +2/+1/0/−1/−2, reflects the chosen month's existing value; list with × remove; Clear all). Stored in **appSettings (synced — Boss's call)**, pushed into the singleton engine via `calendarMath.applyCalendarPrefs` on load. Verified: both mode + corrections flow through the engine's `_engine()`/`_getCumulativeCorrection`-aware `hijriToGregorian`/`daysInMonth` — exactly the functions `buildMonthGrid` uses → the grid reflects them. Boss: **"All pass."**
> - **§C.2d — Style-Setter "Calendar" category.** CalendarPanel's `--cal-*` defaults moved off the `.cal-root` declaration into inline `var(--x, default)` fallbacks (so a body `styleOverride` / the preview draft INHERIT and win — a local declaration would block them). A `calendar` element + category (three-zone) + a **full-center-zone** centre preview that renders the REAL `<CalendarPanel>` under the `.ss` draft wrapper. **Per Boss, each element has its OWN colour + size** (Calendar font + 23 colours + 9 per-element size tokens `--cal-{pill,subtitle,today,nav,day,subdate,moon,weekday,week}-size` = 33 controls). Apply path = the existing `mergeStyleOverride`→`styleOverride`→`<body>` single-writer (BUG-015). Boss: **"What a milestone. All passed perfectly."**
> - **§C.2g — Daily-Note launcher dock button RETIRED.** The Calendar fully serves daily notes (click any day); `openDailyNote`/`handleOpenDailyNote` kept (command palette + Calendar page). Left dock = the Calendar button only. (`showDailyLauncher`/`.daily-launcher-*` removed.)
> - **Subtitle = the CROSS-REFERENCE calendar** (Boss refinement): the month pill shows the primary; the subtitle shows the OTHER calendar's range — Gregorian primary → **Hijri** range (e.g. "Dhul-Hijjah 1447 – Muharram 1448 AH"), non-Gregorian → Gregorian range. `RichMonthGrid.gregorianRange`→`subtitleRange` + new `hijriRange()` (Node-verified).
> - **§C.2e close-out.** **Fonts: Amiri already bundled + `@font-face`'d** (`static/fonts/Amiri-*.ttf` + `fonts.css`) → `--cal-font` renders in Amiri, no code. **i18n ×15 COMPLETE** (the full-localization TOP PRINCIPAL): the whole `settings.calendar` (34 keys) + `calendarPanel.weekAbbrev` + the **34 StyleSetter Calendar labels** translated into all 14 non-EN locales (native, via per-locale translator agents; format token `%Y-%m-%d` preserved; localized date-pattern notation per locale); orphan `ribbon.dailyNote` removed ×15. **3-agent migration audit:** invariants **7/7 HOLD**, migration **5/5 SAFE**, no P0/P1; the 3 P2s (StyleSetter-labels localization / orphan key / a documented localStorage-mirror side-effect) all fixed/documented.
> - **No new Rust; no schema-migration risk** — additive appSettings fields (`calendarCorrections`, `calendarCalculationMode`), `?? default` guarded everywhere; **Editor-Surface Gate untouched** (settings/styling/render/launcher only). New CSS-var contract: per-element `--cal-*` colour + size tokens, Style-Setter-editable.
> - **NEXT:** the MIG-080 §B–§F right-rail note-context cascade (Plan approved: `docs/MIG-080-Plan.md`) + the deferred inspector360 Settings-UI bug.
>
> ---
>
> *Below: the v2.90 preamble, retained for diff visibility.*

> **What changed in v2.90** (a marathon: **MIG-079 §C.2d shipped+validated; MIG-080 §A/§A.2 shipped+validated (Calendar relocated to the left dock); MIG-081 OPENED — Eisa's accurate Hijri engine vendored + cultural calendars + a RICH calendar UI ported from his app, Boss-validated "Perfect!"**):
>
> - **MIG-079 §C.2d — Sky read deferred off boot (validated).** `cache_boot_snapshot_sky` is now `#[tauri::command(async)]`; the 234k-row sky read no longer runs at boot (lazy on first Sky-surface open + an after-idle warm-up via `ensureSky()`, `_skyEpoch`/`_skyGeneration` guards). **Measured: cold `graph_ready` ~11 s → ~1.5 s, queue ~10 s → 0.18 s.** + the ×15 i18n cleanup for the §C.2c summary toggles.
> - **MIG-080 — Right Sidebar → Note-Context-Only (Architect+Plan approved; §A/§A.2 shipped+validated).** The right rail = the open note's context only; universe functions relocate. Seed: `docs/Right-Sidebar-Note-Context-Design-Decision.md`. Disposition: STAY ×6 (Properties, Backlinks, Tags-this-note, Sky-local, Provenance, 360.3D); SPLIT ×4 (Knowledge Health, Review Pulse, Source Review, **Tasks**-contextual); RELOCATE ×2 (Calendar→left, Tags-All→Dashboard). **§A:** Calendar removed from the right rail → a left-dock **Daily-Note launcher** (reuses §D-1's `get_daily_note_path(date)`); wrong-library fix; settings-migration + `NOTE_SCOPED_TABS` reconcile. **§A.2:** a separate left-dock **Calendar** button → a full-page month view. **+ file-tree-refresh fix.** §B–§F PENDING — Plan `docs/MIG-080-Plan.md`.
> - **MIG-081 — Cultural Calendars + the rich Calendar (the headline).** Boss: "use my accurate Hijri calendar" → **vendored `eisaShamsi/hijri-calendar`'s `hijri.js`** (`src/lib/calendar/hijri.js`, pinned @`a06be50e`) — an **astronomical** engine (real new-moon conjunctions via JDN + moon-sighting corrections), more accurate than Intl `islamic-umalqura`. + `@js-temporal/polyfill` (Persian/Hebrew). **`src/lib/calendar/calendarMath.ts`** unifies the 4 systems (Hijri via the engine; Persian/Hebrew via Temporal; all keyed to Gregorian ISO). A **Calendar Settings category** (primary/secondary system, week-start, week-numbers, daily-note format/folder). A **rich `CalendarPanel`** ported from Eisa's app (ornate green header, gold AH/sacred pill, Gregorian-range subtitle, dual-date cells, moon glyphs, Islamic-event dots, Wk column) — driven by the engine's `getMonthData`/`getMoonPhase`/`getEvent`/`isSacredMonth`; all colours via `--cal-*` CSS vars (Style-Setter-ready). **Hard invariant: daily-note filenames stay Gregorian ISO.** Boss: **"Perfect!"** Design: `docs/MIG-081-Architect-Cultural-Calendars.md` + `docs/MIG-081-Rich-Calendar-Design.md`.
> - **Boss's 4 NEXT asks (handover):** Month Correction (engine has setCorrection/clearCorrections/getAllCorrections); Calculation Mode selector (astronomical/tabular); a **Style-Setter "Calendar" tab** (wire the `--cal-*` tokens); **retire the Daily-Note dock button** (the Calendar fully serves daily notes). Then §C.2e fonts/×15 + MIG-080 §B–§F. See `lab/reports/HANDOVER-2026-06-17-mig081-rich-calendar.md` + `NEXT-SESSION-PROMPT.md`.
> - **New subsystem:** `src/lib/calendar/` (vendored `hijri.js` + `calendarMath.ts`); dep `@js-temporal/polyfill`. The Calendar is a first-class left-dock surface serving daily notes + cultural calendars.
>
> ---
>
> *Below: the v2.89 preamble, retained for diff visibility.*

**Version 2.89 | 2026-06-17**

> **What changed in v2.89** (**MIG-079 §C.2d — SHIPPED + Boss-validated: the Sky read is deferred off boot; cold `graph_ready` ~11 s → ~1.5 s, a ~7× boot win**):
>
> - **The remaining boot stall WAS the Sky read — confirmed by measurement, then removed.** Post-§C.2c the app was responsive at `hydrated≈1.1 s`, but cold `graph_ready≈11 s` was almost entirely **queue wait**: the graph IPC sat behind the synchronous `cache_boot_snapshot_sky`, which reads the **233,995-row `sky_links`** table cold. The IPC arrival trace nailed it — an **11,374 ms gap with NO other IPC dispatched** (a sync command monopolising the single IPC thread); the same read warm is **250 ms**, so the 11.4 s is purely cold disk I/O.
> - **The fix (Boss-approved Option B+): defer the Sky read off boot + async-ify + warm-up.** (1) `cache_boot_snapshot_sky` is now `#[tauri::command(async)]` (the §9.1 lever — runs on Tauri's worker pool, never monopolises the IPC thread). (2) The boot `loadGraph` no longer reads sky; `graphReady` fires on the graph (tags+aliases) payload only. (3) Sky View / CNS / Lens / Sight load the graph lazily on **first open** via a memoised `ensureSky()` (mirrors `ensureFullLinks`), gated by a new `skyEverOpened` flag (the LL-022 lazy-mount pattern). (4) An **after-idle background warm-up** (`schedule(() => ensureSky())` after `boot:graph-ready`) pre-loads sky on the worker thread so the first open is usually instant. Guards: `_skyEpoch` (universe-switch staleness) + `_skyGeneration` (same-epoch double-load from a `federation:ready` force). New `skyReady` flag; the Sky View spinner now gates on `!skyReady` (reuses the existing `layout.skyViewLoading` string — **zero new i18n**). `handleUniverseSwitch` now resets sky state (closes a pre-existing latent stale-sky race — `skyNodes` was never reset on switch).
> - **MEASURED (cold boot via PC restart, boot-history `[22]`): `graph_ready` 11,000–11,300 ms → 1,555 ms; queue 9,500–9,700 ms → 178 ms; `hydrated` unchanged (~1.3 s).** Boss: Sky View opened **instantly** (warm-up beat the open). Stage-2 PASS: notes untouched (Editor-Surface Gate), Lens shows real clusters, universe-switch shows the right graph in both universes.
> - **Option C ruled out (the handover's open question, answered):** "render nodes-at-boot, stream edges later" is NOT viable — the d3-force/PIXI engine needs all edges at init (no windowing/LOD). Edges + nodes load together on open. **WA#5:** defer-to-open is the Obsidian-v1.7.2 *DeferredView* / lazy-bootstrap standard; eager-full-graph-at-boot is the documented anti-pattern (Logseq/Obsidian freeze on large vaults).
> - **Migration rigor:** Architect (`docs/MIG-079-C2d-Architect-Defer-Sky-Read.md`, 4 mapping agents) → Plan (`docs/MIG-079-C2d-Plan.md`) → Boss-approved Option B+ → Build (§C.2d-1 `23fdd45f`, §C.2d-2 `cff8f827`) → **Audit (4 agents) BEFORE the Boss test**: all 8 invariants + 5 migration scenarios PASS; **two fixes applied** — P1 double-write race (the `_skyGeneration` guard) and P2 Lens-computes-on-empty-sky (`toggleLens` awaits `ensureSky()` first). `/simplify`: diff clean; 3 optional micro-refactors deferred to protect the validated binary. svelte-check 0; `cargo test --lib` 936/0.
> - **Boot-perf note:** the Sky read joins `note_links` (§C.2b) as a deferred-off-boot read; the only 234k cold read remaining at boot is now GONE. The boot pipeline (§15.2) reads core (notes/libs/settings) + graph (tags/aliases) only; sky + full edges are lazy.
>
> ---
>
> *Below: the v2.88 preamble, retained for diff visibility.*

**Version 2.88 | 2026-06-17**

> **What changed in v2.88** (**MIG-079 §C.2c arc — SHIPPED + Boss-validated: the in-memory 234k-edge array is GONE; Backlinks/Outgoing query per-note from SQLite + are virtualized; two summary toggles; a measured pivot corrected the boot diagnosis**):
>
> - **The §C.2b boot win was PARTIAL — and the measurement re-diagnosed the bottleneck (the session's pivotal finding).** §C.2b deferred the 234k `read_links` off boot (proven: boot record `read_links=None`, graph body 164 ms) — but cold `graph_ready` stayed ~17 s because the graph IPC waits `queue≈15 s` behind **`cache_boot_snapshot_sky`**, which reads the **233,995-row `sky_links`** table (the twin of `note_links`). So the Plan's assumption that `read_links` was THE boot cost was wrong: it was ~7 s of a ~24 s boot; **the Sky read is the co-equal/bigger half and is still on boot → the remaining boot win is §C.2d** (defer the Sky read off boot, same WTD/lazy model). Boot is still ~17–20 s until §C.2d.
> - **§C.2c — KILL THE IN-MEMORY EDGE ARRAY (the freeze fix), Boss-validated.** The Backlinks/Outgoing panels used to filter a 234k-edge JS array per note (the read-time-over-everything Rule-8 shape — the freeze + thrashing). Now they query **per-note from SQLite**: `get_backlink_rows(name, aliases)` (reuses the §C.2a alias-aware `target_name_lower IN (...)` set on `idx_nl_tnl`, federated) + `get_outgoing_rows(path)` (`idx_link_source`), behind `appSettings.perNoteLinkQueries` (default ON; the §C.2b array path stays behind OFF for rollback). **Rehearsal-proven** (80 hub/mid/aliased notes: query == brute-force == `incoming_count`, 0 mismatch). The boot edge-array load (`ensureFullLinks`) is a no-op when ON → the array never loads.
> - **Panels VIRTUALIZED (reuse `VirtualList`).** Row markup hoisted to ONE shared `{#snippet}` used by both `{#each}` (<50 rows → byte-identical to today) and `<VirtualList>` (≥50 → renders only the ~visible window; own scrollbar, `max-height:60vh`). Fixes the hub render (a 5,358-backlink note built ~55k DOM nodes per switch). **WA#5-validated** as THE proven pattern (Roam/Logseq/Obsidian froze precisely from NOT windowing). Plus a **leading-edge coalesce** on the per-note fetch (settled single open fires immediately; bursts use the 180 ms trailing debounce) — removes the felt open-lag.
> - **Two summary toggles (Boss-requested), both default OFF (leaner-by-default):** `noteSummariesEnabled` (Settings → Panels → Summaries) gates the NSC headline under each backlink/outgoing ROW (+ head-caps the fetch to 120 when on, + clears the map when toggled off); `noteTitleSummaryEnabled` (Settings → Editor) gates the note's own `.e-summary` headline under its TITLE (gated at the NoteEditor fetch — one guard covers main + second screen + all 7 hosts).
> - **Process arc (logged honestly):** §C.2c-2 shipped the panel swap, then the switch-lag was patched **3×** (the wrong layer — timing, not render) → Boss "Enough patching" → **Stop-On-Correction**: a 4-agent SME panel (diagnostic/virtualization/WA#5/audit) found the root cause = the un-virtualized render, and the sound solution was designed+built as ONE unit (`docs/MIG-079-C2c-Architect-…` + `…-SME-Findings-and-Solution.md`). The LL-014 three-strike / Solve-the-Class lesson, lived.
> - **Audit-deferred (documented, NOT blocking):** split-pane ×N traversal chips in NON-focused panes (P1 — `linkTraversalMap` derives from the focused tab's outgoing only; needs a per-open-tab outgoing map); `status='active'` vs `!='archived'` predicate nit (identical today). Both for a focused follow-up.
> - **User-facing:** Backlinks/Outgoing open + scroll smoothly on heavily-linked notes (no freeze); two new summary toggles (off by default). Boot is faster than pre-§C.2b but NOT yet at the floor (Sky read = §C.2d). Help/Manual: the two toggles (EN; ×15 rides the translation debt). Commits `6a4d5e20`→`44acd79d`. Next: **§C.2d (defer the Sky read off boot — the remaining boot win)**, then the §C.2c audit items, then §D bring-up. Pickup: `lab/reports/SESSION-LOG-2026-06-17.md` + the new handover + `lab/reports/NEXT-SESSION-PROMPT.md`.

> **What changed in v2.87** (**MIG-079 §C.2b + §C.3 BUILT + Claude-verified — the 234k-row boot edge read is DEFERRED off the boot critical path; a covering index makes the lazy scan index-only; pending Boss runtime validation**):
>
> - **§C.2b — the 234k-row `read_links` is OFF the boot path (the actual ~11 s boot win).** `cache_boot_snapshot_graph` (`cache.rs`) no longer reads `note_links` — it returns **tags + aliases only** (both ~ms after §C.1/§C.2a), so cold `graph_ready` drops from ~33 s toward the editor floor. The full edge list now loads from a NEW dedicated command **`cache_full_links`** (the same federated `read_links_in_schema` + legacy fallback), invoked lazily by a memoized frontend **`ensureFullLinks()`** + a **`linksReady`** flag: **idle pre-fetch right after `boot:graph-ready`** + on first note focus / Backlinks/Outgoing/Graph open. The back-compat `cache_boot_snapshot` shim still returns full links (routes through `cache_full_links`).
> - **P0 never-empty: every edge-list consumer guards on `linksReady`.** The Backlinks/Outgoing panels show a localized **Loading…** (`common.loading`, reused — no new i18n) instead of a misleading "no backlinks" while edges load; the sidebar `$effect` re-runs on `allLibraryLinks.length` + `linksReady` so a note focused before edges land auto-fills. Sky View renders from the write-time `sky_*` payload (unchanged); its `buildSkyData` FALLBACK now `await`s `ensureFullLinks()`. The COUNT badges stay independent (write-time `incoming_count`/`outgoing_count`). Four old `allLibraryLinks = graph.links` sites (boot loadGraph, federation:ready, post-init re-invoke, rename cascade) re-routed through `ensureFullLinks()`; the post-init re-invoke's **redundant second full-graph load at boot is removed**. Second screen (`scanLibraryLinks`) + Constellation Map (own IPC) are independent — untouched.
> - **§C.3 — `idx_link_boot` covering index.** New `link_boot_index.rs` background builder (own stamp `link_boot_index`; CREATE INDEX IF NOT EXISTS, busy-tolerant, off-boot — NOT the §C.2a backfill whose stamp is already set) creates a covering index on `note_links(status, source_path, source_name, target_name, link_type, library_name, weight, traversal_count, last_traversed, confidence, annotation)`. The lazy `cache_full_links` scan then reads index leaf pages only. **Column-set note (measurement-justified deviation from the Plan's literal list):** the list INCLUDES `source_name` (required by consumers) + `annotation` — a NON-covering index is ignored for a full-table-returning query, and annotation is only ~1.33 MB total (measured), so including it keeps `USING COVERING INDEX` AND every consumer byte-identical (no per-row annotation IPC). `context` excluded (always `''`).
> - **Verified (Claude-side; pending Boss runtime validation):** `svelte-check` 0 errors; `cargo test --lib` **935/0** (+6 ignored); new `link_boot_index` tests assert `USING COVERING INDEX idx_link_boot` (deterministic) + the stamp gate; **live-DB rehearsal on a copy** proved the plan flips `SCAN note_links` → `SEARCH … USING COVERING INDEX idx_link_boot` on the REAL 234k-row schema (index build 2.0 s). Adversarial diff review: no P0, Editor-Surface Gate clean (only the READ path changed); two P1 races (universe-switch stale-edge; per-keystroke retry-on-failure) **found + FIXED before commit** (`_linksEpoch` epoch guard + `_linksRetryAfter` 3 s cooldown).
> - **Editor-Surface Gate:** §C.2b/§C.3 touch only the link-edge READ path (a derived view of `note_links`) — NO note content / save / lifecycle code → content-integrity class structurally untouched (same class as §C.1/§C.2a).
> - **User-facing:** boot is faster (edges no longer block `graph_ready`); Backlinks/Outgoing briefly show "Loading…" then fill on first cold open. No new help/manual topic (perf). **Next: Boss runtime validation** (boot-history `graph_ready` drop + never-empty panels + the one-time `[link_boot_index]` diag line + the Editor-Surface Gate), then close-out. Pickup: `lab/reports/SESSION-LOG-2026-06-17.md`.

> **What changed in v2.86** (**§C.1 Boss-validated; §C.2a write-time backlink counts BUILT + Boss-validated; a pre-existing link-heavy-save freeze uncovered and fixed; the right-sidebar relocation designed**):
>
> - **§C.1 (write-time `tag_counts`) Boss-validated.** Runtime: tags total 19,542 correct; boot `read_tags` 5,658 ms → 4/36 ms (real boot-perf history tool); live add/remove maintains the count; content intact.
> - **§C.2a (write-time backlink counts) SHIPPED + Boss-validated.** `note_meta.incoming_count` (+ `incoming_link_types`/`incoming_top_rank`/`_json`, the MIG-066 mirror) maintained write-time and matching the Backlinks panel exactly (`COUNT(DISTINCT source_path)`, alias-aware, status-filtered). **The badge it feeds went from counting 47 backlinks in the whole library to the true ~413,660** — the legacy path inverted `outgoing_links_json`, which can't parse your 100%-typed `[[type::target]]` links. Match column = a **VIRTUAL `note_links.target_name_lower`** generated column + plain index `idx_nl_tnl` (an expression index does NOT seek a JOIN — the OR form full-scanned 234k rows/note). Backfill in `incoming_links_backfill.rs`; `constellation_search_link_counts` re-pointed (read it when stamped, else the legacy path).
> - **Maintenance is a save-path RECOMPUTE-AFFECTED diff, not triggers** (`incoming_signature` before `index_note` → `symmetric_difference` after → recompute only changed targets; reconcile = convergent self-heal). **WA#5-validated** as the textbook strategy for a `COUNT(DISTINCT)`+alias+status (non-self-maintainable) aggregate — ±delta would be WRONG (can't honor dedupe; alias/status changes move the count with no edge insert/delete; SQL Server bans `COUNT(DISTINCT)` in incremental views).
> - **Pre-existing link-heavy-save freeze FIXED (`fca3f194`).** `index_note` rebuilt ALL of a note's links on every save → the per-edge Sky/stratum/maturity (MIG-001) + outgoing (MIG-066) triggers re-fired → measured **~40 s on a 531-link note** (independent of §C.2a). Fix: **skip the `note_links` rebuild when the would-be edge set is byte-identical** to what's stored (conservative dirty-check) → a text-only edit changes no links → no rebuild → no trigger cascade → **instant save**. Boss-validated: Monotheism (119 links) text edit went 5–7 s → instant; link add/remove still registers; content intact.
> - **PJ-066 — the freeze when *adding* a link to a link-dense note FIXED & CLOSED (≈2 min → ~3 s, ≈15×; 2026-06-25).** The dirty-check above made text-only saves instant, but a real link add/remove still rebuilt + re-fired the cascade. Four layers closed it: **(A)** the sky `COUNT(DISTINCT)` disjunction → `IN (SELECT id UNION SELECT alias…)` (5,572 ms → 26 ms, 215×); **(B)** the per-edge sky stratum/maturity triggers → a **save-path recompute-affected diff** (`maintain_sky_after_save` + `recompute_all_sky` reconcile — same shape as the incoming diff above); **(C1)** the reindex command → `#[tauri::command(async)]` (off the UI thread); **(C2)** `index_note`'s link rebuild → **incremental diff-edges** (only the changed edge; unchanged edges keep rowid + earned weight/confidence/traversal); **(C3)** the real residual freeze was **`ensure_search_db_ready` taking `db.lock()` just for a readiness check** (blocking on the writer) → a **lock-free `db_ready: AtomicBool`** + a **second READ-ONLY WAL connection `read_db`** (`with_read_conn` routes reads to it; falls back to `db` only pre-init). Phase-4 audited (1 P1 fixed: publish `db`+`read_db`+`db_ready` under one `federation_generation`-validated block). **Follow-up (open):** residual ~3 s on the heaviest notes during the background reindex + route the remaining read commands (search/stats) off the writer lock.
> - **2nd-boot 0-notes bug fixed (`7cd192cc`):** the VIRTUAL-column idempotency guard used `table_info` (which hides generated columns) → re-ALTER → `init_db` loop → 0 notes. Now `table_xinfo`. + regression test.
> - **Right Sidebar → note-context-only — DESIGNED (not built).** `docs/Right-Sidebar-Note-Context-Design-Decision.md`: the right rail becomes the open note's diagnostic suite; universe functions relocate (All-tags→Search Hub, Calendar→left launcher, Cataloger→left, universe Health→Dashboard, universe Review→full-page reviewer). Its own `/migration` AFTER §C.2/§C.3.
> - **User-facing:** backlink counts are now correct (were ~0); link-heavy-note saves are instant. No new help/manual topic. **Next: §C.2b** — defer the 234k-row `read_links` off boot (the actual ~11 s boot win; §C.2a is the enabler) + **§C.3** covering index. Boot is still ~27 s until §C.2b. Pickup: `lab/reports/HANDOVER-2026-06-17-mig079-c2b.md` + `lab/reports/NEXT-SESSION-PROMPT.md`.

> **What changed in v2.85** (**MIG-079 §C.1 BUILT + proven — write-time `tag_counts` replaces the 5.6 s boot tag scan; the additive-aggregate backfill race ELIMINATED by atomicity, not bounded; rehearsed byte-identical on the live DB**):
>
> - **MIG-079 §C.1 SHIPPED (Claude-verified; pending Boss runtime validation) — write-time `tag_counts`.** New `src-tauri/src/tag_counts.rs` + a `tag_counts(tag, n)` table in `init_db`. The boot graph read (`cache::read_tags`, a 5.6 s full-`tags_json` scan every launch — a CLAUDE.md Rule-8 read-time recompute) is replaced by a persisted summary kept current write-time: `index_note` and `reindex_delete_note` apply a per-tag **±delta** (`new − old`) inside their existing transaction, gated on `schema_versions.tag_counts`; `read_tags_in_schema` reads `{schema}.tag_counts WHERE n>0` when stamped, **else falls back to the legacy live scan** (zero-risk); `reconcile_filesystem` rebuilds it authoritatively (self-heal). Boot `read_tags` 5.6 s → ~ms (from the 2nd boot after ship).
> - **Backfill = ONE atomic `json_each` aggregate** (refinement of the handover's "batched, like `note_body_backfill`"). A counter is an *additive* aggregate, so a batched build racing live ±deltas would double-count — the exact race flagged as audit item (b). Building **and** stamping inside ONE `IMMEDIATE` transaction (dedicated connection, the proven reconcile-`walk_conn` pattern) **eliminates** the race: a concurrent save serializes either before the backfill (its delta gated-off; the aggregate sees its note_meta) or after (table stamped; its delta applies) — both converge (proof-by-cases in the session log). Tags have no 128 MB-row problem, so one statement is cheap; the ~6 s lock hold is one-time, background, post-paint, and WAL keeps boot READS unblocked.
> - **Reproduce-First on the data shape:** all 7,653 `tags_json` rows are clean string-arrays (0 NULL/malformed/non-array/non-string/empty/dup); `serde` and `json_each` aggregates are byte-identical (19,542 distinct / 36,193 occ); the 5.6 s is SQLite's wide-row traversal (inline `body_text`), not JSON parsing — the ~19.5k-row summary reads in ms.
> - **Mandatory gates met:** (a) **live-DB rehearsal** — the REAL `recompute_all_in` against a copy of the 1.74 GB DB equals the live `read_tags` aggregate **byte-for-byte** (`lab/tag-counts/`); (b) **adversarial race audit** — eliminated (above); (c) **±delta unit tests** — 4 new (multiset, add/remove/rename+prune, no-op, backfill==Σdeltas==live & idempotent), `cargo test --lib` **930/0**; (d) **Editor-Surface Gate** — tags are a derived view (delta never touches body/content/display) → content-integrity class structurally untouched; Boss test covers NotePane save / Focus round-trip / tab switch / rename probe-pair.
> - **User-facing:** none (identical tag counts, faster boot from the 2nd launch). No new help/manual topics. **Pending Boss validation:** launch the rebuilt binary → `diagnostics.log` `[tag_counts_backfill] completed: 19542 distinct tags`; next boot `read_tags` ~ms (boot-history tool); add/remove a tag → sidebar count moves live; body intact across the gate. Then **§C.2** (defer the 234k `read_links` — the big 11 s) + **§C.3** (covering index).

> **What changed in v2.84** (**MIG-079 §A + §B SHIPPED+validated; a boot-perf HISTORY tool (append-only, every boot); the boot cost MEASURED — it's 100% the graph reads, cold + warm; a guessing correction logged as a standing rule; §C.1 verified + designed, not yet built**):
>
> - **MIG-079 §A SHIPPED+validated (`256d7c5f`) — single-owner activation.** Idempotency guard at the top of `set_active_universe` (requested path == active → `Ok(())`, no invalidate/re-init) + second screen made display-only (removed `SecondScreenPage.svelte:923` `setActiveUniverse`). **Verified: `init_db` now runs ONCE** (was twice — the redundant main-window + second-screen re-activation). Boss: "way better, no freezes."
> - **MIG-079 §B SHIPPED+validated — `safeBootMode` + the editor baseline + a boot-perf history tool.** `appSettings.safeBootMode` (top-level, default false) gates the 4 unconditional satellite boot IPCs (`cache_boot_snapshot_graph`, the `federation:ready` re-invoke, `listFiveActsNotes`, `getFederationWarnings`) → an editor-only boot (`3d115ef0`). **The append-only boot-perf history tool (`c8f7ff9a`):** `write_boot_perf_report` now also appends each report to `.constellation/boot-perf.history.jsonl` (NEVER overwritten — captures every boot, cold+warm, many per session); +`boot_id`/`phase`/`safe_boot_mode` fields; reader `lab/boot-perf/read-boot-history.py`. Built because `boot-perf.latest.json` kept only the last boot, so cold diagnoses were inference.
> - **The boot cost is MEASURED (not inferred), via the history tool — 3 boots, cold via PC restart:** cold-minimal (editor only) **1.7 s** · warm-full 5.6 s · **cold-full 33 s**. The split: the editor spine (DB open + note-list read) is **~0.5–0.65 s regardless of cold/warm**; the **entire** rest is the graph snapshot reads — `read_links` (234k `note_links` rows) **11.3 s cold** / 0.4 s warm, `read_tags` **5.7 s cold** / 0.06 s warm, + **12 s cold** IPC contention. **Conclusion: the editor is never the problem; §C (the write-time graph fix) is the whole fix, cold AND warm** (33 s → toward the ~1.7 s floor).
> - **A guessing correction — now a standing rule.** When the cold boot looked slow, I stated a *mechanism* ("the 13 s is the cold 1.83 GB DB-file read") without measuring it. Eisa corrected it; the measurement **disproved** it (the DB read is ~0.5 s cold). **Standing order reaffirmed: never state a cause without a measurement behind it** (the BASIC RULE / Reproduce-First in action). The earlier "13 s cold (minimal)" was on the pre-tool binary, unmeasured, and did NOT reproduce — not chased.
> - **MIG-079 §C.1 — write-time `tag_counts` — VERIFIED + DESIGNED, intentionally NOT built at session tail** (a save-hot-path-adjacent write-path change earns the full-care class, not a rushed end-of-session edit). **Safety verified (not assumed):** `index_note`'s UPSERT (search.rs:4266) is the SOLE writer of `note_meta.tags_json` (every other `note_meta` write touches different columns; rename moves the path; Add-tag routes through reindex) → a Rust ±delta is complete; delete decrement mirrors `reindex_delete_note` (search.rs:7217). **Design:** `tag_counts(tag,n)` + Rust ±delta gated on `schema_versions.tag_counts` + a non-blocking own-connection backfill (modeled on `note_body_backfill.rs`) + `read_tags` reads the table when stamped else falls back to today's live scan + `reconcile_filesystem` recomputes authoritatively. **Before it touches the save path:** live-DB rehearsal (`tag_counts` == `read_tags` exactly), adversarial audit of the backfill-coexistence race, a ±delta unit test, the Editor-Surface Gate. Then §C.2 (defer the 234k `read_links` off boot) + §C.3 (covering index).
> - **User-facing:** boot is dramatically smoother (single `init_db`, no freezes). No new help/manual topics (perf + internal instrumentation). Next session opens on **MIG-079 §C.1** (handover `lab/reports/HANDOVER-2026-06-15-bringup-mig079.md` Addendum; kickoff prompt `lab/reports/NEXT-SESSION-PROMPT.md`).

> **What changed in v2.83** (**MIG-078 §B1 startup-race SHIPPED+validated; the boot-perf root cause found (graph recompute = Rule-8 disease, systemic); the Boss "bring-up" program OPENED — concept-paper-per-function + app-wide audit; MIG-079 OPENED, §A single-owner activation SHIPPED+validated (init_db now runs ONCE)**):
>
> - **MIG-078 §B1 — the thundering-herd `init_db` race — SHIPPED + data-proven (`fe01e3db`, `44a230fd`, `a7590837`).** Three fixes: (1) a dedicated `SearchState.init_lock: Mutex<()>` + double-checked locking in `ensure_search_db_ready` (init_db runs once; NOT holding `state.db` across it — that 06-14 attempt stays reverted; poison-tolerant; + a universe-switch generation re-check before storing the conn). (2) **Frontmatter-parser root cause:** `parse_frontmatter`/`body_after_frontmatter` used naive `find("---")` that matched a `---` INSIDE a quoted tag value (`cite-q---cites-…`), truncating the YAML before `cid_cn` → one note (`أولي (كائن)`) indexed with `cid_cn=''`. Fixed to line-anchored `\n---` via ONE shared `split_frontmatter` helper (+EOF guard, +tests). (3) The **26–41 s every-boot `mig003_step3` sweep**: the probe `cid_cn IS NULL OR cid_cn=''` forced a full SCAN of the 1.7 GB `note_meta`; since the column is NOT NULL, changed to `cid_cn=''` (rides `idx_note_meta_cid_cn`) + force-reindex the empty row. **Verified on the live DB:** empty-cid_cn 0; the note self-healed; no thundering herd; no lock errors. A 4-lens adversarial audit returned **PASS_WITH_NOTES** (no P0).
> - **Boot-perf root cause FOUND (Reproduce-First + measured).** §B1 fixed the data but boot stayed ~70 s/3-freezes. `boot-perf.latest.json`: paint 0.9 s / hydrated 1.7 s PASS, but **`graph_ready_ms`=32,519** — `cache_boot_snapshot_graph` recomputes the graph EVERY boot (`read_links` 11.6 s over **233,995** links [measured: all `status='active'`, full SCAN — NOT the "656k" an old note assumed], `read_tags` 5.6 s, +11.25 s queue). A **Rule-8 (Write-Time Derivation) violation**. And a MULTIPLIER: the boot called `set_active_universe` redundantly (main window + second screen) → `init_db` ran TWICE.
> - **MIG-079 OPENED (full `/migration`).** Architect (`docs/MIG-079-Architect-Boot-WTD-Graph-Snapshot.md`, 8-agent examine+WA#5 research) + Plan (`docs/MIG-079-Plan.md`). Cure = extend the in-house write-time trigger pattern (notes_fts/sky_nodes/outgoing aggregates), don't invent. Boss decisions: links **defer off boot**; tags **maintained in the indexer (Rust)**; **activation fix first**. **§A SHIPPED + Boss-validated (`256d7c5f`):** (A.1) idempotency guard at the top of `set_active_universe` (requested path == active → no-op, no invalidate/re-init); (A.2) second screen display-only (removed `SecondScreenPage.svelte:923` `setActiveUniverse`). **Verified: `init_db` now runs ONCE** (was twice); Boss: "way better, no freezes." Remaining: §B `safeBootMode` + missing gates → §C boot-graph WTD (the 30 s) → §D phase bring-up.
> - **The Boss "bring-up" program OPENED + the concept-paper governance.** Method (Eisa): disable every function via FLAGS (not scissors), keep only the Editor (the gate), measure the baseline, re-enable one at a time fixing lag/wiring — and **every function gets a concept paper defining its purpose, acting as the checklist**; each covers the element's **right-click** + **multilingual** conformance; a **core Constellation concept paper** sits above all. Shipped: `docs/concept-papers/` — `00-Constellation-Core-Concept-Paper.md` (the root: Five Acts → Conviction; File-Over-App; Living Links; Rule 8; the Editor is the gate; right-click-everywhere; multilingual-by-default), `00-MASTER-Bring-Up-Charter-and-Checklist.md` (charter + 11-section template incl. Right-click §5 + Multilingual §6 + function inventory + bring-up order + `safeBootMode` design + **§7 Debt Register**), `01-Note-Editor.md`, and **31 per-function papers (02–32)**. **The pass became an APP-WIDE AUDIT** (§7 register): Rule-8 recompute is SYSTEMIC (~14 functions); MIG-077 right-click incomplete (4 hand-rolled + ~19 missing menus); hardcoded English ~17 surfaces; missing feature gates (Search Hub has NO `enabledFeatures.search`). **Confirmed defects:** Calendar day-click always opens TODAY (`get_daily_note_path` ignores the date); Tasks `toggle_task` bypasses the Editor reindex gate; Second Screen `:923` (now fixed §A); Review-Pulse `record_note_visit` dead; 6 no-op Command-Palette stubs. **Boss ruling:** Tasks **reframed** (→ "open epistemic loops" serving Tension→Synthesis→Conviction; kept, not removed).
> - **User-facing:** boot is dramatically smoother (no freezes; `init_db` once). No new help/manual topics (perf + internal governance docs). Next session opens on **MIG-079 §B** (`safeBootMode` + the clean editor baseline) → §C (the 30 s graph fix). The concept papers are now the canonical per-function reference + bring-up checklist.

> **What changed in v2.82** (**MIG-078 progresses: §A′.2 reconcile + §BL.1 de-bloat foundation SHIPPED; the trial-generator table-explosion bug fixed + 19 bloated notes cleaned (~145 MB); a thundering-herd boot race surfaced live → now the next priority**):
>
> - **§A′.2 — note_meta↔disk reconcile SHIPPED + verified (`6daac55b`).** New `reconcile.rs` (background, post-paint, reuses `reindex_delete_note`) removes stale `note_meta` rows whose `.md` is gone — the phantom notes pure-index rendering exposed. Safety-capped (skips offline-drive libraries; aborts if > max(200, 10 %) look stale). Verified: 7,667 → 7,653 rows (exactly 14 removed), 0 real notes touched.
> - **Phase BL design + de-risking (`docs/MIG-078-Phase-BL-Design.md`).** Load-bearing decision: NEVER touch `notes_fts` `content=note_meta` (rebuild would churn docids → not byte-identical); keep FTS triggers on note_meta, only change where they READ body; bridge note_meta↔note_body on `path` (never rowid). Live experiment proved MATCH stays **byte-identical** through `DROP COLUMN` once triggers are repointed; the workflow's proposed `content_hash` WHEN-gate was found INVALID (never written) → replaced by a `note_body_au` trigger. SQLite 3.45 (DROP COLUMN ✓), E: 7.9 TB free.
> - **§BL.1 — note_body table + dual-write + resumable backfill SHIPPED (`e0a3cf20`).** `note_body(path PK, body_text)`; `index_note` dual-writes note_body FIRST then the unchanged note_meta UPSERT; `reindex_delete_note` drops note_body too; `note_body_backfill.rs` (mirrors links_backfill; `INSERT…SELECT` keeps the 128 MB note inside SQLite). No reads/triggers changed → search unchanged. Rehearsed on a live-DB backup: 7,653 bodies copied in 7.5 s, 0 divergent.
> - **Importer table-explosion bug fixed + 19 notes cleaned (`2a210309`, `25c8c9e7`).** The bloat (146 MB; Spirochete 122.9 MB = 84 %) came from the **trial-universe GENERATOR** (`lab/trial-universe/generator/html-to-md.mjs::renderTable`) — NOT the shipped app (no in-app Wikipedia importer). Combinatorial `<tr>`-descendant + nested-table re-render on `{{clade}}` trees. Fixed (`closest('table')===self` + flatten nested tables to text + caps); proven: Spirochete 122.9 MB → 25.6 KB. Then surgically excised the garbage table blocks from the 19 affected notes on disk (byte-based detection, backup-first, frontmatter/prose preserved): **146.8 MB → 1.68 MB (~145 MB reclaimed)**. Backups in `lab/backups/importer-cleanup-2026-06-15/` (gitignored). Full report: `lab/reports/IMPORTER-TABLE-EXPLOSION-FINDINGS-2026-06-15.md`.
> - **Phase D gained §D1b (Boss-approved, from a graph-tech assessment):** stable-skeleton in-slot expansion so siblings don't move on expand (drift = 0); GraphMind/Sky View logged as follow-ons; sunburst excluded.
> - **⚠ LIVE FINDING — the thundering-herd `init_db` boot race (NEXT PRIORITY).** On the §BL.1-binary boot, `init_db` ran 8+ times concurrently; `mig003_step3_soft_rebackfill` ran 2× (26 s + 41 s); `note_body_backfill` hit "database is locked"; `reconcile_filesystem` left no trace → the 19 cleaned files were **NOT reindexed** (note_meta still 404 MB). So the cleanup hasn't reflected in the DB yet, and this race is almost certainly the original "0 notes for ~18 s" boot flicker. A prior fix (hold `state.db` lock across `init_db`) was reverted as too invasive → the proper fix is `std::sync::Once`/a dedicated init mutex (= the migration's §B1, brought forward). Secondary: **1 note with a persistently-empty `cid_cn`** triggers the 26–41 s mig003 sweep every boot.
> - **User-facing:** OrgChart/Map open ~instantly (perf, no UI change). No new help/manual topics this session (perf + data-integrity + a test-generator fix). Next session opens on the startup-race fix.

> **What changed in v2.81** (**MIG-078 OPENED (full /migration, Boss-approved scope A′→B→D) — persist the Universe Map / OrgChart tree (Write-Time Derivation); §A′ in-memory tree SHIPPED + Boss-validated (OrgChart/Map open 2 min → < 2 s)**):
>
> - **Record correction — the v2.80 "OrgChart 26 s → instant" + thundering-herd perf fixes were symptom patches; one regressed.** The v2.80 `if (!fullscreen) loadData(); else loading = false;` gate removed an *incidental* OS-disk-cache warm-up, which **exposed** the real cost (`constellation_map_universe`'s `build_tree` walks every folder on disk on every open) and pushed the fullscreen OrgChart open from ~26 s to **2+ minutes** on the large universe. The thundering-herd `init_db` change was **reverted** (too invasive). Eisa: **"Stop patching."** → full `/migration` MIG-078.
> - **MIG-078 — the Write-Time-Derivation fix.** Architect (`docs/MIG-078-Architect-OrgChart-Map-Tree.md`) + Plan (`docs/MIG-078-Plan.md`), produced from an 8-agent investigation (4 code-examination + 4 state-of-the-art research) and **cross-checked against the actual database**. **Root cause:** the Map/OrgChart tree is recomputed at read time from an eager whole-universe disk walk instead of read from an already-current persisted structure (the exact shape CLAUDE.md Rule 8 forbids). **In-house exemplar:** `sky_nodes` is already maintained at write time by `note_meta_sky_ai/ad/au` triggers — Sky View already does what the Map needs. **Boss-approved scope = A′ → B → D, with the `body_text` bloat folded in.**
> - **Ground truth (corrects the inherited "7,600 / 1.7 GB" assumptions).** The hanging universe is **"Eisa Cognitive Knowledge"**: **7,667** `note_meta` rows across **19 libraries** whose notes live under `E:\Cognitive Knowledge\…` (external paths). `note_meta.path` = native Windows backslash absolute paths. The covering index `idx_note_meta_map` **is** used (index-only scan → the map query already avoids `body_text`). DB bloat = `body_text` 404 MB (one note is **122 MB**) + **379 MB freelist** (22 % dead, VACUUM-reclaimable) + large FTS/vocab/links tables. ⇒ the 2-min hang was the **cold directory walk under AV**, not a 7,600-file content read.
> - **§A′ SHIPPED + Boss-validated.** `src-tauri/src/map.rs`: new `build_tree_from_records()` assembles each library's tree purely from indexed note paths (materialized-path) with **no `fs::read_dir`/`read_to_string`/`metadata`**; depth cutoff, child sort, and empty-folder exclusion mirror the legacy `build_tree`; switched on behind `const MAP_TREE_FROM_INDEX = true` (legacy path retained for `constellation_map_data`). **Proven byte-identical** to the disk walk on the 4 real libraries tested (equivalence harness on the live DB, then removed). **Boss test: OrgChart open 2 min → < 2 s cold, instant warm; Physics/Philosophy/History correct.** Also fixed a raw-key bug: OrgChart loading text used the non-existent `layout.loading` → `common.loading` (×15 present).
> - **Finding surfaced by §A′ (next step):** pure-index rendering exposed **14 stale `note_meta` rows** (0.18 %, all in the two test-scratch libraries — rename/delete test notes whose old rows lingered; files confirmed missing on disk). The disk-walk masked them. → reconcile (Plan §A′.2 / §B5) is the immediate next step.
> - **REMAINING MIG-078:** §A′.2 reconcile stale rows → Phase BL (`body_text` → `note_body` table + VACUUM) → Phase B (`tree_node` + `folder_stats` write-time triggers, the durable end-state) → Phase D (lazy + virtualized rendering). Then Phase 4 audit + close.
> - **User-facing:** OrgChart + Constellation Map open effectively instantly now; no UI change. (Help/Manual unchanged this step — perf-only.)

> **What changed in v2.80** (**MIG-077 Phase A continues (A3-R rich contextual menus + file-tree extension SHIPPED — Boss-validated) + three performance fixes (OrgChart 26s → instant, boot zero-flicker, thundering herd)**):
>
> - **MIG-077 A3 through A3-R4 SHIPPED + Boss-validated.** The OrgChart `.oc-fs-ctx` menu + file tree right-click both now use `buildContextMenu()` — a shared, contextual menu builder (`contextMenuBuilder.ts`, NEW). One source of truth: `buildContextMenu(target: {kind, path, name}, actions)` emits items ONLY when an action callback is provided AND fits the kind (note/folder/library). **Notes get:** Open, Open in new tab, Rename, Move (universe-wide), Add tag, Copy path, Copy name, Reveal in tree, Suggest sources, Delete. **Folders get:** New note/folder/base, Expand/Collapse, Rename, Move, Reveal in tree, Delete. **Libraries get:** New note/folder/base, Expand/Collapse. The file tree also gains Open, Open in new tab, Move, Add tag, Copy path/name — previously absent.
> - **Universe-wide Move** (`MoveDialog.svelte`, NEW). The Move action shows a searchable folder picker across ALL libraries (not same-library-only). New Rust IPC `list_universe_folders` returns every folder in the universe. The dialog groups folders under library roots (bold, with glyph). Double-click = instant move. Collision errors shown inline. **`move_item` now reindexes on move** (was missing — a latent bug for ALL moves including drag-drop): drops old note_meta + adds at new path/library.
> - **Content-integrity-safe Add tag.** The `addTagToNote` function uses dual paths: open notes through the model (`composeNoteModel` + `saveTabContent`), closed notes through `writeNote` + `reindexNote`. Never writes to disk behind an open model.
> - **RenameDialog** (`RenameDialog.svelte`, NEW). Reusable input dialog (RTL-aware) used by Rename + Add tag. Styled to match ConfirmDialog.
> - **Reveal in tree** — was dead app-wide. Now wired: `constellation:reveal-in-tree` event listener in `+layout.svelte` + `data-tree-path` attributes on FileTree entries. Switches to tree mode, expands the target library (+ child universe if nested), finds the row by attribute, opens ancestor `<details>`, scrolls into view with a highlight flash.
> - **Three performance fixes (SME-agent-diagnosed, applied this session):**
>   - **OrgChart 26s → instant.** `loadData()` in `onMount` ran in fullscreen mode too, loading all 19 library trees (7,666 file reads via `read_library_tree`, ~30s cold due to antivirus). Fullscreen uses the DB-based `constellation_map_universe` path and never renders `loadData()`'s output. **Fix:** `if (!fullscreen) loadData(); else loading = false;` — fullscreen now loads instantly via the existing DB path.
>   - **Boot soft-rebackfill scan eliminated.** `mig003_step3_soft_rebackfill` scanned all 7,600 rows in the 1.7 GB `note_meta` table every boot (five UPDATE statements checking for NULL/empty `cid_cn`). In steady state, 0 rows need repair. **Fix:** added an `EXISTS` pre-check using the existing `idx_note_meta_cid_cn` unique index — O(1) when nothing to repair; the full scan only runs when actually needed.
>   - **Thundering-herd init_db race fixed.** `ensure_search_db_ready` checked `db.is_some()` under lock, dropped the lock, ran `init_db` (20+s on cold 1.7 GB), then re-acquired — so 3–7 threads passed the check concurrently, each running `init_db`, while `loadAllStats` read zeros mid-contention → "0 notes" flicker. **Fix:** hold the lock across `init_db`; a fast-path pre-check still returns instantly when already initialized.
> - **i18n ×15:** `contextMenu.{open,openInNewTab,expand,collapse,move,addTag}` — all 15 locales, native equivalents.
> - **REMAINING MIG-077:** Phase A **A4** (ConfidencePicker) → **A5** (GraphMind, optional) → `/simplify`; Phase B **B1 List-mode (#1)** → B2–B7; Phase 4 audit + close.
> - **User-facing:** right-click menus on OrgChart + file tree (new/extended); Move dialog (new); Add tag dialog (new); Reveal in tree (fixed). Help + Manual updated (EN). The perf fixes have no user-facing UI change but boot + OrgChart open should be dramatically faster.

> **What changed in v2.79** (**The styled batch-delete dialog (#2) + MIG-077 App-Wide Right-Click Context Menus — OPENED (full /migration, Boss-approved Option A); Phase A through A2 SHIPPED + Boss-validated**):
>
> - **#2 — the styled batch-delete dialog.** The List-mode multi-select Delete now uses the shared `ConfirmDialog` instead of the native browser `confirm()` ("tauri.localhost says…") box; the message names the destination (.trash folder / Recycle Bin). `dialogs.batchDelete*` ×15. Commit `1a220187`.
> - **MIG-077 OPENED — the app-wide right-click pass.** Origin: three Boss observations during the delete-path test (#1 no right-click batch delete in List mode; #2 the native confirm box [shipped]; #3 "right-click should include every aspect of the app"). Full four-phase /migration. **Architect** (`lab/reports/MIG-077-RIGHTCLICK-CONTEXT-MENUS-ARCHITECT.md`) corrected the inventory via a Plan-agent read: 2 shared-`ContextMenu` consumers (file tree + library headers) + **6 hand-rolled menus** (Tabs, Backlinks, Outgoing, Index, OrgChart, GraphMind) + 1 specialized (Editor); Tabs + Index ship hardcoded English. The one real design fork = the confidence picker (a single-select radio group, not a command list). **Boss approved Option A** — shared `ContextMenu` for action-menus + a separate `<ConfidencePicker>`. **Plan** (`...-PLAN.md`): Phase A consolidate the 6 → Phase B extend (List-mode #1 first) → Phase 4 audit; each step one commit + gate. **Boss approved → cascade.**
> - **MIG-077 Phase A — A0/A1/A2 SHIPPED + Boss-validated.** **A0** (`eb3a246f`): additive `separator?`/`disabled?` on `ContextMenu` (backward-compatible). **A1** (`eb3a246f`): the open-note tab menu → shared `ContextMenu` (`getTabContextMenuItems`), `tabContextMenu.*` ×15 (closes the violation); follow-up `381a471e` made the pin toggle reactive (`openTabs.update`) so the 📌 appears instantly. **A2** (`a999c165`): the Index term menu → shared `ContextMenu` (`getIndexTermMenuItems`), `indexPanel.hideTerm/showTerm` ×15; follow-up `8aab9db2` localized the eye-toggle "hidden terms" tooltip (`indexPanel.hiddenTerms` ×15). Every step: svelte-check 0 · proper build (`npm run build` + `cargo build --release`) · bundle-confirmed · Boss gate PASS. The repeating idiom: a per-surface `getXContextMenuItems(): MenuItem[]` builder + remove the inline menu + its dead CSS.
> - **REMAINING MIG-077:** Phase A **A3** ~~(SHIPPED v2.80)~~ → **A4** (extract a shared `<ConfidencePicker>` for Backlinks + Outgoing — the one non-command menu) → A5 (GraphMind, optional) → `/simplify`; Phase B **B1 List-mode (#1)** → B2 Search → B3 Tags → B4 Calendar → B5 Sky View → B6 in-editor wikilinks → B7 lower-value; Phase 4 3-agent audit + close. The approved Plan on disk drives every step.
> - **User-facing:** the batch-delete confirm + the tab/Index right-click menus are now consistent and fully localized; the pin icon updates instantly. (Consistency/i18n refinements of existing menus — no new help/manual topics this version.)

> **What changed in v2.78** (**The Delete-path fix — the "Deleted files" setting now GOVERNS deletion (was a silent no-op); deletes are always recoverable; "Permanently delete" dropped — SHIPPED + Boss-validated**):
>
> - **The data-loss gap closed.** `delete_item` ALWAYS hard-deleted regardless of Settings → Universe & Libraries → "Deleted files" (`// For now, always permanent delete`) — the dropdown was a no-op, so users expecting the Recycle Bin (the default) or who chose ".trash folder" for safety were **silently losing data permanently**. New Rust **`delete_path(app, path, mode, trash_root)`** (`libraries.rs`) routes by the setting: **permanent** → remove · **trash** → move into `<trash_root>/.trash` (de-colliding like `move_to_trash`; cross-volume-safe rename→copy+remove + recursive dir copy) · **system** → OS Recycle Bin (the new **`trash` crate**). All modes `reindex_delete_note`; `.trash` is already excluded from walks+index (`file_kinds.rs` EXCLUDED_DIRS / `embeds.rs` IGNORED_DIRS) so trashed notes vanish from tree+search. One shared frontend **`deleteWithSetting(path)`** (store.ts) resolves the `.trash` root and replaces the hard-coded delete at BOTH real sites (`handleDeleteConfirm` + `handleBatchDelete`).
> - **".trash folder" scope choice (Boss extension).** A sub-option under the dropdown: **within the note's library** OR **at the universe root** (`trashFolderScope: 'library' | 'universe'`). For Eisa's layout the universe-root case is a cross-folder-tree move (libraries under `E:\Cognitive Knowledge\…`, universe root under `E:\Constellation Universes\…`) — handled cross-volume-safe. i18n `settings.files.{trashFolderScope,scopeLibrary,scopeUniverse}` ×15.
> - **"Permanently delete" dropped (Boss ruling — *"who would choose Permanently deleted? I wouldn't"*).** Cross-check: Obsidian offers it, but macOS/Windows file managers have NO global always-permanent mode (permanent is an explicit one-off). For Constellation's protect-your-knowledge ethos the global option is a footgun. The dropdown is now **System trash / .trash folder** (both recoverable); `deleteWithSetting` + `applyParsedSettings` migrate any legacy 'permanent' → 'system'. The engine KEEPS a `permanent` mode in `delete_path` for a FUTURE *explicit* "Delete permanently" action — never a silent global mode.
> - **Boss-validated Stage 1 + Stage 2, all destinations disk-confirmed** (Working Agreement #1): library `.trash` · universe `.trash` (the cross-tree move) · Windows Recycle Bin (verified via Shell.Application + original-location metadata) · permanent-gone-everywhere · folder delete · List-mode batch. The List-mode batch confirm now states the destination instead of falsely claiming "cannot be undone."
> - **Deferred follow-ups (logged, NOT built):** (1) right-click batch delete in List mode · (2) the native `confirm()` box → Constellation's styled dialog · (3) **comprehensive app-wide right-click** (a real initiative — inventory + plan, not piecemeal) · the **unified Trash view** (Boss-chosen over renaming per-library `.trash`; same-name `.trash` is the standard, disambiguated by location — renaming would break the exact-name exclusion + Obsidian trash recognition) · an **explicit "Delete permanently"** action.
> - **User-facing:** delete now honors the setting + is always recoverable. User Manual (*Notes Management → Deleting notes*) + help (*Notes Management*) updated (EN); help mirror rides the batched-translation debt. New dependency: the `trash` crate.

> **What changed in v2.77** (**MIG-076 §E-2 — the write-integrity diagnostics line — SHIPPED + Boss-validated; §E now CLOSED, only §F remains**):
>
> - **§E-2 ships the Settings → Security & Privacy "Write integrity" readout.** New Rust IPC `read_write_journal_stats` (`write_gate.rs`) streams the `write-journal.jsonl` (+ its one rotated `.old`) and returns `{writes, anomalies, would_refuse_identity/stale, last_anomaly_ts, refused_exists, enforce, exists, rotated, dir}`. **Anomalies = the SHADOW-mode would-refuse verdicts** (`would_refuse_identity` + `would_refuse_stale`) — the count that must be 0 for the §F1 enforcement flip. The Settings line (SettingsModal Security section) shows Writes journaled / Anomalies (✓0 green · ⚠N red + the **most-recent-anomaly date**) / Monitoring (shadow mode) / Open journal folder; **re-reads on every section entry**. i18n `writeIntegrity.*` ×15 (native).
> - **The diagnostic earned its keep on first run.** Reading Eisa's live journal, the readout surfaced **2 anomalies** — both `editor_save` from 2026-06-12 19:57/20:02 carrying a stale `…9E76` cid onto different notes: the exact **§C new-note-while-open identity leak** the journal caught on 06-12 and that was then fixed. Zero new anomalies since, across ~640 writes through the §D + §E-1 testing. The `last_anomaly_ts` field was added precisely so this perpetual-but-historical "⚠ 2" reads correctly as stale (date shown).
> - **The build-process lesson (logged to memory + here).** The first §E-2 binary shipped with the Rust IPC but NO Settings line — because `cargo build --release` (run directly) only recompiles Rust and **re-embeds the existing `../build` frontend**; the SvelteKit bundle is regenerated ONLY by `npm run build` (the tauri.conf `beforeBuildCommand`). Stage-0 "binary newer than source" passed yet the binary held a stale frontend. **Rule: any frontend change → `npm run build` THEN `cargo build --release`, and verify by grepping `build/` for the new string** (binary-mtime alone is insufficient). Memory `feedback_frontend_build_before_cargo`.
> - **Boss iteration (3 rounds, all validated).** Stage 1 PASS (line shows 640 / ⚠2 / June 12, matching the Claude-side disk count). Two follow-ups fixed: the "Most recent" label → **"Most recent anomaly"** (it's the last anomaly, not the last note); the panel now **auto-re-reads on section entry** (the manual Refresh looked dead because the Settings modal blocks editing — no new writes occur while it's open, so there was nothing new to fetch). Then **the Refresh button was dropped entirely** (Boss call — the auto-refresh makes it redundant); its dead i18n key removed ×15.
> - **REMAINING MIG-076 scope:** only **§F** — flip `WRITE_GATE_ENFORCE=true` after a clean soak + the refusal/quarantine/freshness recovery UX + permanent regression suite + 3-agent audit + `/simplify` + close. The WriteGate CAS is still in **shadow mode**.
> - **User-facing:** the Write-integrity readout (new) — User Manual §19 (Security and Privacy) updated (EN); help mirror rides the batched-translation debt.

> **What changed in v2.76** (**MIG-076 §E-1 — the universe-wide name-collision dialog — SHIPPED + Boss-validated (Stage 1 + Stage 2, disk + content confirmed); the trash de-collide fix lands with it**):
>
> - **§E-1 ships the name-collision dialog (PJ-003 §E1b).** Creating a note with a **typed** name, or renaming a note, onto a title that already exists **anywhere in the universe** (every library + every federated child universe) now opens one conventional modal — **Change name / Overwrite / Cancel** (`src/lib/components/CollisionDialog.svelte`, NEW) — instead of create's silent auto-suffix or rename's swallowed "already exists" failure. The check is authoritative: it reuses `resolveWikilinkCrossLibrary` (the SAME resolver the `[[wikilinks]]` use, fed by `resolve_universe_libraries` — federated, so cUniverse duplicates are caught by the same code path). **Overwrite is recoverable** — the displaced note is moved to its library's `.trash` first, never hard-deleted. Quick Capture's auto-named notes are NOT interrupted (a separate Rust path); folders are unaffected (`!isDir`). i18n `collision.*` ×15 (native).
> - **Boss-validated, disk + content proven (Working Agreement #1).** Stage 1 (same-library create — the dialog + all three buttons) + Stage 2 (cross-library create + rename collision + Overwrite-to-trash) both **PASS**. Claude-side disk read confirmed: the renamed note kept **its own** cid + body + **old name preserved as an alias** (so old `[[links]]` still resolve — and collision detection is alias-aware as a bonus); the displaced original sits intact in `.trash`. No identity leak, no corruption — the content-integrity class stays closed through the new surface.
> - **The trash de-collide fix (Obsidian-style, Boss-approved this session).** Stage-2 validation surfaced a real edge: `move_to_trash` reused the exact filename, so trashing a second note sharing a name with one already in `.trash` **atomically replaced — and silently lost — the earlier trashed copy**. Fixed (`src-tauri/src/libraries.rs`): on a `.trash` name clash, suffix the stem with ` {n}` (*Foo 1*, *Foo 2*) — matching Constellation's own create-collision naming. **Never clobber.** Release build green (2m40s).
> - **Two trash gaps LOGGED as follow-ups (pre-existing, not §E-1-introduced).** (a) the normal **Delete** action (`delete_item`) currently **ignores the "Deleted files" setting** and always hard-deletes — the System-trash / .trash / Permanent dropdown is a **no-op** (code: `// For now, always permanent delete`); (b) collision-**Overwrite always uses per-library `.trash`** regardless of that setting (safe by default, but inconsistent). Both belong with §F's recovery/safety UX. **Trash is per-library by design** (Boss-confirmed): a Library is a portable, self-contained vault (Obsidian convention — `.trash` at the vault root); a universe-root trash would scatter deletions across folder trees (Eisa's libraries live under `E:\Cognitive Knowledge\…`, not the universe root) and break portability.
> - **Scope decision (Eisa, this session): §E = collision dialog (§E-1, done) + diagnostics line (§E-2, next).** The refusal / quarantine / freshness recovery UX folds into **§F**. §E-2 = a journal-read IPC + a Settings → Debug "Write integrity" readout (writes journaled / anomalies count).
> - **REMAINING MIG-076 scope:** **§E-2** the write-integrity diagnostics line (next) · **§F** flip `WRITE_GATE_ENFORCE=true` after a clean soak + the refusal/quarantine/freshness recovery UX + permanent regression suite + 3-agent audit + `/simplify` + close. The WriteGate CAS is still in **shadow mode**.
> - **User-facing:** the name-collision dialog (new) — Help topic (*Notes Management → When a name already exists*) + User Manual updated (EN); the 14 translated help mirrors ride the standing batched-translation debt.

> **What changed in v2.75** (**MIG-076 §D — quiesced rename + title-rename re-land — SHIPPED + Boss-validated; the orientation-§13 title-rename gap CLOSED; a 6× rename perf win — the on-save embed lock contention — fixed**):
>
> - **§D ships the rename layer on the §C single-ownership foundation.** Title-rename now runs the SAME full quiesced orchestration as a sidebar rename (freeze → flush-through-model → `rename_item` → wikilink cascade → deliberate remount), so every `[[link]]` to a title-renamed note heals — closing the long-open **orientation-§13 title-rename gap** that the reverted `a086e1ee` had opened. Boss Stage-2 (the exact BUG-023 probe: A links B, rename B by title, inspect both files) **PASS** — both notes keep their own identity, the link heals, **zero write-journal anomalies**. The content-integrity class stays closed through the feature that originally broke it (BUG-023).
> - **The freeze overlay** (`src/lib/components/CascadeFreezeOverlay.svelte`, NEW). A read-only "Updating links…" overlay (localized ×15 — `cascade.updating`) covers every affected-library editor pane during the wikilink cascade, blocking input so no keystroke is lost across the remount. Raised **right after `renameItem`** (the proven-safe post-rename reactive context) and lifted in a `finally`. Reactive signal: the new `cascadeFreeze` store (SEPARATE from the non-reactive `cascadingPaths` write-gate Map — the editor surfaces subscribe to the overlay one; the hot-path save gate stays a plain Map lookup). Mounted on the active / split / index editor panes (each made `position:relative`). Focus is intentionally excluded (a rename can't originate from the sidebar-less, read-only-title Focus surface; §C identity-binding covers Focus content).
> - **The flush-from-model seam fix (§D1).** Under §C the write-ahead buffer is fed only on tab teardown (`NoteEditor.handleFlush`), NOT per keystroke — so `flushAllTabsInLibrary` (the pre-cascade flush) was reading a stale buffer and could leave a freshly-typed link to a just-renamed note broken (a sub-1.5s pre-autosave race, latent in shipped §C). It now composes the flush from the MODEL (the §C authority) under `SINGLE_OWNERSHIP` (`saveNoteSession`, identity-guarded, `isNoteDirty`-gated). One caller (the rename path); legacy WAB path preserved in the `else`.
> - **The 6× rename perf win — the on-save embed lock contention (the headline perf finding).** A rename done right after editing a note used to take ~10.6s. Root cause (read + DB-verified, not guessed): `constellation_embed_notes` (the on-save semantic embed; 8,786 vectors active here) held `search_state.db.lock()` across its WHOLE loop **including the multi-second `run_embedding` model inference**, so the next rename's `reindex_single_note` (`db.lock()`) blocked ~10s behind it. **Fix** (`src-tauri/src/embeddings.rs`): the DB lock is now scoped to only the existence-check + the INSERT — **inference runs lock-free**. Rename-after-edit **10.6s → 1.77s**; a normal rename (no recent edit) is **~400ms**. This is a BROAD win, not rename-only: every save's embed used to freeze the whole search DB during inference. (The earlier handover's "the ~7s rename is the whole-universe wikilink scan" was **imprecise** — the trace proved the cascade scan + the tree refresh are fast; the cost was this lock.) Residual ~1.4s (CPU during concurrent embed inference — not a lock) logged as a follow-up PJ.
> - **Process lesson (logged, this session).** instant-freeze **v1** — raise the freeze BEFORE `renameItem`, keyed by tab.id — **regressed** the cascade + tree refresh (both silently skipped) when an edited tab was open. Could not be pinned by reading (`cascadeFreeze` has only the overlay as a subscriber); per **Reproduce-First + Stop-On-Correction**, secured first (reverted to the Part-A-passing version, Boss-chosen), bisection-confirmed the pre-rename raise as the culprit, then shipped **v2** (raise after). Reproduce-First (the write-journal + a one-shot Boss-readable timing trace) is what overturned the wrong slowness assumption and found the real lock. Harness `tests/mig-076/` stays **28/28** (Recipe H = the title-rename lifecycle).
> - **REMAINING MIG-076 scope:** **§E** refusal/recovery dialog + name-collision dialog (PJ-003 §E1b: Change name / Overwrite-trash-backed / Cancel) + i18n ×15 · **§F** flip `WRITE_GATE_ENFORCE=true` after a clean soak + permanent regression suite + 3-agent audit + `/simplify` + close. The WriteGate CAS is still in **shadow mode**.
> - **User-facing:** title-rename now cascades links (a behavior change — the §13 gap closes) + the freeze overlay (new). Help topic + User Manual updated (EN); the 14 translated help mirrors ride the standing batched-translation debt.

> **What changed in v2.74** (**MIG-076 §C — SINGLE CONTENT OWNERSHIP — SHIPPED + BOSS-VALIDATED (5/5). The content-integrity bug class is structurally closed. `milestone/mig-076-single-ownership` tagged + ZIP-backed.**):
>
> - **The cure, ratified end-state.** After the §CB increments failed (see v2.73), Eisa ruled *"Forget the clean slate. Solve this issue for good. NO more patching."* Three top-principal rules were locked into CLAUDE.md (Reproduce-First; Solve-the-Class-Not-the-Instance; the Editor-Surface Gate Checklist). The fix was then **designed whole, built and proven in isolation, and landed as one validated swap behind a toggle** — exactly as the new rules demand.
> - **The architecture (three new leaf modules in `src/lib/editor/`):**
>   - **`noteModel.ts`** — THE single content authority per open note: `{id, path, cid, props, body: CM6 Text, version, savedVersion}` in a non-reactive module Map. Replaces the three drifting copies (editor text / stale `tab.content` / write-ahead buffer) whose mismatch was the entire disease. **Identity-guarded BOTH directions:** `compose(id, expectPath)` refuses to hand out content for the wrong path (read-out guard); `setBody/setProps(id, …, expectPath)` refuse to accept a write addressing the wrong path (write-in guard). Plus `adoptDisk` (freshness-gated external-change reconciliation), `markSaved`/`isDirty`.
>   - **`noteSession.ts`** — the thin testable glue the components call (`open/ensure/editBody/editProps/seedBody/compose-via-save/repath/externalChange/close`). The §CB bugs lived in glue tangled inside `.svelte` lifecycle; pulling it into a plain module is what made it harness-provable.
>   - **`ownershipFlag.ts`** — `SINGLE_OWNERSHIP` (currently **true**). Instant rollback: flip false + rebuild → proven legacy behavior, no half-state (the legacy path is preserved in the `else` branch of every chokepoint).
> - **Integration (commit `1d93732c` swap, `19ff135b` fix, `4cc77c2a` flag-on).** NoteEditor + FocusPane seed via `seedBody` and save via identity-bound `compose`; FocusPane binds to a **captured focus-session identity** (closes the in-focus-switch cross-write); `saveTabContent` + the store lifecycle (`openNoteTab` reuse/new, `loadTabHistoryEntry`, `renameItem`, `moveItem`, `reloadTabsFromDisk`) drive the model **synchronously** from the explicit note-open event (not the async `ensure` $effect); second-screen `onNoteSaved` adopts freshness-gated. `parseFrontmatter`/`buildFullContent` stayed in `store.ts` (hoisted-fn import cycle is eval-safe).
> - **The one Boss-test failure, found + fixed.** The first swap (flag on) passed Focus + rename visually, but the **write-journal caught a new-note-while-open identity leak** the eye couldn't see: creating a new note reused the tab, and the torn-down PropertyEditor for the *previous* note fired a last save that poisoned the reused model with the old note's identity (§C test's cid landed in §C Eisa No. 2.md). Rolled back to safe on Eisa's order; root cause confirmed from journal+code (not guessed); fixed with the **write-in identity guard + synchronous model-open**; reproduced as harness **Recipe G** (the exact poison) → blocked. Re-test: **5/5 PASS, journal 90 writes / ZERO anomalies, real notes untouched.**
> - **The harness (`tests/mig-076/`, 25/25):** `noteModel.test.ts` (acceptance invariants I1–I5), `runtimeHarness.test.ts` (every failure recipe as a view-vs-disk parity scenario, incl. Recipe G), `currentBugRepro.test.ts` (characterizes the pre-cure defects). Permanent CI surface — this bug class cannot silently return.
> - **Still pre-existing (NOT a regression):** the ~7s rename is the whole-universe cascade wikilink-scan (disk rename itself is ~1ms); **§D's quiesced rename owns that UX.**
> - **REMAINING MIG-076 scope:** **§D** quiesce-rename (freeze+progress overlay for the 7s scan) + title-rename re-land · **§E** refusal/recovery dialog + name-collision dialog (PJ-003 §E1b: Change name / Overwrite-trash-backed / Cancel) + i18n ×15 · **§F** flip `WRITE_GATE_ENFORCE=true` after a clean soak + permanent regression suite + 3-agent audit + `/simplify` + close. The WriteGate CAS is still in **shadow mode** (logs would-refuse, allows) until §F.
> - **No user-facing change in §C** (an invisible reliability fix) — help files + User Manual unchanged this version; §E's recovery/collision surfaces will carry those updates.

> **What changed in v2.73** (**the §C territory failed TWICE more → research-first verdict → Eisa ruled Option B: the BUFFER PATTERN (PRIORITY One, all else frozen); §C restructured as §CB-1..6; §CB-1 shipped + Boss gate 6/6 PASS; §CB-2 in flight**):
>
> - **The §C monolith failed, then §C-2 failed** — both with the same symptom (panes painted with shared content), both display-only (the journal proved ZERO real-note disk writes each time), both reverted same-hour on Eisa's order. The §C-2 one-change diff finally **named the root cause**: ANY `openTabs.update` announcement fired from the teardown flush re-enters the `{#key}` render the store itself drives — the old direct-mutation comment (*"no store.update = no cascade"*) was the load-bearing contract. Bonus journal finding: **FocusPane writes disk PER KEYSTROKE** (~170 writes/35s, +1B each) — pre-existing, now owned by §CB-5.
> - **Research-first (Working Agreement #5, Eisa's order: "don't reinvent the wheel — check Obsidian, cross-check")**: 3 parallel agents — Obsidian (developer docs + 1.7.2 deferred views + obsidian-api), VS Code (textFileEditorModel.ts / fileService.ts / Working-Copies wiki — read at source), CM6-author guidance (discuss t/2946) + Emacs/Vim. **Converged verdict: buffers own content (one per open file); tabs/views are disposable viewports owning NOTHING; saves read the model, never the view; no mature system carries content across a view-teardown boundary — the moment doesn't exist.** VS Code's per-resource write queue ≡ our WriteGate L0 (validates §A). The honest cross-check: §C-2 was at the wrong layer — it decorated the teardown hand-off; the hand-off itself is the disease (and our own Rule 8, write-time derivation, already said so).
> - **Eisa ruling (2026-06-12): "I'll go with Option B. Proceed. This is PRIORITY One. We will NOT do anything until we solve this issue for good."** Plus the side request folded in: **two ways to list open notes — tabs at the top AND a vertical list on the side** — which the buffer pattern makes nearly free (both = viewports on one metadata list) → §CB-6. Recorded: ARCHITECT **§7 addendum** (design D1–D6, territory census, risks R1–R5) + PLAN **§C rewritten as §CB-1..6** (old §C-2..5 SUPERSEDED; §C-1 WAB fail-closed stays shipped+validated). Protocol per step: own commit + own binary + own Boss gate; sandbox repro pre-gate on lifecycle steps (§CB-3).
> - **§CB-1 — the buffer registry — SHIPPED (`bb3dc883`) + Boss gate 6/6 PASS** (14:24–14:31, standalone binary 13:49:53 verified by process-start forensics). NEW `src/lib/editor/noteBuffers.ts`: **NON-reactive** module Map<tabId, NoteBuffer{path, cid, props (snapshot-cloned), body: CM6 `Text` rope, paneState? (§CB-3)}> — a dependency LEAF; inert by construction at any lifecycle moment. **16 writer sites mirrored** (census caught two unlisted writers: `loadTabHistoryEntry` Alt-nav + `renameItem`'s reload); `closeTab`→delete; workspace restores→clearAll. DEV-only parityProbe at 4 paired sites — comparing **parsed pieces**, not raw strings (parse→serialize isn't byte-identical on legacy YAML; caught at design time). Dead `updateTabContent` (zero callers) deleted. 12/12 vitest (`tests/mig-076/`), svelte-check 0/1456 files.
> - **The 7-second rename (Boss gate step 5 observation) — journal-proven pre-existing**: rename_title + rename_item completed **1 ms apart** (14:30:46.904/.905); the 7 s is the post-rename cascade title-scan (reads the whole universe) + refresh — §CB-1 added one parseFrontmatter (µs). **§D quiesce-rename owns that UX** (freeze overlay + progress).
> - **§CB-2 — SHIPPED `8ae4451f` → Boss gate FAILED at step 4 (Focus mode) → REVERTED `4fa0b838` same hour (2026-06-12).** The build: frontmatter cluster extracted to `$lib/editor/frontmatter.ts` (store re-exports, zero churn); `composeBuffer` as the only content source with refusals journaled (`journal_compose_refusal` IPC); freshProps/freshBody deleted; 21/21 vitest · 0/1457 · 923/923. **The failure**: FocusPane opened showing ANOTHER note's body and wrote {§CB2's correct props+cid + the foreign body} to disk (564B→233B at 15:43:41; a third note's body by 15:47). **Journal: only the disposable test note written — real notes untouched; ZERO refusals** — the identity guard (which-note/which-path) held honestly; the poison was the buffer's BODY VALUE, swapped at the focus enter/exit transitions (the panes-seeded-from-stale-strings disease §CB-3 exists to remove; §CB-2's `ensureBuffer` re-seed + focus seeding interacted with it). **Standing rule from this failure: the sandbox-reproduction pre-gate now applies to EVERY remaining §CB step**, not just §CB-3. §CB-1 validated state restored. (Side record: ONE pre-existing Sight tradition-isolation test failure, stash-verified unrelated, chip task_30a68ac6.)
> - **No user-facing changes yet** (scaffolding is invisible by design) — help files + User Manual untouched this version; the §E recovery/collision surfaces will carry the doc updates.

> **What changed in v2.72** (**BUG-023 data corruption → the STOP-EVERYTHING directive → MIG-076 Write Integrity OPEN; §A+§B shipped, ★Stage 1 PASS with two pre-existing data-loss bugs caught + fixed by the new write journal**):
>
> - **BUG-023 (evening)**: a title-rename fix (the §13 gap, commit `a086e1ee`) caused **BUG-015-class cross-note corruption** in Eisa's Boss test — the renamed file received the LINKING note's frontmatter (title + cid_cn identity) + empty body. **Reverted same-hour** (`e99a2f56`). Root analysis: the FOURTH incident of one structural class — disk writes assembled from three independently-mutating identity sources (props-by-tab-id + pane text + captured path). Eisa: *"We have to STOP EVERYTHING… My target is 200% guarantee."* LL-014's three-strike law invoked.
> - **MIG-076 — Write Integrity ("Single Write Authority") — OPEN, §A+§B SHIPPED.** Architect (4 parallel agents: writer map / identity model / lifecycle+laws / industry prior art) + Plan at `lab/reports/MIG-076-WRITE-INTEGRITY-{ARCHITECT,PLAN}.md`; approved with the speed rider (*"not at the expense of speed"* — all gate costs on the ≥1.5s debounced save path). Shipped: **§A1** the WriteGate (`src-tauri/src/write_gate.rs` — per-path serialization L0 + atomic temp/fsync/ReplaceFileW replace L1 + the append-only `write-journal.jsonl`); **§A2** ALL ~24 Rust note-writers routed through it (create = create-exclusive: races refuse, never overwrite); **§B1** identity+freshness CAS under the lock (SHADOW mode — journals would-refuse verdicts, blocks nothing until the §F1 flip); **§B2** SELF-ATTESTATION (the incoming content's own cid_cn is compared against the disk's — every writer identity-protected with zero frontend plumbing; the check that would have caught BUG-023's write); **§B3** verified-existing (the boot `mig003_backfill_cid_cn` already injects via the gated path; live DB 7,663 notes / 1 missing cid). Suite 923/923.
> - **★Stage 1 PASS — and the journal earned its keep on day one**: it caught **two PRE-EXISTING data-loss bugs** within hours — **finding #1** the rename-title stomp (a stale write-ahead buffer, faithfully migrated by §140, flushed over `rename_item`'s fresh title 349ms later — journal-proven by byte sizes) and **finding #2** the empty-body resurrection (a zombie editor instance from a double tab-update + the wab restoring an empty-body same-cid snapshot). Fixes: renameItem clears pre-rename buffers (stale by definition) + folds path/name/content/reloadVersion into ONE tab update (one remount, no zombie); resolveNoteContent never restores an empty-body wab over a non-empty disk; **write origins** — all 14 frontend writer sites label their journal entries (editor_save / editor_flush / prop_save / flush_all / …). Final journal verdict: **42 writes, 0 anomalies**. Boss re-tests both passed.
> - **Eisa rulings this evening**: name collisions get the **conventional dialog** (notify → Change name / Overwrite / Cancel; Overwrite = trash-backed) — PJ-003 ruled, lands as MIG-076 **§E1b** for BOTH create and rename flows.
> - **Earlier today** (see v2.71 + the session log): MIG-075 closed + its 4 follow-ups + the Setter CNS category (4 controls, Boss-validated) + the Style Setter Preview Rule (CLAUDE.md) + PJ-060/archive-weight/§H/PJ-010 shipped, PJ-008/009 closed-by-evidence, the state-of-standing snapshot.
> - **Next**: MIG-076 §C (single-snapshot composition + single store writer + WAB fail-closed) → §D (quiesced rename + the title-rename re-land) → §E (refusal/recovery/collision UX ×15) → §F (enforcement flip + lifecycle regression suite + audit).
>
> **What changed in v2.71** (**MIG-075 CLOSED — the CNS rebuilt to its ratified paper: the engine off the DB, the boundary shed to CCS, the founding registers rendered, one color source, three Boss stages PASS, audit clean**):
>
> - **MIG-075 — the CNS rebuild-to-spec — CLOSED 2026-06-11** in one same-day arc (paper ratified v1.1→v1.2 → Plan approved → §A1–§E + 5 Boss-driven fixes → /simplify → 3-agent audit; ★Stages 1–3 all PASS — Stage 3: *"Perfect! Pass."*). Milestone `milestone/mig-075-cns` + ZIP. The headline state:
>   - **The engine**: `constellation_sight_centrality` + `detect_tensions` read the indexed DB (one `note_links` query / three scoped reads incl. `json_each(tags_json)`), both **async** — the corpus walks are structurally gone. **Eisa's lived measure: CNS paints in <3 s** (was a multi-second UI-thread freeze recurring per edit). The CNS panel opens with **zero live link IPCs** (was 3). Dead surfaces deleted (−1,427 lines: the tag_edges walker, v1 ConstellationSight, LensPanel).
>   - **The boundary**: the circulatory blocks left CNS for ONE **"Circulation → CCS"** row; **Hubs** joined CNS off the cached snapshot with **KH's Most-Connected card retired atomically** (Q6); the **CCS→CNS back-link** (Eisa's Stage-1 ask) completes the dual-organ mutual deep-links.
>   - **The registers (the dark half rendered)**: the **Regions color lens** (Louvain finally consumed — color switches library↔region, position never moves per Q2; live legend; hover **mutes the rest to black-and-white**, click **PINS** the region — both Eisa-directed at Stages 2–3) and **Blind Spots** (top-8 gaps with clickable **suggested-bridge** chips — the founding register restored). Every formerly-dark prop is consumed (the paper §11 rule).
>   - **The names**: `lens.title` native ×14 — ar **الجهاز العصبي للمعرفة** (Eisa's term, paper v1.2); **Structural Cohesion / التماسك البنيوي** replaced "Universe Health" (all four sub-metrics readable); the DF-08 wrong-key headers fixed; the graph-layer scope caption ships ×15.
>   - **One color source**: CNS edges + legend read the **Link-Type Registry** (recolors live via subscription; custom types colored; the legend registry-driven). The §B1-fix-2 **canvas size sync** killed the PJ-062-family bug (CSS-bound box + ResizeObserver + view-preserving re-fit).
>   - **/simplify** (4 agents): 2 real fixes (the unreachable ×N branch; one Map.get per node in the draw loop); **audit: invariants 11/11 HOLD · drift CLEAN** (one follow-through: the 7 dead `lens.link*` keys dropped ×15) · **migration path 8/8 PASS, zero risk**; **903/903** Rust lib tests; svelte-check 0-errors held through every commit; zero boot-path changes.
>   - **Docs**: the CNS help topic + User Manual §8b rewritten to the ratified paper (the stale CNS-vs-Sight framing replaced by CNS-vs-Sky-View/CCS/KH/health-tab/360.3D). **Noted follow-ups**: the 14-language help batch (the ar CNS mirror is stale against the rewrite); the null-type predicate centralization (Rust+TS); the Style-Setter CNS *category* wiring (`data-style-target="cns"` is tagged, inert until a category is designed); `diversivity` serialized-but-unread in the centrality payload.
>   - *(2026-06-11 afternoon — all four follow-ups SHIPPED on Eisa's "Pass. Proceed with the follow-ups")*: **FU-2+FU-4** (`57f230c4`) — the ONE null-type definition now lives in `link_types.rs::is_null_type` + `linkTypeRegistry.ts::isNullLinkType` (mirrored, both call sites re-pointed) and `diversivity` is dropped from the centrality payload (computed-never-read); **FU-3** (`aa8626bf`) — the Style Setter gains a **CNS category** (Background live on the well root; hover-label bg/text + **"Text size"** `--cns-label-size` 9–24px (Eisa post-pass ask — the existing ×15 `text_size` key; the label box scales proportionally), mount-read → apply on next CNS open) with a mini gravity-well preview; **FU-1** — the CNS help topic re-translated ×14 (the stale 2026-05-16 mirrors overwritten) **and the CCS topic translated ×14 for the first time** (28 files, locale-verbatim UI vocabulary from each `{lang}.json`, `translation_status: AI-generated 2026-06-11`). The four "noted follow-ups" above are all closed. *(FU-3 corrected same afternoon — Eisa's screenshot caught two defects: raw `styleSetter.labels.*` keys rendered (the `L()` fallback never fired — svelte-i18n echoes the key itself on a miss, which is truthy, so `|| en` was dead for ANY unkeyed label) and **no mini gravity-well** (every category except editor/sky is two-zone = no centre stage, so the FU-3 preview branch was unreachable). Fix: `L()` made miss-robust (key-echo = miss → English fallback — protects every future label), **CNS joined the three-zone preview set** (same MIG-072 §2 rationale as Sky View, plus the harder one: the hover-label vars are mount-read, so ONLY the mini-well can preview them live), and the 4 labels keys added **natively ×15** (ar chip = Eisa's **الجهاز العصبي للمعرفة**) — gated insertion, leaf-delta +4 each. Note the model shift: on the CNS category the Setter no longer live-pushes to the real app (three-zone clears the live layer); the mini-well IS the live preview, **Keep** applies the background to the real well immediately, labels on next CNS open. Mid-re-test Eisa dictated a new standing rule — **the Style Setter Preview Rule** (now in CLAUDE.md): *"Taking advantage of the entire center zone. Not to squeeze any element mimicry in a tiny box."* — the well preview was resized to fill the stage (`--cns` card modifier = the `--sky` pattern; the well `flex: 1`; rings 28/56/84% of its height).)*
> - **§4.2 row 4 (Tension Detector) updated below**: ⚠️ Partial → **✅ DB-sourced on-demand read (MIG-075 §A2)** — indexed, async, per-library, contract frozen; the fs walk removed. (Same engine change as the CNS centrality — §3.5's sight.rs row was already fixed at v2.67.)
>
> **What changed in v2.70** (**the CNS keep-decision ruled · the Concept Paper RATIFIED at v1.1 · the MIG-075 Plan produced — awaiting Plan approval**):
>
> - **The keep decision (Eisa's existential question, answered):** *"I want to avoid patching an earlier version… provide me with a decision: Does the Constellation Cognitive system need it? And why?"* The delivered verdict — **YES, as the structural organ of the Connection question, rebuilt to its founding spec, never patched** — on three grounds: the Connection question is constitutionally two-sided and CCS just shipped the flow side (killing CNS orphans the ratified CCS §4 boundary); **Blind Spots is the only forward-looking instrument in the whole Cognitive system** (structural gaps point at connections NOT yet made — Synthesis instrumented); and the emergent-vs-declared overlay (found Regions over declared library sectors) exists nowhere else. Merge-into-Sky-View rejected (the modes shape that killed Sight v5); retire-to-Wings stated as the honest kill-path, not recommended; keep-as-is ruled out absolutely. **Eisa: "I accept your logic. Proceed!"**
> - **[`docs/Constellation-Nervous-System-Concept-Paper-v1.1.md`](docs/Constellation-Nervous-System-Concept-Paper-v1.1.md) — RATIFIED** (v1.0 draft preserved). Locks: the identity + lineage thesis; **§2.1 The keep decision** (recorded verbatim); the four registers + Hubs; the CCS boundary + the "load-bearing belongs to CCS / CNS says Bridges" vocabulary ruling; §13 rulings **Q1–Q7 all as recommended** — Regions = a color LENS (position stays the user's organization) · the composite renamed **Structural Cohesion / التماسك البنيوي** · Blind Spots restored with suggested bridges · layer peeling = roadmap, content-similarity = deferred · **CNS carries Hubs and KH's most-connected card retires** (explicit MIG-073 reversal) · native titles ×15 (ar **الجهاز العصبي للكوكبة**).
> *(2026-06-11 ~11:05, PLAN APPROVED + the Q7 amendment)* Eisa: **"Approved. But, I will use 'الجهاز العصبي للمعرفة' as the Arabic equivalent to 'CNS'"** → the paper amended at [`v1.2`](docs/Constellation-Nervous-System-Concept-Paper-v1.2.md) (one ruling: the ar title is Eisa's **الجهاز العصبي للمعرفة** — *the Nervous System of Knowledge* — and the other 13 locales follow that semantic; en keeps the brand form). **The build cascades §A1 → §F** under Plan-Approval-Equals-Build-Approval, stopping at ★ Stages 1–3.
>
> *(2026-06-11 ~13:10, BUILD: §A1→§C2 SHIPPED · ★Stage 1 PASS · Stage 2 staged)* **★Stage 1 (Eisa): Test 1 PASS — "less than 3 seconds from click to paint"** (the corpus-walk freeze is dead) · Test 3 PASS · Test 2 failed-then-fixed (**§B1-fix-1**: the open-ccs listener is on `document`, the panel dispatched on `window` — one line) — **re-test A+B PASS**. Stage-1 findings fixed as **§B1-fix-2** (the PJ-062-family bug: the canvas was sized ONCE at mount with no resize handling → off-center well + content clipped beyond the frozen box; now CSS-bound 100% + a ResizeObserver keeps bitmap/dpr/view in step, `userAdjustedView` preserves the user's pan/zoom). Shipped since: **§A1** `76597676` (centrality re-sourced from `note_links`, async; untyped-weight parity; the dead contradictions field dropped end-to-end) · **§A2** `2bab0cfd` (detect_tensions DB-re-sourced via 3 scoped reads incl. `json_each(tags_json)`; detections unchanged; contract frozen; async) · **§A3** `7c9bb6c6` (dead surfaces deleted: tag_edges IPC + v1 ConstellationSight + LensPanel + 3 dead states; −1,427 lines) · **§B1** `63b097c8` (the circulatory shed → ONE "Circulation → CCS" row; the ≤10-link enrichment + annotation hover + thin/thick legend rows removed; locales ×15 −4+1) · **§B2** `7994519a` (**Hubs to CNS** off the cached snapshot — zero live link IPCs on the panel open; **KH's Most-Connected card retired** atomically per Q6; **the CCS→CNS back-link** — Eisa's Stage-1 ask) · **§B3** `7c59f323` (**native titles ×14** — ar الجهاز العصبي للمعرفة; **Structural Cohesion** ×15 with all four sub-metrics; DF-08 headers fixed; the scope caption) · **§C1** `9e7def9f` (**the Regions lens + register** — Louvain finally rendered as a color lens, position untouched per Q2; live legend; hover-dim) · **§C2** `8d38acfb` (**Blind Spots restored** — top-8 gaps with clickable suggested-bridge chips; **every formerly-dark prop now consumed**). 7 new Rust tests (3 sight + 4 tension) all pass; svelte-check 0-errors held throughout; ★Stage 2 staged (the registers + the names + the back-link).
>
> - **[`lab/reports/MIG-075-CNS-PLAN.md`](lab/reports/MIG-075-CNS-PLAN.md) — Phase 2 produced, APPROVED 2026-06-11 (see the dated addendum above).** §A engine (centrality DB-re-source + async, the dead contradictions field dropped · tension re-source with the TensionReport contract frozen · dead-surface deletion) → §B boundary (the shed + "Circulation → CCS" · insights strip → cached **Hubs** + the atomic KH-card retirement · **Structural Cohesion** + native titles + all four sub-metrics + the DF-08 header fix + the scope caption) → §C registers (the Regions lens + list · Blind Spots + suggested bridges) → §D registry/theme alignment (one color source; `data-style-target`) → §E docs → §F /simplify + 3-agent audit + the perf gate (Louvain worker decision measured-not-assumed, LL-015). ★ Boss stages: 1 speed+shed · 2 registers · 3 colors/AR. Accepted-delta class documented up front (occurrence-dedup centrality; per-pair contradictions; wider tag coverage; archived-filter). Build cascades on Plan approval.
>
> **What changed in v2.69** (**the CNS Concept Paper v1.0 DRAFTED from the origin — Boss-directed; MIG-075 now gates on the paper's Q1–Q7, not the audit's D-set**):
>
> - **Boss direction (verbatim):** *"Let's first develop a complete CNS Concept paper, and we will go from there. To develop this paper, I want you to go all the way back to when I first created the Constellation Sight concept. Originally, the CNS was one of many Sight failed projects. You have to UNDERSTAND the whole idea."* The full Sight lineage was read (Eisa's April-2026 PDF original + its v1.1 port + the v2.0/v3.x/v4.x papers + redesign v0.1–0.3 + the v5 purpose-audit + the Subsystem paper + session logs 2026-05-05→20; 3 parallel agents + first-hand reads; PDF port verified faithful via pypdf).
> - **[`docs/Constellation-Nervous-System-Concept-Paper-v1.0.md`](docs/Constellation-Nervous-System-Concept-Paper-v1.0.md) — DRAFT awaiting Eisa's ratification (Q1–Q7).** Thesis from the record: **CNS is not a failed Sight project — it is the ORIGINAL project.** Eisa's founding "Constellation Lens" paper (April 2026: Brandes + Louvain + Burt structural holes + the M/D/E/C composite; *"What patterns and gaps exist in my thinking?"*) IS what CNS is; v2 shipped ~70–80% of it (v1.1 §12); the Sight name then wandered (v3 star chart died on architecture; v5 revoked the network-science framing and failed the outcome test — *"What is unique about Sight? NOTHING!"*; v6 = the 24-tradition domes) and was externalized to Wings (MIG-038) — while v2 survived the 2026-05-14 A/B verdict (**"Sight v2 = Working. I decided to keep it."**) and took Eisa's own anatomical name (**CNS**, pair of Sight=sensory; later completed by CCS=circulatory, MIG-074). The 2026-05-19 Subsystem paper omits CNS from its own diagnostic-subsystem list — the identity gap this paper closes.
> - **The paper's content:** §2 the full dated lineage · §3 Why "Nervous" · §4 the gravity well — *the layout IS the answer* (centrality radius × the user's-own-organization sectors; deterministic + collide-only; Principle-6 resting calm; single-click preview / double-click open locked; I2b no-observer-effect) · §5 the four cognition-named registers (**Regions · Bridges · Blind Spots · Structural Cohesion**) · §6 the CCS dual-organ boundary + the vocabulary ruling (**"load-bearing" belongs to CCS; CNS says "Bridges"** — dissolves gap G4) · §8 the founding principles restated · §11 the truth-status matrix (communities/gaps computed-but-dark today; tag/similarity edges + layer peeling unbuilt; PJ-037 Map↔Sight rejected forever) · §13 **Q1–Q7**: identity ratification · Regions as a color LENS (position stays the user's order) · the score rename (recommend **Structural Cohesion / التماسك البنيوي** — ends the two-healths collision) · Blind Spots restored with suggested bridges · layer-peeling kept on roadmap vs similarity deferred · **Hubs to CNS + KH's most-connected card retires** (reverses MIG-073's "keep" — Eisa-only) · the Arabic title **الجهاز العصبي للكوكبة**.
> - **Process state:** MIG-075's Plan now follows the ratified paper (paper → Plan → build cascade: S1/S2 perf re-sources · the shed + "Circulation → CCS" deep-link · the Q-ruled registers · the DF-01…18 defect sweep · /simplify · 3-agent audit · staged Boss tests). Session log `lab/reports/SESSION-LOG-2026-06-11.md` §3; MoCh `docs/MoCh/MoCh-2026-06-11-0930.md`. **STOPPED — no Plan, no code, awaiting Q1–Q7.**
>
> **What changed in v2.68** (**MIG-075 RE-SCOPED by Boss correction → the FULL CNS AUDIT produced (concept · functions · purpose · boundaries · slowness · defects); awaiting Eisa's D1–D7 ratification — NOW SUPERSEDED in part: the Boss directed the Concept Paper first; the audit's D-set folds into the paper's Q1–Q7 + the MIG-075 Plan**): (**MIG-075 RE-SCOPED by Boss correction → the FULL CNS AUDIT produced (concept · functions · purpose · boundaries · slowness · defects); awaiting Eisa's D1–D7 ratification**):
>
> - **Boss correction (verbatim):** *"No. Since we are reviewing the CNS, I want you to fully audit its concept, functions, and purpose, all against the CCS, KH, and the remaining core plugins. Also, I want you to fix its slowness and design bugs/issues."* Stop-On-Correction applied (nothing built; the v2.67 Q1–Q8 framing withdrawn; a supersession banner added to `MIG-075-CNS-ARCHITECT.md` — its §2 territory facts remain the factual base). **The ratification document is now [`lab/reports/MIG-075-CNS-AUDIT.md`](lab/reports/MIG-075-CNS-AUDIT.md)** (5 parallel read-only agents + first-hand verification).
> - **The audit's central finding: 4 of 9 CNS pipeline outputs are computed and NEVER RENDERED** — `communityAssignments`/`communityColors`/`communityProfiles`/`gaps`(+bridge suggestions)/`contradictions` are declared props (ConstellationSight2:90–97) with zero further references. The help topic documents **"Communities"** and **"Blind Spots (Structural Gaps)"** sections that don't exist in the UI; the Cognitive-Engine paper assigns exactly those registers to CNS. CNS's unique analytical promise is ~half dark — what renders today is a poorer, unthemed, registry-blind node-link canvas substantially overlapping Sky View.
> - **Four identity gaps:** G1 **no CNS concept paper exists** (the only core plug-in never ratified — §6 of the audit drafts the statement; D1 proposes `Constellation-Nervous-System-Concept-Paper-v1.0.md`); G2 the help topic frames CNS vs the Wings-disabled Sight; G3 the **"Universe Health" vs "Knowledge Health" naming collision** (D3 proposes renaming CNS's topology composite — recommend "Structural Cohesion"); G4 "load-bearing" means bridges (CE paper) AND earned weight (CCS) — vocabulary split to document.
> - **Boundary matrix** (audit §3): by_type+by_confidence render in **three** places (KH + CCS + CNS — the shed resolves); most_connected + weak_foundations duplicated (KH + CNS-insights — D4/D5); two honest "structural gap" definitions coexist (tag-cluster, tension.rs vs community-pair, clusterEngine/Burt) — division to ratify: pair LISTS = TensionPanel · gap GEOMETRY+bridges = CNS · counts = CCS · per-note flags = 360.3D. CAT_COLORS badge parity Sight2↔Map verified IDENTICAL (the Badge-Taxonomy invariant holds). Orphans: the well renders ALL notes (no linked-subgraph filter; corrects the MIG-060-era note as applied today).
> - **Slowness anatomy** (audit §4, fixes directed): S1 the sync fs-walk centrality (re-source from `note_links` + async; cUniverse scans already silently swallowed → exact scope parity); S2 the three live panel IPCs incl. two full-table GROUP BYs (shed + cached-snapshot re-points → zero live aggregates on open); S3 main-thread JS Louvain over 233,538 links (measure-first via the surviving MIG-016 `sight:*` marks — LL-015); S4 skyVersion invalidation on normal edits makes S1 recur today. **Defect register DF-01…DF-18** incl. the new visible **DF-08**: the Link-Types legend + a settings header render `searchHub.linksTo` — the UI literally says "Links to"/"الربط إلى" where "Link Types" is meant.
> - **The gate (audit §8): D1** concept paper · **D2** render-vs-cut the dark analytics (recommend RENDER: communities paint the well + a Blind-Spots register with suggested bridges; contradictions stay cut from CNS — CCS counts/TensionPanel pairs/360 flags already serve them) · **D3** the score rename · **D4** most_connected home · **D5** the insights-strip fate · **D6** the tension re-source + the ×N-suffix delta · **D7** the Arabic CNS title (recommend **الجهاز العصبي للكوكبة**). Slowness + defect fixes are Boss-directed, Plan-sequenced — not re-asked. **STOPPED — no Plan, no code.** Session log: `lab/reports/SESSION-LOG-2026-06-11.md` §2.
>
> **What changed in v2.67** (**MIG-075 OPENED — the CNS-modernization `/migration` Phase-1 Architect produced; awaiting Eisa's ratification — SAME-DAY SUPERSEDED: the Q1–Q8 framing was re-scoped by Boss correction to the full CNS audit above**):
>
> - **MIG-075 — the CNS modernization — Architect delivered** ([`lab/reports/MIG-075-CNS-ARCHITECT.md`](lab/reports/MIG-075-CNS-ARCHITECT.md)), executing the `Handover-2026-06-10-CNS.md` §5 kickoff verbatim. The number was **verified free** (§8 topped at MIG-074; repo-wide search finds no MIG-075+ reference). Carries the two Boss-approved items from MIG-074-CCS-ARCHITECT §3-a — **(a) `detect_tensions` Rule-8 re-sourcing** (the brief's gating fact VERIFIED: the tag-cluster input IS DB-queryable — `note_meta.tags_json` is written at index time, frontmatter `tags:` + inline `#hashtags` all scripts; live probe 7,592/7,661 notes tagged · 19,523 distinct tags · 36,176 pairs) and **(b) the CNS-panel boundary cleanup** (the "Link Health BY TYPE"/"BY CONFIDENCE" blocks live in **`SightPanel.svelte`**, mounted by ConstellationSight2 behind the analytics toggle; shed → "Circulation → CCS" deep-link on the existing `constellation:open-ccs` event).
> - **The four handover rulings answered in the doc:** (c) `lens.title` is byte-identical English in ALL 15 locales (probed) → localize ×14, ar recommendation **الجهاز العصبي للكوكبة**; (d) count-caption mechanics pinned (CNS 233,538 = resolved sky-graph edges, both endpoints exist + self-links excluded, vs CCS/KH 234,062 = recorded `note_links` rows incl. unresolved targets — one muted CNS-side caption recommended); (e) **the Universe-Health score is topology-pure** (clusterEngine.ts:332–375 — 25·modularity + 25·(1−dominance) + 25·entropy + 15·connectivity + 10·(1−gap-penalty); zero circulatory inputs; no boundary change needed); (f) `most_connected` is **duplicated today** (KH's cached `fmt_most_connected` card AND CNS SightPanel's live insights tab) — options drafted, Eisa picks.
> - **New territory findings (each an explicit ruling g–j):** `detect_tensions` has exactly ONE caller (the health tab's `loadTensionReport` — the brief's "Sky View legend action" consumer died in `b2a23d4e`); **the CNS open path is itself an fs walk** (`constellation_sight_centrality` → `scan_library_links` per library, sync, full .md re-read on first toggle — ruling h, re-source recommended IN); **CNS analytics are silently own-universe-only** while the well displays the federation (cUniverse scans fail validation and are swallowed — pre-existing MIG-063-family gap, parity kept); SightPanel's "stagnating" insight queries `status='dormant'` = **0 rows since the decay fix** (data-dead; CCS Cooling is the live replacement); the dormant chip's count caps at 200 (LIMIT); **dead surfaces** `constellation_sight_tag_edges` (zero callers) + `ConstellationSight.svelte` v1 (zero importers) — delete recommended; **registry-color drift**: CNS canvas edges + legend + SightPanel all hardcode typed-link colors (user recolors never reach CNS; custom types fall to gray) — registry-align recommended (the MIG-067/072 single-source invariant).
> - **Honest output deltas of the DB re-source, surfaced for ratification:** tag coverage widens (all scripts + frontmatter vs the walk's Latin+Arabic inline-only); the contradictions **×N occurrence suffix is not reproducible** from `note_links` (write-time dedupe per (type,target) per source) → pair rows without the multiplier (Q2); archived links become filterable (zero visible change today — 0 archived rows). **Eight questions (Q1–Q8) gate ratification. STOPPED after Phase 1 per the brief — no Plan, no code.** Also fixed in this version: §3.5/§5.5 still listed `lens.rs` — renamed `sight.rs` by MIG-009. Session log: `lab/reports/SESSION-LOG-2026-06-11.md`.
>
> **What changed in v2.66** (**MIG-074 CLOSED — CCS, the Constellation Circulatory System, ships end-to-end: the seven registers live, the Link Dashboard retired, three Boss-test stages passed, audit clean**):
>
> - **MIG-074 — CCS (الجهاز الدوري) — CLOSED** (Architect→Plan→Build §A–§D→/simplify→3-agent audit; one same-day cascade, ratified Q1–Q8 all as recommended). The **left-dock Core Plug-in beside CNS** (ECG pulse-waveform icon; `enabledFeatures.ccs`, default ON; command palette 🫀; Settings → Plug-Ins entry) renders the **seven ratified registers** (Concept Paper v1.1 §6) from **ONE `constellation_ccs_snapshot`** call on the MIG-073 `link_stats_cache`: Living Connections · Load-Bearing Reasoning · Cooling Inquiries · Conviction & Doubt · The Life of a Connection (the **5-tier usage census**, Q2) · Retired Reasoning (**Restore** + Show-all via the existing lifecycle IPCs) · The Acts of Inquiry (**registry-canonical order** + the "Open inquiries" line for untyped/legacy ids — guardrail 1). **Six additive `ccs_*` cache keys** ride the existing background recompute; **Q3 shipped**: dormancy is now **derived at read time** (`status='dormant'` OR active∧traversed∧idle>90d) and growth/maturity are warm-guarded — the decay-fix's write-orphaned dormancy is healed at the data layer for BOTH KH and CCS. AS-BUILT perf find: the stale/warm boundary is a **string-range predicate riding `idx_link_last_traversed`** (the julianday form walked the whole 234k index, ~2.4s → **37 ms**, plan-verified). `FormulationInsight` gained `source_path`/`target_path` (additive) so register rows open their notes — **never firing `_link_traverse` (the new I2b no-observer-effect invariant)**.
> - **The Link Dashboard is fully retired (ruling 6)** — §D atomic commit `40367d24`: the right-sidebar `links` tab + `LinkDashboard.svelte` (378 lines) + `PanelId 'links'` + the Settings placement row deleted; the **MIG-007 hub re-pointed** (`constellation:open-link-dashboard` → `constellation:open-ccs`; `settings.links.ccs{,Desc,Btn}` ×15 — افتح الجهاز الدوري ←); `linkDashboard.*` dropped ×15 (per-file delta −22, machine-gated). Sections mapped per the Architect §2.4 table; `crossLibrary`/`broken` dropped per **Q6**; most-connected stays in KH, orphans in TensionPanel. **Q5 mutual deep-links**: CCS header ↔ KH header.
> - **Boss-test arc:** Stage 1 PASS (EN+AR; the full registers on the 234k-link universe) with two remarks → **the CNS cross-check** (Boss approved the recommendation: the future CNS MIG also takes the CNS-panel boundary cleanup — shed "Link Health BY TYPE/BY CONFIDENCE" → a "Circulation → CCS" deep-link; recorded in the Architect §3-a addendum) and **the Arabic semantic pass** (round-2 ×20 values; round-3 Boss terms: **الاستدلال المنقطع** + tier **خامل**). Stage 2 PASS ("You create a great function") with the **associative-pill finding resolved as NOT-a-bug** — the ratified MIG-067 §H note-language rule (ترابطي exists ×15; English-titled notes show English pills; the §H design question is parked as Eisa's call). Stage 3 PASS (retirement + hub).
> - **i18n**: the full **`ccs.*` namespace (38 keys) ×15 locales** with native titles (الجهاز الدوري للكوكبة · 星座循环系统 · 星座循環系 · 별자리 순환계 · Кровеносная система Созвездия · Takımyıldız Dolaşım Sistemi · מערכת הדם של קבוצת הכוכבים · دستگاه گردشی صورت فلکی · تारामंडल परिसंचरण तंत्र · برج کا دورانی نظام …), every merge machine-gated (parse + leaf-delta + endings). **CCS joins the localized set** (Style Setter · Sky View · 360.3D · Knowledge Health · CCS).
> - **/simplify** (4 agents: reuse CLEAN · altitude CLEAN): `INSIGHT_COLUMNS` const (7 SQL sites → 1) + the parametric `linkRows` snippet in CCSView (4 registers, one row shape). **Audit: invariants 16/16 + Rule-2/Rule-4 HOLD · drift 9/9 CLEAN (single emitter, two intended listeners, zero retirement leftovers, locale parity 15/15) · migration-path 6/6 PASS** (partial-cache self-heal via the per-key readiness checks; pre-MIG-074 rollback tolerates the extra rows; stored `panelPlacements.links` inert). **Perf**: paint 595 ms; hydrated 26.3 s on the 25-cUniverse federation (within the documented 28 s MIG-061 baseline); 3 new Rust unit tests (`tests_mig074_ccs`) pin the tier census + register boundaries + Q3 disjointness. Docs: EN help topic **"Constellation Circulatory System"** + User Manual + Panels/Knowledge-Formulation updates (14-language help rides the standing batched debt). **Flagged, not changed:** the Guide-§10-vs-code drift (archive zeroes earned weight; restore restarts at 1.0) — Eisa's later call. Milestone `milestone/mig-074-ccs`. Docs: `lab/reports/MIG-074-CCS-{ARCHITECT,PLAN}.md`, session log `lab/reports/SESSION-LOG-2026-06-10.md`.
>
> *(2026-06-10 ~21:45, session close + handover)* PCS'd; **the next session opens the CNS-modernization `/migration`** via [`docs/handover/Handover-2026-06-10-CNS.md`](docs/handover/Handover-2026-06-10-CNS.md) (verbatim kickoff prompt in its §5): the two Boss-approved items (detect_tensions Rule-8 re-sourcing + the CNS-panel boundary cleanup → "Circulation → CCS" deep-link) plus four in/out candidates for the Architect (the `lens.title` localization drift · the link-count caption convention · the Universe-Health score's inputs · most_connected's eventual home). **PJ-060 (the morning handover's Task B) remains pending in its own session** — its prompt is unchanged in `Handover-2026-06-10.md`.
>
> **What changed in v2.65** (**MIG-074 OPENED — the CCS `/migration` Phase-1 Architect produced; awaiting Eisa's ratification**):
>
> - **MIG-074 — CCS (Constellation Circulatory System / الجهاز الدوري) — Architect delivered** ([`lab/reports/MIG-074-CCS-ARCHITECT.md`](lab/reports/MIG-074-CCS-ARCHITECT.md)), executing Handover-2026-06-10 Prompt A against the ratified Concept Paper v1.1. The number was **verified free** (§8 highest = MIG-073; repo-wide search finds no MIG-074+ reference). Territory mapped: the seven §6 registers → the MIG-073 `link_stats_cache` (3 registers ride existing keys; **~4–5 additive `ccs_*` keys** computed inside the existing background recompute — no new scan, no schema change); the **LinkDashboard predecessor map** (7 sections → destinations: `mostTraveled`/`stale`/`archived` → CCS registers, `mostConnected`/`orphan` already live in KH/TensionPanel, `crossLibrary`/`broken` need Eisa's Q6 ruling); the dock/overlay/lazy-mount precedents (Cataloger pattern); the MIG-007 hub seam (`SettingsModal:1391` → `+layout:2300`). **CNS identity pinned in writing for the first time: CNS = `ConstellationSight2.svelte`** — the old Sight-v2 gravity well retitled "Constellation Nervous System (CNS)" ×15, live under `SIGHT_V2_ENABLED=true` (`sight/engine.ts:131`), dock `+layout:4880`, overlay `+layout:5729`.
> - **Proposed rulings (for ratification):** `detect_tensions` fs-walk Rule-8 modernization **OUT** (all four of its outputs are CNS-topology/tension material — queued as the opening § of the future CNS MIG); **read-time dormancy derivation IN** (the `ccs_cooling` key derives cooling/stale from `last_traversed` at recompute time — no status writes; **Q3** separately offers repairing the shared 6-stage `lifecycle` key's write-orphaned dormancy bucket, which would honestly change KH's census numbers); per-register **trends deferred** to v2; **KH coordination = mutual deep-links** (never embedding). A genuine census duality was surfaced (**D2/Q2**): the cache's `lifecycle` key holds the 6-stage life arc (spark→archival, KF-spec §IV.2) while the ratified CCS §6 register text names the 5-tier usage census (fresh→stale, Guide §8) — both canon; Eisa picks. **Eight questions (Q1–Q8) gate ratification. STOPPED after Phase 1 per the brief — no Plan, no code.** Session log: `lab/reports/SESSION-LOG-2026-06-10.md`.
>
> *(2026-06-10 ~14:50, Architect RATIFIED + Plan produced)* Eisa approved **all Q1–Q8 as recommended** (dock adjacent to CNS + ECG-waveform icon · 5-tier census · the Q3 dormancy-bucket repair · trends deferred · mutual KH deep-links · drop crossLibrary/broken documented · detect_tensions OUT · Option B). **Phase 2 Plan produced** ([`lab/reports/MIG-074-CCS-PLAN.md`](lab/reports/MIG-074-CCS-PLAN.md)): §A backend keys + Q3 repair + `constellation_ccs_snapshot` → §B the CCSView surface + dock + 7 registers + `ccs.*` ×15 (★ Boss Stage 1) → §C Retired-Reasoning restore actions (★ Stage 2) → §D atomic LinkDashboard retire + hub re-point (★ Stage 3) → §E docs/PCS → §F /simplify + 3-agent audit + big-universe perf. **Awaiting Plan approval; build cascades after it.**
>
> **What changed in v2.64** (**Knowledge Health fully localized ×15 + the LinkTypePill registry-label fallback; session closed with a two-task handover**):
>
> - **KnowledgeHealthDashboard — 100% localized** (Boss-directed; the chip the MIG-073 simplify agent filed). The overlay was hardcoded English with `t` imported-but-unused while a dormant `knowledgeHealth.*` block sat unconsumed in every locale. Now every string flows through `$t()`: **8 dormant keys reused with their existing translations** (EN headings adopt the key copy — "By cognitive type", "By confidence level", "Most-connected notes", "Potential bias (no opposing view)", "Computing health snapshot…"; Boss-accepted), **13 keys added ×15**, **15 dead keys dropped ×15** (refresh, universeHealth, strongest, tensions, stagnating…) — every locale delta machine-verified (+13/−15/~0, reused values byte-identical). Data names follow the established patterns: lifecycle stage cards reuse **`notePane.stage.*`** (all 15 locales carry the 6 link stages), type bars mirror **LinkTypePill** (`linkTypes.{id}`), confidence gets `knowledgeHealth.confidence.*`. The two Rust-built English annotations are rebuilt client-side from structured fields (hubs: `traversal_count`; bias: `weight` = support count) — no IPC change. Commits `20e333b0` · `848c3049`. **The localized set is now: Style Setter · Sky View · 360.3D · Knowledge Health.**
> - **Arabic-pass fixes (Boss round-2, both validated):** (1) **`linkTypes.relates` added ×15** — the legacy default type id (301 live links) was missing from every locale (ar **يرتبط**); (2) **LinkTypePill app-wide fallback** is now locale → **registry user-given label** → raw id (custom types like Eisa's `inspires` show "Inspires"; per-locale labels for custom types = a future registry-schema item, noted not built); (3) **RTL row alignment root-caused**: the stretched `dir="auto"` name span hugged its left edge in RTL rows → `text-align: match-parent` on `.khd-insight-name` (the line aligns to the ROW's direction; the name keeps its own bidi; LTR unchanged).
> - **Session close + handover:** `docs/handover/Handover-2026-06-10.md` carries the state of play and **two ready kickoff prompts** for fresh sessions: **Task A — the CCS `/migration` Architect** (Phase 1 only; the ratified Concept Paper v1.1 + the MIG-073 `link_stats_cache` as its assumed aggregate layer; explicit in/out rulings required for detect_tensions' Rule-8 modernization and read-time dormancy) and **Task B — PJ-060** (`index_note` mtime short-circuit; SO #8 cross-check mandated first — search.rs has shifted since the 2026-05-19 trace). MoCh ×2 for 2026-06-10; session log closed.
>
> **What changed in v2.63** (**MIG-073 CLOSED — the Knowledge Health snapshot cache; + the decay-write corruption fix; + the right-sidebar health tab actually works**):
>
> - **Decay-write corruption KILLED** (`aa9941ee`, precursor fix). `constellation_link_decay` ran a WRITE loop on a read-time diagnostic — fired from the KH overlay open, app boot, and a 24h job — that (a) mutated the raw `weight` column although decay is **display-only** (Living-Links-Guide §7 / `effectiveLinkWeight`), (b) **compounded** (re-decayed the already-decayed value every call), and (c) scanned 234k rows with per-row `julianday` (~11s) + one UPDATE per row. The IPC is now strictly READ-ONLY (lifecycle census only); the boot + periodic decay triggers are deleted; the frontend `linkDecay()` wrapper is removed. **Consequence: nothing writes `status='dormant'` anymore** — dormancy rows are historical-only until a read-time derivation ships (CCS-era).
> - **MIG-073 — Knowledge Health snapshot cache — CLOSED** (Architect→Plan→Build P1-P3→/simplify→3-agent audit; all Boss-validated). The KH **overlay** (command palette 🧠 / dock) froze ~12-20s per open: 6 read-time aggregates serialized on the DB mutex, the first cold-reading the **1.7 GB** `search.db` (234,061 links). Now a general **`link_stats_cache`** snapshot table (Perf Rule 8; the data layer CCS §8 assumes — KH is its first consumer): the panel makes ONE `constellation_knowledge_health_snapshot` call (6 cached rows, measured **0.17 ms**), **stale-while-revalidate** (2-min window; stale renders instantly + background refresh on a dedicated connection; the open panel **updates in place** via `kh-snapshot-ready`), first-time population one-off on boot (`only_if_empty` — later boots are a single COUNT; MIG-067 walk-free boot intact), unconditional refresh after a reconcile walk. Audit: invariants 8/8, drift 10/10 clean; Scenario-2 hardening shipped (missing table → self-healing `{ready:false}`, never a dead panel). Commits `556d401e` · `77ed2786` · `eafd37fd` + close-out. Docs: `lab/reports/MIG-073-KH-CACHE-{ARCHITECT,PLAN}.md`.
> - **The right-sidebar "Knowledge Health" tab FIXED** (`b2a23d4e`) — the Boss's actual original complaint. The tab renders **TensionPanel** (NOT the KH overlay — an Architect mis-map corrected in-flight): its only loader lived in `enrichNodesBackground`, which the zero-boot-walks rule removed from boot — so the tab said "Loading…" **forever**, and the leftover call scanned `libPaths[0]` (wrong library in any multi-library universe). Now `loadTensionReport()` fires on tab activation ($effect + untrack; no loops), scoped to the **active note's library**, cached per library, retry-on-failure, with honest `analyzing`/`unavailable` states (`tensionPanel.*` ×15 locales). `detect_tensions` itself untouched (MIG-067-correct); its fs-walk Rule-8 modernization is queued for the CNS/CCS reorganization.
> - **Right-sidebar polish set** (Boss-driven): three panel roots (`.tension-panel`/`.rp-panel`/`.prov-panel`) never scrolled — the `.rs-full-height` host is `overflow:hidden` by design and the children owned no scroll; all three are now self-scrolling (`fcd43b3c` — also explains the earlier "Review Pulse not displaying the whole list"). Contradictions **deduped** to one row per (source, target) pair with a ×N count + stable sort (`1325c5e7`). KH overlay card numbers no longer bleed (`flex: 1 1 auto`). EN User Manual §18.4 + EN help topic updated; **the 14 translated manuals are stale snapshots missing the whole Tension chapter — translation debt logged, not patched piecemeal.**
>
> **What changed in v2.62** (**MIG-007 CLOSED — the Links Settings tab; + the Living-Link surface gains a ratified concept: CCS, the Constellation Circulatory System**):
>
> - **MIG-007 — Links Settings tab — CLOSED** (Architect→Plan→Build→/simplify→Audit; Boss-validated A/B). A new **Settings → Links** section consolidates the scattered link-*behaviour* controls — link format, auto-update-on-rename, use-wikilinks (moved out of "Sky View & Links"), plus the Living-Link Lifecycle group: decay on/off, half-life (now with a direct **number-entry** box beside the slider), and confidence back-fill (moved out of Appearance) — and a **"Related" hub** deep-linking to the Style Setter's Link-Type editor (`openStyleSetterToCategory('links')`, a new category-request seam mirroring the inspect seam) and to the Link Dashboard (a `constellation:open-link-dashboard` event; the dashboard branch was pulled out of the right-sidebar note-gate so it opens with no note focused). **The Appearance Settings section was retired** — MIG-071 had emptied it (styling lives in the Style Setter); its last control, **Title alignment, moved to Editor → Display**. Invariants held: single-styling-home (the Links tab carries **zero** styling controls), no settings renamed/lost (unchanged `$appSettings` bindings → **no migration**), i18n in all 15 (`settings.links.*`). Phase-4 audit **6/6 invariants + 5/5 drift clean**. Commits `6752d4a8` · `5a39b6e6` · `63a6ec27` + §6 cleanup. Follow-up filed (`task_19b5319d`): generalize the right-sidebar note-gate (de-dup `review`/`sourceReview`). **Closes PJ-005.**
> - **CCS — Constellation Circulatory System (الجهاز الدوري) — concept RATIFIED** ([`docs/Constellation-Circulatory-System-Concept-Paper-v1.1.md`](docs/Constellation-Circulatory-System-Concept-Paper-v1.1.md)). Triggered by Eisa questioning the old right-sidebar "Link Dashboard" panel's concept. After reading the full links corpus (Living-Link Concept Paper + Knowledge-Formulation + Living-Links Guide + User-Definable-Link-Types) and **cross-checking the other core-plugin concept papers for complementarity**, the Eisa-ratified finding: the founding **dual** biological metaphor (Nervous + Circulatory) maps to **two** universe-scale instruments of the Cognitive-Engine **Connection** question. **CNS** is the Nervous register (topology — communities / centrality / bridges / structural-gaps). **CCS** is the missing **Circulatory** register — weight / decay / dormancy / traversal / lifecycle / confidence-flow — *the pulse of your thinking* (the unbuilt P5 "circulatory health"). Seven cognition-named registers: **Living Connections · Load-Bearing Reasoning · Cooling Inquiries · Conviction & Doubt · The Life of a Connection · Retired Reasoning · The Acts of Inquiry**. Rulings: a **left-dock Core Plug-in** (peer of CNS / the Cataloger); the old right-sidebar "Link Dashboard" panel **retires** into CCS + per-note Backlinks/Outgoing; **coordinate (not subsume)** Knowledge Health; the MIG-007 hub re-points to CCS when it ships. **Next: the CCS `/migration` Architect** (frontend-mostly — reads `note_links`, writes via existing lifecycle commands; no schema change). The `Link-Dashboard-Concept-Paper-v1.0` draft is banner-marked superseded.
>
> **What changed in v2.61** (**LEDGER RECONCILIATION — the §8 Migrations table is refreshed from MIG-035 → MIG-072; companion backlog Pending Jobs v1.13**):
>
> No code shipped this version — it closes a documentation-drift gap. The §8 Migrations table had not been extended since the v2.16 full refresh (2026-05-18) and stopped at **MIG-035**, even though **37 further migration numbers (MIG-036 → MIG-072) were opened** across the three weeks since. §8 below now carries all of them, grounded in the v2.17→v2.60 preambles + session logs (2026-05-19→06-09): **23 shipped/closed** (The Cataloger · NSC + Universe Digest · Constellation Base + Cross-Universe Federation · User-Definable Link Types / the Link-Type Registry · the Style Setter · Sky View-under-Setter), **4 reverted** (the Constellation Mind local-LLM stack 046–048, removed at v2.34 `a9cf4d62`; the first Base attempt 054), **8 reserved/never-opened** (the abandoned Mind roadmap numbers 049–053; the remaining cross-universe federation surfaces 063/064; rank-aware sort 068), and **2 dormant/frozen** (Sight v7 redesign 036 / Time Dome 037, under MIG-038's Sight-disabled umbrella). Highest migration number = **MIG-072**.
>
> The companion **Pending Jobs v1.13** (`docs/Constellation Pending Jobs v1.13.md`) reconciles the PJ backlog over the same window: closes **PJ-016** (via MIG-042); allocates **PJ-059→064**; corrects the 2026-05-29 session-log "PJ-10/11" numbering collision (those fixes are now PJ-061/062 — the canonical PJ-010/011 are untouched); and sets a new top-of-queue led by **PJ-060** (`index_note` cache-hit short-circuit, the highest-leverage open fix). Note: the 2026-06-09 handover's premise that "Pending Jobs v1.9 is the latest" was incorrect — v1.12 was the latest, now v1.13.
>
> **What changed in v2.60** (**MILESTONE: `localization-complete` — the Sky View / Style Setter / 360.3D surface is now fully multilingual across all 15 languages; MIG-072 closed**):
>
> This version marks a milestone tag (`milestone/localization-complete` @ `56598158`, + ZIP at `E:/Backups/Constellation/Constellation-localization-complete-20260609.zip`). The full arc is detailed in the v2.59 block below + `lab/reports/SESSION-LOG-2026-06-08.md` (Post-close sections) + `docs/MoCh/MoCh-2026-06-09-0000.md`. Summary of what is now true:
>
> - **MIG-072 — CLOSED** (Architect→Plan→Build §1–§5 + the second-screen release-packaging fix→§6 audit). The ENTIRE Sky View visual vocabulary (PIXI `graphEngine` **and** Canvas-2D `LocalSkyView` companion) is Style-Setter-controlled via the `skyPalette` seam; the typed-link colours read the user's Link Types registry; `--skyview-bg` canvas background; per-ring frames; stacked rings. Milestone `milestone/mig-072-sky-style`.
> - **Second screen now renders in release builds** for the first time — it was a standalone Svelte entry (`screen-entry.ts`/`static/screen.html`) with no production build step, so it was **blank in every built app** (dev-only). Fix: a dedicated **second Vite pass** (`vite.screen.config.js`, run after the SvelteKit build, `emptyOutDir:false`); `package.json` `build` = `vite build && vite build --config vite.screen.config.js`.
> - **Style Setter — 100% localized** (Boss-directed, exhaustive): the panel was entirely hardcoded English (0 `$t(`); now **every** label / group / category / chrome button / colour-swatch / live-preview / editor note-mock-up / dropdown option / **tooltip + aria-label** flows through an English-fallback helper `L(en)=$t('styleSetter.labels.<slug>')||en` → **`styleSetter.labels` = 303 keys** in `en.json`, translated into all 14 other languages (fonts stay native). Previews are **RTL-correct** (`text-align: start`, follow interface direction). New **Interface → "Centre zone background"** control (`--center-zone-bg`) for the main content area.
> - **Sky View legend** maturity/stratum names now localize (`legendLabel()` in `GraphMindView`, reusing `graphView.ns*` + `nsSeed`/`stratum`; library/folder names stay verbatim as user data). The **"Node scheme" legend** is localized in all 15.
> - **360.3D Inspector** vocab (`inspector360`, 94 keys incl. long help descriptions) backfilled into the 13 English-only locales → all 15 complete.
> - **Sky View help article** translated into all 14 other languages (`docs/help.<lang>/Sky View/Sky View.md`).
> - Pre-existing **`ja.json` maturity typo** fixed (`成營度`→`成熟度`).
> - **Localization method** (reusable): per-language **parallel translator agents** → **deterministic English-fallback merge scripts** (slug-keyed; identical strings dedupe) → all-15 JSON-parse verify → `svelte-check 0` → mtime-verified `--no-bundle` build → staged Boss tests. Each translator reuses the locale's existing terms.
> - **Open follow-ups (not blocking):** other help topics besides "Sky View" may be English-only in some languages; the he/tr/ur help carries an "AI-generated, native-review-recommended" note; **Pending Jobs v1.9 is stale (2026-05-11)** and needs reconciliation against current state. See the handover (`docs/handover/Handover-2026-06-09.md`).
>
> **What changed in v2.59** (**MIG-072 §3–§5 shipped — the full Sky View visual vocabulary is now Style-Setter-controlled in BOTH renderers; + a second-screen release-packaging bug fixed**):
>
> **§3 (`0151d7d8`) — links & overlays.** The untyped edge (colour + opacity), cluster boundary, hover-edge highlight, the out/in direction arrows, semantic links (colour + opacity), the stratum/origin glow strengths, the dim (out-of-focus) opacity, the trail, and the search badges all route through `this.palette.*` (new `badgeColor()` helper; the hardcoded `HIGHLIGHT_EDGE_COLOR`/`DIM_ALPHA`/`BADGE_COLORS` removed). New Setter groups **Edges** (named distinctly from the typed-link Links category) + **Overlays**; **Glow/Stratum strength** added to Glows & MOC; **Dimmed opacity** to Nodes, with live edge/arrow/cluster/trail/badge preview samples. Audit 192/0.
>
> **§4 (`0211fd23`) — labels (full font) + 3D gizmo.** The palette gains `labelFamily`/`labelSize`/`labelWeight`; the node-label `TextStyle` reads colour/family/size/weight and `setPalette` rebuilds the pooled labels on a label-style change. Predecessor reconciliation: the Setter **Label size** wins when set; the Sky View ⚙ "Label font size" slider stays (unset = ⚙ value = today). New `skyLabels` (colour · font · size · thickness) + `skyGizmo` (X/Y/Z/centre) groups; gizmo axes + centre dot route to `palette.gizmoX/Y/Z/Centre`. Audit 200/0. **§4 completed the full *main* Sky View vocabulary under the Setter.**
>
> **§5 (`ba3affc5`) — `LocalSkyView` (second-screen companion) parity + centre-zone fill.** The small companion graph (right-sidebar **Sky View** tab + the second screen) painted from its own divergent `LIBRARY_COLORS` and ignored the Setter. It now resolves the **same `skyPalette`** as `GraphMindView` — untyped-edge colour/opacity, open-note ring, label colour, node-default fallback all follow the Setter and repaint on change (body-class theme observer; **no `getComputedStyle` on draw**, Perf Rule 3; the repaint `$effect` `untrack`s `draw()` so it adds no new redraw triggers; observer disconnected onDestroy) — and receives the **canonical** `buildLibraryColorMap` map as a prop through all four mounts (`+layout` + the 3 `SecondScreenPage` modes), so a library is the same colour in both renderers; the local `LIBRARY_COLORS` is deleted. Also: the SS Sky View panel (`.sc-star-panel`) was capped at a fixed 300px (companion huddled at the top, centre zone empty) → now **fills its definite-height panel body** (Eisa: *"the SS SV should benefit from the whole available space"*).
>
> **BUG (fixed, `1b67f036`) — the second screen was BLANK in every release/built app.** The second screen is a standalone Svelte app (`screen-entry.ts` → `SecondScreenPage`) loaded by `static/screen.html`, which referenced **dev-only paths** (`/src/screen-entry.ts`, `/@vite/client`). SvelteKit's adapter-static copies `static/` **verbatim** and there was **no build step** for the entry, so a `tauri build` shipped `build/screen.html` still pointing at `/src/screen-entry.ts` (absent in the package) → script 404 → blank white window; it only ever rendered under `npm run tauri dev`. Fix: an isolated **second Vite pass** ([`vite.screen.config.js`](vite.screen.config.js), `root: static`, `emptyOutDir: false`) compiles the entry and rewrites `screen.html` to the hashed bundle into `build/`, run after the SvelteKit build (`package.json`: `vite build && vite build --config vite.screen.config.js`); the graph uses no SvelteKit virtuals so only the `$lib` alias is needed. **Surfaced mid-§5** (Stage-2 blank screen) — diagnosed as pre-existing, not a §5 regression; the main window's SPA build is untouched (verified `build/index.html` still references `_app/immutable/*`). Dev keeps working (loses only 2nd-screen HMR).
>
> **§6 audit — PASS (2026-06-09); MIG-072 CLOSED.** Three independent read-only auditors confirmed: **invariants** intact (Perf Rule 3 · BUG-015 · typed-link single-source · dark/light) · **no drift** (`LIBRARY_COLORS` only in canonical `colors.ts`; all 6 graphEngine colour consts gone; sky controls **57/57 wired, 0 dead, 0 orphan**; the 6 remaining inline-hex literals all legitimate) · **migration path safe** (fresh Universe pixel-identical; **zero src-tauri/Rust/IPC/schema change**; `styleOverride` free-form JSON; 2nd-screen build safe in dev/release/rollback). The full Sky View visual vocabulary is now Style-Setter-controlled in **both** renderers and the second screen renders in release builds for the first time. **Remaining follow-ups** (not migration phases): 14-language help (batched) · milestone tag + ZIP backup.
>
> **Post-MIG-072 (2026-06-09) — milestone + full localization.** (1) Milestone `milestone/mig-072-sky-style` tagged + ZIP backup. (2) Sky View **"Node scheme" legend** localized in all 15 languages (`graphView.ns*`; `d35d034e`). (3) **Style Setter — full UI localization** (Boss-directed): the panel was **100% hardcoded English** (0 `$t(`); now every label / group / category / chrome / colour-swatch / preview / editor-mock-up / dropdown string flows through an **English-fallback** helper `L(en)=$t('styleSetter.labels.<slug>')||en` → `styleSetter.labels` = **284 keys** in en.json, translated into all 14 other languages (`fb76cbac`, `c1c6fd8f`; reusing each locale's existing terms + native proverb). Previews made **RTL-correct** (`text-align: left`→`start`, follow the interface direction). New **Interface → "Centre zone background"** control (`--center-zone-bg`) for the main content area (`.main-area`/`.content-area`/`.pane-container`, unset = today's `#e8e8ec`). Fonts stay native. Satisfies the **full-localization TOP PRINCIPAL** for the Setter. (4) **Remaining localization swept** (`8c0f1680`, `6ad5b092`, `7325629b`): Setter **tooltips + aria** localized (styleSetter.labels → **303 keys** × 15, Setter now 100%); the **Sky View legend** maturity/stratum names now translate via a `legendLabel()` helper (raw English state names → `graphView.ns*` + new `nsSeed`/`stratum`; library/folder names stay verbatim); the **360.3D Inspector** `inspector360` vocab backfilled (70 missing keys — strata/dimensions/states + long help text — into the 13 English-only locales → all 15 at 94); the **Sky View help article** (`Sky View.md`, 317 lines) translated into all 14 other languages (`docs/help.<lang>/Sky View/`); and a pre-existing **ja maturity typo** (`成營度`→`成熟度`) fixed. The Sky View / Style Setter / 360.3D surface is now fully localized.
>
> **What changed in v2.58** (**MIG-072 underway — full Style Setter coverage of the Sky View visual vocabulary; §1–§2 shipped**):
>
> A formal **/migration** (Architect → Plan → Build → Audit) to bring the **entire Sky View graph look** under
> the Style Setter. Trigger: Eisa — "what node/link types exist, how are they coloured, what style elements —
> I want the Setter to cover it all." Discovery: the graph's colours were **all hardcoded** in `graphEngine.ts`
> (it read **zero** CSS vars), and the typed-link colours were a **stale duplicate** of the user-editable Link
> Types registry (`linkTypeColor`). Two renderers exist: PIXI `graphEngine` (full Sky View) + Canvas-2D
> `LocalSkyView` (2nd screen). **Mechanism chosen (Option B, "follow the SV colouring mechanism"):** a PURE
> `src/lib/graph/skyPalette.ts` resolves every colour from `--skyview-*` vars (per-Universe `styleOverride` +
> live draft) falling back to today's EXACT value, and the engine is **told** the palette via a new
> `setPalette()` (mirrors `setData`) — it never calls `getComputedStyle` (Perf Rule 3). Docs:
> `docs/MIG-072-skyview-style-coverage-{ARCHITECT,PLAN}.md`; mock-up `docs/MIG-072-stacked-rings-mockup.html`.
>
> **§1 (`a59f46a0`) — palette seam + typed-link single-source.** `skyPalette.ts` (`SkyPalette` /
> `resolveSkyPalette` / `DEFAULT_SKY_PALETTE`); `graphEngine` gains `palette` + `setPalette()`, the hardcoded
> `TYPED_LINK_COLORS` removed; `GraphMindView` resolves `skyPalette` ($derived) from styleOverride + live draft
> + the registry + theme, pushes it on change. **Headline fix:** the Sky View now honours the link-type
> colours set in Style Setter → Links (it previously ignored them).
>
> **§2 (`54fe02f8`) — full node coverage.** Every node decoration is now Style-Setter-controlled, grouped under
> **Sky View** in the Setter (Canvas · Nodes · Maturity rings · Glows & MOC): default fill, the rings
> (open-note / pinned / orphan), maturity (sapling→wilting), MOC, the provenance glows; **each ring has its own
> width + solid/dotted** (`--skyview-frame-<id>-width/-style`); dotted renders as clean round dots; and 2–3
> co-occurring rings now **stack into evenly-spaced concentric bands** (glows behind; inner→outer maturity →
> MOC → orphan → pinned → open-note → selection) with **user-set spacing** (`--skyview-ring-base / -gap`). The
> Sky View Setter surface uses the **centre preview** (three-zone), filling the centre zone with a live
> stacked-example canvas + the per-type bubble groups. A collapsible **"Node scheme" legend** in the Sky View
> shows each ring/glow in the user's colours. Wiring audit **173 producers / 0 dead** (the JS-consumed
> `--skyview-*` palette vars are recognised via `skyPalette.ts`). The "unset = today's exact look" invariant
> holds for COLOURS; ring spacing/width is the owner-approved §2 visual design.
>
> **Pending:** §3 links & overlays (untyped/semantic edges, hover arrows, search badges, trail, clusters,
> gizmo + the in-scope alphas) · §4 labels (full font: type/size/weight/colour, per Eisa) + gizmo · §5
> `LocalSkyView` parity (same palette, kill its divergent `LIBRARY_COLORS`) · §6 audit. 14-language help batched.
>
> **What changed in v2.57** (**MIG-071 theme subsystem removed · universal audit (CRITICAL + HIGH fixes) · Sky View canvas background control**):
>
> **MIG-071 — themes removed entirely (formal /migration).** The v2.56 "Next" (unify Themes + Saved Styles) **evolved under Eisa's direction** into full removal — *"Why are we still testing the Built-in themes? It should be retired completely."* → **all Appearance theme data is wiped** (built-ins AND custom themes), **no backup**, dropping every Universe to the **plain-default base** (`theme.css` `.theme-light`/`.theme-dark`); the **Obsidian Community import is gone**. The **Style Setter is the sole styling home** and was **explicitly left untouched** — its saved Styles, per-Universe `styleOverride`, and named swatches all stay (Eisa: *"the Style Setter is doing better. Don't touch it."*). Eisa reframed Saved Styles as his theme list: *"I consider the 'Saved Styles' to be Constellation theme list — I can easily switch between styles without any hassle."* Commits: `de5378eb` §A (activeStyleId successor, inert) · `2388c06d` §B (unified base resolver) · `8879a7e6` §C (apply path via activeStyleId) · `db26e82e` §D (`BUILTIN_THEMES = []` + one-shot wipe of `customThemes`/`activeThemeId`) · `d4b69efe` §G (remove the Appearance theme UI + Obsidian import; `ObsidianThemeBrowser` + `obsidianImporter` deleted) · `c5a162fd` §K (/simplify — drop `deriveThemeVariables`, the dead theme machinery). The single `+layout` apply `$effect` (the **BUG-015** one-writer guard) now applies **plain-default + `styleOverride` + live draft only** — no theme branch. **Appearance tab now keeps only** Title alignment + Living-Link Lifecycle (its General bits) — **no themes, no Obsidian import**. Architect/Plan: `docs/MIG-071-themes-styles-unification-{ARCHITECT,PLAN}.md`. (Supersedes the v2.55/earlier "themes live in Settings → Appearance" statements in the dated addenda below — those are historical record.)
>
> **Universal audit + CRITICAL/HIGH fixes** (`lab/reports/AUDIT-2026-06-07.md`; the 14-agent system). **CRITICAL:** Git-LFS checkout in CI (`release.yml` → `lfs: true`) — releases were shipping **134-byte LFS pointer stubs** instead of the 118 MB ONNX model + 17 MB tokenizer; also cleared 3 svelte-check type errors (`f77fa393`). **HIGH (13 fixed):** search hardening — SQL-injection in a prefilter key, FTS5 phrase-escaping in 3 fallbacks, Arabic-normalized mentions needle (`e2358daf` H5/H6/H7) · 2nd-screen XSS via the sanitized `renderMarkdown` + 2nd-screen `styleOverride` sync + restored style Import (`ef9bd7ca` H1/H2/H3) · search-result cap + editor re-embed on save + the @xenova offline guard (`062eadd7` H10/H12/H13). **The @xenova offline break (proper fix):** Sky View "Compute Semantic Links" fetched `Xenova/all-MiniLM-L6-v2` from the HuggingFace CDN at runtime and **failed silently offline**; it now routes through the **bundled local Rust ONNX engine** (`constellation_embed_texts` → `multilingual-e5-small`) and **@xenova is dropped** (`1fd74577`). The "8-minute Loading-AI-model" stall was a **separate pre-existing bug** — a sequential `read_file` loop (one IPC per note × 7,660) running *before* embedding — fixed with batched parallel reads (`6726dc03`). **Boss-validated fully offline:** 500 semantic links, internet disconnected. **Deferred (not regressions):** H11 file-tree virtualization · H4 CSP `script-src 'unsafe-inline'` → hash/nonce (naive removal white-screens the SvelteKit inline bootstrap; H3 sanitization is the real XSS fix) · embedding speed-up via reusing search's stored vectors.
>
> **Sky View canvas background — a dedicated, decoupled Style Setter control.** After the theme wipe the graph showed a flat light-gray: the PIXI canvas is transparent (`backgroundAlpha:0`), so `.gm-container`'s `var(--background-secondary)` — the surface **shared with every sidebar/panel** — was the visible backdrop. Eisa chose (AskUserQuestion) **"Own Style Setter control."** New var **`--skyview-bg`**, consumed by `GraphMindView` `.gm-container` and `LocalSkyView` `.local-star` as `var(--skyview-bg, var(--background-secondary))` (**unset = today's look, no regression**); new **Style Setter → Sky View → Canvas → Background** control (writes `styleOverride` via the one apply effect; the Sky View preview card reflects it live; the second screen inherits it via audit H2). **Wiring audit 144/144, 0 dead** (+1 from 143); svelte-check 0 errors; frontend-only.
>
> **What changed in v2.56** (**MIG-070 §C polish — real fonts · named colour swatches · diffuse inspect targets**):
>
> Three follow-up polish items on the closed Style Setter, all `/simplify`-class (frontend-only; settings persist as a free-form JSON blob, so no Rust / schema-boundary crossing). **(A) Real font choices** — a new shared **`src/lib/fonts.ts`** (a curated cross-platform floor + the user's installed fonts via the browser `queryLocalFonts()` API, graceful fallback) feeds the Setter's font pickers AND the Settings → **Language** Font-Sets editor (one source — reused, not duplicated); every font option renders **in its own typeface** (live preview); the old 14-stack placeholder list is gone. **(B) Named colour swatches** — `appSettings.styleSwatches` upgraded `string[]` → **`StyleSwatch[]` (`{hex, name?}`)** with an idempotent back-compat coercion in `applyParsedSettings`; the Setter's compact apply-grid stays, a new **Manage** toggle adds name / rename / remove rows, and **delete is a deliberate two-step confirm** (✕ → Remove / Cancel) — the accidental **right-click-to-delete was removed** from both the Setter and the Links palette (Eisa). **(C) Diffuse inspect targets** — the ⌖ Inspect tool now tags the sidebar's **library** rows (`--ft-library-*`) and **child-universe** rows (`--ft-cuniverse-*`); for **generic buttons** the `cButtons` element was *made real* (Eisa's call) — the home "Show Dashboard" + empty-sidebar "Add Library" buttons now consume `--button-radius` / `--button-padding-*` (current values as fallbacks → no visual change until edited) and are tagged, so the Buttons control paints a real, inspectable button. The **wiring audit held at 143/143, 0 dead** throughout; **LL-032** (no themes in the Setter render path) + **BUG-015** (one apply effect) untouched. Help (English) + User Manual updated in this commit; the 14-language help is a batched follow-up (the §C-close pattern).
>
> **Next (decided, not started):** unify **Themes + Saved Styles** into one **all-local** gallery and **remove the Obsidian Community Themes import** — a formal **/migration** (theme engine ↔ preset engine ↔ apply path; it changes stored data; **LL-032 is the central render-risk** the Architect phase must solve). Eisa chose to secure this polish first. Also captured this session: a spawned task to **toggle the note summary on/off** (`task_119f64e6`, default ON).
>
> **What changed in v2.55** (**MIG-070 §C COMPLETE — the Style Setter is the single styling home; every control wired; help in all 15 languages**):
>
> MIG-070 §C is closed. The **Style Setter** is now the **single home for all styling**: the **Style Settings tab is retired** (its catalog + the 5 closed gaps all live in the Setter), the **Appearance tab is consolidated** into it (fonts, Styles, Link-Types, typed-link display, pills all moved out — Appearance keeps only Themes, Title alignment, Living-Link Lifecycle), the Setter is its **own Settings nav tab** (✦) plus a **dock inspect crosshair**, and a deterministic **wiring audit** confirmed **143/143 controls wired, 0 dead** (fixed a font-size regression; wired code-block radius / blockquote bar / caret / reading-width / border / radius tokens; removed Sidebar-width + Paragraph-spacing as un-wireable). Retired dead code removed (`/simplify`, 225 lines). The **Appearance & Themes help is rewritten in all 15 languages** (Arabic term for "Style Setter" = **منسق المظهر**, Eisa's call; the other 13 use their natural per-language label). Milestone `milestone/style-setter-complete` + ZIP backup. The v2.54 preamble + dated addenda below carry the full per-commit arc.

> **What changed in v2.54** (**MIG-070 §C — the Style Setter session, consolidated: Phase 5 Links + a layout redesign + Option E live preview (Phase A) all shipped & Boss-validated; named Styles in-the-Setter (Phase 6) in flight; help + User Manual synced**):
>
> This is a clean checkpoint of a long Style-Setter run (commits `6fea13eb` → `ab2782d9`). The v2.53 block + its dated addenda below carry the per-commit detail; this preamble is the current state.
>
> **The Style Setter today.** It is the single styling surface. **Persistence = theme base + per-Universe `appSettings.styleOverride`**, merged last in the ONE shared `+layout` apply `$effect` (tracked in `_lastStyleSettingsKeys`; survives theme switch; clears cleanly). The left rail groups everything into **Surfaces** — Interface · Components · **Links** · Editor · Global · Sky View · OrgChart · Index · Cataloger · Shell.
>
> **Layout (redesigned).** The panel is **resizable** (corner grip; size persisted in `localStorage`). Only the **Editor** category keeps the 3-zone layout (left · centre **note preview** · right controls — its rendered note needs the centre). **Every other category is 2-zone** (left sidebar + one wide right space, no centre).
>
> **Live preview — the real app IS the preview (Option E Phase A, Boss-validated).** On a non-Editor category the Setter **docks to the right + goes translucent** and the overlay no longer dims/blocks the app, so the **real chrome — sidebar, dock, tabs, status bar, and the live Sky View/OrgChart — restyles as you drag.** The mechanism: a transient **in-memory `liveStyleDraft`** store merged LAST in that same single apply `$effect` (so there is **no second-writer race** — the BUG-015 guard — and zero IPC/disk on a drag). Header buttons: **Keep** (persist via `mergeStyleOverride` + clear the live layer) · **Discard** (re-seed from the saved look + clear live → the app reverts) · close = discard. The saved look on disk is **never touched until Keep**.
>
> **Links (one integrated surface).** The display controls (Colour-typed-links / Show-labels toggles + pill radius/height/weight) sit above the §G Link-Types editor; each row shows its **real live pill** and **clicking the pill opens the colour picker** (the control IS the preview — no empty colour box, no duplication). A **universal Saved-colours palette** (the same `styleSwatches` the interface uses) remembers picks; auto-saved swatches recolour the highlighted type on click.
>
> **In flight / next.** **Phase 6 — named, reusable Styles inside the Setter** (wire the "+ new" card + name field to `newPresetFromCurrent`/`applyPreset`/`unifiedStyleList`; reuse the frozen MIG-069 preset engine, read-time non-destructive) — *Eisa's current ask, paused mid-design for this PCS.* Then the rest of **Option E**: **D** DevTools-style inspect-to-style for chrome discovery · **C** inline mini-previews for Global atoms. Research + plan: `docs/MIG-070C-non-editor-preview-RESEARCH.md`, `docs/MIG-070C-OptionE-PLAN.md`. **Help + User Manual updated this PCS** (English canonical: the redesigned Setter, live preview, Keep/Discard, Links pill control; the 14-language help is a batched follow-up — the Setter is still gaining Phase 6/D/C).
>
> *(2026-06-05, Phase 6 shipped — Boss test pending)* **Named, reusable Styles now live inside the Setter.** The hardcoded quick-theme rail is replaced by a **Styles gallery** (`unifiedStyleList` — built-in theme Styles + this Universe's custom-theme Styles + the user's saved Styles), each a self-portrait card via `stylePreview`; **clicking a card applies that Style** (`applyPreset`, non-destructive merge — link colours/custom themes merge by id, never replace) and reflects it in the draft. **"+ Save current"** Keeps the current look then saves it as a named Style via `newPresetFromCurrent`. Crucially, a **new `styleOverride` preset section** (additive, in `SECTION_CATALOGUE`; captures `styleOverride` + `perScriptFonts`) means a saved Style now carries the **per-element look you designed in the Setter**, not just a theme — so a built-in theme applied over it leaves your chrome overrides riding on top. The frozen MIG-069 engine is reused read-time; the section add is back-compatible (old Styles still apply). The existing **Settings → Appearance → Styles** panel (rename/duplicate/delete/export/import) stays as the full manager; Setter CRUD/export (Phase 6.3) is the next small follow-up. Then Option E **D** (inspect) → **C** (inline Global).
>
> *(2026-06-05, Phase 6 froze the Setter on open — fixed)* ⚠ The first Phase-6 cut rendered the Styles gallery with **`unifiedStyleList` + `stylePreview` self-portrait cards** (including the built-in themes via `themeToStyle(BUILTIN_THEMES)`) — **the exact "gallery of heavy cards over `BUILTIN_THEMES`" that this very orientation (v2.49) documents as the main-thread freeze abandoned per LL-014.** It made the Setter non-responsive on open (Eisa caught it immediately). **Fixed** by dropping `unifiedStyleList`/`stylePreview` from the Setter: the **Saved styles** list is now **lightweight name rows** (`savedStyles` only; click → `applyPreset`; no built-in-theme cards, no portraits). The save/apply engine (proven-safe, shared with `StylePresetsPanel`) and the `styleOverride` section are untouched. **Built-in themes are no longer in the Setter gallery** (deliberate, to stay clear of the freeze shape). **Lesson reinforced: before adding UI to the Setter, cross-check its documented freeze history — `unifiedStyleList`/`themeToStyle`/heavy-card galleries are banned there.**
>
> *(2026-06-05, Phase 6.3 shipped; theme picker REMOVED — it re-froze the Setter)* The Setter's **Saved styles** rows now carry **rename / delete / export** (hover-revealed ⤓/✎/✕ + inline rename) — the same actions as the Settings → Styles panel, lightweight rows (no cards). ⚠ The same day, a *"lightweight"* theme picker (a plain `<select>` over `BUILTIN_THEMES`) was added to recover the built-in-themes regression — and it **re-froze the Setter on open**. `BUILTIN_THEMES` is a trivial `const[]`, so the cause is an unreproducible Svelte-render interaction, not the data. **LL-032 strengthened + the picker removed:** the Setter render path must touch **NO themes at all** (not a card gallery, not a `<select>` over `BUILTIN_THEMES`). Built-in/custom **themes live only in Settings → Appearance**; the Setter carries saved **styles** + the live per-Universe look.
>
> *(2026-06-05, "Update style" added)* Each saved-style row gains an **↻ Update** action — overwrite that style's captured look with the **current** one (keeps its name + id), distinct from "+ Save current as a style" (which makes a NEW one); a brief **✓** confirms. Theme-free (no freeze risk).
>
> *(2026-06-05, Option E item C — inline Global preview)* The **Global** category (background / text / status / accent shades; type & rhythm; shape & corners) now shows a **live composite sample card** atop its controls — a heading, muted/faint text, an accent chip, a #tag, and the status colours — built purely from the Global CSS vars, so every shade / radius / border / accent edit shows **live** via the CSS cascade off the draft-carrying `.ss` wrapper (no JS; theme-free → no `BUILTIN_THEMES`, no freeze risk).
>
> *(2026-06-06, Note summary + Breadcrumb now styleable)* Two note-page elements join the **Editor** category. **Note summary** — the NSC headline under a note's title — gains a **full type set**: colour, font, size, thickness, and italic on/off (`--summary-color` / `--summary-font` / `--summary-size` / `--summary-weight` / `--summary-style`; defaults = today's italic muted look). **Breadcrumb** — the path line above the paper — gains its own colour + size (`--breadcrumb-color` / `--breadcrumb-size`); its colour **defaults to the interface text** (§3B: the breadcrumb is chrome) but a Setter override now wins when set. NotePane's `.e-summary` / `.e-breadcrumb` read those vars; both render as clickable lines in the centre note preview. Additive (unset = today's look), theme-free.
>
> *(2026-06-06, Option E item D — inspect-to-style; the hybrid (E) is COMPLETE)* The Setter header gains an **⌖ Inspect** toggle: turn it on → the panel hides + goes click-through so the **real app** is hoverable; the chrome element under the cursor highlights + names itself, and **clicking it jumps the Setter to that element's controls** (Esc / Cancel also exit; non-target clicks are swallowed so inspect can't fire an app action). Built from a `data-style-target` registry on the chrome containers — `+layout.svelte` (dock · sidebar toolbar · layout bar · tabs · right sidebar · status bar · sidebar shell · universe), `FileTree.svelte` (tree · folders), `TagsPanel.svelte` (tags); the inspect logic (toggle · hover-highlight · click-to-jump · cleanup) is all in `StyleSetter.svelte`. Diffuse targets (per-library rows, child-universe rows, generic buttons) deferred. Theme-free → no freeze risk. **This completes Option E (A real-app live preview · C inline Global · D inspect).**
>
> *(2026-06-06, inspect — dock shortcut + Settings-hide; Boss-flagged entry-point fix)* Eisa hit a real gap: the Setter is opened *from* Settings, so hiding the panel for inspect left the **Settings modal** covering the app. Fixed two ways: (1) a **dock shortcut** — a crosshair icon **above the Settings gear** (`openStyleSetterInspect` → opens the Setter straight into inspect mode, no Settings in the way); and (2) inspect now also **hides the Settings modal** (a `body.ss-inspecting` class + a `:global(.settings-overlay){display:none}` rule), so the in-panel **⌖ Inspect** works regardless of how the Setter was opened.
>
> *(2026-06-06, coverage — the editor + the Universe panel, Boss-flagged)* Two surfaces were missing. (1) The **editor** is now an inspect target — `.e-desk` (NotePane) carries `data-style-target="text"`, so hovering the note in inspect jumps to Body text / the Editor category. (2) The **Universe / Libraries panel** (the `LibrarySwitcher` popup — Universe header · Own-Libraries list with colour dots + counts) joins the **Interface** category as a styleable element (background · text · section-headers, via `--universe-panel-bg` / `--universe-panel-color` / `--universe-panel-header-color` on `.library-switcher`; default = the interface look) and is inspect-targetable. *(The panel is a popup — open it to live-preview or inspect it.)*
>
> *(2026-06-06, Settings↔Setter duplication audit — Phase 9 prep)* Report: `docs/MIG-070C-settings-duplication-AUDIT.md`. The Setter now duplicates **~95% of the Style Settings tab** (it writes the same CSS vars via `styleOverride`) plus the Appearance **pill-shape / typed-link-display / font-size** controls and the per-script fonts. **~5 Style-Settings-only gaps** remain (cursor/selection · link-hover · code-block radius · blockquote *border* · shadows) — the Setter must cover them to be a **superset** before the tab retires. Themes / title-alignment / link-lifecycle / locale stay in Settings (not styling). **Phase 9 path:** close the gaps → retire the Style Settings *tab UI* (keep its apply path + catalog, still the engine `styleOverride` rides on) → trim the Appearance styling duplicates (keep the Styles cards + Link-Types editor + Themes). Both surfaces write the same stores until the retire commit (BUG-015 discipline; delete duplicate UI last, one tab at a time). Remaining MIG-070 §C: close the §B gaps · the **batched 14-language help** · a `/simplify` pass · Phase 9 retire.
>
> *(2026-06-06, Phase 9 STARTED — gaps closed · Style Setter is now a nav tab · Style Settings tab retired)* Binary `15:29:18`. **(1) The 5 §B gaps are closed** — the Setter is now a **superset** of the old Style Settings tab. Added (additive, generic `styleOverride` apply path — no allowlist, so arbitrary `--vars` flow through): Editor → **Link** gains *Hover colour* (`--link-color-hover`); **Inline code** gains *Block radius* (`--code-block-radius`); **Blockquote** gains *Bar colour* + *Bar width* (`--blockquote-border-color/-width`); a new **Cursor & selection** element (`--caret-color`, `--text-selection`); Global → **Shape & corners** gains *Small/Large shadow* preset selects (`--shadow-s/-l`, "Default" first so an unset var reflects correctly). **(2) The "Open Style Setter" launcher moved** from the Appearance tab body into the **left Settings nav as its own "Style Setter" tab** (✦ sparkles icon; the nav onclick special-cases it to `openStyleSetter()` — no inline body, opens the overlay). **(3) The "Style Settings" tab is RETIRED** — its nav entry is gone (the Style Setter tab replaces it). The body `{:else if activeSection === 'stylesettings'}` branch is now unreachable; left as revertible dead code with a retirement comment, to be removed in `/simplify` along with its orphaned helpers + the `StyleSettingsPanel` import. The catalog + the `+layout` apply path stay (existing per-theme `styleSettingsValues` still render). **Still pending:** Phase 9.3 (trim the Appearance pill-shape / typed-link-display / font-size duplicates — a UX judgment, to confirm with Eisa) · the dead-code `/simplify` · the batched 14-language help.
>
> *(2026-06-06, Phase 9.3 — Appearance fully consolidated into the Setter)* Binary `17:58:53`. A **100/100 parity table** (the actual `constellationStyleSettings.ts` catalog → the Setter's `ELEMENTS`) first confirmed every old Style-Settings control transferred; then Eisa chose **full consolidation**. Removed from the **Appearance** tab (each predecessor verified live in the Setter *before* deleting): the Interface + Note **font-size sliders** (→ Setter Global → Type / Editor → Body text), the **Typed-Link Display** toggles (→ Links toggles), the **Living Link Pills** shape controls (→ Links → pill shape), the **Styles** cards (→ the Setter's own saved-styles UI, same `stylePresets.ts` engine), and the **Link Types** editor (→ the Setter's Links category renders `<LinkTypesEditor embedded />`, StyleSetter L873). Orphaned `updatePillShape` + `resetLinkPills` + the `LinkTypesEditor`/`StylePresetsPanel` imports removed; svelte-check clean. **Appearance now keeps only:** Themes + Customize Theme (theme authoring — the Setter holds styles, not themes), Title alignment (layout), Living Link Lifecycle (behavioural). **→ The Style Setter is now the single home** for all styling, saved styles, and link-type colours. Remaining MIG-070 §C: Stage 2 functional test of the 5 new controls · the dead-code `/simplify` (the retired Style-Settings body + now-unused CSS) · the batched 14-language help.
>
> *(2026-06-06, Phase 9 WIRING AUDIT — every Setter control now does something)* Binary `21:16:45`. Stage 2 testing exposed that some controls didn't paint; Eisa: *"check all Style Setter wiring conditions, and fix them accordingly"* + *"avoid duplications."* A **deterministic audit** (extract every `var: '--X'` the Setter writes → grep all of `src/` for `var(--X)` consumers) found **11 of 147 controls DEAD** (wrote a var nothing reads). Causes + fixes: **font sizes** (a Phase 9.3 *regression* — removing the Appearance sliders left them uncontrollable) → new **`appnum`** control type writing `appSettings.interfaceFontSize/fontSize` (single source, no var↔setting dup); **`--caret-color`** (stomped by a `!important`) → NotePane honours it; **`--code-block-radius`** + **`--blockquote-border-color/-width`** (never wired) → `lineDecoPlugin` `.cm-codeblock-line`/`.cm-blockquote-line`; **`--file-line-width`/`--file-margins`** (dead ch model) → Setter switched to **px** matching `.e-paper`, wired `.e-paper`+`.e-breadcrumb`; **`--border-width`** → all `var(--border)` chrome borders; **`--sidebar-bg`** → `.sidebar` override (layered over the global panel bg — not a dup); **`--radius-l`/`--radius-m`** → `.settings-modal`/`.stat-card`. **2 controls REMOVED** (Eisa's call — can't be honestly wired): **Sidebar width** (the drag-resize handle owns it via a JS inline width) + **Paragraph spacing** (a live-preview *source* editor has lines, not paragraphs). The short-alias layer (`theme.css`: `--bg-secondary: var(--background-secondary)` …) is clean — one source of truth. **Re-audit: 143/143 wired, 0 DEAD.** Commits `2b6d9493` (§1) + this (§2). PCS'd; tag `milestone/style-setter-complete` + ZIP backup.
>
> *(2026-06-06, Phase 9 dead-code /simplify — DONE)* Binary `22:01:12`. Removed the now-unreachable Style-Settings code the tab retirement left behind (0 live refs verified first): the `{:else if 'stylesettings'}` template block (83 lines), the helper cluster (`getActiveTheme`/`ensureCustomTheme`/`exportStyleSettings`/`copyStyleSettings`/`applyStyleSettingsJSON`/`pasteStyleSettingsFromClipboard`/`importStyleSettingsFile` + `ssImport*` state, 93 lines), the dead `StyleSettingsPanel`/`getEffectiveStyleBlocks` imports, and the orphaned `.ss-toolbar*`/`.w-btn-sm*`/`.ss-import*` CSS (47 lines). **225 lines removed; svelte-check clean; no user-facing change.** Kept shared `downloadJSON`/`pickJSONFile`. **Remaining MIG-070 §C: the batched 14-language help only.**
>
> ---
>
...
>
> *(2026-06-05, redesign Boss-validated + two follow-ups)* The redesign passed. (1) **The pill IS the colour control** — the redundant empty colour box is gone; each Links row now renders just its real `<LinkTypePill>`, and clicking the pill opens the OS colour picker (a transparent `<input type=color>` overlays the pill via `.lte-pillpick`). The exact pill already shows shape + colour + text, so nothing is duplicated. (2) **Non-Editor live-preview options researched** (Eisa asked to audit/explore) → `docs/MIG-070C-non-editor-preview-RESEARCH.md`: five options benchmarked against Obsidian Style Settings, VS Code, DevTools, Figma/Material, Storybook, tweakcn, OS pickers; recommendation = **Option E (hybrid)** built incrementally — A: the real app *is* the preview (live-apply behind a side-docked translucent panel; the only honest preview for Sky View/OrgChart) → D: DevTools-style inspect-to-style for chrome discovery → C: inline mini-previews (the Links-pill pattern) for Global atoms. **Awaiting Eisa's direction choice before building.**
>
> *(2026-06-05, Option E chosen + Phase A shipped — Boss test pending)* Eisa chose **Option E**; plan `docs/MIG-070C-OptionE-PLAN.md`. **Phase A = the real app is the preview** for non-Editor categories. A transient in-memory **`liveStyleDraft`** store (`store.ts`) is merged **LAST** in the shared `+layout` apply `$effect` (above `styleOverride`, tracked in `_lastStyleSettingsKeys`) — the **same single writer**, so no second-effect race (the BUG-015 guard), and zero IPC/disk on a slider drag (Rule 3). On a non-Editor category the Setter **docks to the right + goes translucent** and the overlay no longer dims/blocks the app (the real chrome shows + stays interactive); the draft feeds `liveStyleDraft` live → the **real** sidebar / dock / tabs / status bar (and the live Sky View/OrgChart) restyle as you drag. Header: **Keep** (persist via `mergeStyleOverride` + clear the live layer) · **Discard** (re-seed from the saved look + clear live → the app reverts) · close = discard. The saved look on disk is never touched until Keep. The **Editor** category keeps its centred 3-zone note preview (no live layer). Next: **D** inspect-to-style → **C** inline Global atoms.
>
> ---
>
...
**Second-window file**: [`static/screen.html`](static/screen.html) — a **standalone Svelte app** (`screen-entry.ts` → `SecondScreenPage`), compiled into `build/screen.html` by a **dedicated second Vite pass** ([`vite.screen.config.js`](vite.screen.config.js)) that runs **after** the SvelteKit build (`package.json` `build` script). Before MIG-072 §5's fix (`1b67f036`) there was no such pass, so release builds shipped a `screen.html` still pointing at the dev-only `/src/screen-entry.ts` → the second screen was **blank in every built app** (it rendered only under `tauri dev`).
...
| `sight.rs` | 419 | Brandes' betweenness centrality + tag-shared edges (renamed from `lens.rs`, MIG-009; fs-walk re-source = MIG-075 ruling h) |
...
| 4 | Tension Detector | `tension.rs` | ✅ **DB-sourced on-demand read (MIG-075 §A2, 2026-06-11)** — three indexed per-library reads (note_meta · note_links active · `json_each(tags_json)`), async, detections unchanged, contract frozen; the per-run fs walk removed |
...
| 7 | Review Pulse / **Reviewer** | `review.rs`, `priorities.ts`, `ReviewerView.svelte` | ✅ Write-time (MIG-083) + **rich Reviewer (MIG-084)** — `get_due_notes` returns SIX lenses (Mode-1/3 SELECT ∪ Mode-2 staleness JOIN ∪ **Orphan** `incoming_count==0 & word_count>20` ∪ **Fragile** `incoming_count>=5 & out-derives<=1` ∪ Never), all from write-time `note_meta` columns (no FS walk). Per-note **Prescription** + a **computed two-axis Priority** (`priorities.ts`, frontend, read-time) with a nullable `note_meta.review_priority` OVERRIDE; `get_note_review_status` is the O(1) note-tab read. MIG-080 §F split: left-dock reviewer (universe) + right-rail note tab. `.constellation/review-pulse.json` durable action source; reconcile self-heals. |
...
- **sight.rs** (419; renamed from `lens.rs` in MIG-009) — Brandes' betweenness O(VE), weighted by link_type (supports=1.0, causes=0.9, contradicts=0.8). **At >500 nodes**: approximate sampling (200 sources). Tag-shared edges command: weight 0.6 × shared_tag_count, top 500 — **zero frontend callers (dead; MIG-075 ruling i)**. Both commands are fs walks via `scan_library_links`/`scan_note_tags_recursive` (MIG-075 ruling h).
...
## 8. Migrations (active state, 2026-06-11 — v2.61 refresh: MIG-036→072 folded in; MIG-073/074 closed; MIG-075 current)
...
| **MIG-007** Links Settings tab | `lab/reports/MIG-007-LINKS-SETTINGS-{ARCHITECT,PLAN}.md` | ✅ **Closed 2026-06-09** (Architect→Plan→Build→/simplify→Audit; Boss-validated A/B). New **Settings → Links** section consolidates link-behaviour controls (auto-update · link format · use-wikilinks · decay/half-life · confidence back-fill) + a Style-Setter/Dashboard hub; **Appearance section retired** (Title alignment → Editor → Display); 15-locale i18n; audit 6/6 invariants + 5/5 drift clean. Commits `6752d4a8`/`5a39b6e6`/`63a6ec27`. **Closes PJ-005.** |
...
| **MIG-036** Sight v7 Form-Aligns-To-Purpose redesign | (paused) | 🟡 **Dormant** (2026-05-19). Ground-up density/stack-primitive redesign; deemed over-engineered, paused under MIG-038. (orientation v2.18) |
| **MIG-037** Sight v6.3 Time Dome (25th tradition) | (frozen) | 🟡 **Frozen**. Time Dome tradition + time-family i18n landed; P2/P3 frozen under MIG-038. (orientation v2.18) |
| **MIG-038** Disable Sight + Map → external "Wings" | (Eisa direction 2026-05-19) | ✅ **Shipped 2026-05-19** (`57cd7638`). `SIGHT_V6_ENABLED=false`; Map force-off; both re-categorized as future external "Constellation Wings" plug-ins. Code intact, flags off. |
| **MIG-039** The Cataloger (CECE first left-dock Core Plug-in) | — | ✅ **Shipped 2026-05-20**. CECE promoted to left-dock button + full-page view; long-standing memory leak fixed. See §4.5. |
| **MIG-040** NSC — Note Summary Creator | — | ✅ **Shipped 2026-05-20**. Author-precedence summaries; both-axes disambiguation fix; cross-instance sync; "Classify a note…" picker. See §4.6. |
| **MIG-041** `term_vocab` bigram shrink | — | ✅ **Shipped**. Deleted ~5.19M dead bigram rows; DB 2.35 GB → 1.75 GB (~26%); retired the MIG-015 boot-blocker. (orientation v2.24) |
| **MIG-042** Drop `term_vocab.bridge_concept_id` column | — | ✅ **Shipped**. Dropped dead column + index; +2 lurking bugs fixed. **Closes PJ-016.** (orientation v2.25) |
| **MIG-043** NSC Core Plug-in Phase 1 | — | ✅ **Shipped + Boss-validated**. Engine headline variant + shared `summaryStore`; search results + editor header wired. (orientation v2.27) |
| **MIG-044** NSC Core Plug-in Phase 2 | — | ✅ **Shipped**. Backlinks / Outgoing / Index / Sky View hover summaries; +3 wrong-target bugs caught mid-test. (orientation v2.28) |
| **MIG-045** NSC Core Plug-in Phase 3 — Universe Digest | — | ✅ **Shipped + Boss-validated**. Left-dock Digest view (Library→Folder→Note), virtualized + searchable. (orientation v2.29) |
| **MIG-046** Constellation Mind Phase 0a — inference abstraction | — | 🔴 **Reverted** (with 047/048). Trait surface for inference/embedding providers. (orientation v2.31, reverted v2.34) |
| **MIG-047** Constellation Mind Phase 0b — real local inference | — | 🔴 **Reverted** (with 046/048). `llama-cpp-2` + Fanar 1.9B Q4_K_M; first real Arabic inference. (orientation v2.32, reverted v2.34) |
| **MIG-048** Constellation Mind Phase 1 — chat RAG surface | — | 🔴 **Reverted**. The whole local-LLM stack removed at v2.34 (`a9cf4d62`): CPU ~5 tok/s didn't justify it; cloud `ai/` bridge is again the only LLM surface. |
| **MIG-049** (Mind Phase 2) | — | ⚪ **Never opened** — reallocated to Bases Phase 1, then renumbered to MIG-054 for audit clarity. (orientation v2.35/v2.36) |
| **MIG-050 – MIG-053** (Mind roadmap numbers) | — | ⚪ **Never opened** — reserved for Mind phases (Jais / CECE rewire / cloud refactor); abandoned with the MIG-046–048 revert. |
| **MIG-054** Constellation Base Phase 1 (renum from 049) | — | 🔴 **Reverted** same-day (upstream `parse_frontmatter` bug); superseded by the MIG-055 clean rebuild. (SESSION-LOG-2026-05-26) |
| **MIG-055** Constellation Base — clean rebuild (Five Acts lens system) | — | ✅ **Shipped + Boss-validated**. 4 dimensions + parser + validator + `execute_lens` + LensBlock renderer; 84/84 lens tests. (orientation v2.37) |
| **MIG-056** Cross-Universe Federation foundation | — | ✅ **Shipped**. lens / status-bar / libraryStats / search federate; skip-unavailable; 47 federation tests; §K hotfixes (scatter-gather + RRF). (orientation v2.37/v2.39) |
| **MIG-057** Lexicon expansion + prefix-wildcard coexistence | — | ✅ **Shipped**. Fixes the lemma-vs-prefix collision that hid `الرباط` for typed `الربا`. (orientation v2.38) |
| **MIG-058 / MIG-059** Federated search latency | — | ✅ **Shipped**. FTS5 `snippet()` custom-tokenizer bottleneck (~500 ms/row) → Rust-side snippet (µs); 15–25 s → sub-second; Arabic input truncation resolved as a side effect. (orientation v2.39) |
| **MIG-060** Constellation Base Phase 1.5 — threading gestures | — | ✅ **Shipped**. 3 inline open-in-surface buttons per Lens row (360.3D / CNS / Cataloger); 52 vitest tests. (orientation v2.40) |
| **MIG-061** Federate boot-snapshot IPCs (CNS / Sky View) | — | ✅ **Shipped**. Federated `cache_boot_snapshot_sky`/`_graph`; closed 5 of 14 audit federation surfaces. Filed PJ-061/062 (federation-scale render polish). (orientation v2.41) |
| **MIG-062** Federate filesystem-walk surfaces + Tag Browser | — | ✅ **Shipped**. Filesystem-walk federation + universe-wide Tag Browser; 8 of 14 surfaces now closed. (orientation v2.42/v2.43) |
| **MIG-063 / MIG-064** Remaining federation surfaces (P2 read / P2+P4 write) | (reserved) | 🟢 **Reserved** — the remaining ~6 of 14 audit surfaces (Unlinked Mentions / Index / Knowledge-Health reads; Cataloger / Classifier / NSC writes with FK constraints). Not yet built. |
| **MIG-065** Unified Constellation Base | — | ✅ **Shipped + Boss-validated**. Unified two Bases onto one engine (~77 commits); full 15-locale localization. (orientation v2.45) |
| **MIG-066** Living-Links columns in Base | — | ✅ **Shipped**. Per-type link counts + columns, canonical-ordered via the Link-Type Registry, rank-aware sort. (orientation v2.47) |
| **MIG-067** User-Definable Link Types — "The Living Vocabulary" | — | ✅ **Shipped**. Shared Link-Type Registry; predicate-first `[[type::target]]` storage + type-first authoring; Settings editor; one source → every surface. See §4.4. (orientation v2.47/v2.48) |
| **MIG-068** Rank-aware column sort (Cognitive Engine) | (deferred PJ) | 🟢 **Reserved/deferred**. Ranked dimensions sort by canonical seniority, not alphabetically; lands with the CE columns. |
| **MIG-069** Style Presets | — | ✅ **Shipped**. Named app-global Styles as preview cards (8-section bundles, export/import); 5/5 tests. (orientation v2.48) |
| **MIG-070** Constellation Style Setter | (multi-phase: iters 1–2 + §3 + §C Phases 0–6) | ✅ **Closed**. The single styling home: live click-to-edit + Apply; every interface + Markdown element editable; link styling; named swatches; real fonts; help in all 15 languages. (orientation v2.49–v2.56) |
| **MIG-071** Theme subsystem removal | — | ✅ **Closed**. Built-in + custom themes removed; the Style Setter is the sole styling home; + universal audit (CRITICAL+HIGH) + Sky View canvas-background control. (orientation v2.57) |
| **MIG-072** Sky View visual vocabulary under the Style Setter | Architect→Plan→Build→Audit | ✅ **Closed 2026-06-09**. PIXI `graphEngine` + Canvas-2D `LocalSkyView` both palette-controlled via the `skyPalette` seam; typed-link single-source; second screen now renders in release builds. Milestones `milestone/mig-072-sky-style` + `milestone/localization-complete`. See v2.59/v2.60 preambles. |
| **MIG-073** Knowledge Health snapshot cache (circulatory-aggregate layer) | Architect→Plan→Build P1–P3→/simplify→Audit | ✅ **Closed 2026-06-10**. Boss-validated end-to-end (instant open · live in-place refresh · health-tab fix · scroll + dedupe polish). Audit 8/8 invariants + 10/10 drift clean; Scenario-2 self-healing shipped. Perf-Rule-8 fix for the KH overlay's ~11s cold-scan freeze: general `link_stats_cache` (stale-while-revalidate, background recompute on a dedicated connection) — the data layer CCS §8 assumes; KH = first consumer. Precursor fix `aa9941ee`: `constellation_link_decay` made READ-ONLY (the old write-loop compounded decay into raw weights on every panel/boot/24h call — decay is display-only per Living-Links-Guide §7). P1 `556d401e` backend; P2 `77ed2786` panel. Found during build: the right-sidebar 'health' tab = **TensionPanel** (`detect_tensions`), NOT the KH overlay — its eternal-"Loading…" was then fixed the same session (`b2a23d4e` + scroll/dedupe polish). Docs: `lab/reports/MIG-073-KH-CACHE-{ARCHITECT,PLAN}.md`. |
| **MIG-074** CCS — Constellation Circulatory System (left-dock Core Plug-in; the circulatory register of the Connection question) | `lab/reports/MIG-074-CCS-{ARCHITECT,PLAN}.md` | ✅ **CLOSED 2026-06-10** (Architect→Plan→§A–§E→/simplify→audit, one same-day cascade; three ★ Boss stages PASS). Seven §6 registers on the MIG-073 `link_stats_cache` (+6 `ccs_*` keys in the existing recompute; ONE `constellation_ccs_snapshot` IPC; index-riding stale/warm predicates, 2.4s→37ms); **Q3 read-time dormancy derivation shipped** (heals KH's census too); dock beside CNS (ECG icon, `enabledFeatures.ccs`); Retired Reasoning Restore via existing lifecycle IPCs; **LinkDashboard fully retired** + MIG-007 hub re-pointed (`constellation:open-ccs`); KH mutual deep-links; `ccs.*` ×15 native titles; new invariant **I2b no-observer-effect** (CCS never fires `_link_traverse`). Audit 16/16 invariants + 9/9 drift + 6/6 migration-path; 3 Rust unit tests; perf within baselines. Queued onward: the CNS-panel boundary cleanup + detect_tensions Rule-8 (→ MIG-075); the §H pill-language question + the archive-weight Guide-§10 drift (Eisa's later calls). Milestone `milestone/mig-074-ccs`. |
| **MIG-075** The CNS modernization → **the full CNS audit** (concept · functions · purpose · boundaries vs CCS/KH/all core plug-ins · slowness · defects DF-01…18) | `lab/reports/MIG-075-CNS-AUDIT.md` (ratification doc) + `MIG-075-CNS-ARCHITECT.md` (territory base, ruling framing superseded) | 🟡 **RE-SCOPED 2026-06-11 by Boss correction — AUDIT produced, AWAITING D1–D7.** Headline: **4 of 9 CNS pipeline outputs computed-never-rendered** (communities/profiles/gap-list+bridges/contradictions — unused props Sight2:90–97; the help topic documents sections that don't exist); **no CNS concept paper exists** (D1 drafts it); "Universe Health"-vs-"Knowledge Health" naming collision (D3); by_type/by_confidence triplicated KH+CCS+CNS (the Boss-approved shed resolves); slowness anatomy S1–S4 (sync fs-walk centrality → `note_links` re-source + async; 3 live panel IPCs → shed/snapshot; JS Louvain measure-first; edit-time invalidation); tension re-source per D6 (tags verified DB-queryable: `note_meta.tags_json`, 19,523 distinct). Fixes Boss-directed, Plan-sequenced. The paper was drafted from the Sight origin, the keep-decision ruled, ratified v1.1→v1.2 (the ar-title amendment **الجهاز العصبي للمعرفة**), the Plan approved, and the build cascaded §A1→§F the same day. ✅ **CLOSED 2026-06-11**: engine DB-sourced + async (Eisa: **"<3 seconds"**, was the recurring corpus-walk freeze) · the circulatory shed → "Circulation → CCS" + the CCS→CNS back-link · Hubs in (KH card retired, Q6) · **Regions lens** (hover B/W-mute + click-PIN, Eisa-directed) + **Blind Spots** with suggested bridges (the founding register) · native titles ×14 + **Structural Cohesion** · one registry color source · the PJ-062-family canvas bug killed · /simplify 2 fixes · audit 11/11 + drift CLEAN (+ the 7 dead lens.link* keys dropped ×15) + path 8/8 · **903/903** tests · ★Stages 1–3 PASS. Milestone `milestone/mig-075-cns`. Docs: `lab/reports/MIG-075-CNS-{AUDIT,PLAN}.md`, the paper v1.2, `SESSION-LOG-2026-06-11.md`. |
| **MIG-076** Write Integrity — "Single Write Authority" (the stop-everything response to BUG-023; kills the 4-incident cross-identity write class once and forever) | `lab/reports/MIG-076-WRITE-INTEGRITY-{ARCHITECT,PLAN}.md` | 🟢 **IN FLIGHT — §A+§B+§C+§D+§E (§E-1 + §E-2) SHIPPED + Boss-validated; only §F remains.** The WriteGate (`write_gate.rs`): L0 per-path serialization + L1 atomic ReplaceFileW replace + the `write-journal.jsonl`; ALL ~24 Rust note-writers routed (creates = create-exclusive); L2 identity+freshness CAS in SHADOW (`WRITE_GATE_ENFORCE=false` until §F1); **self-attestation** (incoming cid_cn vs disk, zero frontend plumbing). **§C single content ownership** (`noteModel`/`noteSession`/`SINGLE_OWNERSHIP`) — the content-integrity bug class structurally closed (Boss 5/5; `milestone/mig-076-single-ownership`). **§D quiesced rename + title-rename re-land** + the 6× embed-lock perf win (rename-after-edit 10.6s→1.77s; lock scoped off `run_embedding` inference); the §13 title-rename gap closed. **§E-1 the universe-wide name-collision dialog** (`CollisionDialog.svelte`; create-typed-name + rename → Change name / Overwrite-to-trash / Cancel; `resolveWikilinkCrossLibrary`, cUniverse-inclusive; Quick Capture + folders exempt) + the **trash de-collide fix** (`move_to_trash` suffixes on a `.trash` name clash — never clobber); Boss Stage 1+2 PASS, disk+content proven (renamed note kept its own cid + old name as alias; displaced original intact in `.trash`). **§E-2 the Write-integrity diagnostics line** (Settings → Security & Privacy; `read_write_journal_stats` streams `write-journal.jsonl` + `.old`; anomalies = would_refuse_identity/stale; ✓0/⚠N + most-recent-anomaly date; re-reads on section entry; Refresh button dropped per Boss; `writeIntegrity.*` ×15) — Boss-validated; on first run it surfaced the 2 historical §C anomalies (06-12, since fixed). Eisa rulings: speed rider; collision = conventional (§E1b). **§E CLOSED. Remaining: §F** enforce flip + refusal/quarantine/freshness recovery UX + permanent regression suite + 3-agent audit + `/simplify` + close. Two trash-setting gaps logged (delete_item ignores `trashDestination` — always permanent; Overwrite always per-library `.trash`). Tests: harness 28/28; gate 14 + suite 923/923. Docs: v2.72–v2.76 preambles, `SESSION-LOG-2026-06-11.md`. |
| **MIG-077** App-Wide Right-Click Context Menus | `lab/reports/MIG-077-RIGHTCLICK-CONTEXT-MENUS-{ARCHITECT,PLAN}.md` | 🟢 **IN FLIGHT — Architect + Plan Boss-approved (Option A); Phase A A0/A1/A2 SHIPPED + Boss-validated.** Origin: 3 Boss right-click observations from the delete-path test (no right-click batch delete · native confirm box [fixed] · "right-click should cover the whole app"). Goal: consolidate the **6 hand-rolled menus** (Tabs/Backlinks/Outgoing/Index/OrgChart/GraphMind) onto the shared `ContextMenu` + a separate `<ConfidencePicker>` (the one radio-group menu), then extend to ~6 missing surfaces (List-mode #1 first). **Done:** A0 (ContextMenu `separator`/`disabled`, `eb3a246f`) · A1 (tab menu → shared, `tabContextMenu.*`×15 + instant-pin fix `381a471e`) · A2 (Index menu → shared, `indexPanel.*`×15 + tooltip fix `8aab9db2`) — each ALSO closes a hardcoded-English i18n violation. **Remaining:** A3 OrgChart · A4 `<ConfidencePicker>` (Backlinks+Outgoing) · A5 GraphMind (opt) · Phase B **B1 List-mode** → B2 Search → B3 Tags → B4 Calendar → B5 Sky View → B6 wikilinks → B7 lower-value · Phase 4 3-agent audit + close. Idiom: a per-surface `getXContextMenuItems(): MenuItem[]` builder + drop the inline menu & its dead CSS. See the v2.79 preamble + `SESSION-LOG-2026-06-11.md`. |
| **MIG-087** Broader i18n pass + plural-aware count system (+ live counter) | (Boss-directed mid-session; AskUserQuestion approvals, not the formal 4-phase) | ✅ **CLOSED 2026-06-26 (Boss-validated).** §A 34 all-14-English keys localized ×14 (22 UI labels + 12 brand keys per Boss ruling: Sight/Map/CNS/Dataview/Return-to → native, `app.name`/HTML/CSV/CJK/360.3D kept English). §B **plural-aware counts** via native `Intl.PluralRules` (CLDR) — `tn`/`tnIn` helper + `plurals.<noun>` namespace (27 nouns ×15, ar word-only-for-1&2 per Boss) wired to ~50 call sites app-wide; fixes the wrong-Arabic status bar ("19 مكتبات"→"19 مكتبة"). §C **live word/char counter** — one-way display-only `onLiveStats` observer (outside the BUG-015/§C-2 vector; `noteModel` stays non-reactive), now body-only (frontmatter no longer inflates the count). Verified: svelte-check 0, MIG-076 harness 28/28, byte-identical locale round-trips, Boss Stage-1 (status bar, Arabic) PASS + live-counter re-test PASS. **§D (2026-06-26, the filed remainder) — SHIPPED + Boss Stage-1 PASS:** the 3 full-sentence count embeds (batch-delete dialog ×2 / reindex toast / import "…and N more") now use a `{noun}` placeholder filled by `$tn('plurals.<noun>', count)` (notes ← `plurals.notes`, andMore ← `plurals.more`; no new noun data); rewritten ×15 in native word order by a 15-rewriter→15-verifier Workflow (Arabic passive framing `ستُحذف {noun}` makes the dual ملاحظتان grammatical); svelte-check 0, 60/60 byte-identical, simulation-proven. **§E (2026-06-27, item 2) — SHIPPED + Boss-validated:** the live **PROPERTIES (خصائص)** count — a one-way display-only `onLiveProps(tabId,count)` observer from PropertyEditor→(NotePane→NoteEditor→)+layout, emitted in `debouncedSave()` (non-empty-key count = settled count via `parseFrontmatter if(key)`), `propCount` `$state` + baseline `$effect` + focused-tab-guarded handler (mirrors §C; never writes content; noteModel non-reactive; both PropertyEditor instances + both NoteEditor mounts); MIG-076 harness 28/28. PLUS an **all-noun Arabic singular-"one" refinement** (Boss-ruled): all 27 `ar plurals.*.one` → "{noun} واحد/واحدة" (gender-correct; e.g. خاصية واحدة, ملاحظة واحدة), which also improved the §D sentence embeds at count 1. **Remaining:** 382 scattered partial-locale gaps = documented broader remainder (item 5, triage-first). See the v3.10 preamble + `SESSION-LOG-2026-06-26.md`. |
| **MIG-092** Collections on the Search Hub (the ONE hand-picked-set mechanism) | `docs/MIG-092-Collections-Architect.md` + `…-Plan.md` | ✅ **SHIPPED + Boss-validated (v3.26, ~11 commits + close-out).** A **Collection** = a named, persistent, hand-picked basket of notes (membership only; never writes note content) — gap-defined vs Search (query, transient) · Bases (dynamic view/table) · File Explorer (hierarchy). The **Search Hub** gained a **Results \| Collections** header tab (`SearchHub.svelte`) where the user creates/switches/renames/deletes named collections, removes members, and narrows the view with **four state chips** (due/unlinked/contested/forming — `collectionChips.ts`); **"Add to collection ▸"** is a right-click submenu on both search results and file-tree notes (`+layout.svelte`). The former **Bookmarks** feature is **UNIFIED in** as a pinned, undeletable **"Starred"** collection: the sidebar "Bookmarks" section now renders Starred (flat list; each row shows a *cUniverse / library / folder* location breadcrumb on the reading-END side; per-title RTL row direction; a right-click note menu) and the ⭐ star toggles Starred membership. The "done"/sweep concept was **dropped** (Boss ruling — a note is in a collection or you ✕ it out). **RETIRED (repurposed):** the **MIG-090 Workbench** — `WorkbenchView`, the `enabledFeatures.workbench` flag + its Settings toggle, the vestigial `bookmarks` store, and `save_universe_bookmarks` were all removed; its durable-basket machinery was repurposed (workbench.json→collections.json, `workbench_hydrate`→`collections_hydrate`, the store, the shared `NoteRow`/`NoteList`, the chips). Key files: `src-tauri/src/collections.rs`, `universe.rs` (`read/save_universe_collections`; legacy `workbench.json` migrated-then-retained), `src/lib/libraries/collectionsLogic.ts` (pure reducers) + `store.ts`, `src/lib/components/CollectionsPanel.svelte`, `collectionChips.ts`, `SearchHub.svelte`, `+layout.svelte`. See the v3.26 preamble + `SESSION-LOG-2026-07-05.md`. |
| **MIG-093** the Quick Switcher — the pure title jumper | `docs/MIG-093-Architect-Plan-Quick-Switcher.md` | ✅ **SHIPPED + Boss-validated (v3.27).** Reproduce-First: Boss traces showed Ctrl+O's embedded content search at 20-27s per keystroke-pause (FTS5 native `snippet()` re-tokenizing EVERY matching row before LIMIT; stale runs piling on the db lock) and exact-title hits buried by the unranked cap-20 local filter. Boss ruling: titles only. Shipped: **§D/§D-2** two-phase rank-then-fetch in BOTH `lexical_search_in_schema` and the universal `search_contents` (native snippet() gone from every production path; `include_snippet` honored) · **§A** `read_notes` federated → `allNotes` spans the ONE universe (~9.1k titles) · **§B** the shared `$lib/searchFold.ts` fold (IndexPanel re-pointed) · **§C** `$lib/switcherRank.ts` banded ranking (exact>prefix>word-boundary>fuzzy; recency/compactness tie-breaks; alias rows; pinned test: `islam` → the exact title #1) + the `QuickSwitcher.svelte` rewrite (titles+aliases only, ZERO IPC per keystroke, empty=recents, pinned Create-note + Search-in-Search-Hub rows). **Boss-measured: 25-29s → 1.6-2.7ms over 9,161 candidates.** See the v3.27 preamble + `SESSION-LOG-2026-07-06.md`. |
...
- **Always-mounted lazy-mount** (LL-022): `*EverOpened` flags for Map / OrgChart / **Sky (`skyEverOpened`, MIG-079 §C.2d)**.
...
- **Async command to free the IPC dispatch thread** (the §9.1 lever, reused by MIG-079 §C.2d): `#[tauri::command(async)]` runs a synchronous-rusqlite command on Tauri's worker pool so a cold 234k-row scan can't monopolise the single IPC thread (the §C.2d 11.4 s boot stall). Applied to `constellation_map_universe` (§9.1) and `cache_boot_snapshot_sky` (§C.2d).
- **Defer the heavy read off boot + after-idle warm-up** (MIG-079 §C.2b edges, §C.2d sky): keep the derived view write-time-materialised in SQLite, but read it lazily on first surface-open (memoised `ensureFull*`/`ensureSky`) and pre-warm it on `requestIdleCallback` after `boot:graph-ready`. Cold `graph_ready` ~11 s → ~1.5 s. Universe-switch staleness guarded by an epoch counter; same-epoch forced-refresh races by a generation counter.
...
| Title-heading rename gap | **CLOSED (MIG-076 §D, 2026-06-13).** Re-landed on the §C single-ownership + §D quiesced-rename foundation: `NoteEditor.handleTitleChange` delegates to the host's `handleRenameComplete` (via the `onTitleRename` prop, gated on `SINGLE_OWNERSHIP`) — the SAME quiesced path the sidebar uses (freeze → flush-through-model → `rename_item` → wikilink cascade → deliberate remount). The 2026-06-11 `a086e1ee` attempt caused BUG-023 because the **pre-§C** cascade composed writes from drifting parts (props-by-id + pane-text + captured path); §C's identity-bound `compose` (refuses a cross-path write) + §D's freeze (input blocked during the cascade) remove that vector — proven by harness **Recipe H** + the Boss Stage-2 re-run (link heals in the linker, the renamed note keeps its own identity, zero write-journal anomalies). Second screen keeps the direct-renameItem fallback (display-not-domain). |
| **BUG-023** title-rename delegation corruption | **Vector REVERTED 2026-06-11** (same-hour). The `onTitleRename` delegation caused a cross-note write: `Rename Probe v2.md` received `Probe Pointer`'s frontmatter (incl. its `cid_cn`) + empty body; the linking note's own file was fine (the cascade had rewritten it correctly). Forensic file snapshots in `SESSION-LOG-2026-06-11.md` §BUG-023. Reproduction: create A linking `[[B]]`; open B; rename B via the editor TITLE; B's file is overwritten with A's identity. |
...
| **Living Link — the concept** (why; the 8 acts + canonical order; **untyped = the question**; **facts rest**) | [`docs/Living-Link-Concept-Paper-v1.0.md`](docs/Living-Link-Concept-Paper-v1.0.md) (RATIFIED) · mechanics: [`docs/Living-Links-Guide-v1.0.md`](docs/Living-Links-Guide-v1.0.md) · vocabulary registry: [`docs/USER-DEFINABLE-LINK-TYPES-ARCHITECT.md`](docs/USER-DEFINABLE-LINK-TYPES-ARCHITECT.md) |
| **CCS — Constellation Circulatory System** (the circulatory register for the Connection question; peer of CNS; the pulse of the link graph — SHIPPED, MIG-074) | [`docs/Constellation-Circulatory-System-Concept-Paper-v1.1.md`](docs/Constellation-Circulatory-System-Concept-Paper-v1.1.md) (RATIFIED) · `lab/reports/MIG-074-CCS-{ARCHITECT,PLAN}.md` (closed) · help: `docs/help.uConstellation.World/Constellation Circulatory System/` |
| The Cognitive Engine in one picture (four questions: Development · Altitude · Origin · Connection → Conviction) | [`docs/Cognitive-Engine-One-Picture-Concept-Paper-v1.0.md`](docs/Cognitive-Engine-One-Picture-Concept-Paper-v1.0.md) |
...
3. `cache_boot_snapshot_graph` (**tags + aliases only** — edges deferred §C.2b; sky deferred §C.2d) — deferred via `requestIdleCallback`. Sets `graphReady`.
4. **`cache_boot_snapshot_sky` is NO LONGER read at boot (MIG-079 §C.2d).** Sky View / CNS / Lens / Sight load it lazily on first open via the memoised `ensureSky()` (gated by `skyEverOpened`), and an after-idle background warm-up (`schedule(() => ensureSky())` after `boot:graph-ready`, on the async worker thread) pre-loads it so the first open is usually instant. `skyReady` gates the Sky View spinner. This removed the last 234k-row cold read from the boot path (cold `graph_ready` ~11 s → ~1.5 s).
...
- ~~**Why the alias-aware sky snapshot path (`cache_boot_snapshot_sky`) is bypassed at boot**~~ — **MOOT (MIG-079 §C.2d):** the historical silent-fallback root cause was the stratum column-type crash, fixed by MIG-061 §K; and as of §C.2d the sky read is **deliberately NOT on boot** at all (deferred to first Sky-surface open + an after-idle warm-up). So "bypassed at boot" is now the intended design, not a mystery.
