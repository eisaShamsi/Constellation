...
version: 2.0
date: 2026-05-29 (Dual-World reconciliation — Eisa's "both worlds, one progressive surface" + "Strong yet Simple, by default")
status: Dual-World direction locked. Governs MIG-065+ (the Unified Progressive Base). Supersedes v1.4's clean-slate-only framing.
direction_holder: Eisa
drafter: Claude (Opus 4.8)
supersedes: v1.4 (preserved at docs/Constellation-Base-Concept-Paper-v1.4.md as historical record)
predecessor_versions:
  - v1.0 — pre-decisions draft
  - v1.1 — 7 of 8 closed
  - v1.2 — all 8 + 360.3D bridge folded in
  - v1.3 — CNS bridge added
  - v1.4 — all 10 closed + Cataloger bridge (clean-slate framing; refused the familiar/generic Base)
...
# Constellation Base — Concept Paper v2.0

> **What changed in v2.0** — Eisa's Dual-World direction (2026-05-29): *"have both worlds"* — the familiar Obsidian-style Base **and** the PKF-powered Constellation Base, unified into **one progressive feature**. v2.0 reconciles the vision v1.4 left clean-slate-only:
>
> - **§5.0 added (governing principle): "Strong yet Simple, by default."** The Base opens familiar and uncluttered; cognitive strength is one column-add away, never crowding the first screen. The tie-breaker for every Base decision.
> - **§3's refusal reframed.** v1.4 refused the familiar/generic Base outright. v2.0 narrows the refusal to its true target — *empty aspirational structure* — and welcomes the familiar editable table as the **on-ramp**. The cognitive columns are why users stay.
> - **One engine, not two.** The shipped lens engine (`execute_lens`, SQL, write-time, federated) is **extended**, not replaced; the old MVP's live-scan `query_base` is **retired**; the old MVP's familiar table becomes the Simple face. The two-Bases split is closed.
> - **§12 roadmap superseded** by the MIG-065+ sequence: MIG-065 (Simple foundation — familiar editable table on the clean engine) → MIG-066 (Living Link columns) → MIG-067 (Epistemic Source / Content-type columns) → MIG-068+ (Cognitive Engine columns, which carry their own write-time-derivation groundwork) → aggregations, card/board views, the remaining Five Acts templates.
> - **Implementation map:** `docs/MIG-065-constellation-base-unified-ARCHITECT.md` + `…-PLAN.md`.
> - v1.4 preserved as historical record (SO #6). Everything below this block is v1.4's content, still valid except where §5.0, the §3 reframe, and the §12 roadmap supersede it.
>
> ---
...
**Reconciled in v2.0 (Dual-World):** this refusal targets *empty aspirational structure* — schemas invented to feel organized — **NOT** *familiar surfaces*. A plain, editable table of fields the user has actually populated is not productivity theater; it is the **on-ramp**. v2.0 welcomes the familiar Obsidian-style table as the Simple default and makes the cognitive columns (§6) the reason users stay. "Strong yet Simple, by default" (§5.0) is how the two reconcile: the default reveals existing structure simply; depth is opt-in, never aspirational filler. The line we still hold: a Base column must answer a real question, not decorate.
...
The governing principle (§5.0) sits atop five founding principles (§5.1–§5.5), in priority order. When they conflict, higher-numbered yield to lower-numbered; §5.0 governs all.

### 5.0 Strong yet Simple, by Default *(governing — Eisa, 2026-05-29)*
The Base opens uncluttered and instantly familiar — a table of the note's own fields, nothing more. Strength is always present and one gesture away (the cognitive columns of §6), but it **never crowds the first screen**. When any design decision is in tension, the *default* leans Simple; the *ceiling* stays Strong. This is the reconciliation of "both worlds" — traditional-yet-new, simple-yet-unique. It governs every choice about defaults, the add-column picker, and onboarding, and it is why v2.0 retired v1.4's outright refusal of the familiar surface (§3). Power that requires learning a query language (the Obsidian Dataview cliff) violates this principle; power that is "add a column" honors it.
...
## 12. Roadmap (Updated v2.0 — the Dual-World MIG sequence supersedes v1.4's phase numbering)

Sequencing, not commitments. Each step is its own `/migration` MIG.

**The authoritative sequence (v2.0):**

- **MIG-055 — the lens engine** (SQL, write-time, dimension registry, Five Acts "Recent Captures"). ✓ Shipped.
- **MIG-056 — cross-universe federation** of `execute_lens`. ✓ Shipped.
- **MIG-060 — host-note threading gestures** (Open-in-360.3D / CNS / Cataloger per lens row). ✓ Shipped.
- **MIG-065 — the Simple foundation (this MIG line).** Unify the two Bases: extend the lens engine with a familiar editable **table** (arbitrary frontmatter columns via `note_meta.properties_json`, filter / sort / edit-in-place, standalone `.base` files + inline blocks, the tiered add-column picker) and **retire** the old live-scan `query_base`. ← the "Strong yet Simple" default surface.
- **MIG-066 — Living Link columns** (§6.1). `link.confidence` / `link.weight` / `link.type` / `link.traversal_count` — cheap (already `note_links` columns).
- **MIG-067 — Epistemic columns** (§6.12). `note.source` / `note.content_type` — cheap (already `note_meta` columns, MIG-021).
- **MIG-068+ — Cognitive Engine columns** (§6.10). Stratum / Maturity / Provenance — **carries its own write-time-derivation step** (not persisted today; computed on-demand from scanners).
- **Later** — CNS network columns (§6.11, needs the freshness strategy); NSC headline default column (§6.2); aggregations (count / sum / avg / min / max over linked notes); card / board / calendar views; the remaining four Five Acts templates (§8); cell-edit on typed links (§6.9).

**The v1.4 conceptual phase map** (retained for reference; realized via the MIGs above):

- **Phase 0 — Concept** (this paper) ✓
- **Phase 1 — Rule 8 Migration (MIG-054).** Cheap-lookup `query_base` via SQL against `note_meta.properties_json`. *(MIG-054 was reverted; its goal — SQL not live-scan — is delivered by the MIG-055 lens engine + MIG-065's `query_base` retirement.)*
