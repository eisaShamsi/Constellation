...
**Version 2.15 | 2026-09-14 — PJ-466 PANEL-RULED, and the ledger's own entry corrected. A read-only design panel (13 agents) found this entry wrong in two ways: it is FOUR exits, not three — the omitted one is where the Boss has ALREADY seen a write failure worded "skipped" — plus a fifth honesty exit nobody had scoped; and the question it carried for him was not open, because the app answers it (`phantom_prune.rs:590-592`: a second-look disagreement is "Never an error"). Withdrawn under the law written today. The panel also found the house ALREADY SHIPS this receipt — `PruneReceipt`, rendered in the same Settings page as this door's own entry point — after three of four lenses proposed inventing one. Root cause: `MoldRepairOutcome` carries only `ok: bool`, so nothing can separate "left alone" from "still broken". One field closes PJ-466 + PJ-469 + PJ-470 + PJ-471. Filed PJ-489…PJ-492. NEXT: the Boss's copy + priority ruling, then the build.**

> **What changed in v2.15** — PJ-466 rewritten from the panel verdict (scope corrected, the Boss
> question withdrawn with evidence, the shipped `PruneReceipt` named as the shape to reuse);
> PJ-469 / PJ-470 / PJ-471 re-ranked from LOW/Group-3 to FOLDED INTO PJ-466; PJ-489…PJ-492 filed
> under WA#6 from the same panel. **No code changed. Nothing closed.**
>
> **► NEXT ACTION: PJ-466** — awaiting two rulings from the Boss (the copy; and the priority — now,
> ahead of PJ-456's 107-file wave, or after). Panel recommends now. Then PJ-475 (switch a Saved
> Style from the command palette — *his* request, timed by him as "after PJ-461 closes"). Then
> PJ-456 · PJ-457 · PJ-458 · PJ-459 · PJ-462 · PJ-463 · the multi-folder templates setting · the
> PJ-264/PJ-378 unpack · PJ-434 · PJ-438.
>
> - **PJ-466 is a small `/migration`**, by CLAUDE.md's own boundary test — it changes the IPC report
>   shape (Rust ↔ Svelte). ~24 new keys + one new plural noun × 15 locales, plus SO#2 docs.
> - **Eleven candidate questions were answered by the app** and recorded in the verdict's *Already
>   decided* section rather than spent on his time. Two reached him.
...
### 🔶 PJ-466 *(MED → a small `/migration`; Group 2 — honesty; PANEL-RULED 2026-09-14, awaiting the Boss's priority call)* — the repair door's receipt is untrue on every exit
**CORRECTED 2026-09-14 by a read-only design panel (`wf_6d74a307-646`, 13 agents) — this entry as
written in v2.13/v2.14 was wrong in two ways, and a build scoped to it would have shipped a
confirmed-wrong screen.** Full verdict: `lab/reports/PJ-466-PANEL-VERDICT-2026-09-14.md`;
account: `SESSION-LOG-2026-09-14.md` §21.

**The complaint stands:** on a halted run the summary says **"Templates fixed"**
(`moldRepair.summaryTitle`) over 0 fixed; **"0 fixed, 1 skipped."** (`summaryLine`) for a WRITE
FAILURE; **"A backup of every file was saved first"** (`backupAt`) when batch 2 never ran; and
nothing says the ordinary files were not attempted. The Boss saw that screen in the rehearsal
(disclosed beforehand — `SESSION-LOG-2026-09-12.md:442`).

**Correction 1 — it is FOUR exits, not three, plus a fifth honesty exit nobody had scoped.**
`MoldRepairDialog.svelte:173` (delicate halt) · `:182` (between-batches blocker) · **`:194` (normal
completion)** · `:198` (the catch). The omitted normal-completion exit is where the Boss has
**already** seen a genuine write failure worded "skipped" — Recovery 1 of the same rehearsal,
`SESSION-LOG-2026-09-12.md:444`: *"3 fixed, 1 skipped."* over a `✕ … ReplaceFileW error 5` row. The
fifth: a thrown `scan_stamped_molds` (`:99-106`) dismisses the door in silence while
`+layout.svelte:624`'s `catch { moldRepairCount = 0; }` retracts the banner — a scan that could not
run reported as a universe with zero molds.

**Correction 2 — the question this entry carried for the Boss is WITHDRAWN; asking it would have
violated the 2026-09-14 law.** *"Should a stale-file refusal halt the cascade at all?"* — the app
already answers **no**: `phantom_prune.rs:590-592` defines that exact class, *"the file reappeared,
or the row no longer classified as a phantom. **Never an error.**"*, and the door's own header
(`MoldRepairDialog.svelte:25-26`) says the halt exists *"so a systematic problem never reaches the
ordinary files"* — which an already-fixed file is not. A coding defect, not a design choice. The
sibling question is answered too: `mig108.rs:1044-1046` — a consented, journaled, batched file
operation halts on the first genuine failure, so a **write** failure must keep halting.

**The root cause, and why this closes four entries at once.** `MoldRepairOutcome` carries only
`ok: bool` and free text (`mold_repair.rs:56-62`), so no sentence in any language can separate
*"left alone, nothing owed"* from *"still broken"*. `failed` is computed by subtraction
(`:463-467`) and the halt reads `if (r1.failed > 0)`. That one missing field is why a benign
refusal halts the run (**PJ-466**), why a previewed `will_apply:false` candidate is still written
and then restored by an unchecked `let _ = std::fs::write` (**PJ-469** — therefore NOT cosmetic),
why `relinked_sources` over-counts on this very card (**PJ-470**), and why the receipt speaks
English in fifteen languages (**PJ-471**). One field, one job.

**Do NOT invent a taxonomy — the house already ships one.** `phantom_prune.rs:585-604`
`PruneReceipt { removed, skipped, failed, unknown, stopped_early, refused }`, rendered at
`SettingsModal.svelte:2753-2790` in the **same Settings page as this door's own entry point**:
neutral title, one sentence per non-zero class, a refusal replacing the lot, `$tn('plurals.entries')`
on every count. `repairReport` (`:2791+`) is a second shipped instance. Three of the four panel
lenses proposed inventing a thirteenth vocabulary while telling the panel no precedent existed.

**Also established:** there are **ten** non-success sites in `repair_one`, not nine (`:507`
"Could not read it" was in nobody's taxonomy); `:537` leaves the file **changed** on disk while
reporting `ok:false`; `:540`'s rollback is unchecked while `:541` asserts *"so it was undone"*;
`:517` is provably unreachable; `mold_repair.rs:443-447` is dead code. A session claim that
`report` can be `null` at the between-batches blocker was **refuted by reading** — the only `await`
between the two blocker checks is inside `if (delicate.length > 0)`.

**Shape:** a small `/migration` by CLAUDE.md's own boundary test (the IPC report shape crosses
Rust ↔ Svelte). ~24 new keys + one new plural noun (`plurals.templates`) × 15 locales; SO#2 docs.
**Closes PJ-466 + PJ-469 + PJ-470 + PJ-471.** **Out of scope, ruled:** PJ-468 (the shared write
gate), PJ-467 (performance), a "try again" button (the banner already returns — Constraint as
Design), and rerouting Exit 2 to the `blocked` screen (it renders no receipt and its retry
double-counts `report`).

**Awaiting the Boss:** the copy (his vocabulary by standing ruling) and the priority — now, ahead
of PJ-456's 107-file wave, or after. Panel recommends now.


---

## 🧾 Filed 2026-09-14 (later) — from the PJ-466 panel, under WA#6 (discovered, therefore filed — not quietly parked)

These three share PJ-466's disease — *a run that stopped or failed reports as if it completed* — but
belong to **different engines**, so the panel ruled them OUT of PJ-466's build and INTO the ledger.
Folding them in would double the surface under one Boss test.
...
### 🔗 PJ-469 *(FOLDED INTO PJ-466 by the 2026-09-14 panel — build together, close together)* — the repair batches are keyed on `needs_care` and ignore `will_apply`
...
### 🔗 PJ-470 *(FOLDED INTO PJ-466 by the 2026-09-14 panel — build together, close together)* — `relinked_sources` can over-count
...
### 🔗 PJ-471 *(FOLDED INTO PJ-466 by the 2026-09-14 panel — build together, close together)* — `moldRepair.*` counts against the MIG-087 plural rule; `repair_one` details are English literals
