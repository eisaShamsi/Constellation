...
**Version 1.12 | 2026-04-30**

> **What changed in v1.12**: a sector-layout course-correction (commit §109). Boss reported §107's single-ring layout for note "1902" still showed visual overlap (typed clusters and untyped scatter colliding at the same radius) and that the result didn't match the widget. Re-reading the prior directive: the "all nodes in one circle" rule from §107 applied to **avoiding ring-per-group's full concentric rings**, not to flattening the depth-based ring system that the compact widget itself uses.
>
> **Single change in §109**:
>
> - **Restored depth-based sector rings** `[160, 270, 380]` (matching the compact widget's `[56, 89.6, 123.2]` scaled up to the full-window viewBox 1200×800). Each typed group's nodes still cluster at their SECTOR_MAP compass angle with the widget's 8°-per-node spread, but the ring radius is now determined by note depth: depth 1 → inner ring 160, depth 2 → middle 270, depth 3 → outer 380. Untyped nodes scatter around the full 360° on the same depth-based rings.
>
> The §107 uniqueId hover fix is preserved — each rendered node still carries a `uniqueId: 'n${idx++}'`, hover state remains `hoveredId` keyed to that. Path-based hover collisions stay fixed.
>
> **Result for note "1902"** (15 supports + 1 derives-from + 30 untyped, all expected to be mostly depth 1): supports cluster at top inner ring 160 (~120° span), derives-from single node at left inner ring 160, untyped distributed across all three rings depending on the IPC's reported depth values.
>
> **Known residual overlap risk**: an untyped depth-1 node at angle 0° lands on inner ring 160 at the same x,y as a supports depth-1 node at angle 0°. This is a real collision for "1902"-class data because most untyped from second-order connections are depth 2 (less common to land at the same ring as typed depth 1), but some can be depth 1 (direct untyped wikilinks). If Boss reports visible overlap on retest, next iteration adds **gap-filling**: untyped only at angles outside the typed sector ranges of the same depth ring.
...
| `inspector360.rs` | 445 | Aggregates 9 phase data per note (read-only) |
...
| 12 | 360° Inspector ✅ enabled v1.8 §93, hardened v1.9 §96–§104 | `inspector360.rs` (445 lines) | ⚠️ **Read-time aggregation, but actual perf is fine** — the per-fetch cost was theorised as 1–3 s but Boss's lived experience is "almost instantly". MIG-010 (cache `note_360_view` write-time) priority dropped to LOW. Frontend mitigations still in place: debounce 200 ms, sequence guard, last-fetched-key dedup, lazy mount, dedupe-by-path in the layout. |

**Inspector 360° aggregator** ([`inspector360.rs:1`](src-tauri/src/inspector360.rs:1)): "360° spherical knowledge inspector: the note is the CORE of a sphere. The ATMOSPHERE surrounding it is every dimension of understanding." Aggregates `Note360View` from typed/untyped links (7 types) + stratum + maturity + contradictions + orphan/SPOF flags + provenance + stage + review + trail membership + lens groups + missing-link-types gap analysis.

**Frontend Inspector 360 surface** (post-v1.9 §96–§104):

- Two display modes via the `compact` prop. Compact = right-sidebar tab (always-on glance widget). Full-window = ribbon-dock button (deliberate-study surface, replaces editor area).
- **Hybrid layout**, automatic. `Inspector360.svelte::layoutMode = $derived.by(() => maxTypedCount <= 8 ? 'sector' : 'rings')`. Sector mode renders typed-link groups at their `SECTOR_MAP` compass angle on three depth-based ring radii (160/270/380), 50° wedge per sector — original §93 design with §103 minimal nodes. Rings mode renders one concentric ring per typed-link group, sorted by count ascending (smaller inner, larger outer).
- **Multi-hop back stack** shared between compact and full-window. State: `inspector360BackStack: $state<Array<{path, name}>>`. Forward node-click pushes current; back click pops one entry; bar shows `← {previous}` until empty. Universe switch resets to `[]`.
- **Group labels** switch with the layout: ring-top labels in rings mode, sector-rim labels (radius 415) in sector mode. Untyped is rendered as `Untyped` (literal — i18n key was missing across all 15 locales; the OR fallback chain was broken).
- **Node sizes** depth 1 / 2 / 3 = 6 / 4 / 3 px radius. Always-on labels removed; hover-only with 13 px font + 3 px black SVG stroke for legibility. 6 px invisible hit-area expansion around each node.
- **Frontend dedup-by-path** in `ringsLayout` so the same note returned from outbound + inbound + second-order sources renders once. Per-group dedup; cross-group dedup deferred (a note can appear on both supports and untyped rings, which is semantically valid).
- **Mode-specific decorations**: Atmospheric Rings has rotating tilted ellipses as ambient background (always). Neural Web has bright glowing nodes + organic synaptic lines + faint dashed ring outlines (rings mode only). Cosmic Sphere has stars background + per-group solid orbital rings (rings mode) or three fixed concentric rings (sector mode) + sector lines and rim labels at SECTOR_MAP angles.
...
- **inspector360.rs** (445) — see §4.2 row 12.
...
*End of v1.12. Maintained per Standing Order #6.*
