...
**Version 2.14 | 2026-09-14 — PJ-461 CLOSED, Boss-passed on every stage: 17 CSS names that matched nothing the theme defines, 16 corrected and 1 kept by design, across 21 files. It grew well past a rename: a CSS-specificity defect that made the Template Studio fix silently not work, a preview clipping fault on short windows that PRE-DATED the job, an Arabic right-to-left fault the sibling lens had fixed years earlier, and a redesign of the Note-graph preview he approved (wedges 1.64x longer, names moved to callout rows). Four inspection rounds, a design panel, a wrap panel and two findings-verifiers between them refuted a dozen of my own claims — including two FALSE comments I had already shipped. A new LAW earned: Check What Is Already Built BEFORE You Ask. Filed PJ-475…PJ-488. NEXT: PJ-466 (halt-screen wording) before PJ-456's 107-file wave; PJ-475 is his named follow-up.**

> **What changed in v2.14** — PJ-461 closed; PJ-475…PJ-488 filed; a new top-of-all-rules law added to
> `CLAUDE.md`; and two of his rulings recorded that had lived only in chat.
>
> **► NEXT ACTION: PJ-466** — the halt screen's wording, one concern across three exits (panel-ruled:
> before PJ-456's wave). Then **PJ-475** (switch a Saved Style from the command palette — *his*
> request, timed by him as "after PJ-461 closes"). Then PJ-456 · PJ-457 · PJ-458 · PJ-459 · PJ-462 ·
> PJ-463 · the multi-folder templates setting · the PJ-264/PJ-378 unpack · PJ-434 · PJ-438.
>
> - **PJ-461 — CLOSED.** Boss-passed Stage 1 (5 of 5), Stage 2 Step 1, Stage 3, and he ruled Stage 4.
>   **Final accounting of the 17 names:** 16 corrected, 1 (`--link-tip-font-size`) deliberately kept
>   as the undefined-until-set member of the Style Setter's `--link-tip-*` family, plus
>   `--bg-modifier-border` struck (zero live uses). Zero `var()` consumers of any retired name remain
>   anywhere in `src/`.
> - **Two rulings of his recorded that would otherwise have been lost** (the failure mode SO#10 exists
>   for): **the Style Setter IS Constellation's theme control and Saved Styles ARE the themes**; and
>   **the built-in dark theme is one he never uses and considers unsuitable for the app**.
> - **A new LAW in `CLAUDE.md`** — *Check What Is Already Built BEFORE You Ask* — earned by three
>   violations in one sitting, the worst being a three-option design question about the Calendar's
>   typeface put to him without opening the Style Setter's Calendar category, which he built and which
>   carries 33 controls including "Calendar font".
...
### 🆕 PJ-466 *(MED — Group 2 — honesty; panel-ruled "own job, before PJ-456's wave")* — the halt screen's wording is wrong in three places, across three exits
On a halted run the summary says **"Templates fixed"** (`moldRepair.summaryTitle`) over 0 fixed;
**"0 fixed, 1 skipped."** (`summaryLine`) for a WRITE FAILURE; **"A backup of every file was saved
first"** (`backupAt`) when batch 2 never ran; and nothing says the ordinary files were not
attempted. The Boss saw this raw screen in the rehearsal (disclosed beforehand). The panel's
position: ONE concern across the three exits in `MoldRepairDialog.svelte` (delicate-batch failure
:171-175; between-batches blocker :179-184; the invoke-error catch :195-199) — fix as one job, and
his question with it: **should a stale-file refusal ("no longer a stamped template") halt the
cascade at all?** i18n ×15.
...
### 🆕 PJ-469 *(LOW — Group 3)* — the repair batches are keyed on `needs_care` and ignore `will_apply`
...
### 🆕 PJ-470 *(LOW — Group 3)* — `relinked_sources` can over-count
...
### 🆕 PJ-471 *(LOW — Group 3 — i18n)* — `moldRepair.*` counts against the MIG-087 plural rule; `repair_one` details are English literals
