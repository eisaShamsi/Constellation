...
Um **Resumo da Nota** é um breve resumo de uma nota — algumas frases que lhe dizem do que a nota trata num relance. Os resumos são produzidos pelo **Note Summary Creator (NSC)**. Você os verá **por todo o aplicativo, onde quer que uma nota apareça**: sob o título de cada cartão na fila do **Classificador** / **Revisão de Origem** (onde você decide como classificar um cartão sem abrir a nota por trás dele), como uma linha tênue em itálico sob cada resultado nos **Resultados de busca** (que lhe diz *sobre o que* uma nota é, ao lado do trecho que mostra por que ela correspondeu), como uma faixa fina acima da nota quando você a abre no **Editor** (para que a essência da nota esteja em contexto enquanto você lê ou escreve), sob cada origem vinculada no painel de **Backlinks** e sob cada destino vinculado no painel de **Outgoing Links** (para que uma longa lista de notas relacionadas seja percorrida como ideias em vez de meros títulos), sob cada menção de nota quando você expande um termo no **Índice** (para que as notas de um termo sejam reconhecíveis num relance), dentro do tooltip ao passar o cursor na **Vista Estelar** ao apontar para uma bolha (para que um grafo carregado continue legível sem precisar clicar), e como *conteúdo primário* de cada linha no novo painel do **Resumo do Universo** (onde toda a base de conhecimento é uma lista rolável de resumos — veja *O Resumo*).
...
- **Painel de Backlinks** — sob cada linha de origem que se vincula à nota que você está lendo. Uma longa lista de menções entrantes se torna percorrível: você lê a essência da nota que faz o link como uma única linha em itálico sob seu título, sem precisar abrir cada uma para se lembrar do que se trata.
- **Painel de Outgoing Links** — sob cada linha de destino para a qual a nota que você está lendo aponta. Mesma forma dos Backlinks; você vê num relance do que trata cada conexão de saída.
- **Índice** — quando você expande um termo, cada nota que usa o termo mostra o resumo como uma linha tênue sob seu título (e sob o trecho de contexto correspondente, quando houver). Um termo que aparece em dezenas de notas se torna uma lista de ideias em vez de apenas nomes de arquivo.
- **Hover na Vista Estelar** — quando você passa o cursor sobre uma bolha no grafo da **Vista Estelar**, o tooltip flutuante mostra o nome da nota na primeira linha e a manchete do seu resumo numa segunda linha em itálico, para que você possa ler o que uma bolha *significa* sem deixar o grafo.
- **Resumo do Universo** — o lugar *primário* onde os resumos vivem: um painel dedicado no dock esquerdo que lista cada nota do seu Universo (escalonada Library → Pasta → Nota), com cada linha mostrando a manchete sob o nome. Clique numa linha para expandi-la e ler o resumo completo de várias frases em linha. Buscar/filtrar restringe toda a lista. Veja *O Resumo* para o tópico completo.

Por padrão os resumos se preenchem **preguiçosamente e gentilmente**: à medida que os cartões rolam para dentro da visão, à medida que os resultados de busca aparecem, quando você abre uma nota, quando você expande um termo, passa o cursor sobre uma bolha, ou rola o Resumo, o Constellation computa os resumos ausentes alguns de cada vez, pausando sempre que uma varredura de classificação da Biblioteca está em execução para que os dois nunca competam por recursos. Isto mantém o aplicativo responsivo — você pode ver brevemente um cartão / resultado / nota aberta / linha / tooltip / linha do Resumo antes que seu resumo apareça, e então o resumo surge um momento depois.
