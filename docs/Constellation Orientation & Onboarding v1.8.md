...
**Version 1.8 | 2026-04-29**

> **What changed in v1.8**: Three things landed today, in order. (1) **MIG-003 integrated to main.** It closed yesterday on the side branch `claude/frosty-stonebraker-75c9bf` (tag `milestone/mig-003-closed` at `25c65ef`) but was never merged. Today's `git merge --ff-only` brought the seven MIG-003 commits onto `main` — `origin/main` is now at `8cb80ac`. The previously stranded closure docs (CANONICAL-FILENAME-ARCHITECTURE.md changes, SESSION-LOG-2026-04-28.md §85–§89 entries, this orientation v1.7 itself) became tracked content. Source ↔ binary parity restored at the main checkout by copying the post-MIG-003 release artifacts (`constellation.exe` Apr 28 18:19, msi/nsis bundles Apr 28 13:07/13:10) from the frosty worktree. **(2) CE Phase 12 360° Inspector re-enabled** (compact right-sidebar tab + full-window overlay via dock button). Backend `get_360_view` IPC was already shipped; only the import + UI wiring was gated at `+layout.svelte:84`. The IPC walks the full library on every call (acknowledged Rule-8 violation; expect 1–3 s per fetch on the 7,600-note Universe); frontend mitigations are debounce 200 ms + sequence guard + last-fetched-key dedup + lazy mount. Bringing this surface to Rule-8 compliance via a cached `note_360_view` table is queued as MIG-010-scale work, contingent on Boss's tutorial-test perf verdict. **(3) CE Phase 9 Multi-Lens approved for re-wire on Path B** (Rule-8 compliant) — queued after MIG-006 §3 redo. The dead-code `lenses.rs::apply_lens` will be made load-bearing only as part of that future MIG-010-scale migration, not before.
...
| 12 | 360° Inspector ✅ enabled v1.8 §93 | `inspector360.rs` (445 lines) | ❌ **Hybrid violation acknowledged** — read-time aggregation across 9 phases; on the 7,600-note Universe each `get_360_view` call walks the file system + runs O(n²) inbound iteration → expect 1–3 s per fetch. Frontend mitigations (debounce 200 ms, sequence guard, last-fetched-key dedup, lazy mount) absorb thrashing but don't reduce per-fetch cost. Rule-8 compliance via cached `note_360_view` table is queued as MIG-010-scale work. |
...
- **Actual `get_360_view` latency on the 7,600-note Universe** (v1.8 §93). The 1–3 s estimate is from reading `inspector360.rs` + extrapolating; not measured. Boss tutorial test will produce the first real number. If it's >3 s or jarring during tab navigation, the MIG-010 (cached `note_360_view` table) prioritization climbs above MIG-006 §3 redo.
- **Whether the Inspector 360 first-fetch shows a confusing empty state during the 1–3 s wait** (v1.8 §93). The component falls back to "Open a note to see its 360° view" when `data === null`; subsequent fetches keep stale data while the next one resolves. If first-load UX is jarring, a `loading` prop on `Inspector360.svelte` is the lightest follow-up.
...
*End of v1.8. Maintained per Standing Order #6.*
