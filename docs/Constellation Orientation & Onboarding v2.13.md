...
| **MIG-020 → MIG-025** | (gap — backfill pending; see v2.12 preamble doc-drift note) | ⚠️ Doc-drift — these MIG rows missing from this table; commit history is intact in `git log`. Backfill scheduled at MIG-026 ship-gate. |
| **MIG-026** Sight tradition expansion (19 new + user-definable) | `lab/reports/MIG-026-SIGHT-REGISTER-EXPANSION-{ARCHITECT,PLAN}.md` | ⏳ **In flight.** Phase 0 (K1 rename `register` → `tradition`) + Phase α (multi-shape `TraditionModule` foundation + 7 stub renderers) + Phase β (A3+A6 chip UI: family categorization + 4 favorites + dropdown) shipped (commits `fba5031` / `13577b9` / `0218c38`). Paused at Phase β for MIG-027 pivot; **resumes at Phase γ** (Polanyi + Mohist modules). Phases γ → μ remain per Plan. |
| **MIG-027** Sight theme inheritance | (focused single-subsystem MIG; no Architect/Plan doc — opened mid-MIG-026 pivot 2026-05-17) | ✅ **Closed** (2026-05-17). Initial + §-fix-1 + §-fix-2 cascade (commits `686ee58` / `2f190dc` / `593af51`) shipped Sight following the active interface theme. Chrome split into theme-aware `ChromePalette` (8 fields incl. `highlightedRing`) + theme-agnostic `SEMANTIC_COLORS`. Theme-conditional `--sight-highlight` CSS var family (default bright amber `#fbbf24` for dark; `:global(body.theme-light) .sight-v6-root` override deep amber `#b45309` for light). Boss-tested Stage 1 / 1.1 / 1.2 / 2 / 2.5 — all PASS across Constellation/Nord/Solarized × Light/Dark. CNS theme-aware deferred (likely MIG-028 if requested). |
