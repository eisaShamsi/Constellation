...
> **What changed in v1.7**: MIG-003 (Human-name Filenames) closed. § 6 rewritten to reflect the inverted architecture: `cid_cn` is now the immutable internal id (lives in frontmatter), filenames are human-readable. The Canonical Filename Architecture design doc is marked historical with a Post-MIG-003 banner. § 8 migration table updated. Visible behavior change: every `.md` file on disk now has a human title as its filename; renames cascade through every dependent table.
