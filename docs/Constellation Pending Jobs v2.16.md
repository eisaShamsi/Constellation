...
### 📌 METHOD NOTE (not a PJ, but it must not be lost) — `%APPDATA%` reads are NOT trustworthy from this session
The `ui-inspector` proved it while gating the PJ-466 test: reading
`%APPDATA%\world.uconstellation.app\universes.json` returns a **frozen copy dated 7 August** naming
a single universe (`كون عيسى`) that is not the current registered set — a byte-identical shadow also
sits under `…\Packages\Claude_…\LocalCache\Roaming\world.uconstellation.app\`, the signature of a
virtualized read. **A wrap-panel lens moved to BLOCK the whole test on the strength of that file.**
Its adversary refuted it using a non-AppData witness (each universe's own `.constellation/owner.info.json`
and `boot-perf.latest.json`, on `E:`). **Rule: no claim about anything under `%APPDATA%\world.uconstellation.app`
may be made from this session without a witness that lives on `E:`.**
