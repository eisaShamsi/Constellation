...
**Version 2.08 | 2026-08-31 — session close: the Linked Universe naming ruling applied to EVERY living user doc (26 files, per-file verified) · PJ-331 re-scoped: doc half DONE, UI half is the job · handover written. ► Next: PJ-433.**
...
### 🚨 PJ-433 *(HIGH — Group 1 — silent failure + a persisted wrong choice)* — an unreachable universe fails silently at boot, and the fallback is then saved as the user's choice

`+layout.svelte:3644-3647` is a bare `catch { continue; }` — the error is never bound. With other
universes registered, the loop opens one of them and the user gets a **normal-looking window on the
wrong universe**, no notice. `set_active_universe` (`universe.rs:1245`) then writes that fallback to
`active_id`, so it keeps opening the fallback after the drive returns.

Location-agnostic in mechanism, but **the population it hits is precisely the one the ruling invites**:
a universe on a USB stick, an external drive or a share. The remedy pattern already exists one status
bar slot away — `federation.warningBadge` with a path/reason popup (`+layout.svelte:10707-10740`).
Do not persist a fallback as a deliberate choice.
