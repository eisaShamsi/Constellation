...
**Version 2.92 | 2026-06-20**

> **What changed in v2.92** (**MIG-082 OPENED — the clickable Calendar; §A + §B COMPLETE, Boss-validated: cells are interactive (dots open items, empty space → daily note, task dots open the note AT the task's line, toggle-complete with a single-ownership reconcile); Indian/Buddhist + Chinese/Korean lunisolar calendars; per-calendar year-display + native/phonetic month-name settings (Arabic phonetic Boss-verified); month numbers everywhere; ×15 i18n**):
>
...
> - **REMAINING:** §C only (close-out: the non-destructive cultural-date frontmatter field, the remaining §A-era i18n strings like CalendarPanel `dailyNote`/`completeTask`, the migration audit, + the deferred inspector360 bug). Per-feature behaviour in the body §4.x; the Plan drives the steps.
>
> ---
>
