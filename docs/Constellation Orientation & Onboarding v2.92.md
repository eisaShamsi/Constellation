...
> **What changed in v2.92** (**MIG-082 OPENED — the clickable Calendar; §A.1/§A.2/§A.3 SHIPPED + Boss-validated + pushed: cells are interactive (dots open items, empty space → daily note), task dots open the note AT the task's line, and tasks toggle complete from the calendar with a single-ownership reconcile**):
>
...
> - **REMAINING:** §A.4 (Indian + Buddhist calendars + the Persian double-era fix), §B (Chinese + Korean lunisolar, Intl-only branch), §C (close-out: cultural-date frontmatter + ×15 i18n + audit + the deferred inspector360 bug). Per-feature behaviour in the body §4.x; the Plan drives the steps.
>
> ---
>
