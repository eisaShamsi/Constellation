...
**Version 1.13 | 2026-04-30**

> **What changed in v1.13**: a sector-layout fix (commit §110). The §109 depth-based rings didn't help "1902" because `inspector360.rs::get_360_view` always stamps outbound and inbound links with `depth = 1`. Result: every typed connection lands on the inner ring 160, and any untyped depth-1 nodes also land on 160 → collision at the same compass angle as typed clusters. Boss-confirmed in retest: "Nothing changed!"
>
> **Single change in §110**:
>
> - **Count-based ring assignment** for sector mode. Typed groups sorted by count, distributed across the inner two rings only (smallest typed → inner 160, largest typed → middle 270). **Untyped always on the outer ring** 380, regardless of count or depth. Each typed group still clusters at its SECTOR_MAP compass angle with the widget's 8°-per-node spread. Depth is now used **only for node size** (radius), not ring placement.
>
> **Result for note "1902"** (3 groups: 1 derives-from, 15 supports, 30 untyped):
> - derives-from (rank 0, smallest) → inner ring 160 at the LEFT compass angle (single yellow node).
> - supports (rank 1, largest typed) → middle ring 270 at the TOP compass angle (~120° spread for 15 nodes).
> - untyped (always outer) → outer ring 380, scattered around the full 360°.
>
> Three reliably distinct rings, each with at most one type group on it. No typed/untyped angular collision because untyped is on a separate ring radius.
>
> **For notes with more typed groups**: with 4+ typed groups, the inner-two-ring split assigns roughly half to inner and half to middle (using `Math.floor(g * 2 / numTyped)`). Multiple typed groups can share a ring at different compass angles — no collision because their angles are different (e.g. supports at top + contradicts at bottom on the same inner ring don't visually conflict).
...
*End of v1.13. Maintained per Standing Order #6.*
