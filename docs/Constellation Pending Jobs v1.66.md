...
> **What changed in v1.66** (**Boss ruled "All 31" on the triage register. The four APP-KILLERS are closed and Boss-validated — and an adversarial audit found three of my four fixes wrong before they shipped. Ultracode**):
>
> **► NEXT ACTION — the rest of the 31.** Group 1 (the four app-killers) is **DONE**. The register's §3 "fix soon" batch is next, with **PJ-207 (the unreachable repair pass) moved to its front** — several other fixes are only safe *because* they assume a self-heal that is not wired. Then §4 (12 medium). Behind them: MIG-105 Phase-2 Plan (awaiting approval), MIG-104 Slices 9–15, MIG-109 and MIG-110 (both reserved, not scheduled).
>
> ### ✅ CLOSED — the four ranked APP-KILLERS
> - **PJ-200 · "couldn't read it" ≠ "you have none"** — the shared `read_persisted_json` (absent = `Ok(None)`; unreadable/corrupt/truncated = `Err`), applied to the universe registry (6 write-back sites), review history (3), custom link types and saved styles. `libraries.rs` was found **already correct** and left alone. The lenient twin is kept and documented as read-only everywhere.
> - **PJ-201 · the fallback editor that never saved** — `EditorView.updateListener` appeared exactly ONCE in `NotePane.svelte`, in the primary state; the fallback editor built on a different list had none. A note opened that way looked normal and nothing typed into it ever reached disk. Both states now share `changeListener()`.
> - **PJ-203 · "Overwrite" destroying the note you KEPT** — `moveToTrash` removed the file and left the tab live, owning a path the survivor was about to occupy. Cured by the shared `releaseTabsForVacatedPath`. **Boss-validated live**: the survivor's body was intact across a full relaunch.
> - **PJ-202 · REFUTED, not fixed.** The claimed restart-discards-the-rescue-copy path cannot occur: a tab enters `pendingCidEnsure` only when its content has NO `cid_cn`, while net-recovered content is `identityProven` — and therefore cid-bearing — by construction. A prior pass had already checked and refuted this; the register re-reported it. Kept one genuine hardening (`drainCidEnsure` adopts through `externalChange`, inheriting three guards) and corrected the comment that had asserted the impossible mechanism.
>
> ### ☠️ THE AUDIT ON MY OWN WORK — three of four fixes were wrong
> 37 agents, every finding refuted twice before acceptance.
> **(1) The link-types fix landed on a DEAD COMMAND** — `read_universe_link_types` is registered with zero callers; the editor reads via `list_link_types` → the lenient `read_deltas`. It protected nothing. **(2) The style-presets fix was cancelled at the UI layer** — `catch { return [] }` turned a backend refusal back into "you have none"; added the read-succeeded latch. **(3) My registry fix LOCKED THE USER OUT** — with a corrupt `universes.json`, all four routes into the app returned an error naming a hidden file. Refusing to overwrite had become refusing to continue; `PersistedError` now separates **Unreadable** (transient → refuse) from **Corrupt** (set aside, preserving it, and proceed from a true empty). **(4) My tab teardown DESTROYED UNSAVED WORK** — it erased the write-ahead net and disposed the model without saving, so a note whose last save had failed lost its only copy while the "your edit is safe" banner stayed on screen with dead buttons. `preserveWorkBeforeVacating` now flushes first and KEEPS the net when the flush cannot be proven.
>
> ### 🆕 FILED — PJ-208…211
> - **PJ-208** — **MIG-110 "Tabs in Every View"** Architect. Concept paper `docs/concept-papers/34-tabs-in-every-view.md`. **Q2 must be answered first**: is the open set global or per-pane? Global today (`openTabs` + two pointers); per-pane strips would make it a *data-model* change touching session restore and the second screen. That answer decides the whole size.
> - **PJ-209** — ✅ the always-available close affordance (× on the note, ⋯ → Close, file-tree right-click). **Boss-ruled visibility:** the page × shows only when the note has no visible tab — closing is an act on the *open set*, so it belongs to the tab strip.
> - **PJ-210** — ✅ split view collapses to normal when **fewer than two** notes remain (Boss-ruled; zero was the wrong threshold — a lone pane in a split layout has no tab bar, so the survivor was stranded).
> - **PJ-211** — ✅ **DONE at session close.** Help (`Notes Management` → *"Closing a note — four ways, and none of them touches the file"*) + User Manual (§3 → *"Closing a note"*), English, both written. **PJ-212 filed in its place:** the translated help folders carry **21 topics against English's 42**, and none has a `Notes Management` topic at all — pre-existing drift far wider than this change, and a job of its own.
>
> ### ⚖️ THE PATTERN, COMMITTED BY ME, TWICE, IN THE SAME FEATURE
> The triage named **"half a sweep"** — a fix applied to one branch and not its twin — as the dominant defect class here. I then committed it twice while fixing it: wiring **1 of 6** note-action builders (and not the file tree the Boss actually right-clicks), then hiding the × on the Index preview while leaving its menu item live and dead.
>
> Both were cured the same way, and the shape is the lesson: **replace the promise a caller must remember with a structure that cannot forget.** First a shared helper (`wireCloseNote`, called from every builder); then a **derived** value (`isClosableTab`, read from `openTabs` membership) — because a prop is just another promise, and this feature had already proved promises get dropped.
>
> ### 📌 STILL NOT TRIAGED — stated, not buried
> The 2026-07-30 inspection lost **25 candidates to server errors**. That register says so itself: *"an under-count, not an all-clear."* They are not in the 31 and nobody has ever looked at them. Re-running that scope is still owed.
>
> **Gates:** vitest **812/812** (71 files) + Sight lane **84/84** · svelte-check **0** · Rust **1339/0** · i18n **15/15 ✓**. Boss-validated across 12 live tests.
>
> ---
