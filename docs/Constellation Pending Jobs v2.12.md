...
**Version 2.12 | 2026-09-01 — PJ-460 CLOSED, Boss-passed all four steps: the repair door's blocked-screen curtain is porous (faintly dimmed, clickable — his ruling), so the tab × behind it works and "try again" does its job; six safety-inspection rounds hardened the running state until the invariant held whole — while busy or running, the dialog cannot be dismissed, no key reaches the app, no tab may mount by any enumerated path. Filed: PJ-462 (failed-close-flush residual, Boss: fix after the drain) and PJ-463 (wall-clock perf tests fail under CPU load). NEXT: Eisa Universe's 39 through the same door; then the deliberate-failure rehearsal; then PJ-461.**
