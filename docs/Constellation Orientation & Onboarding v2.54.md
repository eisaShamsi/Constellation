...
>
> *(2026-06-05, Option E item C — inline Global preview)* The **Global** category (background / text / status / accent shades; type & rhythm; shape & corners) now shows a **live composite sample card** atop its controls — a heading, muted/faint text, an accent chip, a #tag, and the status colours — built purely from the Global CSS vars, so every shade / radius / border / accent edit shows **live** via the CSS cascade off the draft-carrying `.ss` wrapper (no JS; theme-free → no `BUILTIN_THEMES`, no freeze risk). Remaining: Option E **D** (inspect-to-style); then the batched 14-lang help + a `/simplify` pass.
>
> ---
>
