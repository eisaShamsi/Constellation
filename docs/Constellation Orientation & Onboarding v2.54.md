...
>
> *(2026-06-05, Phase 6.3 + a safe theme picker — Boss test pending)* The Setter's **Saved styles** rows now carry **rename / delete / export** (hover-revealed ⤓/✎/✕ + inline rename) — the same actions as the Settings → Styles panel, kept as lightweight rows (no cards). And a **lightweight theme picker** returns the built-in/custom themes to the Setter as a **name `<select>` dropdown** (explicitly NOT the freeze-prone `stylePreview` card gallery — LL-032): pick one → `updateSettings({activeThemeId, colorScheme})`, the user's `styleOverride` rides on top. Recovers the one regression from the freeze fix. Remaining Setter work: Option E **D** (inspect-to-style) → **C** (inline Global previews); then the batched 14-lang help + a `/simplify` pass.
>
> ---
>
