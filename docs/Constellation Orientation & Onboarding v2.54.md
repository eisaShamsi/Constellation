...
>
> *(2026-06-05, "Update style" added)* Each saved-style row gains an **↻ Update** action — overwrite that style's captured look with the **current** one (keeps its name + id), distinct from "+ Save current as a style" (which makes a NEW one); a brief **✓** confirms. Theme-free (no freeze risk). Remaining Setter work: Option E **D** (inspect-to-style) → **C** (inline Global previews); then the batched 14-lang help + a `/simplify` pass.
>
> ---
>
