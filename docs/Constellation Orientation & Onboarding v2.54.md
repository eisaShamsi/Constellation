...
>
> *(2026-06-06, inspect — dock shortcut + Settings-hide; Boss-flagged entry-point fix)* Eisa hit a real gap: the Setter is opened *from* Settings, so hiding the panel for inspect left the **Settings modal** covering the app. Fixed two ways: (1) a **dock shortcut** — a crosshair icon **above the Settings gear** (`openStyleSetterInspect` → opens the Setter straight into inspect mode, no Settings in the way); and (2) inspect now also **hides the Settings modal** (a `body.ss-inspecting` class + a `:global(.settings-overlay){display:none}` rule), so the in-panel **⌖ Inspect** works regardless of how the Setter was opened. Remaining MIG-070 §C: the **batched 14-language help** · a `/simplify` pass · Phase 9 (retire the old Appearance/Style-Settings tabs at the parity gate).
>
> ---
>
