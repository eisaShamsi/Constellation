...
**Version 1.45 | 2026-07-22**

> **What changed in v1.45** (**A SHIPPED REGRESSION of mine caught by the per-build inspection and fixed; the same class fixed in the shared library loader. PJ-139 filed. SO#9. Ultracode**):
>
> ### ★ THE RE-PRIORITIZED BACKLOG
>
> **► NEXT ACTION — MIG-103 §4's SURFACE.** `discover_shapes` exposed as a Tauri command + the Studio panel showing the 21 discovered kinds with their proposed names. The engine is complete, prior-art-hardened and unit-proven, but reaches no user.
>
> **CLOSED THIS JOB:**
> - **APP-KILLER (MY REGRESSION, shipped in `98b71440`, fixed in `bb886dee`)** — the PJ-136 nested-map guard was checked in composeFrontmatter's REMOVE loop but **not in SET/ADD**, and the immutable set was derived from the **props array** rather than the file. The app manufactures a dishonest props array by itself: PropertyEditor caches `tab.content` via `buildFullContent`, `reconstructFrontmatter` dropped the block's children from that cache, and re-parsing it re-typed the key as ordinary text — so the *next* edit of any property spliced the real block out and wrote `source: ""`. Durable, silent. **Fixed twice over:** the immutable set is now read from the FILE (`nestedMapKeys`), and `nestedRaw` makes the cache lossless. 3 tests incl. the full two-save chain. Boss-validated on the running app with the exact edit→switch-away→edit sequence.
>   - **Recorded reasoning error:** the original commit claimed *"zero blast radius by design."* I verified that keeping `value` empty made `reconstructFrontmatter` byte-identical and stopped there, never tracing the re-parse of the lossy cache — I checked the step I thought of and called it proof. The deeper mistake: **I made the file's own structure something the caller declares rather than something the file states.** A props array can lie; the file cannot.
> - **APP-KILLER — the same class still live in the SHARED library loader** (`bb886dee`). `load_libraries` collapses any failure into `vec![]`; `add_library` then pushes one entry onto it and atomically renames a ONE-ENTRY `libraries.json` over a 19-library registry. **This is the anti-pattern fixed the same morning at `universe.rs:380` — I fixed the one call site instead of the shared loader, so the class survived. Solve the class, not the instance.** `try_load_libraries` refuses on an unreadable/unparsable registry and backs up before refusing; the three mutate-then-save commands use it; reads still degrade gracefully. 3 Rust tests.
>
> **NEWLY FILED:**
> - **PJ-139** *(UX · two related gaps, Boss-found)* — **(a) a library cannot be removed from its own right-click menu.** Removal lives only in *Manage libraries*, reached from the Universe button at the sidebar footer — nowhere near the thing being removed. **(b) A library whose folder no longer exists still renders as completely normal in the sidebar** — no marker, no indication it cannot open (observed live: the folder was deleted, the registration remained, the sidebar showed it unchanged). Proposal: render such a library as **unavailable** (greyed + marker) offering *Remove* / *Locate…*, and add *Remove library* to the right-click menu. **Explicitly NOT auto-removing it** — a missing folder means an unplugged drive, a disconnected share, or a sync client mid-restore at least as often as a deletion, and silently dropping the registration would be the exact class of bug fixed twice today. *Absent is not gone.* Boss's call on the visual treatment.
>
> **STILL OPEN:** **PJ-135** (English-only noise list — awaiting a Boss ruling) · **PJ-137** (retire the hand-rolled property parser — the permanent cure behind PJ-136; **this job is the second app-killer to originate in that parser split**, which strengthens the case) · **PJ-138** (translated help sets are a subset of EN) · the **PJ-130** backlog · **PJ-124** (the inspection ignores `args.files` — confirmed a 6th time) · PJ-125/126/127/128/129/131/132/133.
>
> **MIG-103 remaining phases:** §2 request path · §3 the Studio as a core-plugin app-within-app + the visual gallery · **§4's surface** · §5 the manuscript builder (proposer, NOT a wizard) · §6 the Studio's own style setter · §7 tending.
>
> ---

**Version 1.44 | 2026-07-22**

> **What changed in v1.44** (**PJ-136 CLOSED & Boss-validated. PJ-137 filed as the permanent cure. SO#9. Ultracode**):
>
> ### ★ THE RE-PRIORITIZED BACKLOG
>
> **► NEXT ACTION — MIG-103 §4's SURFACE.** `discover_shapes` exposed as a Tauri command + the Studio panel showing the 21 discovered kinds with their proposed names. The engine is complete, prior-art-hardened and unit-proven, but reaches no user.
>
> **CLOSED THIS JOB:**
> - **PJ-136 — a nested map property no longer reads "Empty"** (Boss-validated, two rounds). The row now shows its child field names as chips plus a *read-only* label, and the **write path refuses** to overwrite or splice the block — protection by construction, not by the widget staying inert. Boss's round-1 catch: the chips were read-only *but the key name was still an editable field with autocomplete*, so renaming it would have looked like it worked and silently done nothing — the same silent-failure class. Closed at the ROW level: key renders as plain text (reusing the existing `tags`/`aliases` pattern), the × delete is gone, and right-click drops *Remove property* while *Copy value* now copies the field names actually shown. 7 tests incl. "a write is refused, not applied" and "dropping the row does not splice the block". i18n ×15. **Disk verified byte-identical through rename + property edits.**
>
> **NEWLY FILED:**
> - **PJ-137** *(structural · the permanent cure behind PJ-136)* — **retire the hand-rolled property parser; one YAML-document authority.** `store.parseFrontmatter` is a line scanner with no concept of nesting; `yamlDoc` is a real CST that handles it correctly and writes byte-exactly. TWO parsers, disagreeing, feeding the same panel — that split is the root of this whole class: it is where the 2026-07-21 rename APP-KILLER lived (a line scanner discarding indentation) and where PJ-136's "Empty" lie came from. **PJ-136's read-only row is CONTAINMENT, not a decision** — Boss, 2026-07-22: *"showing it read-only is a temporary procedure, until you research for a solution and fix it for good."* Once the panel and the write path share one model of what a property IS, nested fields become editable safely, because the write path is byte-exact by construction rather than by refusal. **/migration-sized**: it changes the property projection every consumer reads and touches PropertyEditor, the BUG-015/023 surface. Research first.
> - **PJ-138** *(docs debt · pre-existing)* — **the translated help sets are a SUBSET of English.** EN carries 42 topics; ar/de/zh carry 19–21, and **Properties is not among them** — so a user-facing property change can only be documented in English today. Surfaced while doing SO#2 for PJ-136. Decide whether the 14 locales get the missing topics or whether the subset is deliberate.
>
> **STILL OPEN:** **PJ-135** (English-only noise list — awaiting a Boss ruling) · the **PJ-130** backlog (**54 remaining**; needs re-triage against HEAD; **Batch 1 built but Boss-untested**) · **PJ-124** (the inspection ignores `args.files` — confirmed a 5th time) · PJ-125/126/127/128/129/131/132/133.
>
> **MIG-103 remaining phases:** §2 request path (+ the Arabic skeletons from R3, 10 items awaiting Boss validation) · §3 the Studio as a core-plugin app-within-app + the visual gallery · **§4's surface** · §5 the manuscript builder (proposer, NOT a wizard) · §6 the Studio's own style setter · §7 tending.
>
> ---

**Version 1.43 | 2026-07-22**

> **What changed in v1.43** (**THREE APP-KILLERs fixed & Boss-validated. PJ-136 filed + Boss-ruled. SO#9. Ultracode**):
>
> ### ★ THE RE-PRIORITIZED BACKLOG
>
> **► NEXT ACTION — PJ-136** (Boss-ruled: render a nested map property **read-only with a summary of its children**, not as an editable row labelled "Empty"), **then MIG-103 §4's SURFACE** (`discover_shapes` exposed as a Tauri command + the Studio panel showing the 21 kinds — the engine is complete and unit-proven but reaches no user).
>
> **CLOSED THIS JOB:**
> - **APP-KILLER — rename destroyed a nested frontmatter value** (`6cb55169`, Boss-validated Stage 1; disk confirms `source.title: Muqaddimah`, `author`, `year` all intact through rename + a tag edit). Third strike on trim-the-line parsing (LL-014); both sites now key off column 0. A second independent bug in the same function (alias list left open across a root key) fixed alongside. 4 tests, 3 red first.
> - **APP-KILLER — a locked `libraries.json` could delete every library** (`5144c610`, Boss-validated Stage 2A). Absent is a fact; unreadable is an unknown.
> - **APP-KILLER — an external file change could land in the wrong note** (`5144c610`, Boss-validated Stage 2B). Fixed at source AND structurally — `adoptDisk` now takes the `expectPath` guard every other model mutator already proves.
> - **The 2026-07-21 whole-app inspection** returned 57 confirmed (3 APP-KILLER · 9 HIGH · 34 MED · 11 LOW) and **zero findings in the new `template_discovery.rs`**. All three app-killers fixed; the remaining 54 fold into PJ-130.
>
> **NEWLY FILED:**
> - **PJ-136** *(HIGH · data-loss trap · pre-existing)* — **a nested map property renders as "Empty".** `yamlDoc.projectProps` skips nested maps by design ("preserved in the CST, not editable here — Boss decision"); `store.parseFrontmatter`, the hand-rolled line parser that feeds the note MODEL and therefore the visible panel, has no such rule and emits `{key, value: ''}`. Untouched the data is safe (proven on disk). **Typing into the row composes `source: <typed>` over the whole block** — Muqaddimah, Ibn Khaldun, 1377 gone, no error; asserted by a characterization test at `tests/mig-103/nestedMapProperty.test.ts` that MUST go red when fixed. **BOSS RULED: render it read-only with a summary of its children** (e.g. *source · title, author, year*) — visible, honest, un-typeable. Touches PropertyEditor, the BUG-015/023 surface, so it gets a proper architectural review, not a patch.
>
> **STILL OPEN:** **PJ-135** (English-only noise list — awaiting a Boss ruling) · the **PJ-130** backlog (**54 remaining** after the three app-killers; needs re-triage against HEAD; **Batch 1 built but Boss-untested**) · **PJ-133**/**PJ-134** — *note PJ-134 is now CLOSED, it was app-killer #2 above* · **PJ-124** (the inspection ignores `args.files` — confirmed a 5th time) · PJ-125/126/127/128/129/131/132.
>
> **MIG-103 remaining phases:** §2 request path (+ the Arabic skeletons from R3, 10 items awaiting Boss validation) · §3 the Studio as a core-plugin app-within-app + the visual gallery · **§4's surface** · §5 the manuscript builder (proposer, NOT a wizard) · §6 the Studio's own style setter · §7 tending.
>
> ---

**Version 1.42 | 2026-07-21**

> **What changed in v1.42** (**MIG-103 §4 recognition + §4B naming BUILT & committed. Two Boss-directed prior-art audits (55-agent naming round). SO#9. Ultracode**):
>
> ### ★ THE RE-PRIORITIZED BACKLOG
>
> **► NEXT ACTION — MIG-103 §4's SURFACE.** The recognition engine is complete and unit-proven but reaches no user: `discover_shapes` is not exposed as a Tauri command and no Studio panel shows the discovered kinds. Everything below it is done; nothing above it exists. **Then D4** (apply a template to a note already started — the bridge between capture and composition), **then §1's use-side**.
>
> **CLOSED THIS JOB:**
> - **MIG-103 §4 — the recognition engine, hardened against prior art** (`18f41bea`; audit `docs/concept-papers/MIG-103-Shape-Discovery-Algorithm-Audit.md`). A kind is stored as `{core keys} + {key → count, fill}`, not a rigid set — the answer JSON/NoSQL schema inference and Wikipedia infoboxes reached independently (`Infobox person`: 142 parameters, ZERO required). Real Universe: **106 rigid signatures → 21 coherent kinds**. Four designs discarded with evidence (closed itemsets · maximal itemsets · Jaccard clustering · signature-only cores), plus a membership-overlap redundancy filter the audit did not name but the data demanded. 12 tests.
> - **MIG-103 §4B — naming, per the Boss ruling "propose a name from the members, let me edit it"** (`35263d3d`). Differential cluster-relative scoring over SIX independent evidence families, gated on the Wilson lower bound, requiring corroboration by ≥2 families. Kills `source=Wikipedia` and `the` with no stopword list (the only approach that survives fifteen languages); kills the imported slug `template-film-date-with-1-release-date` by independence, since no scalar statistic can separate it from `film`. 55-agent prior-art round; six survivors changed the code. Real Universe: 3 of 21 named, 18 honestly unnamed. 23 tests + an `--ignored` real-Universe harness.
> - **Two avenues tested and REJECTED rather than assumed:** retuning the confidence level (at z=1.645 not one lost name returns — corroboration binds, not confidence) and outgoing links as a seventh family (the only tokens clearing both gates are JSTOR/ISSN identifiers).
>
> **NEWLY FILED:**
> - **PJ-135** *(HIGH · Language-First violation · pre-existing)* — **the shape engine's noise list is English-only.** `is_noise` in `template_discovery.rs` strips `created`/`modified`/`cid_cn`, so identity keys written in ANY other language pass straight through and become a top-ranked "kind". Measured: in **Eisa Universe** the #1 shape is `أنشئ · حُدث` (241 notes) and in **Constellation Test** it is #3 at 538 notes — both "created · modified" in Arabic. The shapes are real but meaningless, and they outrank every genuine kind by support. `أنشئ` is NOT traceable to a current locale string, so the origin of those keys is unknown. **Needs a Boss ruling**, because the options trade off badly: (a) translate the ~16 noise keys × 15 locales = 240 strings — deterministic, but a permanent maintenance burden that still misses unlisted languages; (b) a language-agnostic value-shape test (a key whose values carry a time-of-day is a machine-written audit stamp) — covers every language but is a heuristic that could mis-fire on genuine time-valued keys; (c) leave it — the bogus kinds get no name and can be ignored, but they head the list. **Found while validating §4B across the Boss's other Universes; NOT introduced by this job.**
>
> **STILL OPEN:** the whole **PJ-130** safety backlog (**60 confirmed** — 4 APP-KILLER · 24 HIGH · 20 MED · 12 LOW — needs re-triage against HEAD; **Batch 1 built but Boss-untested**) · **PJ-133** and **PJ-134** (both APP-KILLER, pre-existing) · **PJ-124** (the inspection ignores `args.files` — re-confirmed a 4th time this job) · PJ-125/126/127/128/129/131/132.
>
> **MIG-103 remaining phases:** §2 request path (+ the Arabic skeletons from R3, 10 items awaiting Boss validation) · §3 the Studio as a core-plugin app-within-app + the visual gallery · **§4's surface (the next action above)** · §5 the manuscript builder (proposer, NOT a wizard) · §6 the Studio's own style setter · §7 tending.
>
> ---
