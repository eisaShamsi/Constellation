...
mode" in category "Appearance", so **typing "theme" returns No results** — which is what he hit. No
default shortcut (absent from `DEFAULT_SHORTCUTS`), and no Settings control writes `colorScheme`
(`handleToggleTheme`, `+layout.svelte:6100-6108`, is its only writer). Smallest fix: a keyword/alias
field on commands, or rename it. **Frame it correctly:** this is one switch inside the theme system,
NOT a missing theme control.

### 🆕 PJ-477 *(Group 2 — a design collision, his call)* — the light/dark toggle and Saved Styles are two theme layers that fight
A user has ONE theme; the app has two independent layers, and the toggle changes only the built-in
one. **Measured on Eisa Cognitive Knowledge:** 174 `styleOverride` keys, 81 pinned hex colours — 36
very light, 13 very dark; `--background-primary` and `--text-*` left FREE while `--background-secondary`,
`--topbar-bg`, `--dock-bg`, `--statusbar-bg`, `--center-zone-bg` are pinned light. Toggling dark
therefore flips only the unpinned half and yields the hybrid he called **"the worst theme I've ever
seen"**. His words: the built-in dark theme is *"not suitable for the app."* Options: a dark variant
per Saved Style · the toggle selecting between two Saved Styles · retiring the built-in toggle. **His
design call; same panel treatment as PJ-475.**

### 🆕 PJ-478 *(LOW — Group 3)* — Cairo is offered but not bundled
`fonts.ts:23` lists **Cairo** in `CURATED_FONTS`, so it appears in every font picker including the
Calendar font control — but no Cairo file ships in `static/fonts/` and no `@font-face` declares it
(Amiri, by contrast, is bundled and registered at `fonts.css:28,35`). A user picking Cairo without it
installed gets a silent fallback with no indication. Either bundle it or mark unbundled families in
the picker.

### 🆕 PJ-479 *(MEDIUM — Group 2 — hygiene, found while measuring)* — five checkouts of this repository, and one tracked file inside the ignored worktrees
`git worktree list` reports **five** trees: the working one, three under `.claude/worktrees/`
(2026-05-09, 05-18, 08-07 — **887 MB**), and `E:\مشاريع كلاود\Constellation-wtSC` (2026-06-12, detached
HEAD), which no document mentions. **WA#2 is explicit:** one location, no worktrees, no parallel
paths. Each holds a full copy of `src/`, so any audit grepping from the root silently reads
months-old code — my own `find .` hit the 08-07 copy today. And `.claude/worktrees/` is gitignored
(`.gitignore:37`, commit `261e0c89`) **but one file inside it is still TRACKED** —
`.claude/worktrees/upbeat-proskuriakova/.claude/settings.local.json` — because an ignore rule never
untracks. It carries absolute paths under `C:\Users\ealsh\` and is pushed to GitHub.

### 🆕 PJ-480 *(LOW — Group 3)* — the Digest panel chevron's hover is invisible by construction
`DigestPane.svelte:538` now paints `--background-modifier-hover`, but `.dg-chev-btn` is a **child** of
`.dg-note` (markup :334-336) whose own `:hover` paints the identical token (:519-521). Hovering the
chevron always hovers the row, so there is no distinct feedback — and the block's own grammar for
child feedback is non-background (the name underlines, the headline shifts colour). The rename was
correct; the state it reaches for has no token. `--interactive-hover` and `--background-modifier-hover`
both resolve to `--color-base-25`, so `theme.css` has two tokens with one value and no pressed/active
step between hover and accent.

### 🆕 PJ-481 *(LOW — Group 3)* — the Ledger lens's tier dots are painted from the wrong vocabulary
`NoteLedgerGraph.svelte:156,157,158,159` — **four renderings**, not one. `:157`
`.lt-load-bearing { background: var(--rel-supports, #879A39) }` borrows the **Supports relationship**
colour for a **traversal** tier (`f.tier` comes from `linkLifecycle()`, a pure function of traversal
count and staleness — link type is nowhere in it), so recolouring Supports in the Style Setter
repaints the load-bearing dot. `:158` borrows the global accent. `:156` is the base dot that
**`emerging` AND `fresh` both fall through to** — there is no `.lt-emerging` rule at all. The
canonical chain is `--link-tier-*` (`BacklinksPanel.svelte:421-438`, `OutgoingLinksPanel:277-303`,
`StyleSetter:2184-2187`; the token is `--link-tier-loadbearing`, one word). `cockpitGraphData.ts:59-61`
documents this exact anti-pattern being fixed for **confidence** — the same bug, unfixed, one file
over. Changes a Boss-validated lens's default colours, so it needs his eye.

### 🆕 PJ-482 *(LOW — Group 3)* — a dead parallel style registry that a future audit will trust
`src/lib/theme/constellationStyleSettings.ts` (~120 controls), `StyleSettingsPanel.svelte` and
`generateStyleSettingsCSS` (`styleSettings.ts`) have **zero live consumers** — only `CORE_BLOCK_IDS` is
imported (`+layout.svelte:66`), to filter blocks OUT. `+layout.svelte:2490` records why: *"MIG-071 —
the theme subsystem was removed."* It still advertises a `sidebar-width` control the live registry
deliberately removed (`StyleSetter.svelte:534-535` explains the removal). **It matters to method:** it
is a plausible-looking source of custom-property names — exactly the input a "which names are defined?"
...
(`FileTree:243`). No token owns it; a maintainer restyling selection has six literals to find. Also
`ConstellationMap.svelte:862` — two lines above a row PJ-461 fixed — is still a frozen
`rgba(124,58,237,0.1)`, the app-default accent hard-coded, so it ignores his accent and the theme.

### 🆕 PJ-484 *(LOW — Group 3)* — `--accent-bg` is misnamed, and `.s-result.active` is invisible under the pointer
`--accent-bg` (`theme.css:169`) aliases `--background-secondary-alt`, a background tier, not an accent
tint — and it resolves to the same `--color-base-25` as `--bg-hover`. So `+layout.svelte:11425`
`.s-result.active` renders **identically to `.s-result:hover` at :11424**, including its `color`. A
live pre-existing collision of the same class PJ-461 fixed at the Template Studio. Fixing the token
moves every consumer, so it needs its own sitting.

### 🆕 PJ-485 *(LOW — Group 3)* — dead CSS and orphaned i18n keys
`.w-option-input` / `.w-option-form` (`+layout.svelte:12305-12311`) have **no markup user anywhere** and
are pruned from the shipped bundle. `settings.noteGraphStyle` / `settings.noteGraphStyleDesc` exist in
all 15 locale files with no component reference — leftovers from the lens picker removed at `e8b392d0`.

### 🆕 PJ-486 *(LOW — Group 3)* — an unbuilt lens is reachable from a persisted setting
`cockpitFlag.ts:36` `normalizeGraphStyle` keeps any id present in `NOTE_GRAPH_STYLES`, and `heartwood`
is listed with `built: false`. The picker filters on `built`, so a user who selected it before the
Settings picker was removed (`e8b392d0`) has it persisted, and their second screen silently mounts
`NoteRadialGraph` — a lens no picker offers — permanently and with no notice. None of his universes is
in that state. A behaviour change, hence its own item.

### 🆕 PJ-487 *(LOW — Group 3)* — the lens text plates are sized by constants calibrated against the wrong font size
`NoteButterflyGraph.svelte:76` (`title.length * 8.4 + 34`), `:260` (`* 7 + 16`),
`NoteOrreryGraph.svelte:493` (`* 3.2`), `:522` (`* 7.4 + 26`), `:572` (`* 6.4`) size text plates from
character counts calibrated while every label rendered at the inherited 14px. PJ-461 restored the
authored 9–15px, so every fit changed. **Measured before shipping: every plate still fits, and two
that were tight are now roomier** — but the constants should be re-derived from the real sizes rather
than left coincidentally correct.

### 🆕 PJ-488 *(MEDIUM — Group 2 — PJ-461's mirror image, and arguably worse)* — the fossil-key class
PJ-461 was **sites reading a name nothing writes**. This is the inverse: **a CONTROL writing a name
nothing reads.** `--font-interface-size` is present in all three of his `styleOverride` maps, is in no
current Style Setter catalog, and has **zero `var()` readers in `src/`** — yet it is re-applied to
`<body>` on every boot, forever. **Nothing filters `styleOverride` against the current catalog**, so
every retired control leaves a fossil that outlives it. Needs a sweep and a filter.

---

## 📚 Lessons-Learned entries owed from this cycle
1. **Bundle verification has THREE emit locations**, not one: `build/_app/immutable/assets/*.css`,
   `build/_app/immutable/chunks/*.js`, and `build/assets/screen-*` (the second-screen entry from
   `vite.screen.config.js`). `LESSONS-LEARNED` §611's documented command scans only
   `build/_app/immutable/` and would have verified **38 of 78** occurrences in this job. Grep patterns
   for custom properties must be **boundary-anchored** (four retired names are substrings of their own
   replacements), `grep -c` counts lines and minified CSS is one line, and `build/` must be proven to
   post-date HEAD before any of it means anything.
2. **A register is not evidence.** Seven items in this job's own evidence register were wrong — a line
   count, a locale tally, two reachability mechanisms, the rem baseline, where the font JS writes, and
   a tier-chain citation — and every lens that trusted one inherited its error. The panels' value came
   entirely from attackers re-measuring what the register asserted.
3. **A stylesheet filename is not proof a build shipped your change.** CSS comments are stripped in
   production, so a comment-only CSS edit yields a byte-identical file and the same content hash. Verify
   the chunk carrying the actual change.

---

> **What changed in v2.13** — PJ-454 closed (43/43); the rehearsal done; the scan's double-walk
> fixed; PJ-464…PJ-473 filed; PJ-461 re-scoped.
>
> **► NEXT ACTION: PJ-461** — 75 CSS-token usage sites across 21 files (the entry below carries the
> replacement table and three open decisions); its own build and its own Boss sitting (visual,
> light + dark). Then **PJ-466** (the halt screen's wording, one concern across three exits — panel:
> before PJ-456's 107-file wave). Then PJ-456 · PJ-457 (evidence now stronger) · PJ-458 · PJ-459 ·
> PJ-462 (after the drain) · PJ-463 · the multi-folder templates setting · the PJ-264/PJ-378
> unpack · PJ-434 · PJ-438.
>
> - **PJ-454 — CLOSED (`7d79daba` + `d58955c8`, committed AFTER the Boss's pass).** The repair half
>   is complete: **43 of 43** molds repaired. Before Eisa Universe's 39 were run, a read-only
>   measurement (verified independently) showed the scan would have listed **74 rows for 39 files**
>   — Eisa Universe registers four libraries NESTED under its root and `collect_md_paths` has no
>   own-library boundary — so three delicate files would have run twice and the second pass's
>   refusal would have HALTED the run before the 35 ordinary files. Panel-ruled fix (top-level own
>   roots + attribution by `library_name_for_path`, no dedupe), RED→GREEN. The diff-scoped
>   inspection then caught a regression of the fix (a nested library the parent's walk cannot
>   enter — dot-folder / junction — vanished from the door): closed with the collector's own reach as
>   the oracle, RED→GREEN, re-inspection zero. **Boss-passed 2026-09-13:** Stage 1 (the rehearsal —
>   halt on one ✕ "ReplaceFileW error 5", batch 2 untouched; then an ordinary ✕ not halting its
>   siblings; then recovery) and Stage 2 (39 · 34/1/4 · "39 fixed, 0 skipped" · banner gone) and the
>   relaunch (healer `injected=0 templates=39`). Disk-confirmed: 39 backups each carrying its stamp,
>   journal +39, the four duplicate-folder copies now byte-identical to the repaired originals.
> - **The deliberate-failure rehearsal — DONE** (owed since v2.11). Halt-on-failure after the
>   delicate batch and the ordinary batch continuing past a failure were both witnessed on his
>   screen. Still unproven, disclosed: the between-batches open-tab halt, the invoke-error catch, a
>   verify-failure undo, any failure on a real non-copy file.
> - **PJ-461 — re-scoped:** not 3 names / 5 sites but **75 usage sites across 21 files** (audit:
>   31 names, one refuter each, synthesis). Entry updated below.
> - **Filed PJ-464 … PJ-473** (section "Filed 2026-09-13" below): the two closed defects for the
>   record; the halt-screen wording; the scan on every watcher flush; no journal line on a failed
>   repair; batches ignore `will_apply`; `relinked_sources` over-count; strings vs the plural rule
>   and English literals; `library_name_for_path`'s trailing separator; `run_cold_start`'s
>   redundant walk; four out-of-scope finds from the CSS audit.
>
> ---

> **What changed in v2.12** — PJ-460 closed; the running-state invariant completed under six inspection rounds.
>
> **► NEXT ACTION: repair Eisa Universe's 39** through the same door with Eisa Universe active
> (own-libraries only — write sovereignty), then the owed **deliberate-failure rehearsal on copies**
> (halt-on-failure and re-scan-between-batches have never fired; no test), then **PJ-461**. PJ-462
> waits for after the drain (his ruling). Then the umbrella unpack (PJ-264, PJ-378).
>
> - **PJ-460 — CLOSED, Boss-passed (`<this commit>`).** Panel ruled fix (a): the blocked screen's
>   overlay is porous — `pointer-events:none`, an 18% tint (his "faintly dimmed, clickable") — so the
>   tab strip's × is reachable; the click-time re-check is the real guard and stays. Six diff-scoped
>   inspection rounds then found and fixed, in order: ✕ clickable mid-run (MED) · keyboard through the
>   curtain (LOW) · second-screen `open-in-main` mounting a candidate mid-run (MED → a shared
>   `moldRepairRunning` signal gates `openNoteTab`) · no re-entrancy latch (LOW → `busy`) · entry-only
>   gate (LOW → re-read before each mount) · history navigation as an unlisted mount + a NO-OP gate I
>   had placed after the tab-store write + the busy-window close (LOW/LOW/MED). Round six: zero.
>   Boss test: block fires → tab × behind the card closes it → try again runs → **try again with the
>   tab still open stays blocked.**
> - **PJ-462 filed** — class-wide residual (panel): a tab whose close-flush FAILS leaves the old stamped
>   text in the recovery net → dirty on reopen → re-stamp after repair. Pre-existing, on the manual
>   path too. **Boss ruled: file it, fix after the drain cycle.**
> - **PJ-463 filed** — `tests/sight-v6/tradition-perf.test.ts` asserts wall-clock ≤16 ms; 3 of 27
>   failed while a release build ran in parallel and passed 27/27 alone. Load-sensitive CI hazard.
>
> ---

> **What changed in v2.11** — the repair door, panel-designed and Boss-tested end to end.
>
> **► NEXT ACTION: panel + fix PJ-460** (the blocked screen's recovery button). Then repair Eisa
> Universe's 39 through the same door (it is the active universe's own molds only — write
> sovereignty). Then the owed deliberate-failure rehearsal (halt-on-failure is UNEXERCISED) before
> PJ-456's 107-file wave. Then the umbrella unpack (PJ-264, PJ-378).
>
> - **PJ-454 repair door — BUILT + BOSS-TESTED (Stages 1 and 2, real files).** Panel resolved the
>   "app must be closed" contradiction into a click-time open-tab block (load-bearing: a dirty open
>   mold's delayed save would re-stamp after the engine verifies). Boss ruled the three forks as
>   recommended: read-only pre-flight (`preview_mold_repair`, with `needs_care`), banner-when-needed
>   + Settings fallback, refuse-and-name if open. Safety inspection found a MED in the engine
>   (federated mold → file written, index not updated, clean success reported) — fixed at the root:
>   scan is own-libraries only (`load_libraries`, the write path's own resolver) and `repair_one`
>   refuses foreign paths. **موسوعة عيسى: 4 of the 43 repaired**, backup verified, disk-confirmed.
> - **PJ-460 filed** — the Boss: *"I cannot close the tab while the 'Close these files first' dialog
>   box is on."* The modal overlay blocks the tab strip, so "I've closed them — try again" cannot do
>   its job. Workaround works (close dialog → close tab → reopen); the affordance is wrong. Design
>   choice → panel.
> - **PJ-461 filed** — the class that made the dialog black: CSS variable names that do not exist in
>   `theme.css`. Mine fixed; two pre-existing files use `--bg-primary` / `--bg-active` /
>   `--bg-modifier-border` (5 places: `CalendarPanel.svelte:287,291` with a `#fff` fallback so it
>   renders light but would be wrong in dark; `+layout.svelte:11553,12291,12309` with NO fallback).
> - Halt-on-failure and re-scan-between-batches are **not exercised** by any test (all four
>   succeeded); disclosed to the Boss, rehearsal owed.
>
> ---

> **What changed in v2.10** — the drain cycle produced its first validated work.
>
> **► NEXT ACTION: continue the DRAIN CYCLE.** PJ-454's guard is closed; its **repair half** (the 43
> molds) needs the settings door + the file list shown to the Boss + a dry run + his test before it
> runs. Then the two queued decisions (PJ-456 second wave, PJ-457 duplicate). Then the umbrella
> unpack: PJ-264 (~100) and PJ-378 (58) into numbered entries; then PJ-434, PJ-438.
>
> - **PJ-454 guard — CLOSED (`cc507561`, guard proven this session).** Boss-tested live on the 15:08
>   binary, all three cases. **The count is 43** (39 Eisa Universe, 4 موسوعة عيسى, 0 elsewhere) —
>   reproduced twice, exact reconciliation; 102, 67 and 0 are refuted. Repair engine built + tested
>   (`f1799826`, `mold_repair.rs`, suite 1,629/0), deliberately not invocable.
> - **PJ-455 — CLOSED (`<this commit>`), Boss-tested all four steps.** Tree-refresh at the write for
>   four creation commands + the outside-universe folder warning (15 locales). Found by the Boss
>   while testing PJ-454.
> - **PJ-456 filed** — the 107-file second wave of stamped molds (64 obvious / 24 ambiguous / 19 real
>   notes that must never be touched). Approve by enumeration in folder batches; no safe rule exists.
> - **PJ-457 filed** — the 18 GB duplicate inside Eisa Universe (39% of that universe's index; traced
>   to a 2026-08-07 MIG-108 Copy then de-registered). Recommendation: link the original, verify 13
>   links, bin the copy.
> - **PJ-458 filed** — two more creation commands announce nothing to the tree (`quick_capture`,
>   `get_daily_note_path`) but may return an EXISTING path, so PJ-455 deliberately left them; decide
>   per-command whether creation-vs-reuse can be distinguished, then wire. Also `create_base` (screen
>   refreshes) and `save_clipboard_image` (latent — mints an invisible `attachments/` folder, no
>   caller yet).
> - **PJ-459 filed** — documentation drift: the Templates help topic exists in English only (the 14
>   translated help sets never had one), and 6 of 14 translated User Manuals lack a template section.
>   Pre-existing; surfaced during the PJ-455 doc sync.
>
> ---
...
> **⚠ AMENDED 2026-09-15 — this entry's PREMISE is the shadow artifact; re-verify before working it.**
> It rests on reading `%APPDATA%/world.uconstellation.app/universes.json` from inside a session.
> That path is REDIRECTED into the Claude container and frozen at 2026-08-07 (same NTFS file ID as
> the container copy); the real file has provably been rewritten 37+ times since. **PJ-321 was
> CLOSED for exactly this trap** (*"the app was never at fault; I was reading a shadow copy"*).
> This entry may dissolve the same way. Do not touch code for it until the registry's real contents
> are obtained through the app itself, or from a copy the app writes to `E:`.
>
...
### ✅ PJ-455 — CLOSED (2026-09-01, Boss-tested all four steps) — a newly created template did not appear in the File Explorer until reload

**Found by the Boss during the PJ-454 guard test.** Saving a note as a template wrote the file
correctly but announced nothing, so the File Explorer had no reason to redraw — the template
appeared only after a relaunch or Ctrl+R. Panel-confirmed the mechanism and found **a second broken
surface the Boss had not hit**: Template Studio's "Keep" (`adopt_discovered_kind`) writes a mold into
the same folder and its screen refreshes nothing at all.

**Fix — the announcement moved to the write** (`libraries::announce_created`, emits the app's own
`note-created`), wired at four creation commands: `create_template` and `adopt_discovered_kind` (the
broken two) plus `create_note` and `create_folder` (which worked only because their screens
remembered to refresh — the drift that produced this bug). Placement is Rust-side at the write, not
in the screen, so every present and future caller inherits it.

**Plus the setting warning the Boss ruled (his "Yes").** Pointing the Template folder at a path
OUTSIDE the universe now shows a bordered warning under the field: the templates save correctly but
will not appear in the File Explorer. Only an absolute escaping path warns (a relative value always
resolves inside); silent if the universe root cannot be resolved. Localized in all 15 app locales +
the English help topic and manual + 8 translated manuals (the 6 without a template section → PJ-459).

**Gates:** cargo/svelte-check clean · i18n 15/15 parity · diff-scoped safety inspection 0 findings
(both hunters completed — not an all-died empty) · suite 1,629/0 · **Boss-tested: all four steps
passed** on the 10:07 binary (verified to contain the change).

---

### ✅ PJ-460 — CLOSED (2026-09-01, Boss-passed all four steps) — the blocked screen's "try again" button was unreachable; the curtain is now porous and the running-state invariant is complete

**His words:** *"I cannot close the tab while the 'Close these files first' dialog box is on. So, I
closed the dialog box, closed the tab, then re-clicked 'Review templates to fix'."*

The open-file block is correct and load-bearing (a dirty open mold's debounced save fires AFTER the
engine verifies and the write gate lets it through — silent re-stamp). But the blocked screen is a
modal overlay, so the tab strip behind it cannot be clicked, which makes **"I've closed them — try
again"** unusable as designed. The user's workaround (close dialog → close tab → reopen the door)
works. **Fix is a design choice → panel first:** (a) make the blocked overlay non-blocking so the
tab × is reachable; (b) a "Close these files for me and continue" action that flushes dirty content
then closes the offending tabs, then re-checks and proceeds; (c) reword the screen to the
workaround and demote the button. `MoldRepairDialog.svelte` blocked mode.

### 🆕 PJ-462 *(LOW — Group 3 — class-wide residual; Boss ruled: after the drain)* — a failed close-flush leaves stamped text in the recovery net

Found by the PJ-460 panel, pre-existing, present on the manual close-tab path too: `closeTab` flushes
a dirty tab but never reports whether the write landed; on a failed save the old (stamped) text sits in
the recovery net, is restored DIRTY on reopen, and its next debounced save re-stamps a file the repair
had fixed. Needs the close primitive to report the flush result (a store-surface change) and a decision
on what the reopen does with a net entry older than the file. **Boss ruled 2026-09-01: file it, fix
after the drain cycle.**

### 🆕 PJ-463 *(LOW — Group 3 — CI hazard surfaced 2026-09-01)* — wall-clock perf tests fail under CPU load

`tests/sight-v6/tradition-perf.test.ts` asserts `elapsed < 16` ms per tradition switch (and < 400 ms
for the full cycle). With `cargo build --release`, a safety inspection and svelte-check running in
parallel, 3 of 27 failed; rerun alone, 27/27 passed. The file imports only `sight/v6` layout math —
nothing else can move it. Options: skip under a `CI_LOAD` flag, a relative budget against a warm-up
baseline, or retry-once. Not a correctness defect; a false-red generator.

### 🆕 PJ-461 *(LOW — Group 3 — pre-existing; the class that made the repair dialog black)* — CSS variable names that do not exist in the theme

> **Re-scoped 2026-09-12 (audit `wf_349bfa30-f69`: 31 names, one adversarial refuter each,
> synthesis; every site opened and read).** **75 usage sites across 21 files** — DigestPane,
> ConstellationMap, TemplateStudioRow, OrgChart, livePreview.ts, +layout, CalendarPanel,
> SettingsModal, TasksPanel, BaseTab, LinkTypesEditor, Mig108UnifyDialog, NoteButterflyGraph,
> NoteOrreryGraph, NoteLedgerGraph, NoteRadialGraph, StyleSetter, SightV7, facetSidebar, SightV6,
> tour. **UNDEFINED (17 names → the real `theme.css` token):** `--font-sans` (27 uses) →
> `--font-interface-theme` :8 · `--font-monospace` (11) / `--font-mono` (1) / `--mono-font` (3) →
> `--font-monospace-theme` :10 · `--font-text` (3) / `--text-font` (2) → `--font-text-theme` :9 ·
> `--interface-font` (6) → `--font-interface-theme` · `--library-accent` (9) → `--interactive-accent`
> :108 · `--bg-primary` (4) → `--background-primary` :81 (or `--bg` :159 at Tier-3-alias sites) ·
> `--background-modifier-border-hover` (4) / `--border-color` (2) → `--background-modifier-border`
> :85 · `--background-modifier-active-hover` (2) → an 18% `--interactive-accent` tint (the
> MoveDialog form) · `--background-modifier-active` (1) → `--background-modifier-hover` :87 ·
> `--bg-active` (1) → `--accent-bg` :169 · `--border-faint` (1) → `--border-light` :164 ·
> `--link-tip-font-size` (2) → keep the literal; add the missing Style Setter control ·
> `--sidebar-width` (1) → the literal · `--bg-modifier-border` (0 live uses). **Keep as-is (defined
> dynamically / statically):** `--library-color`, `--selection-color`, `--border-width`, `--text`,
> `--taxo-color`. **Template-literal artefacts by design (7 families)** — no change; one site inside
> `--rel-` flagged for a different reason (`NoteLedgerGraph.svelte:157` borrows `--rel-supports`
> instead of the traversal-tier chain its siblings use). **Three decisions for the build:** the
> `--link-tip-font-size` control's unit (`rem` preserves behaviour); `--bg` vs
> `--background-primary` as the house name at Tier-3 sites; OrgChart connector weight (`-border`
> vs `-border-focus`, cosmetic). Its own build and its own Boss sitting (visual, light + dark).
> The synthesis text is in `SESSION-LOG-2026-09-12.md §3b/§3c`.

`theme.css` defines `--background-primary` / `--background-secondary` / `--background-modifier-border`
and `--bg-hover` / `--bg-tertiary` / `--bg-secondary` — but NOT `--bg-primary`, `--bg-active` or
`--bg-modifier-border`. A rule using a non-existent name falls to its literal fallback (or to
nothing). The repair dialog did this and rendered black on the Boss's light theme (fixed this
session by reading the theme, not guessing). **Two pre-existing files carry the same class:**
`CalendarPanel.svelte:287,291` (`--bg-primary` nested with a `#fff` fallback — renders light, would be
wrong in dark) and `+layout.svelte:11553,12291,12309` (`--bg-active`, `--bg-primary` with no
fallback). Fix = rename to the real tokens; verify each in `theme.css` first.

### 🆕 PJ-456 *(Group 2 — the PJ-454 second wave)* — ~107 more stamped molds the strict rule does not reach

Beyond the strict 43, a read-only investigation found **107 files needing judgement: 64 obvious
molds (A), 24 ambiguous (B), 19 real notes that must never be touched (C)**. **No safe rule exists**
— the best discriminator catches 41 of 64, misses 23, and also catches the Boss's own **unfinished
concrete-formwork note**, which on disk is indistinguishable in shape from a mold. **Approve by
ENUMERATION in folder-sized batches, never by predicate.** Two facts make that safe: 0 of the 107
appear in `earned.jsonl`, and exactly one has a human inbound link. `القوالب الخرسانية` is the
standing counter-example to any future rule proposal. Brief in `SESSION-LOG-2026-09-01.md §8`.

### 🆕 PJ-457 *(Group 2 — federation hygiene)* — an 18 GB drifted duplicate inside Eisa Universe

`Eisa Universe\موسوعة عيسى` — 9,074 files, **39% of that universe's `note_meta` rows**, 60% of its
links — is a copy of the موسوعة عيسى universe. Traced from evidence: a **MIG-108 unification executed
as Copy on 2026-08-07 16:48–16:53**, then de-registered as a library with the files left behind (who
de-registered it is unknown — the diagnostics log starts after that window). 9,054 of 9,074 shared
files byte-identical; 18 of the 20 that differ, differ ONLY in their identity stamp; original holds 9
newer notes; nothing references the copy; 13 links from outside resolve into it. **Recommendation:**
add `E:\موسوعة عيسى` as a **Linked Universe** → verify the 13 links → move the copy to `.trash`,
behind a backup, Boss tests links first. His call. Brief in `SESSION-LOG-2026-09-01.md §8`.

### 🆕 PJ-458 *(LOW — Group 3 — completeness of PJ-455)* — two more creation commands announce nothing

`quick_capture` and `get_daily_note_path` emit no `note-created`, but each may return an **existing**
path rather than always creating — so PJ-455 deliberately did not wire them (announcing a creation
that did not happen is a new small wrong). Decide per-command whether creation-vs-reuse can be
distinguished, then wire. Also: `create_base` (its screen refreshes; move the guarantee to the write
for consistency) and `save_clipboard_image` (latent — would mint an invisible `attachments/` folder,
but has no caller in `src/` yet, so a trap for whoever wires paste, not a live defect).

### 🆕 PJ-459 *(LOW — Group 3 — documentation drift, pre-existing)* — the Templates help topic is English-only

The `Templates/Templates.md` help topic exists only in `help.uConstellation.World` (English); the 14
translated help sets never had one (help.ar has 21 topics to English's 44). Separately, 6 of 14
translated User Manuals (fr, he, hi, pt, tr, ur) have no template section at all. Surfaced during the
PJ-455 doc sync; not created now because translating whole topics/sections is out of that fix's
scope.

---

## 🧰 Filed 2026-09-13 — from the PJ-454 close (the panel's attacks, the `/simplify` lenses, the two inspections, the CSS audit)

### ✅ PJ-464 *(MED — CLOSED 2026-09-13, `7d79daba`, Boss-passed)* — the mold scan listed a nested library's templates twice and would have halted the run
`scan_stamped_molds` walked every own library with `collect_md_paths`, which carries no own-library
boundary; on a universe with libraries nested under its root (Eisa Universe: root + four) every
nested mold appeared twice, the delicate batch carried three files twice, and the second pass's
"no longer a stamped template" refusal counted as a failure and halted before the ordinary files.
Fixed caller-side (top-level own roots; foreign dropped after collection; owner by
`library_name_for_path`; no dedupe), plus the oracle second pass for nested roots the parent's
walk cannot enter (dot-folder / junction — the inspection's catch). Guards: three RED→GREEN tests
+ the `#[ignore]`d real-registry harness. Verified on Eisa Universe: 39 / 39 / 0.

### ✅ PJ-465 *(LOW — CLOSED 2026-09-13, `d58955c8`, Boss-passed)* — the mold banner's dismiss flag and count leaked across universe switches
`moldRepairDismissed` was the one notice flag absent from `handleUniverseSwitch`'s reset block;
`moldRepairCount` was never cleared. Both reset with their siblings. Proven: ✕ in Eisa Universe →
switch → Scratch's own banner returned.

### ✅ PJ-466 *(CLOSED 2026-09-15, `5e3e6857`, Boss-passed Stage 1 on his screen — all 8 items)* — the repair door's receipt now tells the truth
**Closes PJ-469 + PJ-470 + PJ-471 with it.** Panel verdict:
`lab/reports/PJ-466-PANEL-VERDICT-2026-09-14.md`. Test: `lab/reports/PJ-466-BOSS-TEST-2026-09-14.md`.
Account: `SESSION-LOG-2026-09-14.md` §21–§22 and `-15.md`.

**The defect, stated once.** `MoldRepairOutcome` carried only `ok: bool` and free text, and `failed`
was computed by subtraction (`attempted - repaired`). Nothing could separate *"left alone, nothing
owed"* from *"still broken"*. So a file that needed nothing **halted the whole cascade**, and a
genuine write failure was reported to the user as **"skipped"** — in fifteen languages.

**What shipped.** `MoldOutcomeKind` (`repaired` | `kept` | `failed`) + a stable `reason` code +
`backed_up`, counted BY CLASS. Modelled on the shipped `PruneReceipt`
(`phantom_prune.rs:585-604`, rendered at `SettingsModal.svelte:2753-2790`) — **not invented**; three
of four design-panel lenses proposed inventing a taxonomy while telling the panel no precedent
existed. The halt reads `failed`, so a benign refusal can no longer stop the run. Also: the verify
rollback is CHECKED (`verifyNotRestored` is its own code — it used to discard its `Result` while the
message asserted *"so it was undone"*); `gone` no longer swallows a permission denial;
`relinked_sources` counts only successful re-indexes (**PJ-470**); an unreadable registry refuses the
RUN instead of blaming all 43 files; the engine proves the edit before writing, so a file that cannot
be repaired costs no write (**PJ-469**, fixed in the engine, not the door).

**The copy, panel-ruled at his request** (*"I want to hear from the panel"*). Five lenses refuted
**eleven** of my sentences. The title ladder is gone — *"Some templates were fixed"* would have
printed over a run that fixed nothing, the same flattery he saw live. One neutral heading,
**"What the fix did"** (his choice); one sentence per non-zero class; a translated reason per file.
Four sentences promised what the code cannot prove, including *"it will catch up on the next
launch"* — `reconcile.rs:1014-1016` says in its own words that *"every drifted note in the report is
still drifted when the user reads it."* 22 new keys + `plurals.templates` (37 CLDR forms) × 15
locales (**PJ-471**); `summaryLine` and `done` retired. **"skipped" has left this door in every
language.**

**Three defects of mine, each caught by a different gate — the part worth re-reading:**
1. **A BOOT REGRESSION I introduced.** Counting notes instead of link rows was right, but it turned a
   ...
   **Measured on `Eisa Cognitive Knowledge` (2.03 GB): 7,768 ms**, against ~23 ms before. Rule 8's
   hard constraint. Fixed by walking first and asking about the ~43 identities found: **70 ms**.
2. **A GUARD THAT GUARDED NOTHING.** My run-level refusal used `load_all_libraries` — cached and
   federation-recursive — to predict what the per-file check does through `load_libraries` —
   uncached, own-only. On a warm cache it would pass and print the 43 false refusals it existed to
   prevent. **`index_repair.rs` already enforces this rule as a test; `mold_repair.rs` was simply not
   in its list.** It is now.
3. **PJ-469 AT THE WRONG LAYER.** My first version had the DOOR skip files a minutes-old preview had
   judged. A file re-stamped since would have been reported **"kept · already fixed"** — a false
   statement on the screen this job exists to make truthful.

**And two translators (zh, ar) caught a fourth**, independently and unprompted: three strings took
`{noun}` while the component still passed `{count}` — the screen would have rendered the literal
`{noun}` in all fifteen languages. Neither inspection round had seen it.

**Gates.** Rust **1,639 / 0** · svelte-check **0** · i18n-parity **15/15** (RED before the merge) ·
55 i18n tests · two diff-scoped safety inspections, both a **genuine** zero (3 candidates raised and
refuted each time) · `/simplify` 4 lenses · the structural no-write test RED-proved twice. The Boss
test went auditor → inspector → **wrap panel (14 edits)** → inspector ×3 → APPROVED; the wrap panel
found the fifth defect the inspector structurally cannot see — a **required step presented as
optional**, which would have produced a false PASS.


---

## 🧾 Filed 2026-09-15 — from the PJ-466 build and its gates (WA#6: discovered, therefore filed)

### 🆕 PJ-493 *(MED — Group 2 — honesty; the "fifth exit" the copy panel CUT from PJ-466 for a good reason)* — a scan that cannot run reports a universe with zero molds
`scan_stamped_molds` (`mold_repair.rs`) returns `Ok(…)` **unconditionally** — it has no error path at
all — and `load_libraries` returns an EMPTY VECTOR on a registry read failure by design. So an
unreadable registry yields "no molds found", and both call sites swallow a frontend throw as well:
the dialog closes without ever painting (`MoldRepairDialog.svelte:99-106` sets `candidates = []` and
calls `onDone`), and `+layout.svelte:624`'s `catch { moldRepairCount = 0; }` **retracts the banner**.
A user whose scan cannot run is told nothing, and their broken templates become invisible.

**Why it was cut from PJ-466 rather than fixed there:** the copy panel proved there is no screen and
no reachable cause for the wording I was about to write, so building it would have been inventing UI
for something that cannot happen *as currently coded*. The GAP is real; the fix is an engine change
(make the scan able to fail) plus an honest surface — its own job, on its own terms. The run-level
refusal PJ-466 added to `repair_stamped_molds` (`try_load_libraries` → `Err`) is the shape to copy.

### 🆕 PJ-496 *(LOW — Group 3 — docs parity)* — six translated User Manuals have no template-repair passage at all
Measured 2026-09-15 with each locale's own shipped `moldRepair.recheck` string as the probe (an
English-label grep returns a false 0/14 — see the method note): **8 manuals carry the passage**
(`ar de es fa ja ko ru zh`) and were corrected + extended with the receipt paragraph in PJ-466's
close; **6 do not carry it at all** — `fr he hi pt tr ur`. The gap pre-dates this job; PJ-466 widened
it by adding a paragraph to the other nine. Also owed: `docs/help.{lang}/` has no per-language
`Templates/` topic folder, so the help-topic side of this door exists only in English.

### 🆕 PJ-494 *(LOW — Group 3 — perf, pre-existing)* — the repair door's two lists are not virtualized
`{#each receiptRows}` and the review list render every row inside a fixed-height scroll box.
CLAUDE.md's threshold is 50; `receiptRows` is now `outcomes + notReached`, and Eisa Universe's own run
was 39. A universe with more molds crosses it. Not introduced by PJ-466 — flagged by its efficiency
review so it is not forgotten if the candidate set grows.

### 🆕 PJ-495 *(LOW — Group 3 — test residue, now cleared; recorded so the pattern is not repeated)* — three rounds of mold-repair rehearsals left residue in Scratch
Rounds on 2026-09-11 and 2026-09-13 left `mold-rehearsal/` and `mold-rehearsal-2/` (4 files each) plus
**10** backup files in `Scratch\.constellation\pj454-backup\`, none ever cleared. **All removed
2026-09-15 on the Boss's word**, together with this round's own staging. **The residual exposure:** the
8 deleted files still have rows in Scratch's `search.db`, so Scratch will show a drift/phantom notice
on its next boot — which is Constellation's own machinery working as designed. Recorded because a
test that stages files must also state, in the tutorial, what it will remove and when — PJ-466's
tutorial did, and that is the standard from here.

### 📌 METHOD NOTE (not a PJ) — the shadow trap is PER-FILE, and it caught me TWICE in one day
**The test, use it per file:** `fsutil hardlink list <path>` — **`Error 50` = genuine**; a
path under `Packages/Claude_…/LocalCache/Roaming/` = **shadowed, contents not trustworthy**.

`%APPDATA%/world.uconstellation.app/universes.json` is shadowed: `fsutil file queryfileid` returns
the **same NTFS file ID** for the `%APPDATA%` path and the container path — one file object, two
paths, not two copies. Its siblings `style-presets.json`, `app-prefs.json` and `write-journal.jsonl`
all return `Error 50` and are genuine.

**Trap 1 (a wrap-panel lens):** moved to BLOCK the whole PJ-466 Boss test on the strength of that
frozen file. Refuted by `E:`-resident witnesses.

**Trap 2 (mine, the same day):** I noticed the app had written `write-journal.jsonl` into that folder
**today**, concluded the folder was live and the shadow mechanism false, and was about to rewrite
four documents to say so. **The inference does not hold — a pass-through file always looks live
whether or not its neighbour is shadowed.** A `findings-verifier` refuted me with the file-ID test;
I reproduced it myself before accepting it. My own quick per-file loop ALSO mis-reported
`universes.json` as genuine — the raw command disagreed with my own matcher.

**Provably stale without reading it:** `save_registry` sits on `set_active_universe`'s unconditional
success path (`universe.rs:1490-1494`), boot calls it (`+layout.svelte:4356`), the live journal holds
**37** `session_restore_begin` markers after 2026-08-07 09:56, and `diagnostics.log` shows zero save
failures. **The real registry's CONTENTS remain unverifiable from this session.**

### 🆕 PJ-489 *(MED — Group 2 — honesty, same class as PJ-466)* — the importer shows a success tick over a run that imported nothing
`ImporterModal` renders its completion state without consulting the per-file error count, so an
import in which every file failed presents the same tick as one that succeeded. Same shape as the
mold door's "Templates fixed" over 0 fixed; different engine (`importers.rs`), so it needs its own
pass and its own test. **Verify the exact component and line before building** — the panel's
enumeration of this one was not independently re-read, and the same lens was caught with a false
module count elsewhere in its sweep.

### 🆕 PJ-490 *(LOW — Group 3)* — `MigrationProgressStrip` has no failure state
The strip that surfaces long migrations (the MIG-041 `migration:term_vocab_v2` channel, reused by
the standing defrag rule) has phases for running and done but none for *errored* — a migration that
fails is indistinguishable from one still working. Same honesty class; different engine.

### 🆕 PJ-491 *(LOW — Group 3)* — `Mig108UnifyDialog` computes a `skipped` count it never renders
An omission rather than a false claim — its summary is reached only after the engine returns `Ok`
(`:215`) and every failure routes to resume, which is why the panel ruled MIG-108's dialog HONEST
and out of PJ-466's scope. Filed so the unrendered count is not lost.

### 🆕 PJ-492 *(LOW — Group 3 — docs)* — six translated User Manuals carry no mold-door passage at all
`fr he hi pt tr ur` have no passage describing the template-birth-date repair; the other eight
(`ar de es fa ja ko ru zh`) do, and will gain the halt paragraph with PJ-466. A missing passage
pre-dates this job.

### 🆕 PJ-467 *(MED — Group 2 — Rule 8 / Rule 3 exposure)* — the mold scan runs on every watcher flush, not once after boot
`refreshMoldRepairCount()` is scheduled at the tail of `refreshLibraryCaches` (`+layout.svelte`
~4814), which also runs from the 800 ms watcher-flush debounce (~3577) and the reindex path
(~3562) — so `scan_stamped_molds` re-reads every `.md` under every top-level own root after every
external-change burst, not the "one deferred post-boot scan" its comments describe. PJ-464 halved
the cost on a nested layout; the trigger remains. Fix: gate the scan to boot + the door's Done, or
maintain the count at write time.

### 🆕 PJ-468 *(LOW — Group 3 — the journal has a hole, PJ-406's family)* — a failed repair leaves no durable record
`gate_write` returns on the write error BEFORE `journal_ext` (`write_gate.rs:441` vs `:448`), so a
blocked repair is recorded nowhere but the screen. Measured in the rehearsal: two blocked attempts,
zero journal lines.

### ✅ PJ-469 *(CLOSED 2026-09-15 with PJ-466, `5e3e6857`, Boss-passed — the previewed-refusal write is gone — fixed in the ENGINE (it proves the edit before writing), not in the door, where the first attempt would have put a false "kept · already fixed" line on the receipt)* — the repair batches are keyed on `needs_care` and ignore `will_apply`
`MoldRepairDialog.svelte:158-159` splits candidates by `needs_care` only; a candidate whose preview
said `will_apply:false` is still sent to the engine (which refuses it — safely — but counts the
refusal as a failure, which in the delicate batch halts the run).

### ✅ PJ-470 *(CLOSED 2026-09-15 with PJ-466, `5e3e6857`, Boss-passed — `relinked_sources` counts only successful re-indexes; the discarded `Err` no longer inflates a number rendered to the user)* — `relinked_sources` can over-count
`reindex_sources_linking_to` (`mold_repair.rs`) reports every source path it attempted; a
`reindex_single_note` failure is `let _` and the path is still counted as refreshed.

### ✅ PJ-471 *(CLOSED 2026-09-15 with PJ-466, `5e3e6857`, Boss-passed — the four `{count}`+fixed-noun keys are `$tn` with the new `plurals.templates` noun (37 CLDR forms × 15), and the engine’s English `detail` literals became translated `reason` codes)* — `moldRepair.*` counts against the MIG-087 plural rule; `repair_one` details are English literals
`moldRepair.count` / `repair` / `inbound` / `relinked` use `{count}` with a fixed noun ("1 template
files…" was on the Boss's screen); the engine's outcome strings ("Repaired: stamp … removed…",
"The change could not be written: …") are English in all 15 languages.

### 🆕 PJ-472 *(LOW — Group 3 — a resolver out of step with its siblings)* — `library_name_for_path` does not trim a trailing separator
`libraries.rs:237-250` normalizes `\`→`/` and lowercases but does not `trim_end_matches('/')`,
unlike `nested_library_paths`, `foreign_roots_of`, `path_is_under_any` and `run_full`'s `norm`;
a root registered WITH a trailing separator resolves its own files to `None`. Every caller falls
back (the walking library / the row's stored name) so it is cosmetic today; fix at the resolver.

### 🆕 PJ-473 *(LOW — Group 3 — tidy-up; NO behaviour claim)* — `run_cold_start` walks nested own libraries redundantly
`index_repair.rs:827-899` collects per library and removes only foreign roots, so a cold start of
the root library also reads every nested own library's files (attribution per file, upsert — no
double count). Who submits `Scope::ColdStart` per library was not read; do not write "no behaviour
change" until it is.

### 🆕 PJ-474 *(LOW — Group 3 — four out-of-scope finds from the PJ-461 audit; verified by two agents each, not re-read by the synthesiser)*
(a) `static/fonts/EBGaramond-Variable.ttf` is a saved HTML page, not a font (`fonts.css:45`
references it); (b) `SecondScreenPage.svelte:595-597` applies `styleOverride` keys with no
`removeProperty`, so a reset var stays on the second screen; (c) the nine Relationship-colour
pickers show `#888888` for an unset var (`StyleSetter.svelte:848-860`); (d) workspace-restore
`validTabs` (`+layout.svelte:~10588`) omits `structure` / `inspector360` / `sourceReview`.

### 📍 STATE AT 2026-09-01 SESSION CLOSE — SUPERSEDED — PJ-454 CLOSED 2026-09-13, see v2.13
...
- The **Two-Signal Choke Point** in `canonical.rs` — nothing acquires a stamp from here on.
  **✅ BOSS-TESTED 2026-09-01, all three cases (guard CLOSED).** *(The commit had preceded his test,
  against his standing order; he ruled "leave it — test it next", and this session did — via
  tutorial-auditor → ui-inspector → panel, on a binary verified to contain the code.)*
