...
> - **§CB-2 — SHIPPED `8ae4451f` (2026-06-12 ~15:00), Boss gate pending** (saves compose from the buffer; identity travels with content; refusals journaled): the frontmatter cluster extracted VERBATIM to `$lib/editor/frontmatter.ts` (store.ts re-exports — zero call-site churn app-wide); `composeBuffer(tabId, expectPath, surface)` is the ONLY content source for handleSave/handleFlush/handlePromote/PE saves/saveTabContent/FocusPane — a path mismatch is REFUSED + journaled into the same write-journal stream (`journal_compose_refusal` IPC; write_gate `journal_event`); `freshProps()/freshBody()` DELETED (the Frankenstein joins are gone); `ensureBuffer` covers non-store NoteEditor hosts (index preview/dashboard); FocusPane's per-keystroke parseFrontmatter died (perf win ahead of §CB-5). 21/21 vitest · 0/1457 svelte-check · 923/923 Rust. (Full-suite note: ONE pre-existing Sight tradition-isolation test failure, stash-verified unrelated, chip task_30a68ac6.)
