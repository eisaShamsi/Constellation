...
> - **§CB-2 IN FLIGHT** (saves compose from the buffer; identity travels with content; refusals journaled): write_gate.rs `journal_ext`→`journal_line` refactor + `journal_event` (frontend compose-refusals share the ONE journal stream) landed in the worktree; next: `composeBuffer`/`setBufferBody`/`setBufferProps` + the 7 call-site flips (freshProps/freshBody die) + `journal_compose_refusal` IPC.
