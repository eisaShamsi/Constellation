...
**Version 2.67 | 2026-06-11**

> **What changed in v2.67** (**MIG-075 OPENED — the CNS-modernization `/migration` Phase-1 Architect produced; awaiting Eisa's ratification**):
>
...
| **MIG-075** The CNS modernization (detect_tensions Rule-8 re-source · the CNS-panel boundary cleanup → "Circulation → CCS" · the c–j ruling set) | `lab/reports/MIG-075-CNS-ARCHITECT.md` | 🟡 **OPENED 2026-06-11 — Architect produced, AWAITING RATIFICATION (Q1–Q8).** Carries the two MIG-074 §3-a Boss-approved items + the four handover rulings (c) `lens.title` ×15 localization · (d) the 233,538-vs-234,062 count caption · (e) Universe-Health inputs (VERIFIED topology-pure — no change) · (f) most_connected's home (duplicated today: KH card + CNS insights tab) — plus territory-surfaced rulings: (g) the Knowledge-Insights section's mixed registers (incl. the data-dead "stagnating" tab — `status='dormant'` = 0 rows since the decay fix), (h) `constellation_sight_centrality`'s own fs walk (the CNS open cost; re-source recommended), (i) dead `constellation_sight_tag_edges` + v1 `ConstellationSight.svelte` (delete recommended), (j) the hardcoded typed-link colors in CNS edges/legend/SightPanel (registry-align recommended). Gating fact verified: tags ARE DB-queryable (`note_meta.tags_json`, 19,523 distinct in the live universe). No Plan, no code until Eisa rules. |
