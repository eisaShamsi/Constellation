...
**Version 2.00 | 2026-08-25 (evening)**
...
### 🆕 PJ-387 *(MED — Group 2 — silent-data-loss + index-divergence)* — the 234 notes with no identity: a delete leaves no record, and 13 of them DO have one
**Boss-ordered 2026-08-25:** *"Regarding the 234 notes, file them as their own job."*

**Concept (the horse):** *a note the app cannot name is a note the app cannot remember destroying.*

**Measured** on `Eisa Universe`, read-only: **234 of 2,731 rows (8.6%) carry an empty `cid_cn`;
all 234 files exist on disk; all 234 carry body text (32,625,753 characters); 223 sit in one
folder** (`Constellation PKM\Constellation Working Docs`).

**They are TWO populations, and the split is the job:**
- **13 rows are an INDEX DIVERGENCE.** Their files carry a real `cid_cn` in frontmatter while the
  index holds none — so they would purge unarchived *despite having an identity*. **Why the
  indexer holds an empty value for a file that names one is NOT KNOWN and must be investigated
  before anything is changed.**
- **221 have no frontmatter at all.** Giving them identities means writing to the Boss's own
  notes — his decision, not a mechanical fix.

**What breaks:** `build_delete_archive` returns an empty `Vec` for an empty cid, Phase 2 is gated
on non-empty, Phase 3 purges anyway and returns `Ok`. Bounded for a trash delete (the `.md`
survives); for a permanent delete or a reconcile-gone it destroys the last copy with **no record**.

**Three remedies, complements not alternatives:** reindex the 13 (an index bug — fix regardless) ·
PJ-384's root fix, which covers all 221 without touching a file · back-fill identities (Boss
decision).

**Group 2, not 1, only because the app is now honest about it on the very surface that makes the
promise** — and that honesty is conditional on this close's string fixes landing. **Escalates to
Group 1 the moment PJ-386 (restore) is approved.** Not a duplicate of PJ-384: that is the
mechanism, this is the population.
