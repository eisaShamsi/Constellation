...
| **A5** | **DONE 2026-08-21** — `is_known_type` and `is_structural_type` **deleted**, `structural_frontmatter_targets` is registry-taking, the `snapshot()` alias is gone. The compiler enumerated **34 sites**; 20 were legitimately about the active universe and now say so via `active_universe_vocabulary()`, and 14 were threaded. Parse chain (`extract_wikilinks` / `parse_link_body` / `emit_frontmatter_links` / `extract_typed_links` / `extract_frontmatter_typed_links`) and `index_note_impl` now carry `&LinkTypeRegistry`. **The two filesystem re-walkers lost a real defect in passing:** `strata.rs` and `inspector360.rs` re-read the process-global **once per directory**, so a walk spanning a vocabulary change could classify half a library under one vocabulary and half under another — they now use one value passed from the top. **Census test pins all 37 remaining ambient reads by file and count**, and it demonstrably fails on a one-site discrepancy. Suite **1523 / 0**. |
| ~~A5 (earlier status)~~ | ~~PART-LANDED — the plan said "not started" and that was false~~ | `active_universe_vocabulary()` exists (link_types.rs) and `snapshot()` survives as a transitional alias delegating to it. The RENAME landed; the DELETION of the three ambient readers and the ~26 call-site threading did not. Corrected after the panel caught the doc contradicting the code. |
| A6–A8 | not started | |
