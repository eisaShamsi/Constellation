...
- **Resumen del Universo** — el lugar *primario* donde viven los resúmenes: un panel dedicado en el dock izquierdo que lista cada nota de tu Universo (escalonada Library → Carpeta → Nota), con cada fila mostrando el titular bajo el nombre. Haz clic en una fila para expandirla y leer el resumen completo de varias frases en línea. Buscar/filtrar reduce toda la lista. Ver *El Resumen* para el tema completo.

Por defecto los resúmenes se rellenan **de forma perezosa y suave**: a medida que las tarjetas entran en vista, a medida que aparecen los resultados de búsqueda, a medida que abres una nota, a medida que expandes un término, pasas el cursor sobre una burbuja, o desplazas el Resumen, Constellation calcula los resúmenes faltantes de a pocos, deteniéndose siempre que un escaneo de clasificación de Library está en ejecución para que los dos nunca compitan por recursos. Esto mantiene la app responsiva — puedes ver brevemente una tarjeta / resultado / nota abierta / fila / tooltip / fila del Resumen antes de que aparezca su resumen, y luego el resumen aparece un momento después.
