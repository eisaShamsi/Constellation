...
| Title-heading rename gap | **OPEN — fix attempt REVERTED (BUG-023, 2026-06-11)**: the gap is real (`NoteEditor.handleTitleChange` calls `renameItem` only; the cascade runs only on file-tree renames), but the first fix (delegating to `handleRenameComplete` via an `onTitleRename` prop, commit `a086e1ee`) produced **BUG-015-class cross-note corruption** in Eisa's Boss test — the renamed file ended up on disk with the LINKING note's entire frontmatter (title + cid_cn identity) and an empty body. Vector removed by revert the same hour. Root cause unestablished (the title-blur rename originates INSIDE the live editor whose own save/flush/{#key} lifecycle is active — a different interleaving than the sidebar rename the orchestration was designed for). **Do not re-attempt without the full /migration treatment**: NotePane spec §2.6 + the BUG-015 forensics in `lab/forensics/` + the Rename Function Concept Paper are required reading first. |
