...
|---|---|---|---|
| a | **`detect_tensions` fs-walk Rule-8 modernization** | **OUT of MIG-074** — queued as the opening § of the future CNS-modernization MIG (number allocated when opened). *(Addendum 2026-06-10, Boss-approved at the Stage-1 cross-check:)* **the same future CNS MIG also takes the CNS-panel boundary cleanup** — CNS's right panel today carries two circulatory blocks ("Link Health BY TYPE" + "BY CONFIDENCE" in `ConstellationSight2.svelte`) that duplicate CCS's Acts-of-Inquiry + Conviction-&-Doubt registers with worse rendering (raw `lens.link*` i18n keys shown verbatim; custom types shown as keys, not registry labels). Approved shape: CNS **sheds both blocks** and gains a **"Circulation → CCS" deep-link**; the i18n/registry defects vanish with the shed. Also note for that MIG: a caption convention for the CNS-vs-CCS link-count difference (resolved sky edges 233,538 vs all recorded note_links rows 234,062 — both true, different layers), and a look at the Universe-Health score's inputs. | Its four outputs are all topology/tension (CNS register, §2.7); CCS invariant I4 (complement-not-overlap) forbids CCS owning them. Folding it in would push MIG-074 across the Rust tension-subsystem boundary for zero circulatory gain. The boundary case — contradiction *counts* — is already circulatory-served by `stats.by_type` (the Acts of Inquiry shows `contradicts` distribution); the contradiction *pair list* stays TensionPanel's. *(If Eisa prefers IN: it lands as a self-contained §-step re-sourcing tension.rs from note_links/note_meta — with the tags-in-DB fact verified first; adds ~1 session + a Rust-side test surface.)* |
