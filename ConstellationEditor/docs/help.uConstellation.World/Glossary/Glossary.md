...
aliases:
  ...
  - Concordance
  - Term index
description: An automatic word index that extracts every word from all notes across all libraries, showing occurrence counts and source notes.
---

# Glossary

The Glossary is an automatic word index. It extracts every word from every note across all your libraries, counts how many times each word appears, and shows you which notes contain it. Click any note name to jump straight to it.

You can open the Glossary from the **book icon** in the left ribbon, or via Mission Control (**Ctrl+P** > "Open glossary").

---

## How it works

The glossary scans all `.md` files in all registered libraries and builds a word index:
...
- Or press **Ctrl+P** (Mission Control) and type "Open glossary".
...
- A library is added or removed.
- Files are created, modified, or deleted in any watched library.
- The app starts up.
...
When you have multiple libraries, the glossary merges word data across all of them:

- The same word found in different libraries has its counts summed.
...
> [!tip] Use the filter for large libraries
> In libraries with many notes, the word index can be very large. Use the filter input to quickly find specific words you're looking for.
